  ...
  return useCallback(
    (event: AgentStreamEvent, source: AgentEventSource = 'live') => {
      ...
      if (seq > 0) {
        ...
        // Main remaps live frames onto its own wire seq; only the recorder
        // value (`sourceSeq`) is a valid cursor into the host's run-event log,
        // so resume tracks it separately.
        const previousSourceSeq =
          resumeSourceSeqByConversationRef.current.get(conversationId) ?? 0
        const nextSourceSeq = nextAgentRecorderSeqCursor(
          previousSourceSeq,
          event,
        )
