...
function useWelcomeMessage(conversationId) {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                ...
                if (cancelled)
                    return;
                const welcome = events.find((e) => e.type === "assistant_message" && e.payload);
                if (welcome?.payload) {
                    const text = welcome.payload.text;
                    if (text)
                        setMessage(text);
                }
            }
...
export function WelcomeDialog({ conversationId, onConnect, onSignIn, }) {
    const { hasConnectedAccount } = useAuthSessionState();
    const [open, setOpen] = useState(() => uiState.getItem(WELCOME_DIALOG_SEEN_KEY) !== "true");
      ...
      <DialogContent fit size="md" className="welcome-dialog-content">
        ...
        <DialogBody className="welcome-dialog-body">
          ...
          <div className="welcome-dialog-cards">
            ...
            <div className="welcome-dialog-card welcome-dialog-card--interactive" onClick={handleConnect} role="button" tabIndex={0} onKeyDown={(e) => e.key === "Enter" && handleConnect()}>
              <div className="welcome-dialog-card-icon">
                <Smartphone size={20}/>
              </div>
              <div className="welcome-dialog-card-text">
            ...
            {!hasConnectedAccount && (<div className="welcome-dialog-card welcome-dialog-card--interactive" onClick={handleSignIn} role="button" tabIndex={0} onKeyDown={(e) => e.key === "Enter" && handleSignIn()}>
                <div className="welcome-dialog-card-icon">
                  <LogIn size={20}/>
                </div>
                <div className="welcome-dialog-card-text">
          ...
          <button type="button" className="welcome-dialog-cta" onClick={handleClose}>
            Get started
          </button>
        </DialogBody>
      </DialogContent>
