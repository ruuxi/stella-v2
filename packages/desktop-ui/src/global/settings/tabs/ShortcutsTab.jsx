...
function formatShortcutForDisplay(shortcut, offLabel) {
    if (!shortcut)
        return [offLabel];
    return shortcut
        .split("+")
...
export function ShortcutsTab() {
    const t = useT();
    const actionLabel = useCallback((action) => t(`settings.shortcuts.actions.${action}`), [t]);
    const [shortcuts, setShortcuts] = useState({
        dictation: "Alt",
        voice: "CommandOrControl+Shift+D",
    });
    const [loaded, setLoaded] = useState(false);
    const [savingShortcut, setSavingShortcut] = useState(null);
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                ...
                if (!cancelled) {
                    setShortcutError(getSettingsErrorMessage(error, t("settings.shortcuts.errors.load")));
                }
            }
            finally {
                if (!cancelled)
    ...
    const saveShortcut = useCallback(async (action, shortcut) => {
        const setShortcut = action === "dictation"
        ...
        try {
            ...
            if (!result.ok) {
                const message = result.error ?? t("settings.shortcuts.errors.unavailableShortcut");
            ...
            showToast({
                ...
                description: shortcut
                    ? t("settings.shortcuts.toasts.updatedDescription", {
                        keys: formatShortcutForDisplay(shortcut, t("settings.shortcuts.off")).join(" + "),
            ...
            const message = getSettingsErrorMessage(error, t("settings.shortcuts.errors.update"));
            setShortcutError(message);
            showToast({
    ...
    useEffect(() => {
        if (!capturingShortcut)
            return;
        const setShortcutCaptureSuspended = (suspended) => window.electronAPI?.system?.setGlobalShortcutsSuspended?.(suspended) ??
    ...
    const renderShortcutRow = (action, description) => (<div className="settings-row">
      <div className="settings-row-info">
        <div className="settings-row-label">{actionLabel(action)}</div>
        <div className="settings-row-sublabel">{description}</div>
      </div>
      <div className="settings-row-control">
        <Keybind keys={formatShortcutForDisplay(shortcuts[action], t("settings.shortcuts.off"))}/>
        <Button type="button" variant="ghost" className="pill-btn" disabled={!loaded || savingShortcut !== null || capturingShortcut !== null} onClick={() => setCapturingShortcut(action)}>
          {capturingShortcut === action
            ? t("settings.shortcuts.capturing")
            : t("settings.shortcuts.change")}
        </Button>
        <Button type="button" variant="ghost" className="pill-btn" disabled={!loaded ||
            savingShortcut !== null ||
            capturingShortcut !== null ||
            !shortcuts[action]} onClick={() => void saveShortcut(action, "")}>
    ...
    return (<div className="settings-tab-content">
      <div className="settings-card">
        ...
        {shortcutError ? (<p className="settings-card-desc settings-card-desc--error" role="alert">
            {shortcutError}
          </p>) : null}
        {renderShortcutRow("dictation", t("settings.shortcuts.dictation.description"))}
        {renderShortcutRow("voice", t("settings.shortcuts.voiceAgent.description"))}
      </div>
    </div>);
}
