 ...
 *
 * The transient buffer may contain plaintext response text, reasoning, and
 * tool arguments because the relay must continue receiving the upstream
 * stream after a client disconnects. It never stores request bodies, prompts,
 * input messages, tool definitions, credentials, or request headers; upstream
 * response-storage policy remains independently provider-specific.
 */
...
/**
 * Bind an idempotent reservation to the canonical logical client request,
 * before mutable audience/default/provider resolution. This lets a retry
 * resume the first resolved route even if billing audience or backend defaults
 * changed after dispatch, while still storing only a one-way digest.
 */
const canonicalRelayJson = (value: unknown): unknown => {
  if (Array.isArray(value)) return value.map(canonicalRelayJson);
  if (value && typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value as Record<string, unknown>)
        .sort(([left], [right]) => left.localeCompare(right))
        .map(([key, child]) => [key, canonicalRelayJson(child)]),
    );
  }
  return value;
};

export const relayResumeRequestBinding = async (args: {
  method: string;
  pathname: string;
  agentType: string;
  requestJson: Record<string, unknown>;
}): Promise<string> => {
  const canonicalBody = JSON.stringify(canonicalRelayJson(args.requestJson));
  const digest = await crypto.subtle.digest(
    "SHA-256",
    new TextEncoder().encode(
      `stella-relay-request-v2\0${args.method.toUpperCase()}\0${args.pathname}\0${args.agentType}\0${canonicalBody}`,
    ),
  );
...
// Exactly-once billing tombstones outlive every stream/retry window so a late
// duplicate finalizer can never charge the same logical request again.
export const STELLA_RELAY_BILLING_RECEIPT_TTL_MS = 24 * 60 * 60 * 1000;
...
export const STELLA_RELAY_CLEANUP_MAX_BILLING_DOCS = 4;
