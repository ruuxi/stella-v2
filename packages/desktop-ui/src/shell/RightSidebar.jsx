...
const computeDefaultWidth = () => DISPLAY_PANEL_DEFAULT_WIDTH;
...
const resolveDisplayPanelWidth = (preferredWidth) => {
    const desired = preferredWidth ?? computeDefaultWidth();
    return clampNumber(desired, DISPLAY_PANEL_MIN_WIDTH, Math.max(DISPLAY_PANEL_MIN_WIDTH, computeMaxWidth()));
};
...
// each other's writes.
const lastAppliedDisplayPanelWidthPx = new WeakMap();
 ...
 * tab's `render()`.
 */
export const RightSidebar = forwardRef(function RightSidebar({ portalTarget }, ref) {
    ...
    useImperativeHandle(ref, () => ({
        ...
        update(rawPayload) {
            ...
            displayTabs.openTab(spec, panelOpen
                ? { activate: false }
    ...
    const handleResizeStart = useCallback((event) => {
        ...
        const measuredStartWidth = aside?.getBoundingClientRect().width ??
        ...
        const onMove = (ev) => {
            ...
            if (snappedToExpanded) {
                if (delta > -DISPLAY_PANEL_EXPAND_SNAP_THRESHOLD)
                    return;
            ...
            latestWidth = Math.max(DISPLAY_PANEL_MIN_WIDTH, Math.min(maxWidth, rawWidth));
            if (frame === 0) {
    ...
    return createPortal(<aside ref={asideRef} className={`right-sidebar right-sidebar-panel${shellVisible ? " right-sidebar--shell-visible" : ""}${panelOpen ? " right-sidebar--open" : ""}${panelOpen && panelExpanded ? " right-sidebar--expanded" : ""}`} aria-label="Workspace" aria-hidden={!shellVisible}>
      {panelOpen ? (<div className="right-sidebar__resize-handle" role="separator" aria-orientation="vertical" aria-label="Resize display panel" onPointerDown={handleResizeStart} onDoubleClick={handleResizeDoubleClick} title="Drag to resize · double-click to reset"/>) : null}
      <div className="right-sidebar-inner right-sidebar-panel__frame">
        ...
        <div className="right-sidebar-panel__body">
          <div className={`right-sidebar__active${panelOpen ? "" : " right-sidebar__active--kept"}`}>
            <SidebarSectionBody />
          </div>
        </div>
