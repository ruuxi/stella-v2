...
Hard budget: Maximum ${MEMORY_MAP_MAX_ENTRIES} entries and
${MEMORY_MAP_MAX_CHARS} characters (HTML comments are not counted). Writes that
would exceed the budget are REJECTED with an error — curate (merge, prune,
tighten) instead of truncating.
Prune stale routes: prune entries older than ${MEMORY_MAP_STALE_DAYS} days
unless recent usage shows they are still useful.
...
export const truncateUnicodeAtLineBoundary = (
  ...
  if (prefix === "") {
    // No complete line fits the budget (e.g. a single very long line); fall
    // back to a code-point-safe hard truncation so the cap fills the budget
    // instead of collapsing to just the marker.
