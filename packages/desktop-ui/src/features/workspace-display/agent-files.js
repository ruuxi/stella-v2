 ...
 */
export function deriveAgentFilesMap(activities, eventFilter) {
 ...
 */
export function mergeAgentFileEvents(activities, fileEvents) {
    if (fileEvents.length === 0)
        return activities;
    const seenIds = new Set();
    for (const event of activities)
        seenIds.add(event._id);
    const extras = fileEvents.filter((event) => !seenIds.has(event._id) &&
        eventAgentId(event) !== null &&
        eventContributesFiles(event));
    if (extras.length === 0)
        return activities;
    const before = (a, b) => a.timestamp < b.timestamp ||
        (a.timestamp === b.timestamp && a._id <= b._id);
    const merged = [];
    let i = 0;
    let j = 0;
    while (i < activities.length && j < extras.length) {
        if (before(activities[i], extras[j]))
 ...
 */
export function eventContributesFiles(event) {
 ...
 */
export function agentFilesSignature(activities) {
    let signature = "";
    for (const event of activities) {
        const agentId = event.payload
            ?.agentId;
        if (typeof agentId !== "string" || agentId.length === 0)
            continue;
        if (!eventContributesFiles(event))
            continue;
        const payload = event.payload;
        const fileCount = (Array.isArray(payload.fileChanges) ? payload.fileChanges.length : 0) +
