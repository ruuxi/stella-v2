...
import { respondToConnectorConnect, useConnectorConnectRequest, } from "@/features/chat/connector-connect-store";
import "./connector-connect-card.css";
export const ConnectorConnectCard = ({ compact = false, conversationId, }) => {
    ...
    const sub = request.phase === "connecting"
        ? isBrowserExtension
    ...
    const title = request.phase === "connected"
    ...
    return (<div className={`connector-connect-card connector-connect-card--${request.phase}${compact ? " connector-connect-card--compact" : ""}`} role="status">
      <div className="connector-connect-card__icon" aria-hidden>
        {request.phase === "connected" ? (<Check size={compact ? 14 : 16}/>) : request.phase === "error" ? (<AlertCircle size={compact ? 14 : 16}/>) : showIconImage ? (<img src={request.iconUrl} alt="" onError={() => setIconFailed(true)}/>) : (<Globe size={compact ? 14 : 16}/>)}
      </div>
      ...
      {request.phase === "offer" ? (<div className="connector-connect-card__actions">
          <Button type="button" variant="ghost" className="pill-btn connector-connect-card__decline" onClick={() => respondToConnectorConnect(request.requestId, "decline")}>
            Not now
          </Button>
          <Button type="button" variant="primary" className="pill-btn pill-btn--primary connector-connect-card__accept" onClick={() => respondToConnectorConnect(request.requestId, "accept")}>
            {isBrowserExtension ? "Install" : "Connect"}
          </Button>
        </div>) : request.phase === "connecting" ? (<div className="connector-connect-card__actions">
          <Button type="button" variant="ghost" className="pill-btn connector-connect-card__decline" onClick={() => respondToConnectorConnect(request.requestId, "cancel")}>
            Cancel
          </Button>
        </div>) : null}
    </div>);
