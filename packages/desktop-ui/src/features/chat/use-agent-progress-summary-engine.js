...
const summarySessionKey = (agentId) => `progress-summary:${agentId}`;
...
export const buildAgentProgressSignature = (task) => {
...
export const buildAgentProgressUserText = (task, priorPhrases) => {
    const reasoning = redactSensitiveText(task.reasoningText ?? "")
    ...
    if (task.toolActivity) {
        lines.push(`Tool: ${redactSensitiveText(task.toolActivity.label)} (${redactSensitiveText(task.toolActivity.toolName)})`);
        if (task.toolActivity.argsHint) {
            lines.push(`Tool context: ${redactSensitiveText(task.toolActivity.argsHint)}`);
        }
    }
    if (reasoning) {
        lines.push("", "Latest reasoning:", reasoning);
    }
    lines.push("", `Previous phrases: ${priorPhrases.length > 0 ? redactSensitiveText(priorPhrases.join("; ")) : "(none yet)"}`);
    return lines.join("\n");
};
const requestSummary = async (task, priorPhrases) => {
...
const closeSummarySession = (agentId) => {
    const agentApi = window.electronAPI?.agent;
    if (!agentApi?.oneShotCompletion)
        return;
    void agentApi
...
export function useAgentProgressSummaryEngine(liveTasks) {
    ...
    const runningIdsKey = useMemo(() => liveTasks
    ...
    useEffect(() => {
        const publish = () => {
            ...
            if (!api?.publishReasoningSummaries)
                return;
