 ...
 *
 * `putDriveObject` writes to a key derived from the path, so an agent
 * reporting a path it never hydrated overwrites the user's object in place —
 * the C3 defect. A turn proves it read the current version by echoing the
...
export function registerCloudDriveRoutes(http: HttpRouter) {
  ...
  http.route({
    ...
    handler: httpAction(async (ctx, request) => {
      ...
      const rows = (await ctx.runQuery(
        ...
        { ownerId: token.ownerId, limit: 1, paths },
      )) as Array<{
        path: string;
        r2Key: string;
        sizeBytes: number;
      ...
      for (const row of rows) {
        ...
        attachments.push({
          ...
          url: await r2.getUrl(row.r2Key, { expiresIn: 120 }),
        });
      }
  ...
  http.route({
    ...
    handler: httpAction(async (ctx, request) => {
      ...
      files = files.filter((file) => admitted.has(file.path));
      ...
      for (const file of files) {
        ...
        recorded.push({
          ...
          // Metadata-only rows say so: their bytes are still in the workspace,
          // so a download URL for them would presign a missing object.
