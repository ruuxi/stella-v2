...
Treat the helper as maintained developer infrastructure, not a one-off script. Give it composable subcommands, useful `--help`, machine-readable output (prefer JSON), bounded timeouts, and errors that say what prerequisite or recovery command is needed. Add `--dry-run` to commands with material destructive side effects. Expose the richest diagnostics the actual runtime supports—for example logs, network/console output, settling, and CDP performance traces for Electron/web; Simulator screenshots and logs for iOS—without inventing capabilities the stack cannot prove.
