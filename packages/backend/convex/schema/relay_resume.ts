...
export const relayResumeSchema = {
  stella_relay_response_streams: defineTable({
    ...
    provider: v.string(),
    model: v.string(),
    requestBinding: v.optional(v.string()),
  ...
  stella_relay_billing_receipts: defineTable({
    relayRequestId: v.string(),
    ownerId: v.string(),
    turnId: v.optional(v.string()),
    ...
    model: v.string(),
    agentType: v.string(),
    /** New receipts delegate charging to the exact physical dispatch receipt. */
    ...
    terminalStatus: v.optional(relayBillingTerminalStatusValidator),
    success: v.optional(v.boolean()),
    estimatedInputTokens: v.number(),
    estimatedOutputTokens: v.number(),
    hasActualUsage: v.boolean(),
    actualInputTokens: v.optional(v.number()),
    actualOutputTokens: v.optional(v.number()),
    actualTotalTokens: v.optional(v.number()),
    actualCachedInputTokens: v.optional(v.number()),
    actualCacheWriteInputTokens: v.optional(v.number()),
    actualReasoningTokens: v.optional(v.number()),
    actualCostMicroCents: v.optional(v.number()),
    durationMs: v.optional(v.number()),
    acceptedAt: v.optional(v.number()),
    /**
     * Terminal stream state can be observed before the producing action has
     * drained its final usage frame. Billing becomes deliverable only after
     * that producer explicitly settles, or cleanup proves it was abandoned.
     */
    billingReady: v.optional(v.boolean()),
    billingReadyAt: v.optional(v.number()),
