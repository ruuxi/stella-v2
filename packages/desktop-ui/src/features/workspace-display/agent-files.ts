...
export function deriveAgentFilesMap(
  ...
  for (const [agentId, events] of eventsByAgent) {
    // Agent ids identify durable threads and survive follow-ups/retries.
    // Replay every authoritative mutation attributed to that durable id so
    // the current card remains a cumulative view of the thread's work.
