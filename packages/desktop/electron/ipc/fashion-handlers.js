...
import { IPC_FASHION_DELETE_BODY_PHOTO, IPC_FASHION_GET_BODY_PHOTO_DATA_URL, IPC_FASHION_GET_BODY_PHOTO_INFO, IPC_FASHION_GET_LOCAL_IMAGE_DATA_URL, IPC_FASHION_PICK_AND_SAVE_BODY_PHOTO, IPC_FASHION_PICK_TRY_ON_IMAGES, IPC_FASHION_START_OUTFIT_BATCH, IPC_FASHION_START_TRY_ON, } from "@stella/contracts/desktop/ipc-channels";
import { registerPrivilegedHandle, } from "./privileged-ipc.js";
import { waitForConnectedRunner } from "./runtime-availability.js";
import { MAX_IMAGE_REFERENCE_BYTES, readValidatedImageFileNoFollow, } from "@stella/runtime/kernel/tools/image-reference-policy";
...
const fashionDir = (root) => path.join(root, "fashion");
const tryOnDir = (root) => path.join(fashionDir(root), "try-on");
const mediaOutputsDir = (root) => path.join(root, "media", "outputs");
const hiddenFashionConversationId = (root) => `fashion:${Buffer.from(root).toString("base64url").slice(0, 24)}`;
const sameFile = (left, right) => left.dev === right.dev &&
    left.ino === right.ino &&
    left.size === right.size &&
    left.mtimeMs === right.mtimeMs &&
    left.ctimeMs === right.ctimeMs;
const readPickedFileNoFollow = async (filePath, options = {}) => {
    ...
    const handle = await fs.open(initialResolved, constants.O_RDONLY | constants.O_NOFOLLOW);
    try {
        ...
        if (!before.isFile() ||
            ...
            throw new Error("Fashion images must be regular files no larger than 20 MB.");
        }
        const postOpenResolved = await fs.realpath(requested);
        const postOpenPathStat = await fs.stat(postOpenResolved);
        if (postOpenResolved !== initialResolved ||
            !sameFile(selectedPathStat, before) ||
            !sameFile(before, postOpenPathStat)) {
            throw new Error("Fashion image pathname changed before its selected file was opened.");
        }
        const chunks = [];
        let total = 0;
        for (;;) {
            const chunk = Buffer.allocUnsafe(Math.min(256 * 1024, MAX_IMAGE_REFERENCE_BYTES + 1 - total));
        ...
        while (verificationOffset < total) {
            const { bytesRead } = await handle.read(verification, verificationOffset, total - verificationOffset, verificationOffset);
            if (bytesRead === 0)
                break;
...
const convertHeicToJpeg = async (bytes, directory, options = {}) => {
    ...
    await fs.access(sipsPath, constants.X_OK).catch(() => {
        throw new Error("HEIC conversion is unavailable in this Stella environment.");
    });
    ...
    try {
        await (options.execSips ?? execFileAsync)(sipsPath, ["-s", "format", "jpeg", sourcePath, "--out", outputPath], {
            timeout: timeoutMs,
            killSignal: "SIGKILL",
            maxBuffer: 256 * 1024,
        }).catch((error) => {
...
export const prepareFashionImage = async (sourcePath, directory, conversionOptions = {}) => {
    ...
    if (sourceExt === "heic" || isHeicBytes(bytes)) {
        if (!isHeicBytes(bytes))
            throw new Error("Selected HEIC file is invalid.");
        return {
    ...
    const ext = decoded.mimeType === "image/jpeg"
        ? "jpg"
...
const mimeForPreparedExtension = (ext) => ext === "jpg" ? "image/jpeg" : `image/${ext}`;
const writePreparedFashionImage = async (filePath, prepared) => {
    await materializeMediaArtifact({
        filePath,
        validateExisting: async (candidate) => await validateDecodedImageFile(candidate, mimeForPreparedExtension(prepared.ext)),
...
const replaceBodyPhoto = async (root, prepared) => {
    const directory = fashionDir(root);
    await fs.mkdir(directory, { recursive: true });
    const staged = path.join(directory, `.body-next-${randomUUID()}.${prepared.ext}`);
...
const findExistingBodyPhoto = async (root) => {
...
const removeAllBodyPhotos = async (root) => {
    const dir = fashionDir(root);
    await Promise.all(SUPPORTED_EXTENSIONS.map(async (ext) => {
        try {
...
const assertAllowedLocalImagePath = (root, rawPath) => {
...
export const stashTryOnImagePaths = async (root, batchId, rawPaths) => {
    ...
    if (paths.length === 0)
        return [];
    if (path.basename(batchId) !== batchId ||
...
const getBodyPhotoInfo = async (root) => {
    const found = await findExistingBodyPhoto(root);
    if (!found)
        return { hasBodyPhoto: false };
...
export const registerFashionHandlers = (options) => {
    const requireRoot = () => {
        const root = options.getStellaDataDir();
        if (!root)
            throw new Error("Stella root not initialized.");
        return root;
    };
    ...
    registerPrivilegedHandle(options, IPC_FASHION_GET_BODY_PHOTO_INFO, async () => {
    ...
    registerPrivilegedHandle(options, IPC_FASHION_PICK_AND_SAVE_BODY_PHOTO, async (event) => {
    ...
    registerPrivilegedHandle(options, IPC_FASHION_GET_BODY_PHOTO_DATA_URL, async () => {
        const root = requireRoot();
        const found = await findExistingBodyPhoto(root);
        if (!found)
            return null;
    ...
    registerPrivilegedHandle(options, IPC_FASHION_GET_LOCAL_IMAGE_DATA_URL, async (_event, payload) => {
        const root = requireRoot();
        const filePath = assertAllowedLocalImagePath(root, payload?.path);
        const allowedRoots = await Promise.all([fashionDir(root), tryOnDir(root), mediaOutputsDir(root)].map(async (candidate) => await fs.realpath(candidate).catch(() => null)));
        const image = await readValidatedImageFileNoFollow(filePath, {
            allowedRoots: allowedRoots.filter((candidate) => candidate !== null),
        });
        return `data:${image.mimeType};base64,${image.bytes.toString("base64")}`;
    });
    registerPrivilegedHandle(options, IPC_FASHION_PICK_TRY_ON_IMAGES, async (event) => {
        ...
        if (result.filePaths.length >= MAX_MANAGED_IMAGE_REFERENCE_ITEMS) {
            throw new Error(`Choose at most ${MAX_MANAGED_IMAGE_REFERENCE_ITEMS - 1} clothing images; the body photo uses the first image_gen reference slot.`);
        }
        return { canceled: false, paths: result.filePaths };
    });
    registerPrivilegedHandle(options, IPC_FASHION_START_TRY_ON, async (_event, payload) => {
        const root = requireRoot();
        const found = await findExistingBodyPhoto(root);
        if (!found) {
        ...
        const requestedPaths = Array.isArray(payload?.imagePaths)
            ? payload.imagePaths.filter((value) => typeof value === "string" && value.trim().length > 0)
            : [];
        if (requestedPaths.length + imageUrls.length >=
            MAX_MANAGED_IMAGE_REFERENCE_ITEMS) {
            throw new Error(`Fashion try-on accepts at most ${MAX_MANAGED_IMAGE_REFERENCE_ITEMS - 1} clothing references because the body photo occupies the first image_gen reference slot.`);
        }
        const stashedPaths = await stashTryOnImagePaths(root, batchId, requestedPaths);
        if (stashedPaths.length === 0 && imageUrls.length === 0) {
            throw new Error("Attach at least one image of the clothes you want to try on.");
        }
    ...
    registerPrivilegedHandle(options, IPC_FASHION_START_OUTFIT_BATCH, async (_event, payload) => {
        const root = requireRoot();
        const found = await findExistingBodyPhoto(root);
        if (!found) {
