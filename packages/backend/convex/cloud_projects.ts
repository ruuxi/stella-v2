...
const cloudProjectRowValidator = v.object({
  _id: v.id("cloud_projects"),
...
/**
 * Revoke the GitHub App installation itself during owner reset/deletion.
 * A 404 is the same terminal state as 204: no external grant remains. Callers
 * must retain their installation locator/outbox row for every other outcome.
 */
export const deleteGithubInstallationGrant = async (
  installationId: string,
): Promise<void> => {
  if (!INSTALLATION_ID_PATTERN.test(installationId)) {
    throw new Error("Cannot revoke an invalid GitHub installation id.");
  }
  ...
  throw new Error(
    await githubErrorMessage(
      response,
      `GitHub installation revocation failed with status ${response.status}`,
    ),
  );
};
...
const requireLiveOwnedProject = async (
  ctx: Pick<QueryCtx, "db"> | Pick<MutationCtx, "db">,
  ownerId: string,
  projectId: string,
) => {
  const project = await requireOwnedProject(ctx, ownerId, projectId);
...
export const createMyProject = mutation({
  ...
  returns: publicProjectValidator,
  handler: async (ctx, args) =>
...
export const renameMyProject = mutation({
  ...
  handler: async (ctx, args) => {
    const ownerId = await requireOwnerId(ctx);
    await assertOwnerDataWriteAllowed(ctx, ownerId);
    const project = await requireLiveOwnedProject(ctx, ownerId, args.projectId);
    const name = args.name.trim();
    if (!name || name.length > 80) {
      throw new ConvexError("Give the project a name of 1–80 characters.");
    }
...
export const finishGithubConnect = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!row.ownerGeneration) {
      return refuse(
        "That connect code predates the current account state. Start the GitHub connection again.",
      );
    }
    await assertOwnerDataWriteAllowed(ctx, ownerId, row.ownerGeneration);
...
export const listMyGithubRepositories = action({
  ...
  > => {
    ...
    // Resolution crossed an action/query boundary. Re-check the exact
    // generation immediately before minting a provider credential and again
    // before returning provider data, so reset cannot race either seam.
    await assertExpectedOwnerGenerationActive(ctx, ownerId, generation);
...
export const mintInstallationTokenInternal = internalAction({
  ...
  handler: async (
    ...
    // Minting and identity lookup both cross transaction boundaries. Refuse to
    // release the short-lived token if this owner's generation changed while
    // either was in flight.
    await ctx.runMutation(
...
export const createInstallStateInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const row of candidates) {
      if (row.expiresAt <= args.now) await ctx.db.delete(row._id);
    }
...
export const consumeVerifyStateInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await assertOwnerDataWriteAllowed(ctx, row.ownerId, row.ownerGeneration);
...
export const beginGithubVerificationInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!row.ownerGeneration) return { ok: false, verifyStateId: "" };
    await assertOwnerDataWriteAllowed(ctx, row.ownerId, row.ownerGeneration);
    await assertOwnerMigrationWriteAllowed(ctx, row.ownerId);
...
export const setInstallationStatusInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await assertOwnerDataWriteAllowed(ctx, row.ownerId);
