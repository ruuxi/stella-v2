 ...
 * same 13px default, same muted ink. Static by design — while a row is
 * running the title shimmer carries the motion, the star just sits quietly
 * in the leading slot.
 */
...
/** Matches desktop's `.agent-activity-star { opacity: 0.85 }`. */
...
export function StellaStarGlyph({
  ...
  size?: number;
  /** Muted ink — the settled row text color. */
  color: string;
}) {
  return (
    <Svg
