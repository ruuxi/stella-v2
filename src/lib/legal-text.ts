...
Additional Backend Services — Our backend also provides authentication, an offline fallback responder, connector integrations (Slack, Telegram, etc.), social features, and media generation capabilities.
...
Offline Responder — When your desktop application is not running or not reachable, you may interact with Stella through the mobile app or connected channels (Slack, Telegram, etc.). In this case, your message is sent to our backend, processed by a minimal fallback AI agent, and a response is returned. These offline interactions are transient — they are processed in memory and are not persistently stored in our systems beyond what is required to deliver the response and record usage for billing purposes.
...
Offline Responder — When your desktop is offline and you interact with Stella via the mobile app or a connected channel (Slack, Telegram, etc.), your message is sent to our backend and processed by a minimal fallback AI agent. The interaction is transient — it is processed in memory and not persistently stored beyond what is needed to deliver the response and record usage metadata.

Connector Message Routing — If you connect Stella to third-party messaging platforms (Slack, Telegram, Google Chat, Microsoft Teams), we store connection metadata (which external account is linked to which Stella account, conversation mapping identifiers) and transient message events (inbound and outbound messages held temporarily for delivery, with a short time-to-live, automatically cleaned up). We do not permanently store the text content of connector messages.
