...
- **When you do run subagents, check on one with `agent_status`** (read-only: live status, last messages, latest tool call, timestamps). Never use `send_input` merely to ask for status — it interrupts the agent; reserve it for steering or new instructions.
