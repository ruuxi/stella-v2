...
import {
  maybeSpawnDreamRun,
  shutdownDreamRuns,
} from "../kernel/agent-runtime/dream-scheduler.js";
import {
...
import { registerApiProvider } from "../ai/api-registry.js";
import type {
  Api,
  AssistantMessage,
...
import type { ResolvedLlmRoute } from "../kernel/model-routing.js";
...
describe("Dream scheduler shutdown", () => {
  const activeRoots = new Set<string>();
  afterEach(async () => {
    await shutdownDreamRuns();
  ...
  it("shutdownDreamRuns aborts the in-flight run, releases the mkdir lock, and clears the in-flight flag before resolving", async () => {
    const rootPath = path.join(
      os.tmpdir(),
      `stella-dream-interrupt-${Date.now()}-${Math.random()
        .toString(36)
        .slice(2)}`,
    );
    activeRoots.add(rootPath);
    const state = { requested: false, aborted: false };
    ...
    const result = await maybeSpawnDreamRun({
      ...
      resolvedLlm: fakeHangingRoute(state),
      trigger: "manual",
    });
    expect(result.scheduled).toBe(true);

    await waitFor(() => state.requested);
    const lockDirPath = path.join(rootPath, "locks", "dream");
    expect(existsSync(lockDirPath)).toBe(true);

    await shutdownDreamRuns(rootPath);

    // The LLM call was aborted and the run's finally block completed
    // (lock released) before shutdown resolved.
    expect(state.aborted).toBe(true);
    expect(existsSync(lockDirPath)).toBe(false);

    // inFlight was cleared: a fresh spawn is not rejected as in_flight.
    const followUp = await maybeSpawnDreamRun({
      stellaDataDir: rootPath,
      store: {
        dreamInboxStore: { countUnprocessed: () => 1 },
      } as RuntimeStore,
      resolvedLlm: fakeHangingRoute({ requested: false, aborted: false }),
      trigger: "manual",
    });
    expect(followUp.reason).not.toBe("in_flight");
