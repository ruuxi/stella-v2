...
import { SETTINGS_TABS, } from "@/global/settings/settings-tabs";
...
const LegalDialog = lazy(() => import("@/global/legal/LegalDialog").then((m) => ({
 ...
 */
export const SettingsScreen = ({ activeTab: activeTabProp, onActiveTabChange, onSignOut, embedded = false, }) => {
    ...
    const tabStripRef = useEdgeFadeRef();
    return (<>
      ...
      <div className="settings-screen" data-search-active={isSearching ? "true" : "false"}>
        <div className={`settings-layout ${embedded ? "settings-layout--sidebar" : "settings-layout--standalone"}`}>
          ...
          <header className="settings-tab-rail" role="tablist" aria-label={t("settings.title")}>
            ...
            return (<button key={tab.key} id={`settings-tab-${tab.key}`} type="button" role="tab" aria-selected={isActive} aria-controls={`settings-tabpanel-${tab.key}`} tabIndex={isActive ? 0 : -1} className={`settings-tab-rail-item${isActive ? " settings-tab-rail-item--active" : ""}`} onClick={() => {
                    if (isSearching)
                        setSearchQuery("");
          ...
          <SettingsPanel>
            {isSearching ? (<SettingsSearchResults query={deferredQuery} onSelect={handleResultSelect} onClear={() => setSearchQuery("")}/>) : (<div id={`settings-tabpanel-${activeTab}`} role="tabpanel" aria-labelledby={`settings-tab-${activeTab}`}>
                <SettingsTabContent activeTab={activeTab} onSignOut={onSignOut} onOpenLegal={setActiveLegalDoc} pendingScrollTarget={pendingScrollTarget} onScrollTargetHandled={() => setPendingScrollTarget(null)}/>
              </div>)}
          </SettingsPanel>
        </div>
      </div>
...
function SettingsTabContent({ activeTab, onSignOut, onOpenLegal, pendingScrollTarget, onScrollTargetHandled, }) {
    ...
    useEffect(() => {
        if (!pendingScrollTarget)
            return;
        if (pendingScrollTarget.tab !== activeTab)
            return;
