...
- **Home cover + greeting + actions**: `desktop/src/app/home/HomeContent.tsx`.
...
- Stella product source has one `package.json` at the repo root — install its
  dependencies there. Standalone apps created with `create-stella-app` are the
  exception: each external app owns its package and dependencies inside its
  generated `workspace/apps/<slug>` directory.
...
- [create-stella-app](../create-stella-app/SKILL.md) — scaffold a standalone Vite React user app in Stella's writable workspace (also covers canvas and game surfaces)
