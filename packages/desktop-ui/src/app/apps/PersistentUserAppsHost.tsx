...
import type { UserApp } from "./user-apps-registry";
import {
...
type UserAppLoadState = {
  status: "loading" | "ready" | "error";
  url: string | null;
  error: string | null;
};

const appLeases = new Map<string, AppLease>();

const startApp = (slug: string): Promise<string> => {
...
const acquireApp = (slug: string): Promise<string> => {
  ...
  if (!lease) {
    const nextLease: AppLease = {
      refs: 0,
      stopTimer: null,
      failed: false,
      startPromise: Promise.resolve(""),
    };
    nextLease.startPromise = startApp(slug).catch((error: unknown) => {
      nextLease.failed = true;
      throw error;
    });
    lease = nextLease;
    appLeases.set(slug, nextLease);
  }
  lease.refs += 1;
...
const releaseApp = (slug: string): void => {
