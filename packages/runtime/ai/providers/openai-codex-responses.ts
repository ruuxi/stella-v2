...
export const streamSimpleOpenAICodexResponses: StreamFunction<"openai-codex-responses", SimpleStreamOptions> = (
	...
	const reasoningEffort = resolveOpenAICodexReasoningEffort(
		model,
		options?.disableReasoning ? "off" : options?.reasoning,
	);
...
export const resolveOpenAICodexReasoningEffort = (
	model: Model<"openai-codex-responses">,
...
async function parseErrorResponse(response: Response): Promise<{ message: string; friendlyMessage?: string }> {
	...
	try {
		...
		if (err) {
			...
			if (typeof code === "string" && code && !friendlyMessage && !message.includes(code)) {
