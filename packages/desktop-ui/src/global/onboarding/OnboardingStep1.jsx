...
import { PHASE_ACTS, SPLIT_PHASES, SPLIT_STEP_ORDER, } from "./onboarding-flow";
...
export const OnboardingStep1 = ({ initialPhase = "intro", onComplete, onInteract, onDiscoveryConfirm, onEnterSplit, onSelectionChange, onPhaseChange, isAuthenticated, discoveryWelcomeExpected = false, discoveryWelcomeReady = false, }) => {
    const t = useT();
    ...
    useEffect(() => {
        ...
        const probe = async () => {
            try {
                const result = await window.electronAPI?.system?.detectTechnicalUserSignals?.();
                if (cancelled)
                    return;
    ...
    const handlePhaseChange = useCallback((nextPhase) => {
        onSelectionChange?.(nextPhase === "browser" && discoverySelectionsRef.current);
        onPhaseChange?.(nextPhase);
    }, [onPhaseChange, onSelectionChange]);
    const { phase, leaving, rippleActive, maxVisitedSplitStepIndex, visibleSteps, nextSplitStep, prevSplitStep, continueIntro, } = useOnboardingFlow({
        initialPhase,
        onComplete,
        onEnterSplit,
        onInteract,
    ...
    useEffect(() => {
        if (initialNotificationSentRef.current)
            return;
    ...
    const canReturnNext = canGoNext && maxVisitedSplitStepIndex >= splitStepIndex + 1;
    const platform = getPlatform();
    const phaseAct = PHASE_ACTS[phase];
    const renderActiveSplitPhase = (activePhase) => {
        switch (activePhase) {
            case "capabilities":
            ...
            case "permissions":
                return (<OnboardingPermissions splitTransitionActive={leaving} onContinue={nextSplitStep}/>);
            case "extension":
                return (<OnboardingExtensionPhase splitTransitionActive={leaving} onContinue={nextSplitStep}/>);
            case "browser":
                return (<OnboardingBrowserPhase availableProfiles={discovery.availableProfiles} browserEnabled={discovery.browserEnabled} categoryStates={discovery.categoryStates} platform={platform} selectedBrowser={discovery.selectedBrowser} selectedProfile={discovery.selectedProfile} showNoneWarning={discovery.showNoneWarning} splitTransitionActive={leaving} onContinue={discovery.confirmDiscovery} onSelectBrowser={discovery.selectBrowser} onSelectProfile={discovery.setSelectedProfile} onToggleBrowser={discovery.toggleBrowser} onToggleCategory={discovery.toggleCategory}/>);
            case "theme":
                return (<OnboardingThemePhase colorMode={appearance.colorMode} gradientColor={appearance.gradientColor} gradientMode={appearance.gradientMode} sortedThemes={appearance.sortedThemes} splitTransitionActive={leaving} themeId={appearance.themeId} onContinue={nextSplitStep} onSelectColorMode={appearance.setColorMode} onSelectGradientColor={appearance.setGradientColor} onSelectGradientMode={appearance.setGradientMode} onSelectTheme={appearance.selectTheme} onThemePreviewEnter={appearance.previewTheme} onThemePreviewLeave={appearance.cancelThemePreview}/>);
            case "personality":
    ...
    return (<div className={`onboarding-dialogue ${isSplit ? "onboarding-dialogue--split" : ""}`} data-phase={phase} data-leaving={leaving} style={{ display: isComplete ? "none" : undefined }}>
      {phase === "intro" && (<div className="onboarding-moment onboarding-moment--ripple" data-active={rippleActive}>
      ...
      {isSplit && (<>
          {phase === "browser" ? (<OnboardingMockWindows activeWindowId={discovery.activeMockId} stageState="current"/>) : null}
          <div className="onboarding-split-right">
            <div className="onboarding-split-stage" data-phase={phase} key={phase}>
              {phaseAct ? (<div className="onboarding-act-label">
                  {t(ACT_LABEL_KEYS[phaseAct])}
                </div>) : null}
              {STEP_TITLE_KEYS[phase] ? (<div className="onboarding-split-title">
                  {t(STEP_TITLE_KEYS[phase])}
                </div>) : null}
          ...
          <div className="onboarding-progress" aria-hidden="true">
            {progressGroups.map((group) => (<div className="onboarding-progress__group" key={group.act}>
                {group.steps.map((step) => {
                    const stepIndex = SPLIT_STEP_ORDER.indexOf(step);
                    const state = step === phase
                        ? "current"
                        : stepIndex < splitStepIndex
                            ? "done"
                            : "todo";
                    return (<span key={step} className="onboarding-progress__dot" data-state={state}/>);
                })}
              </div>))}
          </div>

          <div className="onboarding-phase-nav">
            <button type="button" className="onboarding-phase-nav-btn onboarding-phase-nav-btn--prev" disabled={!canGoPrev || leaving} onClick={prevSplitStep} aria-label={t("onboarding.previousStep")}>
              <ChevronLeft size={14}/>
            </button>
            {canReturnNext ? (<button type="button" className="onboarding-phase-nav-btn onboarding-phase-nav-btn--next" disabled={leaving} onClick={nextSplitStep} aria-label={t("onboarding.nextStep")}>
