...
import { extractOutput, saveOutputToStella, } from "./media-store";
import { openDisplayPayloadTab } from "@/features/workspace-display/open-payload";
import { showToast } from "@/ui/toast";
import { friendlyImageGenerationFailure } from "./media-error-copy";
import { capInMemory, failedNotifiedJobs, markMediaJobMaterialized, materializedJobs, persistFailedNotifiedJobs, publishMaterializedMediaPayload, } from "./media-materializer-state";
export { markMediaJobMaterialized, publishMaterializedMediaPayload, useMaterializedMediaPayload, useMaterializedMediaPayloadSnapshot, } from "./media-materializer-state";
const toMediaAsset = (output) => {
    switch (output.kind) {
        case "image": {
            const filePaths = output.localPaths?.filter((p) => typeof p === "string" && p.length > 0);
 ...
 */
export const useMediaMaterializer = ({ onMaterialized, suppress = false, }) => {
    ...
    const jobs = useQuery(api.media_jobs.listSucceededSince, hasConnectedAccount ? { since: bootSince, limit: 50 } : "skip");
    const failedJobs = useQuery(api.media_jobs.listFailedSince, hasConnectedAccount ? { since: bootSince, limit: 50 } : "skip");
    useEffect(() => {
        if (!jobs || jobs.length === 0)
            return;
        // Process oldest-first so multiple completions in one tick land in the
        // right order in the sidebar.
        const ordered = [...jobs].sort((a, b) => (a.completedAt ?? a.updatedAt) - (b.completedAt ?? b.updatedAt));
        for (const job of ordered) {
            if (materializedJobs.has(job.jobId))
                continue;
            if (inFlightRef.current.has(job.jobId))
                continue;
    ...
    useEffect(() => {
        if (!failedJobs || failedJobs.length === 0)
            return;
        const ordered = [...failedJobs].sort((a, b) => (a.completedAt ?? a.updatedAt) - (b.completedAt ?? b.updatedAt));
        for (const job of ordered) {
            if (!IMAGE_CAPABILITIES.has(job.capability))
                continue;
            if (failedNotifiedJobs.has(job.jobId))
                continue;
