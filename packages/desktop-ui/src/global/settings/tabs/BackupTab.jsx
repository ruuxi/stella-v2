...
function formatBackupTimestamp(timestamp, neverLabel) {
    if (!timestamp) {
        return neverLabel;
    }
...
export function BackupTab() {
    ...
    const isBackupUpgradeRequired = hasConnectedAccount &&
        billingStatus !== undefined &&
        billingStatus.plan === "free";
    const loadBackupState = useCallback(async () => {
        const systemApi = window.electronAPI?.system;
        if (!systemApi?.getLocalSyncMode ||
    ...
    useEffect(() => {
        ...
        void (async () => {
            try {
                ...
                if (!cancelled) {
                    setBackupError(getSettingsErrorMessage(error, t("settings.backup.errors.load")));
                    setRemoteBackups([]);
                }
            }
            finally {
    ...
    const handleSyncModeChange = useCallback(async (value) => {
        ...
        try {
            ...
            setBackupError(getSettingsErrorMessage(error, t("settings.backup.errors.updateMode")));
        }
        finally {
    ...
    const handleBackupNow = useCallback(async () => {
        ...
        try {
            ...
            const message = getSettingsErrorMessage(error, t("settings.backup.errors.startBackup"));
    ...
    const handleRestoreBackup = useCallback(async (snapshotId) => {
        const systemApi = window.electronAPI?.system;
        ...
        try {
            ...
            const message = getSettingsErrorMessage(error, t("settings.backup.errors.restore"));
            setBackupError(message);
            showToast({
    ...
    return (<div className="settings-tab-content">
      <div className="settings-card">
        ...
        {backupError ? (<p className="settings-card-desc settings-card-desc--error" role="alert">
            {backupError}
          </p>) : null}
        {isBackupUpgradeRequired ? (<p className="settings-card-desc">
        ...
        <div className="settings-row">
          <div className="settings-row-info">
            ...
            time: formatBackupTimestamp(backupStatus?.lastSuccessAt, t("settings.backup.never")),
        })}
            </div>
            <div className="settings-row-sublabel">
              {t("settings.backup.automatic.lastRemote", {
            time: formatBackupTimestamp(backupStatus?.lastRemoteSuccessAt, t("settings.backup.never")),
        })}
            </div>
            {backupStatus?.lastRemoteError ? (<div className="settings-row-sublabel">
          ...
          <div className="settings-row-control">
            <Select className="settings-runtime-select" value={syncMode} onValueChange={(value) => void handleSyncModeChange(value)} disabled={!backupLoaded || isSavingSyncMode || isBillingStatusLoading} aria-label={t("settings.backup.automatic.ariaLabel")} options={[
        ...
        <div className="settings-row">
          ...
          <div className="settings-row-control">
            <Button type="button" variant="ghost" className="pill-btn" onClick={() => void handleBackupNow()} disabled={!backupLoaded || isRunningBackup || Boolean(restoringSnapshotId)}>
              {isRunningBackup
        ...
        {hasConnectedAccount && remoteBackups.length === 0 ? (<div className="settings-row">
            <div className="settings-row-info">
              <div className="settings-row-sublabel">
        ...
        {hasConnectedAccount
            ? remoteBackups.map((backup) => (<div key={backup.snapshotId} className="settings-row">
                <div className="settings-row-info">
                  <div className="settings-row-label">
                    {formatBackupTimestamp(backup.createdAt, t("settings.backup.never"))}
                    {backup.isLatest
                ...
                <div className="settings-row-control">
                  <Button type="button" variant="ghost" className="pill-btn" onClick={() => void handleRestoreBackup(backup.snapshotId)} disabled={isRunningBackup ||
