...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @stella/ai <command> [provider]
...
Examples:
  npx @stella/ai login                # interactive provider selection
  npx @stella/ai login anthropic      # login to specific provider
  npx @stella/ai list                 # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @stella/ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @stella/ai --help' for usage`);
	process.exit(1);
}
