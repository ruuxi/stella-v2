...
export const readyMediaArtifactSize = async (filePath) => {
    try {
...
const cleanupStalePartials = async (filePath) => {
...
export const materializeMediaArtifact = async (args) => {
    ...
    const timeout = args.producerTimeoutMs
        ? setTimeout(() => timeoutController.abort(new Error("Media artifact download timed out.")), args.producerTimeoutMs)
        : null;
    ...
    try {
        ...
        let rejectProducerAbort;
        const producerAborted = new Promise((_resolve, reject) => {
        ...
        if (producerSignal.aborted)
            throw abortError(producerSignal);
        if (bytes.length === 0)
            throw new Error("Media artifact was empty.");
