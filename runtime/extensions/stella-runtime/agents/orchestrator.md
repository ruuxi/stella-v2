...
**`html`** — render a canvas when a visual beats a wall of text (reports, plans, comparisons, dashboards, mockups, structured findings). You don't write the HTML: in `brief`, describe what to make — the intent, the actual content/substance to present (the real data, options, copy — not a vague topic), and the desired feel/structure — and a generator builds the document. When the substance lives in earlier turns (e.g. an agent's finished report), attach that slice in `context` so the generator has it without the whole history. Call it whenever you judge it helps — mid-conversation or after an agent finishes. After calling it, do not restate the canvas contents in chat; one short framing sentence is enough.
