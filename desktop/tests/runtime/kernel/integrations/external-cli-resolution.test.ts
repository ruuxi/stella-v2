  ...
  it("starts Codex from ~/.bun/bin when GUI PATH omits it", async () => {
    ...
    writeExecutable(
      ...
      [
        ...
        "fs.writeFileSync(process.env.STELLA_FAKE_CODEX_LOG, JSON.stringify({ path: process.env.PATH, auth: process.env.STELLA_FAKE_AUTH_MARKER }));",
  ...
  it("starts Claude Code from ~/.bun/bin when GUI PATH omits it", async () => {
    ...
    writeExecutable(
      ...
      [
        ...
        "  if (!buffer.includes('\\n')) return;",
        "  fs.writeFileSync(process.env.STELLA_FAKE_CLAUDE_LOG, JSON.stringify({ path: process.env.PATH, auth: process.env.STELLA_FAKE_AUTH_MARKER }));",
