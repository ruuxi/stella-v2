 ...
 * below (unknown frames are dropped, malformed records never reach the store).
 *
...
/**
 * Decodes one journal record. A record that does not typecheck is dropped
 * rather than rendered: the seq still counts (the caller advances its cursor
 * on the raw seq) so one unreadable row can never wedge the stream.
 */
export const decodeRecord = (value: unknown): JournalRecord | null => {
