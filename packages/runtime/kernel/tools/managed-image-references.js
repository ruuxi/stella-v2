...
import { MAX_IMAGE_REFERENCE_BYTES, readAuthorizedImageReference, } from "./image-reference-policy.js";
...
export const MAX_MANAGED_IMAGE_REFERENCE_SERIALIZED_CHARS = Math.ceil(MAX_MANAGED_IMAGE_REFERENCE_TOTAL_BYTES / 3) * 4 +
...
const dataUri = (mimeType, bytes) => `data:${mimeType};base64,${Buffer.from(bytes).toString("base64")}`;
const validatedDataUriBytes = async (value) => {
...
export const normalizeManagedImageReferenceBytes = async (bytes, mimeType, maxBytes = MAX_MANAGED_IMAGE_REFERENCE_ITEM_BYTES) => {
    ...
    try {
        ...
        for (const edge of edgeCandidates) {
            ...
            const resized = width === originalWidth && height === originalHeight
                ? null
                : photon.resize(original, width, height, photon.SamplingFilter.Lanczos3);
            ...
            try {
                for (const quality of qualities) {
                    ...
                    if (!verified || verified.mimeType !== "image/jpeg")
                        continue;
                    return {
    ...
    throw new Error(`managed reference could not be normalized below ${maxBytes} bytes`);
};
export const prepareManagedImageReferences = async (args) => {
    ...
    if (itemCount > MAX_MANAGED_IMAGE_REFERENCE_ITEMS) {
        throw new Error(`image_gen accepts at most ${MAX_MANAGED_IMAGE_REFERENCE_ITEMS} reference images`);
    }
    const inlineCount = args.paths.length +
    ...
    const out = [];
    const appendInline = async (bytes, mimeType) => {
        const itemBudget = Math.min(MAX_MANAGED_IMAGE_REFERENCE_ITEM_BYTES, Math.floor(remainingBytes / Math.max(1, remainingInline)));
        const prepared = await normalizeManagedImageReferenceBytes(bytes, mimeType, itemBudget);
        remainingInline -= 1;
    ...
    for (const filePath of args.paths) {
        const reference = await readAuthorizedImageReference(filePath, args.context);
    ...
    for (const url of args.urls) {
        ...
        if (url.length > MAX_MANAGED_IMAGE_REMOTE_URL_CHARS) {
            throw new Error(`managed reference URL exceeds ${MAX_MANAGED_IMAGE_REMOTE_URL_CHARS} characters`);
        }
        out.push(url);
    }
    return out;
