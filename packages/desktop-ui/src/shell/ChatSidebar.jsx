...
import { ComposerContextRow, useComposerContextSuggestions, } from "@/app/chat/ComposerContextRow";
...
import { ComposerMicButton, ComposerSubmitButton, ComposerStopButton, ComposerTextarea, } from "@/features/chat/ComposerPrimitives";
import { useDictation } from "@/features/dictation/hooks/use-dictation";
import { DictationRecordingBar } from "@/features/dictation/components/DictationRecordingBar";
import { attachComposerAppSelectionContext, deriveComposerState, hasAttachedComposerChips, } from "@/features/chat/composer-context";
...
export function ChatPanelTab({ openRequest, wideLayout = false, messages, conversationId, isStreaming, isStreamingResponseText, runtimeStatusText, activeToolCallId, activeToolName, latestCompletedTool, hasToolActivity, isToolActive, pendingUserMessageId, queuedUserMessages, removeQueuedUserMessage, hasOlderMessages, isLoadingOlder, isInitialLoading, onLoadOlder, onSend, onStop, }) {
    ...
    const { message: inputText, setMessage: setInputText, messageRef: inputTextRef, } = useComposerMessageState();
    ...
    const [surfaceActive, setSurfaceActive] = useState(() => typeof document === "undefined" ||
    ...
    const agentModelConfigByThread = useAgentModelConfigs(chatRuntime?.conversation.tasks ?? []);
    ...
    const sidebarScrollApi = useMemo(() => ({
    ...
    const contextSuggestions = useComposerContextSuggestions(surfaceActive, chatContext, setChatContext);
    ...
    const submitFromDictationRef = useRef(() => { });
    const attachAnnotation = useCallback((selection) => {
    ...
    const handleCancelQueued = useCallback((message) => {
    ...
    return (<div className={`chat-panel-tab chat-panel-tab--sidebar${wideLayout ? " chat-panel-tab--wide" : ""}`} {...dropHandlers}>
      <div className={wideLayout
            ? "full-body-row chat-panel-tab__row"
            : "chat-panel-tab__body"}>
        <div className="chat-sidebar-inner">
          <div className="chat-sidebar-main">
            <CompactConversationSurface className="chat-sidebar-messages" variant="sidebar" scroll={sidebarScrollApi} messages={messages} conversationId={conversationId} agentModelConfigByThread={agentModelConfigByThread} isStreaming={isStreaming} runtimeStatusText={runtimeStatusText} pendingUserMessageId={pendingUserMessageId} queuedUserMessages={queuedUserMessages} onCancelQueued={removeQueuedUserMessage ? handleCancelQueued : undefined} indicator={indicatorProps} hasOlderMessages={hasOlderMessages} isLoadingOlder={isLoadingOlder} isLoadingHistory={isInitialLoading} contentContainerStyle={wideLayout ? WIDE_PANEL_CONTENT_STYLE : SIDEBAR_CONTENT_STYLE} estimatedItemSize={wideLayout ? 140 : undefined}/>

            <div className="chat-sidebar-composer">
              <ConnectorConnectCard compact conversationId={conversationId}/>
              <ComposerLeadRow replyPeek={assistantReplyPeek.visible
            ? {
                text: assistantReplyPeek.previewText,
                onJumpToBottom: () => sidebarScroll.scrollToBottom("smooth"),
                onDismiss: assistantReplyPeek.dismiss,
            }
            : null} showActivityPill={showActivityPill}/>

              <div ref={shellRef} className={`chat-sidebar-shell${isDragOver ? " chat-sidebar-shell--drag-over" : ""}`}>
                <div ref={shellContentRef} className="chat-sidebar-shell-content">
                  {hasAttachedComposerChips(chatContext, selectedText) && (<div className="composer-attached-strip composer-attached-strip--compact">
                      <ComposerContextRow variant="compact" chatContext={chatContext} selectedText={selectedText} setChatContext={setChatContext} setSelectedText={setSelectedText} onPreviewScreenshot={setPreviewScreenshotIndex}/>
                    </div>)}
                  <form ref={formRef} data-composer-context-menu="native" className={`chat-sidebar-form${formExpanded ? " expanded" : ""}`} onSubmit={(event) => {
