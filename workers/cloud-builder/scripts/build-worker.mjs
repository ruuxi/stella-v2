...
export const workerBuildDirectory = path.join(
  workerRoot,
  ".wrangler",
  "worker-build",
);
...
export const buildWorker = async ({ outdir = workerBuildDirectory } = {}) => {
  ...
  const modules = Object.entries(result.metafile.outputs).filter(([file]) =>
    file.endsWith(".js"),
  );
  ...
  const relativeOutput = (file) =>
    path
      .relative(outdir, path.resolve(workerRoot, file))
      .replaceAll(path.sep, "/");
  const manifest = {
    ...
    modules: modules.map(([file, output]) => ({
      ...
      imports: output.imports
        ...
        .map((value) => ({
          file: relativeOutput(value.path),
          dynamic: value.kind === "dynamic-import",
        })),
    })),
    totalBytes: modules.reduce((sum, [, output]) => sum + output.bytes, 0),
    eagerBytes: [...eager].reduce(
      (sum, file) => sum + (byPath.get(file)?.bytes ?? 0),
      0,
    ),
  };
    ...
    await Promise.all(
      result.outputFiles.map(async (file) => {
        ...
        if (relative.startsWith("..") || path.isAbsolute(relative))
          throw new Error("Worker output escaped its build directory.");
