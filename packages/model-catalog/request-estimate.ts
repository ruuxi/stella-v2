import type { ManagedModelAudience, ModelConfig } from "./model";
import {
  resolveStellaModelConfigForSelection,
  STELLA_DEFAULT_MODEL,
} from "./aliases";

/** Raw relay request body, before any provider-specific shaping. */
export type StellaRequestBody = Record<string, unknown>;

/** Pre-flight token reservation derived from the raw request body. */
...
export function downgradeUnsupportedRequestImages(
  ...
  if (managedModelSupportsImageInput(resolvedModel)) {
    return requestBody;
  }
