...
/** The `MODEL_GATEWAY_CONTROL` entrypoint as OwnerGate and the Orchestrator call it. */
export type ModelGatewayControl = ManagedRequestControl & {
...
export const fetchWithManagedCancellation = async (args: {
  ...
  const cancel = (): void => {
    ...
    cancelWork ??= (async () => {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          await args.control.cancelManagedRequest({
  ...
  try {
    ...
    cleanup();
    void responseWork
      .then((response) => response.body?.cancel(error))
      .catch(() => undefined);
    throw error;
  }
};
