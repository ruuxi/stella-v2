...
export function NewChatDialog({ open, onOpenChange, onSelectFriend, onCreateGroup, }) {
    ...
    const toggleSelection = useCallback((ownerId) => {
        setSelectedIds((prev) => {
    ...
    const handleCreateGroup = useCallback(async () => {
        ...
        const didCreateGroup = await onCreateGroup(groupName.trim() || "Group", [...selectedIds]);
    ...
    const handleSelectFriend = useCallback(async (otherOwnerId) => {
    ...
    if (mode === "group") {
        ...
        <DialogContent fit className="friends-dialog-content">
          ...
          <DialogBody className="friends-dialog-body">
            ...
            <TextField label="Group name" hideLabel placeholder="Group name (optional)" value={groupName} onChange={(e) => setGroupName(e.target.value)}/>
            ...
            <section className="friends-section">
              ...
              <div className="friends-list">
                ...
                return (<button key={friend.profile.ownerId} type="button" className="new-chat-item" data-selected={isSelected ? "true" : undefined} onClick={() => toggleSelection(friend.profile.ownerId)} disabled={isCreatingGroup}>
                      <Avatar fallback={friend.profile.username} src={friend.profile.avatarUrl} size="normal"/>
                      <span className="new-chat-item-name">
                        @{friend.profile.username}
                      </span>
                      {isSelected ? (<Check size={16} className="new-chat-item-check" aria-hidden/>) : null}
                    </button>);
            })}
              </div>
            </section>
            ...
            <div className="friends-dialog-footer">
              <button type="button" className="pill-btn pill-btn--lg" onClick={() => setMode("pick")} disabled={isCreatingGroup}>
                Back
              </button>
              <button type="button" className="pill-btn pill-btn--primary pill-btn--lg" disabled={selectedIds.size === 0 || isCreatingGroup} onClick={() => void handleCreateGroup()}>
      ...
      <DialogContent fit className="friends-dialog-content">
        ...
        <DialogBody className="friends-dialog-body">
          ...
          <div className="new-chat-list">
            <button type="button" className="new-chat-item" onClick={() => setMode("group")}>
            ...
            {friends.map((friend) => (<button key={friend.profile.ownerId} type="button" className="new-chat-item" disabled={pendingFriendId !== null} onClick={() => void handleSelectFriend(friend.profile.ownerId)}>
                <Avatar fallback={friend.profile.username} src={friend.profile.avatarUrl} size="normal"/>
                <span className="new-chat-item-name">
