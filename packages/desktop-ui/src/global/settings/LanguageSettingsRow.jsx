...
import { useT, useI18n, LOCALE_NATIVE_LABELS } from "@/shared/i18n";
...
export function LanguageSettingsRow() {
    ...
    const localeOptions = useMemo(() => supportedLocales.map((code) => ({
        value: code,
    ...
    return (<div className="settings-card">
      ...
      <div className="settings-row">
        ...
        <div className="settings-row-control">
          <Select className="settings-runtime-select" value={locale} onValueChange={(next) => setLocale(next)} options={localeOptions} aria-label={t("common.language")}/>
        </div>
      </div>
    </div>);
}
