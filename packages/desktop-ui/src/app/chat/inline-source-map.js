...
const INLINE_MAP_RE = /\/\/[#@]\s*sourceMappingURL=data:application\/json;(?:charset=[^;]+;)?base64,([A-Za-z0-9+/=]+)/;
/**
...
const parseInlineMap = (moduleText, moduleUrl) => {
 ...
 */
export const originalPositionFor = async (moduleUrl, generatedLine, generatedColumn) => {
    ...
    for (const segment of segments) {
        if (segment.generatedColumn > targetColumn)
            break;
        match = segment;
    }
    match ??= segments[0];
...
export const resetInlineSourceMapCache = () => {
    mapCache.clear();
};
