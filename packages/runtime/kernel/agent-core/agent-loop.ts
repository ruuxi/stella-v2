	...
	Effect.gen(function* () {
		...
		for (const toolCall of toolCalls) {
			...
			if (preparation.kind === "immediate") {
				results.push(
					yield* emitToolCallOutcome(emit, toolCall, preparation.result, preparation.isError),
				);
			} else {
				...
				results.push(
					yield* finalizeExecutedToolCall(env, assistantMessage, preparation, executed),
				);
			}
		}
...
> =>
	Effect.gen(function* () {
		...
		for (const running of runningCalls) {
			...
			results.push(
				yield* finalizeExecutedToolCall(env, assistantMessage, running.prepared, executed),
			);
		}
	...
	Effect.scoped(
		Effect.gen(function* () {
			...
			const outcome = yield* Effect.tryPromise({
				...
				Effect.map(
					(result): ExecutedToolCallOutcome => ({ result, isError: false }),
				),
				Effect.catch((error) =>
