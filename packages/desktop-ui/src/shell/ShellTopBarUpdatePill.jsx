...
export const ShellTopBarUpdatePill = () => {
    ...
    return (<div className="shell-topbar-update-pill" data-state={isDownloading
            ? "active"
            : isDownloaded
                ? "downloaded"
                : isError
                    ? "error"
                    : "idle"}>
      <button type="button" className="shell-topbar-update-pill__main" onClick={() => void handleUpdate()} disabled={isDownloading} aria-label={label} title={title}>
        {isDownloaded || isError ? (<RefreshCw className="shell-topbar-update-pill__icon" size={12} strokeWidth={2} aria-hidden/>) : (<Download className="shell-topbar-update-pill__icon" size={12} strokeWidth={2} aria-hidden/>)}
