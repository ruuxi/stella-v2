...
import { loadOlderFeatureEntries, refreshFeatureSnapshot, useStoreSidePanelState, } from "@/features/store/store-side-panel-store";
import { openStoreDisplayTab } from "@/shell/display/default-tabs";
import { sectionCollapseStore, useSectionCollapsed, } from "@/shell/section-collapse-store";
import { EMPTY_ACTIVITY_EXPANSION, activityExpansionStore, } from "@/shell/activity-expansion-store";
import { EMPTY_FIRST_SEEN_ORDER, activityRowKey, flattenActivityTasks, getCompactActivityStatusText, getActivityRowCompletedAtMs, getActivityRowSearchText, getActivityRowStatus, groupActivityTasks, orderByFirstSeen, getTaskAgentUpdates, summarizeCompactActivity, updateSeenRunningTaskIds, } from "@/features/chat/lib/event-transforms";
import { deriveConversationFiles, } from "@/features/workspace-display/derive-conversation-files";
import { agentFilesSignature, deriveAgentFilesMap, mergeAgentFileEvents, } from "@/features/workspace-display/agent-files";
...
const activityRowText = (row) => getActivityRowSearchText(row);
 ...
 */
const useActivityRowMotionProps = (orderIndex) => {
...
function WorkspaceSection({ title, sectionId, hideHeader = false, children, }) {
    const collapsed = useSectionCollapsed(sectionId);
    const sectionCollapsed = hideHeader ? false : collapsed;
    return (<section className="chat-workspace-strip__section">
      {!hideHeader ? (<header className="chat-workspace-strip__section-header">
          <button type="button" className="chat-workspace-strip__section-toggle" onClick={() => sectionCollapseStore.toggle(sectionId)} aria-expanded={!sectionCollapsed}>
            {title}
          </button>
        </header>) : null}
      <div className="chat-workspace-strip__section-collapse" data-collapsed={sectionCollapsed ? "true" : undefined}>
...
const compactTaskTooltip = (task) => {
    ...
    const detail = (selectLatestAgentAssistantMessage(task.assistantMessages) ?? "").replace(/\s+/g, " ");
    const clipped = detail.length > 120 ? `${detail.slice(0, 117)}…` : detail;
    return `${label} · ${compactTaskState(task)}${clipped ? ` — ${clipped}` : ""}`;
};
const taskSourceLabel = (task) => task.source === "claude-native" ? "Claude · read-only" : undefined;
const CompactChildState = memo(function CompactChildState({ summary, prioritizeFailure, }) {
    const statusText = getCompactActivityStatusText(summary, prioritizeFailure);
    return (<>
      {summary.usesProgressBar ? (<span className="chat-workspace-strip__compact-progress" aria-hidden="true">
          <span className="chat-workspace-strip__compact-bar">
            {summary.completedCount > 0 ? (<span className="chat-workspace-strip__compact-bar-segment chat-workspace-strip__compact-bar-segment--done" style={{ flexGrow: summary.completedCount }}/>) : null}
            {summary.runningCount > 0 ? (<span className="chat-workspace-strip__compact-bar-segment chat-workspace-strip__compact-bar-segment--running" style={{ flexGrow: summary.runningCount }}/>) : null}
            {summary.errorCount > 0 ? (<span className="chat-workspace-strip__compact-bar-segment chat-workspace-strip__compact-bar-segment--error" style={{ flexGrow: summary.errorCount }}/>) : null}
            {summary.canceledCount > 0 ? (<span className="chat-workspace-strip__compact-bar-segment chat-workspace-strip__compact-bar-segment--queued" style={{ flexGrow: summary.canceledCount }}/>) : null}
          </span>
          <span className="chat-workspace-strip__compact-progress-count">
            {summary.completedCount}/{summary.totalCount}
          </span>
        </span>) : (<span className="chat-workspace-strip__compact-cells" aria-hidden="true">
          {summary.tasks.map((task, index) => (<span key={task.id} className={`chat-workspace-strip__compact-cell chat-workspace-strip__compact-cell--${task.status}`}
            // Stable task identity keeps state changes on this final grid
            // slot; only the local paint fades/pulses, never its position.
            style={{ "--cell-order": index }} title={compactTaskTooltip(task)}/>))}
        </span>)}
      <span className="chat-workspace-strip__compact-status" data-failure={prioritizeFailure && summary.errorCount > 0 ? "true" : undefined}>
        {statusText}
      </span>
    </>);
});
function TaskStatusIcon({ status }) {
    const suffix = status === "completed" ? "done" : status === "error" ? "error" : status;
    return (<AgentLifecycleStatusIcon status={status} className={`chat-workspace-strip__task-icon chat-workspace-strip__task-icon--${suffix}`} size={15} strokeWidth={2} aria-hidden="true"/>);
}
const TaskRow = memo(function TaskRow({ task, expanded, onToggle, onSelect, files, onOpenFile, orderIndex, metaText, childContent, compactChildren, compactFailurePriority = false, isTopLevel, }) {
    ...
    const agentUpdates = task.status === "running" ? getTaskAgentUpdates(task) : EMPTY_UPDATES;
    ...
    const compactTasks = useMemo(() => (compactChildren ? flattenActivityTasks(compactChildren) : undefined), [compactChildren]);
    const compactSummary = useMemo(() => (compactTasks ? summarizeCompactActivity(compactTasks) : undefined), [compactTasks]);
    const compactMotionActive = useContinuousAnimationGate({
        active: task.status === "running" && (compactSummary?.runningCount ?? 0) > 0,
        elementRef: rowRef,
    });
    const hasChildContent = Boolean(childContent);
    const hasDetail = hasChildContent || Boolean(parentDetail) || hasAgentUpdates || hasFiles;
    const detailOpen = expanded && hasDetail;
    return (<motion.li {...motionProps} className="chat-workspace-strip__task-row" ref={rowRef} data-status={task.status} data-source={task.source} data-read-only={task.readOnly ? "true" : undefined} data-expanded={expanded ? "true" : undefined} title={label} data-continuous-animation={compactMotionActive ? "true" : undefined}>
      <div className="chat-workspace-strip__task-row-head">
        <button type="button" className="chat-workspace-strip__task-button" data-compact={compactSummary ? "true" : undefined} onClick={() => onToggle(task.id, !expanded)} aria-expanded={expanded} aria-label={`${label || "Activity"} — ${expanded ? "collapse" : "expand"}`}>
          {compactSummary ? (<>
              <span className="chat-workspace-strip__compact-main">
                <span className="chat-workspace-strip__task-icon-wrap" aria-hidden="true">
                  <TaskStatusIcon status={task.status}/>
                </span>
                <span className="chat-workspace-strip__task-label">
                  <ActivityTaskShimmer task={task} text={label} isTopLevel={isTopLevel}/>
                </span>
                {sourceLabel ? (<span className="chat-workspace-strip__task-source">
                    {sourceLabel}
                  </span>) : null}
              </span>
              <CompactChildState summary={compactSummary} prioritizeFailure={compactFailurePriority && !expanded}/>
            </>) : (<>
              <span className="chat-workspace-strip__task-icon-wrap" aria-hidden="true">
                <TaskStatusIcon status={task.status}/>
              </span>
              ...
              {metaText ? (<span className="chat-workspace-strip__row-meta chat-workspace-strip__group-status">
                  {metaText}
                </span>) : null}
            </>)}
        </button>
        <button type="button" className="chat-workspace-strip__task-attach" onClick={() => onSelect(task)} aria-label={task.source === "claude-native"
            ? "View Claude conversation"
            : "View activity"} title={task.source === "claude-native"
            ? "View Claude conversation"
            : "View activity"}>
      ...
      <div className="chat-workspace-strip__task-collapse" data-collapsed={detailOpen ? undefined : "true"} inert={!detailOpen}>
        <div className="chat-workspace-strip__task-collapse-clip">
          {hasDetail ? (<div className="chat-workspace-strip__task-detail">
              {parentDetail ? (<p className="chat-workspace-strip__parent-status">
                  {parentDetail}
                </p>) : null}
              {hasAgentUpdates || hasFiles ? (<div className="chat-workspace-strip__task-rail">
                  {hasAgentUpdates ? (<AgentAssistantUpdates messages={agentUpdates} max={AGENT_UPDATE_CAP}/>) : null}
                  {hasFiles ? (<button type="button" className="chat-workspace-strip__file-button chat-workspace-strip__files-toggle" onClick={() => setFilesExpanded((value) => !value)} aria-expanded={filesExpanded}>
                      <span className="chat-workspace-strip__file-name">
                        {`View ${files.length} ${files.length === 1 ? "file" : "files"}`}
                      </span>
                    </button>) : null}
                  {hasFiles ? (
                ...
                <div className="chat-workspace-strip__task-collapse" data-collapsed={filesExpanded ? undefined : "true"} inert={!filesExpanded}>
                      <div className="chat-workspace-strip__task-collapse-clip">
                        <ul className="chat-workspace-strip__list chat-workspace-strip__task-files">
                          {files.map((file) => (<li key={file.path} className="chat-workspace-strip__row" title={file.path}>
                              <button type="button" className="chat-workspace-strip__file-button" onClick={() => onOpenFile(file)}>
                                <DisplayTabIcon kind={displayTabKindForPayload(file.payload)} size={15}/>
                                <span className="chat-workspace-strip__file-name">
...
const TasksList = memo(function TasksList({ rows, isTaskExpanded, isCompactTaskExpanded, onToggleTask, onSelectTask, agentFiles, onOpenFile, nested = false, }) {
    return (<ul className={`chat-workspace-strip__list chat-workspace-strip__list--tasks${nested ? " chat-workspace-strip__group-members" : ""}`}>
      ...
      <AnimatePresence initial={false}>
        {rows.map((row, index) => row.kind === "task" ? (<TaskRow key={activityRowKey(row)} task={row.task} expanded={isTaskExpanded(row.task)} onToggle={onToggleTask} onSelect={onSelectTask} files={agentFiles.get(row.task.id) ?? EMPTY_FILES} onOpenFile={onOpenFile} orderIndex={index} isTopLevel={!nested}/>) : (<TaskRow key={activityRowKey(row)} task={row.hierarchy.status === row.hierarchy.owner.status
                ? row.hierarchy.owner
                : { ...row.hierarchy.owner, status: row.hierarchy.status }} expanded={isCompactTaskExpanded(row.hierarchy.owner)} onToggle={onToggleTask} onSelect={onSelectTask} files={agentFiles.get(row.hierarchy.owner.id) ?? EMPTY_FILES} onOpenFile={onOpenFile} orderIndex={index} isTopLevel={!nested} compactChildren={row.hierarchy.children} compactFailurePriority={row.hierarchy.owner.status === "running"} childContent={<TasksList rows={row.hierarchy.children} isTaskExpanded={isTaskExpanded} isCompactTaskExpanded={isCompactTaskExpanded} onToggleTask={onToggleTask} onSelectTask={onSelectTask} agentFiles={agentFiles} onOpenFile={onOpenFile} nested/>}/>))}
      </AnimatePresence>
    </ul>);
});
const scheduleEntryToAffectedRef = (entry, conversationId) => ({
    kind: entry.kind,
    id: entry.id,
    conversationId,
    name: entry.name,
...
// usual.
export const WorkspaceSections = memo(function WorkspaceSections({ query = "", variant = "strip", searchMode = "complete", includeUserApps = false, renderEmpty, onNavigate, } = {}) {
    ...
    const { hasOlder: activityHasOlder, isLoadingOlder: activityIsLoadingOlder, loadOlder: loadOlderActivity, } = activity;
    ...
    useEffect(() => {
        if (!searching || quickSearch)
            return;
        if (activityHasOlder && !activityIsLoadingOlder)
            loadOlderActivity();
    }, [
        searching,
        quickSearch,
    ...
    useEffect(() => {
        if (!searching || quickSearch)
            return;
        if (filesHaveOlder && !filesAreLoadingOlder)
            loadOlderFiles();
    }, [
        searching,
        quickSearch,
    ...
    useEffect(() => {
        ...
        if (total == null || storeState.olderLoading)
            return;
        const loaded = (storeState.snapshot?.items.length ?? 0) + storeState.olderEntries.length;
        if (loaded < total)
            void loadOlderFeatureEntries();
    }, [
        searching,
        quickSearch,
    ...
    const [openScheduleEntry, setOpenScheduleEntry] = useState(null);
    ...
    const seenRunningTasksRef = useRef(new Set());
    ...
    const isTaskExpanded = useCallback((task) => {
        const override = expandOverrides.get(task.id);
        return (override ??
            (task.status === "running" || seenRunningTasksRef.current.has(task.id)));
    }, [expandOverrides]);
    const isCompactTaskExpanded = useCallback((task) => expandOverrides.get(task.id) ?? false, [expandOverrides]);
    const toggleTask = useCallback((taskId, nextExpanded) => {
    ...
    const [seededConversationId, setSeededConversationId] = useState(undefined);
    ...
    useMemo(() => {
        ...
        if (allTasks.length === 0)
            return;
        seenRunningTasksRef.current = updateSeenRunningTaskIds(seenRunningTasksRef.current, allTasks);
    }, [allTasks]);
    ...
    useEffect(() => {
        if (!conversationId || allTasks.length === 0)
            return;
    ...
    const activityRows = useMemo(() => matchingActivityKeys
        ? groupedRows.filter((row) => matchingActivityKeys.has(activityRowKey(row)))
        : groupedRows, [groupedRows, matchingActivityKeys]);
    ...
    const runningOrderRef = useRef(EMPTY_FIRST_SEEN_ORDER);
    const runningRows = useMemo(() => {
        const running = groupedRows.filter((row) => getActivityRowStatus(row) === "running");
        const { ordered, state } = orderByFirstSeen(running, activityRowKey, runningOrderRef.current, true);
    ...
    const agentFileEvents = useMemo(() => mergeAgentFileEvents(activity.activities, filesFeed.files), [activity.activities, filesFeed.files]);
    const agentFilesCacheRef = useRef(null);
    ...
    // index built here.
    const searchableFiles = useMemo(() => deriveConversationFiles(agentFileEvents).map((entry) => ({
        entry,
    ...
    const storeItems = useMemo(() => {
        ...
        const source = searching && !quickSearch
    ...
    const visibleUserApps = useMemo(() => includeUserApps && searching
        ? listUserApps(userApps, query, "recent").slice(0, 8)
        : [], [includeUserApps, query, searching, userApps]);
    const nowMs = Date.now();
    const visibleDoneRows = useMemo(() => doneRows.slice(0, Math.max(0, caps.activity - runningRows.length)), [doneRows, caps.activity, runningRows.length]);
    ...
    const visibleFiles = useMemo(() => filteredFiles.slice(0, caps.files), [caps.files, filteredFiles]);
    const upNext = useMemo(() => filteredSchedules.slice(0, caps.schedule), [filteredSchedules, caps.schedule]);
    ...
    const dialogAffected = useMemo(() => {
        if (!openScheduleEntry || !conversationId)
            return [];
    ...
    return (<>
      <div className="chat-workspace-strip__panel">
        {hasActivity && (<WorkspaceSection title="Activity" sectionId="activity" hideHeader>
            <TasksList rows={visibleActivityRows} isTaskExpanded={isTaskExpanded} isCompactTaskExpanded={isCompactTaskExpanded} onToggleTask={toggleTask} onSelectTask={handleSelectTask} agentFiles={agentFiles} onOpenFile={handleOpenFile}/>
          </WorkspaceSection>)}

        {hasFiles && (<WorkspaceSection title="Files" sectionId="files">
            <ul className="chat-workspace-strip__list">
              {visibleFiles.map((file) => (<li key={file.path} className="chat-workspace-strip__row" title={file.path}>
                  <button type="button" className="chat-workspace-strip__file-button" onClick={() => handleOpenFile(file)}>
                    <DisplayTabIcon kind={displayTabKindForPayload(file.payload)} size={15}/>
                    <span className="chat-workspace-strip__file-name">
                      {basenameOf(file.path)}
                    </span>
        ...
        {hasSchedule && (<WorkspaceSection title="Schedule" sectionId="schedule">
            <ul className="chat-workspace-strip__list">
              {upNext.map((entry) => (<li key={`${entry.kind}:${entry.id}`} className="chat-workspace-strip__row">
                  <button type="button" className="chat-workspace-strip__file-button" onClick={() => {
        ...
        {hasUserApps && (<WorkspaceSection title="Apps" sectionId="user-apps">
            <ul className="chat-workspace-strip__list">
              {visibleUserApps.map((app) => (<li key={app.slug} className="chat-workspace-strip__row">
                  <button type="button" className="chat-workspace-strip__file-button" onClick={() => {
        ...
        {hasStore && (<WorkspaceSection title="Store" sectionId="store">
            <ul className="chat-workspace-strip__list">
              {storePreview.map((item) => (<li key={item.featureId ?? item.name} className="chat-workspace-strip__row" title={item.name}>
                  <button type="button" className="chat-workspace-strip__file-button" onClick={() => {
                    openStoreDisplayTab();
                    onNavigate?.();
                }}>
      ...
      <ScheduleDetailsDialog open={openScheduleEntry !== null} onOpenChange={(next) => {
            if (!next)
                setOpenScheduleEntry(null);
