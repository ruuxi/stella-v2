...
import {
  ...
  type PreparedHomeContent,
} from "../src/conversation-home-cache.js";
...
function fixture() {
  ...
  const durable = {
    read: () => saved,
    write: (value: ConversationHomeSnapshot) => {
  ...
  const args = {
    ownerId: "owner1",
    metadata: metadata(),
    readContent: async (): Promise<PreparedHomeContent> => {
      reads++;
      return {
        memoryPreference: args.metadata.memory.preference,
        memoryDocuments: args.metadata.memory.preference.memoryEnabled
          ? [
              {
                name: "profile.md",
                displayPath: "profile.md",
                content: "favorite color amber",
              },
            ]
          : [],
        personalityOverride: null,
        skillCatalog: args.metadata.skills,
      };
    },
  };
  return {
...
describe("conversation home content cache", () => {
  test("unchanged authoritative versions survive an isolate restart", async () => {
    const f = fixture();
    const c = new ConversationHomeCache(f.durable);
    const first = await c.load(f.args);
    await c.load(f.args);
    expect(await new ConversationHomeCache(f.durable).load(f.args)).toEqual(
      first,
    );
  ...
  test("changed policy, wipe epoch, generation and owner cannot reuse content", async () => {
    ...
    await c.load(f.args);
    f.args.metadata.memory.preference.ownerGeneration = "g2";
    await c.load(f.args);
    f.args.ownerId = "owner2";
    await c.load(f.args);
  ...
  test("changed document versions retain a mismatched durable copy through a failed read", async () => {
    ...
    const c = new ConversationHomeCache(f.durable);
    const first = await c.load(f.args);
    f.args.metadata.memory.personalityHead = {
      documentId: "personality",
      name: "personality.md",
      displayPath: "personality.md",
      kind: "personality",
      source: "test",
      ownerGeneration: "g1",
      memoryEpoch: "e1",
      revision: 2,
      r2Key: "g1/personality/2",
      sizeBytes: 1,
      updatedAt: 2,
    };
    await expect(
  ...
  test("a changed durable key never exposes old content while its replacement is loading", async () => {
    ...
    const slow = new ConversationHomeCache(f.durable).load({
      ...
      readContent: async () => {
        await pending;
        return await f.args.readContent();
      },
    });
  ...
  test("a purge during a storage read cannot resurrect durable or resident content", async () => {
    ...
    await expect(
      c.load({
        ...f.args,
        readContent: async () => {
          const value = await f.args.readContent();
          f.purge();
          return value;
        },
      }),
  ...
  test("oversized content remains usable without exceeding the KV value limit", async () => {
    ...
    const args = {
      ...f.args,
      readContent: async () => ({
        ...(await f.args.readContent()),
        personalityOverride: "x".repeat(100_001),
      }),
    };
    ...
    expect(f.reads()).toBe(1);
    expect(f.saved()).toBeUndefined();
    await new ConversationHomeCache(f.durable).load(args);
    expect(f.reads()).toBe(2);
  });
});
