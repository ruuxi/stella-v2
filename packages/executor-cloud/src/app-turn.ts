...
const appSource = (spec: AppSpec): string => `import React from "react";

const habits = ${JSON.stringify(spec.habits)};
    ...
    <main className="shell">
      ...
      <section className="grid">
        {habits.map((habit, index) => <article className="habit" key={habit.name}>
          ...
          <div className="track"><i style={{width: \`\${habit.progress}%\`}} /></div><small>{habit.progress}%</small>
        </article>)}
      </section>
      <aside><div><p className="eyebrow">Focus for now</p><h2>${spec.focus}</h2></div><button>Begin a 20 minute focus</button></aside>
