...
export function useApplyResumeSnapshot({ dispatch, refs, streaming, }) {
    ...
    return useCallback((args) => {
        dispatch({
            ...
            activeRun: args.activeRun?.uiVisibility === 'hidden' ? null : args.activeRun,
        });
        if (args.conversationId === activeConversationIdRef.current) {
            setPendingUserMessageId(args.activeRun?.uiVisibility === 'hidden'
                ? null
