...
export function ProviderOnlyPicker({ providers, value, onSelect, disabled = false, ariaLabel, }) {
    ...
    const isConnected = useCallback((key) => {
        if (key === STELLA_PROVIDER_KEY)
            return true;
        if (findApiKey(credentials.apiKeys, key))
            return true;
        if (findOauthCredential(credentials.oauthCredentials, key))
            return true;
        return false;
    ...
    const handleSaveKey = useCallback(async (key, label) => {
    ...
    return (<div className="provider-only-picker" role="radiogroup" aria-label={ariaLabel} data-disabled={disabled || undefined}>
      {providers.map((provider) => {
            ...
            return (<div key={provider.key} className="provider-only-row" data-selected={selected || undefined} data-expanded={expanded || undefined}>
            <button type="button" role="radio" aria-checked={selected} className="provider-only-row-main" onClick={() => handleRowClick(provider.key)} disabled={disabled}>
              ...
              <span className="provider-only-row-text">
                ...
                {provider.description ? (<span className="provider-only-row-desc">
            ...
            {expanded ? (<div className="provider-only-row-auth">
                {supportsOAuth ? (<div className="provider-only-row-auth-line">
                ...
                {supportsApiKey ? (<div className="provider-only-row-auth-line">
                    ...
                    <TextField label={`${provider.label} API key`} hideLabel type="password" placeholder={llmEntry?.placeholder ?? "API key"} value={draftKey} onChange={(event) => setDraftKey(event.target.value)} onKeyDown={(event) => {
                        ...
                        }} autoFocus={!supportsOAuth} disabled={disabled}/>
