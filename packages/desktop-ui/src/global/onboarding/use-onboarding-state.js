...
import { clearPostOnboardingHints, seedPostOnboardingHints, } from "./post-onboarding-hints";
...
const writeLocalOnboardingCompleted = (completed) => {
    if (completed) {
...
const SPLIT_PHASE_SET = new Set(SPLIT_STEP_ORDER);
export const readOnboardingPhase = () => {
    const raw = uiState.getItem(ONBOARDING_PHASE_KEY);
    if (!raw)
        return null;
    if (!SPLIT_PHASE_SET.has(raw)) {
        uiState.removeItem(ONBOARDING_PHASE_KEY);
        return null;
    }
    return raw;
};
const writeOnboardingPhase = (phase) => {
...
const getSnapshot = () => durableHydrated ? durableCompleted : readLocalOnboardingCompleted();
...
const hydrateOnboardingCompleted = async () => {
    ...
    hydrationPromise = (async () => {
        ...
        try {
            const persistedCompleted = (await preferencesApi.getOnboardingCompleted()) === true;
...
export function useOnboardingState() {
    ...
    const persistPhase = useCallback((phase) => {
