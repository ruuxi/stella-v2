...
export const MediaTabContent = ({ item }) => {
    ...
    return (<div className="media-tab">
      <div className="media-tab__surface">
        <div className="media-tab__hero">
          <div className="media-tab__hero-bar" onClick={(event) => event.stopPropagation()}>
            <div className="media-tab__hero-bar-top">
              {item.capability ? (<span className="media-tab__hero-cap">
                  {item.capability.replace(/_/g, " ")}
                </span>) : null}
              <div className="media-tab__hero-actions" role="group" aria-label="Item actions">
          ...
          <div className="media-tab__hero-preview">
            <MediaPreviewCard asset={item.asset} inDialog {...(item.prompt ? { prompt: item.prompt } : {})} {...(item.capability ? { capability: item.capability } : {})}/>
          </div>
        </div>
      </div>
    </div>);
