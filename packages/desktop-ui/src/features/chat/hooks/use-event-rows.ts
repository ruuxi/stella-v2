...
type BackgroundWorkDescriptor = NonNullable<
  AssistantRowViewModel["backgroundWork"]
>;

/**
 * Rebuild a background-work descriptor with `removed` threads dropped from
 * every thread-keyed field, mirroring the supersede pass's field surgery.
 * Callers guarantee at least one thread remains.
 */
const omitThreadsFromBackgroundWork = (
  source: BackgroundWorkDescriptor,
  removed: ReadonlySet<string>,
): BackgroundWorkDescriptor => {
  const remainingThreadIds = source.threadIds.filter((id) => !removed.has(id));
  const descriptions = { ...source.descriptions };
  const spawnedAtMs = { ...(source.spawnedAtMs ?? {}) };
  const statusTexts = { ...(source.statusTexts ?? {}) };
  const progressTexts = { ...(source.progressTexts ?? {}) };
  const toolActivities = { ...(source.toolActivities ?? {}) };
  const startEventIdsByThread = { ...source.startEventIdsByThread };
  const attemptGenerationsByThread = {
    ...(source.attemptGenerationsByThread ?? {}),
  };
  const rootRunIdsByThread = { ...source.rootRunIdsByThread };
  const terminalEventIdsByThread = {
    ...(source.terminalEventIdsByThread ?? {}),
  };
  ...
  return {
    ...
    completedThreadIds: source.completedThreadIds.filter(
      (id) => !removed.has(id),
    ),
    ...(source.pausedThreadIds
      ? {
          pausedThreadIds: source.pausedThreadIds.filter(
            (id) => !removed.has(id),
          ),
        }
      : {}),
    ...(source.failedThreadIds
      ? {
          failedThreadIds: source.failedThreadIds.filter(
            (id) => !removed.has(id),
          ),
        }
      : {}),
    ...(source.supersededThreadIds
      ? {
          supersededThreadIds: source.supersededThreadIds.filter(
            (id) => !removed.has(id),
          ),
        }
      : {}),
    ...
    completionSections: (source.completionSections ?? []).filter(
      (section) =>
    ...
    cardId: `agent-activity:${remainingThreadIds
      ...
      .filter(Boolean)
      .join("+")}`,
    followUpThreadIds: (source.followUpThreadIds ?? []).filter(
      (id) => !removed.has(id),
    ),
  };
};
...
/**
 * Reconcile a completed task that still carries its spawn receipt. A finished
 * background occurrence renders as its settled `AgentCompletionCard` — the
 * documented "settled presentation" (richer: produced files + result excerpt)
 * projected at the completion anchor. Its original `BackgroundWorkCard` receipt
 * must not ALSO render for that same occurrence, or one task draws twice: a
 * settled inline receipt PLUS a standalone completion card with the same body.
 * (See `agent-completion.ts`: completion "enriches the existing spawn-anchored
 * card instead of inserting another timeline row"; `AgentCompletionCard`: "the
 * original BackgroundWorkCard slot switches to this settled presentation. It
 * never creates a second completion-time row.")
 *
 * Matched by OCCURRENCE identity (`startEventId`), never by title: a distinct
 * same-title task owns its own start event, and a `send_input` re-run that is
 * running again keeps its live receipt (a different, not-yet-completed
 * occurrence — it is absent from `completedThreadIds`). Only threads that both
 * (a) the lifecycle fold marks completed and (b) have a completion card
 * actually rendered somewhere are dropped, so a completion whose card aged out
 * of the window still shows its settled receipt. Mutates `rows` /
 * `droppedRowIndices` in place, same contract as the dedup pass it follows.
 */
export const suppressReceiptsForRenderedCompletions = (
  rows: EventRowViewModel[],
  droppedRowIndices: Set<number>,
): void => {
  const renderedCompletionStartEventIds = new Set<string>();
  rows.forEach((row, index) => {
    if (droppedRowIndices.has(index)) return;
    if (row.kind !== "assistant" || !row.agentCompletion) return;
    for (const section of row.agentCompletion.sections) {
      if (section.startEventId) {
        renderedCompletionStartEventIds.add(section.startEventId);
      }
    }
  });
  if (renderedCompletionStartEventIds.size === 0) return;

  rows.forEach((row, index) => {
    if (droppedRowIndices.has(index)) return;
    if (row.kind !== "assistant" || !row.backgroundWork) return;
    const backgroundWork = row.backgroundWork;
    const completed = new Set(backgroundWork.completedThreadIds);
    const removed = new Set(
      backgroundWork.threadIds.filter(
        (threadId) =>
          completed.has(threadId) &&
          renderedCompletionStartEventIds.has(
            backgroundWork.startEventIdsByThread[threadId] ?? "",
          ),
      ),
    );
    if (removed.size === 0) return;
    if (removed.size === backgroundWork.threadIds.length) {
      // Every covered thread now renders through its completion card. If the
      // row has nothing else, drop the synthetic receipt row entirely;
      // otherwise just shed the redundant card.
    ...
    rows[index] = {
      ...row,
      backgroundWork: omitThreadsFromBackgroundWork(backgroundWork, removed),
    };
  });
};
...
export function useEventRows(opts: UseEventRowsOptions): UseEventRowsResult {
  ...
  const allRows = useMemo<EventRowViewModel[]>(() => {
    ...
    suppressReceiptsForRenderedCompletions(computed, droppedRowIndices);
