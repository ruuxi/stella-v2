...
type ChatCompletionToolWithCacheControl =
...
> = (
  ...
  (async () => {
    ...
    try {
      ...
      const getContentIndex = (block: typeof currentBlock) =>
          ...
          if (choice?.delta?.tool_calls) {
            for (const toolCall of choice.delta.tool_calls) {
              ...
              if (currentToolCallBlock) {
                ...
                if (
                  currentToolCallBlock.streamIndex === undefined &&
                  streamIndex !== undefined
                ) {
                ...
                if (toolCall.function?.arguments) {
                  ...
                  currentToolCallBlock.partialArgs +=
                    toolCall.function.arguments;
                  currentToolCallBlock.arguments = parseStreamingJson(
                    currentToolCallBlock.partialArgs,
                  );
                }
                stream.push({
...
function addCacheControlToLastTool(
  ...
  const lastTool = tools[
    tools.length - 1
  ] as ChatCompletionToolWithCacheControl;
...
export function getCompat(
  ...
  return {
    ...
    replayReasoningContentField:
      model.compat.replayReasoningContentField ??
      detected.replayReasoningContentField,
