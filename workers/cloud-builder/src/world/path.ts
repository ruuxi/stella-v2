import { WORLD_PATH_LIMIT_BYTES, WORLD_ROOT } from "./types.js";

const pathBytes = (path: string): number => new TextEncoder().encode(path).byteLength;
...
export const normalizeWorldPath = (
  ...
  if (input.startsWith("/") || input.includes("\\") || input.includes("\0") || input.endsWith("/")) {
...
export const worldRelativeToolPath = (value: unknown): string => {
  ...
  if (resolved === WORLD_ROOT) return "";
  const prefix = `${WORLD_ROOT}/`;
...
export const absoluteWorldPath = (path: string): string =>
  path === "" ? WORLD_ROOT : `${WORLD_ROOT}/${path}`;
