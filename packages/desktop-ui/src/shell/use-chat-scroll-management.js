...
const followRearmThresholdPx = (trailingRegionMinPx) => trailingRegionMinPx + FOLLOW_REARM_EXTRA_PX;
/**
 ...
 */
const followKeyTurnId = (followKey) => followKey.replace(/^assistant-/, '').replace(/-\d+$/, '');
export function useChatScrollManagement({ hasOlderEvents = false, isLoadingOlder = false, onLoadOlder, surface = 'full', } = {}) {
    ...
    const prependRestoreRafRef = useRef(null);
    ...
    const setThumbRef = useCallback((el) => {
        thumbElRef.current = el;
    }, []);
    const thumbFadeRef = useRef(null);
    ...
    const updateThumb = useCallback((scroll, scrollLength, contentLength) => {
        ...
        if (thumbFadeRef.current)
            clearTimeout(thumbFadeRef.current);
    ...
    useEffect(() => {
        ...
        if (!transition.requestSettled)
            return;
        const pending = prependAnchorRef.current;
        if (!pending)
            return;
        if (pending.cancelledByUser ||
        ...
        const restore = () => {
            ...
            if (!current || current.cancelledByUser)
                return;
            ...
            emitChatHistoryPaginationDebug({
                ...
                detail: {
                    ...
                    after: result,
                    thresholdVisible: result.scrollTopAfter <= current.anchor.viewportHeight * 2,
                    attempts,
                },
            });
            prependAnchorRef.current = null;
    ...
    */
    ...
    useEffect(() => {
        ...
        const tryAttach = () => {
            ...
            let wheelActionId = null;
            let lastWheelAt = -Infinity;
            let lastWheelDirection = 'none';
            let pointerActionId = null;
            let keyActionId = null;
            let touchActionId = null;
            let touchStartY = null;
            let activeIntent = null;
            const nextActionId = () => ++paginationActionIdRef.current;
            const markActiveIntent = (id, direction) => {
                activeIntent = {
                    id,
                    direction,
            ...
            const attemptHistoryLoad = (actionId, direction, source) => {
                const metrics = readPaginationMetrics();
                if (!metrics)
                    return;
                const options = historyOptionsRef.current;
                const decision = paginationGateRef.current.consider(actionId, direction, metrics, {
                ...
                if (!decision.request || !options.onLoadOlder)
                    return;
            ...
            const stepFollow = () => {
                ...
                const maxScroll = Math.max(0, attached.scrollHeight - attached.clientHeight);
                ...
                if (followGentle) {
                    ...
                    const stepPx = Math.abs(lerpStep) >= FOLLOW_MIN_STEP_PX
                        ? lerpStep
                ...
                lastFrameTime = now;
                const accel = FOLLOW_SPRING_STIFFNESS * diff - FOLLOW_SPRING_DAMPING * followVel;
                // Stream-follow never runs backward, so clamp velocity ≥ 0.
                followVel = Math.max(0, followVel + accel * dt);
                let step = followVel * dt;
                if (step < FOLLOW_MIN_STEP_PX)
                    step = FOLLOW_MIN_STEP_PX;
                if (step >= diff) {
            ...
            const setTarget = (newTarget, options = {}) => {
                if (!attached)
                    return;
                if (!followRef.current)
                    return;
            ...
            const scrollLatestUserMessageIntoView = () => {
                ...
                if (!followRef.current)
                    return;
                const userRow = attached.querySelector('.event-row--user--just-sent');
                if (!userRow) {
            ...
            const scrollQueuedMessagesIntoView = () => {
                ...
                if (!followRef.current)
                    return;
                const queuedMessages = attached.querySelectorAll('.composer-queued-message:not(.composer-queued-message--leaving)');
                const queuedMessage = queuedMessages.length > 0
                ...
                const messageBottom = messageRect.bottom - containerRect.top + attached.scrollTop;
                const target = messageBottom -
            ...
            const followActiveAssistantRow = () => {
                ...
                const streamingRow = attached.querySelector(`[data-scroll-follow-key="${CSS.escape(followKey)}"]`);
                if (!streamingRow || streamingRow.offsetHeight <= 0)
                    return false;
                ...
                const rowLayoutBottom = rowRect.bottom - containerRect.top + attached.scrollTop;
                ...
                const revealElement = streamingRow.querySelector('[data-reveal-visible-bottom]');
                if (revealElement) {
                    const frontier = Number(revealElement.getAttribute('data-reveal-visible-bottom'));
                    if (Number.isFinite(frontier)) {
                        const revealRect = revealElement.getBoundingClientRect();
                        const revealBottom = revealRect.top - containerRect.top + attached.scrollTop + frontier;
                ...
                const tailBoundary = attached.querySelector('.composer-queued-stack') ??
                    attached.querySelector('.event-list-trailing-region');
            ...
            const followIdleContentGrowth = () => {
                ...
                if (followActiveAssistantRow())
                    return;
                const trailing = attached.querySelector('.event-list-trailing-region');
                const containerRect = attached.getBoundingClientRect();
            ...
            const handleWheel = (event) => {
                noteManualScroll();
                const now = performance.now();
                const direction = event.deltaY < 0 ? 'up' : event.deltaY > 0 ? 'down' : 'none';
                if (wheelActionId === null ||
            ...
            const handleTouchMove = (event) => {
                noteManualScroll();
                if (touchActionId === null || touchStartY === null)
                    return;
                const y = event.touches[0]?.clientY;
                if (y === undefined)
                    return;
                const direction = y > touchStartY ? 'up' : 'down';
