/**
 * The top spinner: the pose the working indicator's mark holds while thinking,
 * replacing the three-dot ellipsis. The mobile half of the desktop rig's
 * spinner block, tuned to the same reference design.
 *
 ...
 *
 * Everything here is a Reanimated worklet evaluated on the UI thread from one
 * frame callback, so no per-frame work crosses to JS. It is also plain
 * arithmetic over numbers, which is what makes it directly unit-testable under
 * `bun test` with no renderer.
 *
 * All distances are in the spinner's own 100-unit cell; the component scales
 * the cell into its rendered size.
 */
...
/** Half the figure-eight's horizontal travel, as a fraction of the mark's own
 *  size. The indicator's mark box is widened by exactly this overhang so the
 *  travel stays inside the bubble. */
...
/** The dots park well outside the body's cell, so their layer gets its own
 *  wider viewBox rather than being clipped to the spinner's. */
export const DOT_EXTRA = 60;
export const DOT_VIEW_SPAN = CELL + DOT_EXTRA * 2;

/** A dot's lifecycle: orbiting the top, armed to drop, falling to its station,
 *  parked as one character of the ellipsis, then flying back into orbit. */
...
/**
 * Dot state lives in one flat number array rather than an array of objects:
 * it is a Reanimated shared value written from the UI thread every frame, and
 * a plain number array is the shape that survives that round trip without
 * being reallocated. Fields are addressed as `i * S_STRIDE + S_FIELD`.
 */
const S_MODE = 0;
const S_ARMED_AT = 1;
const S_X = 2;
...
/**
 * The sheen is one band repeated across a strip that is slid behind the
 * diamond to read as the top's surface turning. rn-svg has no
 * `spreadMethod="repeat"`, which the desktop rig uses, so the repeat is baked
 * into the stop list and the strip is made wide enough to cover a full slide.
 */
...
/**
 * Everything one frame needs, as flat numbers. The component reads these off a
 * single shared value, so each animated prop is one property access on the UI
 * thread rather than its own worklet recomputing the pose.
 */
...
/**
 * The whole pose for one instant. `morph` runs 0 (full spinner) to 1 (full
 * character), so the same call covers the grow-into-character handoff. `state`
 * is mutated in place: the dots carry spring velocity and drop/collect mode
 * across frames, which is the only thing here that is not a pure function of
 * `now`.
 */
export function computeSpinnerFrame(
  ...
  // Dots change mode only on the frame the top sweeps past their station, and
  // only in the sweep direction — so the drop reads as the top leaving the dot
  // behind. Even laps drop, odd laps collect.
  if (dotsOn) {
