...
import { Copy, Download, FileSpreadsheet, FileText, FolderOpen, Presentation, } from "@/ui/icons";
...
const formatFromPath = (sourcePath) => {
...
const titleForPreview = (previewRef, snapshot) => snapshot?.title ?? previewRef.title;
const parsePreviewMeta = (html, kind) => {
...
const injectPreviewChrome = (html, zoomPercent) => {
    const style = `
...
const StatusPanel = ({ error, loading, }) => (<div className="display-artifact-status">
...
export function OfficeArtifactPanel({ previewRef, }) {
    ...
    const meta = useMemo(() => parsePreviewMeta(snapshot?.html, kind), [snapshot?.html, kind]);
    const srcDoc = useMemo(() => snapshot?.html
        ? injectPreviewChrome(snapshot.html, zoomPercent)
        : undefined, [snapshot?.html, zoomPercent]);
    useEffect(() => {
        const frame = frameRef.current;
        if (!frame || kind !== "spreadsheet")
            return;
        const applySheetSelection = (index) => {
            const doc = frame.contentDocument;
            if (!doc)
                return;
            doc
                .querySelectorAll(".sheet-tab[data-sheet]")
                .forEach((tab) => {
                tab.classList.toggle("active", Number(tab.getAttribute("data-sheet")) === index);
            });
            doc
                .querySelectorAll(".sheet-content[data-sheet]")
                .forEach((content) => {
                content.classList.toggle("active", Number(content.getAttribute("data-sheet")) === index);
            });
        };
        const attachSheetHandlers = () => {
            ...
            const active = doc.querySelector(".sheet-tab.active[data-sheet]") ??
                doc.querySelector(".sheet-tab[data-sheet]");
            const activeIndex = Number(active?.getAttribute("data-sheet"));
            if (Number.isFinite(activeIndex))
                applySheetSelection(activeIndex);
    ...
    const countLabel = kind === "spreadsheet" && meta.count > 0
        ? `${meta.count} sheet${meta.count === 1 ? "" : "s"}`
        : kind === "presentation" && meta.count > 0
            ? `${meta.count} slide${meta.count === 1 ? "" : "s"}`
            : labelForKind(kind);
    return (<section className={`display-artifact-panel display-artifact-panel--${kind}`}>
      <header className="display-artifact-panel__header">
        <div className="display-artifact-panel__identity">
          ...
          <div className="display-artifact-panel__title-group">
            ...
            <div className="display-artifact-panel__title" title={previewRef.sourcePath}>
              {title}
            </div>
          </div>
        </div>
        ...
        <div className="display-artifact-panel__actions">
          <Select className="display-artifact-panel__zoom" aria-label="Preview zoom" value={String(zoomPercent)} onValueChange={(value) => {
            setZoomPercent(Number(value));
        }} options={ZOOM_OPTIONS.map((value) => ({
            value: String(value),
            label: `${value}%`,
        }))}/>
          <button type="button" className="display-artifact-panel__action" onClick={() => {
            window.electronAPI?.system?.showItemInFolder?.(previewRef.sourcePath);
        }} title="Reveal in Finder" aria-label="Reveal in Finder">
            <FolderOpen size={14}/>
          </button>
          <button type="button" className="display-artifact-panel__action" onClick={handleSave} title="Save" aria-label="Save">
            <Download size={14}/>
          </button>
          <button type="button" className="display-artifact-panel__action" onClick={handleCopy} title="Copy" aria-label="Copy">
            <Copy size={14}/>
          </button>
        </div>
      </header>
      ...
      <div className="display-artifact-panel__body">
        {ready ? (<iframe ref={frameRef} className="display-artifact-panel__frame" title={`${labelForKind(kind)} preview: ${title}`} sandbox="allow-scripts allow-same-origin" srcDoc={srcDoc}/>) : (<StatusPanel error={error} loading={loading}/>)}
      </div>

      {(actionStatus || snapshot?.status === "ready") && (<footer className="display-artifact-panel__footer">
          <span>
            {snapshot?.updatedAt
