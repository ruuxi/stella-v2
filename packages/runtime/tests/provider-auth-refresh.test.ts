...
describe("provider authorization failures", () => {
  it("recognizes bounded nested errors without accepting malformed status carriers", () => {
    expect(isUnauthorizedProviderError({ cause: { response: { error: { status: 401 } } } })).toBe(true);
    expect(isUnauthorizedProviderError({ cause: { cause: { cause: { cause: { status: 401 } } } } })).toBe(false);
    expect(isUnauthorizedProviderError({ status: 500, cause: { status: 401 } })).toBe(false);
    expect(isUnauthorizedProviderError({ status: Infinity, response: { status: 401 } })).toBe(true);
    expect(isUnauthorizedProviderError({ status: NaN })).toBe(false);
    expect(isUnauthorizedProviderError({ status: "401" })).toBe(false);
    expect(isUnauthorizedProviderError(Object.assign([], { status: 401 }))).toBe(false);
    expect(isUnauthorizedProviderError(new Error("Unauthorized request"))).toBe(true);
  ...
  it("refreshes a rejected credential once before exposing a result", async () => {
    ...
    const result = await requestWithAuthRefresh({
      apiKey: "old",
      refreshApiKey: async () => { refreshes++; return " fresh "; },
      request: async key => {
        keys.push(key);
  ...
  it("preserves the original failure if credential renewal fails", async () => {
    ...
    await expect(requestWithAuthRefresh({
      apiKey: "old",
      refreshApiKey: async () => { throw new Error("renewal failed"); },
