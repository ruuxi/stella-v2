...
const requestHeaders = (options, idempotencyKey, requestHash) => ({
...
const parseErrorResponse = async (response) => {
    const text = (await readResponseBodyBounded(response, { maxBytes: 1024 * 1024 }).catch(() => Buffer.alloc(0))).toString("utf8");
    if (!text)
        return `request failed with status ${response.status}`;
    try {
        const parsed = JSON.parse(text);
        const message = asNonEmptyString(parsed.error) ?? asNonEmptyString(parsed.message);
...
const parseJobJson = async (response) => JSON.parse((await readResponseBodyBounded(response, {
...
const isManagedMediaJob = (value) => {
    if (!value || typeof value !== "object")
        return false;
    const record = value;
    return Boolean(asNonEmptyString(record.jobId) &&
        asNonEmptyString(record.capability) &&
        asNonEmptyString(record.profile) &&
        parseJobStatus(record.status));
};
const extractRemoteImages = (output) => {
...
const extensionFor = (url, mimeType) => {
...
const materializeImages = async (args) => {
    ...
    for (const [index, image] of args.images.entries()) {
        ...
        const expectedMimeType = declaredMimeType ??
        ...
        const saved = await materializeMediaArtifact({
            ...
            validateExisting: async (candidate) => await validateDecodedImageFile(candidate, expectedMimeType),
            producer: async (downloadSignal) => {
                ...
                if (!decoded) {
                    throw new Error("Image artifact was partial or had an unsupported MIME type.");
                }
                if (decoded.mimeType !== expectedMimeType) {
                    throw new Error(`Image artifact bytes are ${decoded.mimeType}, not ${expectedMimeType}; refusing a misleading file extension.`);
                }
                mimeType = decoded.mimeType;
                return bytes;
            },
...
const statusText = (job) => job.status === "queued"
...
export const submitAndWaitForManagedImageJob = async (options) => {
    const fetchImpl = options.fetchImpl ?? fetch;
    const now = options.now ?? Date.now;
    const sleep = options.sleep ?? abortableSleep;
    const timeoutMs = options.timeoutMs ?? MANAGED_IMAGE_JOB_TIMEOUT_MS;
    const artifactGraceMs = options.artifactGraceMs ?? MANAGED_IMAGE_ARTIFACT_GRACE_MS;
    ...
    const stellaDataDir = options.context.stellaDataDir ??
    ...
    const idempotencyKey = createManagedImageIdempotencyKey(options.context, operation.operationId);
    ...
    let accepted = operation.jobId
        ? { jobId: operation.jobId, reattached: true }
        : null;
    let lastSubmitError = "";
    const reconcileAcceptance = async () => {
        try {
    ...
    try {
        ...
        for (let attempt = 0; !accepted && attempt < SUBMIT_ATTEMPTS; attempt += 1) {
            throwIfAborted(signal);
            try {
                const response = await fetchImpl(new URL("/api/media/v1/generate", options.baseUrl), {
                    method: "POST",
                    headers: {
                        ...headers,
                ...
                if (response.ok) {
                    accepted = (await parseJobJson(response));
                    break;
                }
                lastSubmitError = await parseErrorResponse(response);
        ...
        while (!accepted && now() < deadline) {
            ...
            if (accepted)
                break;
            await sleep(Math.min(reconciliationPollMs, Math.max(1, deadline - now())), signal);
            reconciliationPollMs = Math.min(maxPollMs, Math.max(initialPollMs, reconciliationPollMs * 1.5));
        }
        ...
        if (!jobId) {
            return finish({
                ...
                message: "Stella could not confirm whether the image request was accepted. It will not submit a duplicate.",
                ...
                if (!response.ok) {
                    ...
                    if (value.status === "failed" ||
                        ...
                        return finish({
                            ...
                            jobId,
                            status: value.status,
                            code: asNonEmptyString(value.error?.code)?.toLowerCase() ??
                                value.status,
                            message: asNonEmptyString(value.error?.message) ??
                        ...
                        if (images.length > 0) {
                            try {
                                const materialized = await materializeImages({
                                    ...
                                    fetchImpl,
                                    signal,
                                    downloadTimeoutMs: Math.max(1, Math.min(artifactDownloadTimeoutMs, artifactDeadline - now())),
                                });
                                return finish({
                                    ok: true,
                                    job: value,
                                ...
                                if (now() >= artifactDeadline) {
                                    return finish({
                                        ...
                                        message: `Image completed but its artifact could not be saved: ${error.message}`,
                                        reattached,
                                    });
                                }
                            }
                            ...
                            if (now() >= artifactDeadline) {
                                return finish({
                                    ...
                                    message: "Image generation completed without a downloadable artifact.",
                                    reattached,
                                });
                            }
                        }
