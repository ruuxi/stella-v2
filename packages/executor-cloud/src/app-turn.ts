...
const assertToolCommandSucceeded = (
  ...
  // exec_command's model-facing `result` is formatted text. Machine-readable
  // lifecycle fields live in `details`; retain the old object fallback so a
  // rolling runtime/executor deploy remains compatible in either direction.
  const payload =
...
const appSource = (
  spec: AppSpec,
): string => `import React, { useEffect, useMemo, useRef, useState } from "react";
...
export const runAppTurn = (
  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      yield* Effect.try({
        try: () =>
          assertToolCommandSucceeded(hydrate, "Dependency hydration failed"),
        catch: asError,
      });
      ...
      yield* Effect.try({
        try: () => assertToolCommandSucceeded(build, "Production build failed"),
        catch: asError,
      });
      const productionBuildMs = performance.now() - buildStarted;
      const disk = yield* Effect.tryPromise({
        try: async () => {
          const proc = Bun.spawn(["du", "-sb", "/workspace"], {
            stdout: "pipe",
          });
