...
export const getShellBreakpointState = (width) => ({
    hideWorkspaceStrip: width > 0 && width <= SHELL_WORKSPACE_STRIP_AUTO_HIDE_WIDTH,
    displayPanelTakeover: width > 0 && width <= SHELL_DISPLAY_PANEL_TAKEOVER_WIDTH,
});
let snapshot = getShellBreakpointState(typeof window === "undefined" ? 0 : window.innerWidth);
const listeners = new Set();
const sameBreakpointState = (left, right) => left.hideWorkspaceStrip === right.hideWorkspaceStrip &&
...
export const shellBreakpointStore = {
    ...
    setWidth(width) {
        ...
        if (sameBreakpointState(snapshot, next))
            return;
        snapshot = next;
...
export const useShellBreakpointState = () => useSyncExternalStore(shellBreakpointStore.subscribe, shellBreakpointStore.getSnapshot, shellBreakpointStore.getSnapshot);
