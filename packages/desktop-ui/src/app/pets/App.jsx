...
import { Compass, Download, MoreHorizontal, Plus, Search, User, } from "@/ui/icons";
import { useMutation, usePaginatedQuery } from "convex/react";
import { api } from "@/convex/api";
import { Button } from "@/ui/button";
import { Select } from "@/ui/select";
import { StellaLogoIcon } from "@/ui/stella-logo-icon";
import { Dialog, DialogBody, DialogCloseButton, DialogContent, DialogHeader, DialogTitle, } from "@/ui/dialog";
...
import { DEFAULT_PET_ID, normalizePet, } from "@/shell/pet/built-in-pets";
import { BUNDLED_PETS } from "@/shell/pet/bundled-pets";
import { useTagFacets } from "@/shell/pet/pet-catalog-context";
import { getCachedPetCatalogFirstPage, writeCachedPetById, writeCachedPetCatalogFirstPage, } from "@/shell/pet/pet-catalog-cache";
import { readPetOpenPreference, useSelectedPetId, writePetOpenPreference, } from "@/shell/pet/pet-preferences";
import { useAuthSessionState } from "@/global/auth/hooks/use-auth-session-state";
import { CreatePetDialog } from "./CreatePetDialog";
import { SharePetDialog } from "./SharePetDialog";
import { useMyUserPets, usePublicUserPets, useUserPetMutations, } from "./user-pet-data";
import "./pets.css";
const ALL_TAG = "all";
...
const formatDownloads = (value) => downloadCountFormatter.format(Math.max(0, Math.floor(value)));
/**
 ...
 */
const filterBundledPets = (tag, search, sort) => {
    const trimmed = search.trim().toLowerCase();
    const filtered = BUNDLED_PETS.filter((pet) => {
        if (tag !== ALL_TAG && !pet.tags.includes(tag))
            return false;
        if (!trimmed)
            return true;
        return (pet.displayName.toLowerCase().includes(trimmed) ||
 ...
 */
function PetCard({ pet, state, removable, ownIdleStrip = false, badge, menu, onOpen, onGet, onSelect, onRemove, }) {
    ...
    return (<div className="pets-card pets-card-wrapper" data-pet-state={state} data-selected={state === "selected" ? "true" : "false"} role="button" tabIndex={0} onClick={onOpen} onKeyDown={(event) => {
      ...
      <div className="pets-card-sprite">
        {animateFull ? (<PetSprite spritesheetUrl={pet.spritesheetUrl} state="idle" size={84}/>) : useIdleStrip ? (<PetIdlePreview previewUrl={pet.previewUrl} size={84}/>) : (<div className="pets-card-sprite-placeholder" aria-hidden>
            <StellaLogoIcon size={20} aria-hidden/>
          </div>)}
      </div>
      <div className="pets-card-name-row">
        <span className="pets-card-name">{pet.displayName}</span>
        {badge ? (<span className="pets-card-visibility-badge" data-tier={badge.tier}>
            {badge.label}
          </span>) : null}
      </div>
      <div className="pets-card-meta">
        <span className="pets-card-creator">by {pet.creator}</span>
        <span className="pets-card-downloads" title={`${pet.downloads.toLocaleString()} selections`}>
          <Download size={11} aria-hidden="true"/>
          {formatDownloads(pet.downloads)}
        </span>
      </div>
      <div className="pets-card-actions" onClick={(event) => event.stopPropagation()}>
        {state === "uninstalled" ? (<Button type="button" variant="primary" size="small" className="pill-btn pill-btn--primary" onClick={onGet}>
            <Plus size={12}/>
            Get
          </Button>) : (<>
            <Button type="button" variant={state === "selected" ? "secondary" : "primary"} size="small" className={state === "selected" ? "pill-btn" : "pill-btn pill-btn--primary"} onClick={state === "selected" ? undefined : onSelect} disabled={state === "selected"}>
              {state === "selected" ? "Selected" : "Select"}
            </Button>
            {removable ? (<Button type="button" variant="secondary" size="small" className="pill-btn" onClick={onRemove}>
                Remove
              </Button>) : null}
          </>)}
      </div>
...
function PetDetailsDialog({ pet, state, removable, onOpenChange, onGet, onSelect, onRemove, }) {
    const [mainState, setMainState] = useState("idle");
    const primaryLabel = state === "selected" ? "Selected" : state === "installed" ? "Select" : "Get";
    ...
    return (<Dialog open onOpenChange={onOpenChange}>
      <DialogContent fit className="pet-detail-dialog">
        ...
        <DialogBody className="pet-detail-body">
          <div className="pet-detail-stage" aria-label={`${pet.displayName} preview`}>
            <PetSprite spritesheetUrl={pet.spritesheetUrl} state={mainState} size={220} continuous/>
          </div>

          {blurb ? <p className="pet-detail-blurb">{blurb}</p> : null}

          <div className="pet-detail-actions">
            <Button type="button" variant={state === "selected" ? "secondary" : "primary"} size="normal" className={state === "selected"
            ? "pill-btn pill-btn--lg"
            : "pill-btn pill-btn--primary pill-btn--lg"} onClick={() => void handlePrimary()} disabled={state === "selected"}>
              {primaryLabel}
            </Button>
            {removable && state !== "uninstalled" ? (<Button type="button" variant="secondary" size="normal" className="pill-btn pill-btn--lg" onClick={() => {
                onRemove();
                onOpenChange(false);
            }}>
                Remove
          ...
          <section className="pet-detail-states-section">
            ...
            <div className="pet-detail-states" role="tablist" aria-label="Animation states">
              {ANIMATION_STATES.map((entry) => (<button key={entry.state} type="button" role="tab" aria-selected={mainState === entry.state} aria-label={entry.label} title={entry.label} className="pet-detail-state-thumb" data-active={mainState === entry.state || undefined} onClick={() => setMainState(entry.state)}>
                  <PetSprite spritesheetUrl={pet.spritesheetUrl} state={entry.state} size={52} continuous/>
                </button>))}
            </div>
          </section>
        </DialogBody>
...
export const PetsApp = () => {
    ...
    const [petOpen, setPetOpenState] = useState(() => readPetOpenPreference());
    ...
    const { setVisibility: setUserPetVisibility, deletePet: deleteUserPet, recordInstall: recordUserPetInstall, } = useUserPetMutations();
    const ownedUserPetIds = useMemo(() => {
        const set = new Set();
        for (const pet of myUserPets ?? [])
            set.add(pet.petId);
        return set;
    }, [myUserPets]);
    ...
    const cachedFirstPageRef = useRef(getCachedPetCatalogFirstPage());
    useEffect(() => {
    ...
    const paginationArgs = useMemo(() => ({
        sort,
        tag: trimmedSearch.length === 0 && activeTag !== ALL_TAG
            ? activeTag
            : undefined,
        search: trimmedSearch.length > 0 ? trimmedSearch : undefined,
    }), [activeTag, sort, trimmedSearch]);
    const { results, status, loadMore } = usePaginatedQuery(api.data.pets.listPublicPage, paginationArgs, { initialNumItems: PAGE_SIZE });
    const remotePets = useMemo(() => {
        return results
            .map(normalizePet)
            .filter((pet) => pet !== null);
    }, [results]);
    ...
    const visiblePets = useMemo(() => {
        ...
        const networkPets = remotePets.length > 0
            ? remotePets
        ...
        for (const pet of remoteFiltered)
            seen.add(pet.id);
    ...
    const userPetIds = useMemo(() => {
        const set = new Set();
        for (const pet of publicUserPets)
            set.add(pet.petId);
        for (const pet of myUserPets ?? [])
            set.add(pet.petId);
        return set;
    ...
    const recordOneInstall = useCallback(async (petId) => {
        if (incrementedRef.current.has(petId))
            return;
    ...
    const handleSetUserPetVisibility = useCallback(async (pet, next) => {
        try {
    ...
    const handleDeleteUserPet = useCallback(async (pet) => {
        try {
    ...
    return (<main className="pets-page">
      ...
      <div className="pets-toolbar">
        <label className="pets-search">
          ...
          <input type="search" placeholder="Search by name, description, or creator" className="pets-search-input" value={query} onChange={(event) => setQuery(event.currentTarget.value)}/>
        </label>
        <div className="pets-sort">
          <span className="pets-sort-label">Sort</span>
          <Select className="pets-sort-select" value={sort} onValueChange={(value) => setSort(value)} aria-label="Sort" options={Object.keys(SORT_LABELS).map((option) => ({
            value: option,
            label: SORT_LABELS[option],
        }))}/>
        </div>
        <div className="pets-toolbar-actions">
          <Button variant="secondary" size="normal" className="pill-btn pill-btn--lg" onClick={() => {
            if (viewMode === "mine") {
          ...
          <Button variant="primary" size="normal" className="pill-btn pill-btn--primary pill-btn--lg" style={{
          ...
          <Button variant={petOpen ? "secondary" : "primary"} size="normal" className={petOpen
            ? "pill-btn pill-btn--lg"
            : "pill-btn pill-btn--primary pill-btn--lg"} style={petOpen
            ? undefined
            : {
                borderColor: "color-mix(in oklch, var(--primary-foreground) 30%, transparent)",
      ...
      {viewMode === "discover" && myUserPets && myUserPets.length > 0 ? (<section className="pets-your-section">
          ...
          <div className="pets-grid">
            {myUserPets.map((pet) => {
                ...
                const cardState = isSelected
                    ? "selected"
                    : "installed";
                return (<PetCard key={pet.petId} pet={builtIn} state={cardState} removable={false} ownIdleStrip badge={pet.visibility === "private"
                        ? { label: "Private", tier: "private" }
                        : pet.visibility === "unlisted"
                            ? { label: "Unlisted", tier: "unlisted" }
                            : null} onOpen={() => setDetailsTarget(builtIn)} onGet={() => handleGet(pet.petId)} onSelect={() => handleSelect(pet.petId)} onRemove={() => handleRemove(pet.petId)} menu={<div onClick={(event) => event.stopPropagation()}>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <button type="button" className="pets-card-menu-btn" aria-label={`More actions for ${pet.displayName}`}>
                          <MoreHorizontal size={14}/>
                        </button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end" sideOffset={6} className="store-card-menu">
                        <DropdownMenuLabel>Visibility</DropdownMenuLabel>
                        <DropdownMenuRadioGroup value={pet.visibility} onValueChange={(value) => {
                            if (value === "public" ||
                                value === "unlisted" ||
                                value === "private") {
                        ...
                        <DropdownMenuItem data-variant="destructive" onSelect={() => {
                            const ok = window.confirm(`Delete "${pet.displayName}"? This cannot be undone.`);
                            if (ok)
                                void handleDeleteUserPet(pet);
                        }}>
                          Delete pet
                        </DropdownMenuItem>
                      </DropdownMenuContent>
      ...
      {viewMode === "discover" ? (<div className="pets-tags" role="tablist" aria-label="Filter by tag">
          <button type="button" role="tab" className="pets-tag-pill" data-active={activeTag === ALL_TAG ? "true" : "false"} aria-selected={activeTag === ALL_TAG} onClick={() => setActiveTag(ALL_TAG)}>
            All
          </button>
          {tagOptions.map((tag) => (<button key={tag} type="button" role="tab" className="pets-tag-pill" data-active={activeTag === tag ? "true" : "false"} aria-selected={activeTag === tag} onClick={() => setActiveTag(tag)}>
              {tag}
            </button>))}
        </div>) : null}

      {viewMode === "discover" ? (visiblePets.length === 0 && !isLoadingFirstPage ? (<div className="pets-empty">
              ...
              {visiblePets.map((pet) => {
                ...
                const cardState = isSelected
                    ...
                    : installed
                        ? "installed"
                        : "uninstalled";
                return (<PetCard key={pet.id} pet={pet} state={cardState} removable={!isBundledPetId(pet.id)} ownIdleStrip={userPetIds.has(pet.id)} onOpen={() => setDetailsTarget(pet)} onGet={() => setDetailsTarget(pet)} onSelect={() => handleSelect(pet.id)} onRemove={() => handleRemove(pet.id)}/>);
            })}
            </div>
            {showPagingFooter && (<div ref={sentinelRef} className="pets-grid-sentinel" data-loading={isLoadingMore ? "true" : "false"} aria-hidden="true">
                {isLoadingMore ? "Loading more…" : ""}
              </div>)}
          </>)) : (<section className="pets-your-section">
          <div className="pets-your-header">
            <span className="pets-your-title">My pets</span>
            {myUserPets && myUserPets.length > 0 ? (<span className="pets-your-count">{myUserPets.length}</span>) : null}
          </div>
          {!myUserPets ? (<div className="pets-empty">Loading…</div>) : myUserPets.length === 0 ? (<div className="pets-empty">
              You haven't created any pets yet.
            </div>) : (<div className="pets-grid">
              {myUserPets.map((pet) => {
                    const builtIn = userPetToBuiltIn(pet);
                    const isSelected = pet.petId === selectedPetId;
                    const cardState = isSelected
      ...
      {shareTarget ? (<SharePetDialog open onOpenChange={(next) => {
                if (!next)
                    setShareTarget(null);
            }} pet={shareTarget}/>) : null}
      {detailsTarget ? (<PetDetailsDialog key={detailsTarget.id} pet={detailsTarget} state={detailsTarget.id === selectedPetId
                ? "selected"
                : isInstalled(detailsTarget.id)
                    ? "installed"
                    : "uninstalled"} removable={!isBundledPetId(detailsTarget.id)} onOpenChange={(next) => {
                if (!next)
                    setDetailsTarget(null);
            }} onGet={() => handleGet(detailsTarget.id)} onSelect={() => handleSelect(detailsTarget.id)} onRemove={() => handleRemove(detailsTarget.id)}/>) : null}
    </main>);
};
