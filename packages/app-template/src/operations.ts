 ...
 *
 * One document, two hands: the UI's controls and the Stella agent both call
 * the deterministic functions below. The model only picks a verb and
 * arguments; this ordinary code applies the change to live state. Keep
 * durable state in `stella.storage` (persist the state document after every
 * mutation, hydrate it on load) while operations mutate live in-memory state
 * so results render immediately.
 *
 * Convention:
 *  - `operationFns(api)` returns the named deterministic functions. UI
 *    controls call these directly.
 *  - `createAppOperations(api)` binds the same functions to SDK operation
 *    definitions for `stella.operations.register`. No separate "AI path".
 *  - Argument validation lives here, in the app.
 */
  ...
  return [
    {
      ...
      args: [
        { name: "habit", type: "string", required: true, description: "Habit name" },
    ...
    },
    {
      ...
      args: [
        { name: "focus", type: "string", required: true, description: "New focus text" },
      ],
