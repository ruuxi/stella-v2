...
const createContextProvider = (args?: {
  ...
  createRunnerContext({
    ...
    // The runner context now spins up the cloud transcript/journal outbox
    // drains; empty listings keep them no-ops in this boundary test.
    runtimeStore: {
      listCloudTranscriptOutbox: () => [],
      listCloudJournalOutbox: () => [],
      countCloudTranscriptOutbox: () => 0,
      countCloudJournalOutbox: () => 0,
    } as never,
