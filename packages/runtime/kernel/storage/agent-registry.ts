...
const parseDescendantBoundaryState = (
  ...
  const parsed = parseJsonValue(value) as unknown;
...
export type AgentRecordInput = {
  threadId: string;
  conversationId: string;
  /** Transcript ownership for this thread; defaults to "local". */
  storageMode?: "cloud" | "local";
  /** Exact owner-data epoch for cloud lifecycle publication and replay. */
  ownerGeneration?: string;
  agentType: string;
...
export class AgentRegistry {
      ...
      .prepare(
        `INSERT INTO agent (
         ...
         WHERE excluded.attempt_generation >= agent.attempt_generation
           AND excluded.conversation_id = agent.conversation_id
           AND excluded.storage_mode = agent.storage_mode
           AND excluded.owner_generation IS agent.owner_generation
  ...
  private deserializeAgentRow(row: any): PersistedAgentRecord {
