...
import { deriveLatestAgentPresentationStatus, deriveOwnedAgentPresentationStatus, latestAttemptSupersedesAuthoritative, } from './agent-activity-presentation';
/**
 ...
 */
export function selectFreshActivityTasks(tasks, nowMs = Date.now()) {
    return tasks.filter((task) => isManagedActivityTask(task) &&
        (task.status === 'running' ||
 ...
 */
export function buildActivityTasks(records, decorations) {
    return records
        ...
        const latestAttempt = candidateDecoration
            ...
            : undefined;
        const status = deriveLatestAgentPresentationStatus(authoritative, latestAttempt);
        const running = status === 'running';
        const decoration = running ? candidateDecoration : undefined;
        const latestAttemptOwns = latestAttemptSupersedesAuthoritative(authoritative, latestAttempt);
        ...
        return {
            ...
            status,
            attemptGeneration: (latestAttemptOwns
                ? candidateDecoration?.attemptGeneration
                : undefined) ?? record.attemptGeneration,
            runId: (latestAttemptOwns ? candidateDecoration?.runId : undefined) ??
                record.rootRunId,
            ...
            completedAtMs: running
                ...
                : latestAttemptOwns
                    ...
                    : record.completedAt,
            lastUpdatedAtMs: (latestAttemptOwns ? candidateDecoration?.observedAtMs : undefined) ??
                record.updatedAt,
            outputPreview: running || !recordOwnsAttempt
                ? undefined
...
export function isGenericTaskDescription(description) {
...
export function fallbackTaskDescription(agentId) {
    const slug = (agentId ?? '').trim();
    if (!slug || /^(task|legacy)-/i.test(slug))
        return 'Task';
    ...
    if (words.length < 2 && letterCount < 4)
        return 'Task';
    if (letterCount === 0)
        return 'Task';
 ...
 */
export function isActivityFeedTask(task) {
...
export function isManagedActivityTask(task) {
    return task.source === 'stella';
}
export function isStandaloneTaskStatusText(statusText) {
...
export function normalizeTaskDisplayStatusText(statusText) {
    return normalizeDisplayStatusText(statusText);
}
export function getTaskDisplayText(task) {
...
export function getTaskWorkingIndicatorText(task) {
    const statusText = normalizeTaskDisplayStatusText(task.statusText);
    if (task.status === 'running' &&
        statusText &&
...
function createEventGuard(type, requiredFields) {
    return (event) => event.type === type &&
        typeof event.payload === 'object' &&
        event.payload !== null &&
        (requiredFields === undefined ||
            requiredFields.every((field) => field in event.payload));
}
export const isToolRequest = createEventGuard('tool_request', ['toolName']);
export const isToolResult = createEventGuard('tool_result');
...
export const isAgentStartedEvent = createEventGuard('agent-started', ['agentId']);
export const isAgentCompletedEvent = createEventGuard('agent-completed', ['agentId']);
export const isAgentFailedEvent = createEventGuard('agent-failed', ['agentId']);
export const isAgentCanceledEvent = createEventGuard('agent-canceled', ['agentId']);
export const isAgentProgressEvent = createEventGuard('agent-progress', ['agentId', 'statusText']);
export function extractToolTitle(event) {
...
export function extractStepsFromEvents(events) {
    const steps = [];
    const stepIndexByRequestId = new Map();
    for (const event of events) {
        ...
        const status = event.payload.error
            ? 'error'
            : 'completed';
 ...
 */
export function getCurrentRunningTool(messages) {
    const events = [];
...
export const flattenActivityTasks = (rows) => {
    const tasks = [];
    const append = (row) => {
        if (row.kind === 'task') {
            tasks.push(row.task);
...
const compareCompactTaskRecency = (a, b) => b.lastUpdatedAtMs - a.lastUpdatedAtMs ||
    (b.completedAtMs ?? 0) - (a.completedAtMs ?? 0) ||
    b.startedAtMs - a.startedAtMs ||
    a.id.localeCompare(b.id);
const compactLatestText = (task) => {
    const assistantText = selectLatestAgentAssistantMessage(task.assistantMessages);
    if (assistantText)
...
const compareCompactAssistantRecency = (a, b) => {
    ...
    if (aHasText !== bHasText)
        return Number(bHasText) - Number(aHasText);
    if (aHasText && bHasText) {
        ...
        const byTimestamp = (b.assistantMessagesUpdatedAtMs ?? 0) -
...
export function summarizeCompactActivity(tasks) {
...
export function getCompactActivityStatusText(summary, prioritizeFailure) {
...
export const activityRowKey = (row) => row.kind === 'task'
    ? `task:${row.task.id}`
 ...
 */
export const deriveTopLevelActivityWorkUnits = (tasks) => {
    const latestById = new Map();
    for (const task of tasks) {
        if (!isActivityFeedTask(task) || !isManagedActivityTask(task))
            continue;
...
export const countActiveTopLevelActivityWorkUnits = (tasks) => deriveTopLevelActivityWorkUnits(tasks).filter((unit) => unit.status === 'running').length;
export const getActivityRowStatus = (row) => row.kind === 'task' ? row.task.status : row.hierarchy.status;
export const getActivityRowCompletedAtMs = (row) => {
...
export const getActivityRowSearchText = (row) => {
    if (row.kind === 'task') {
 ...
 */
export function groupActivityTasks(tasks) {
    ...
    const buildRows = (siblings, ancestors) => {
        const visibleChildren = (task) => (childrenByParentId.get(task.id) ?? []).filter((child) => !ancestors.has(child.id) && child.id !== task.id);
        const rows = [];
        for (const task of siblings) {
            ...
            if (children.length > 0) {
                ...
                const status = isManagedActivityTask(task)
                    ? deriveOwnedAgentPresentationStatus(task.status, managedChildStatuses)
                    : task.status;
                rows.push({
                    kind: 'hierarchy',
                    hierarchy: {
    ...
    if (unvisited.length > 0)
        rows.push(...buildRows(unvisited, new Set()));
    return rows;
}
const countActivityTasks = (rows) => rows.reduce((total, row) => {
    if (row.kind === 'task')
        return total + 1;
    return total + 1 + row.hierarchy.descendantCount;
}, 0);
/** Agent-authored prose for Activity rows, active or completed. */
export function getTaskAgentUpdates(task) {
 ...
 */
export function updateSeenRunningTaskIds(seen, tasks) {
    let changed = false;
...
export const EMPTY_FIRST_SEEN_ORDER = {
    order: new Map(),
    next: 0,
};
/**
 ...
 */
export function orderByFirstSeen(items, keyOf, prev = EMPTY_FIRST_SEEN_ORDER, descending = false) {
    const order = new Map();
    let next = prev.next;
