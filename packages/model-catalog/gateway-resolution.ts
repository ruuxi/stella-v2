...
import { managedModelSupportsImageInput, resolveRequestedStellaModel } from "./request-estimate";
...
export const resolveManagedModelDescriptor = (args: {
  ...
  const selection = resolveRequestedStellaModel(args.agentType, { model: args.requestedModel }, args.audience);
  const provider = resolveManagedGatewayProvider({ model: selection.resolvedModel,
  ...
  return managedModelDescriptor({
    ...
    provider,
    protocol: resolveCloudManagedProtocol({ relayProvider: provider, configuredApi: selection.config.api }),
    config: selection.config,
  });
};

/** Shared wire metadata construction, used by prediction and authoritative routing. */
export const managedModelDescriptor = (route: Pick<GatewayModelResolution,
  ...
  ...(route.config.maxOutputTokens !== undefined ? { maxOutputTokens: route.config.maxOutputTokens } : {}),
});
