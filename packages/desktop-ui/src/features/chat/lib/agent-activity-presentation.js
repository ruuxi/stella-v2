const isTerminal = (status) => status !== "running";
/**
 ...
 */
export const latestAttemptSupersedesAuthoritative = (authoritative, latestAttempt) => {
    if (!latestAttempt)
        return false;
    ...
    if (authoritativeAttempt !== undefined && observedAttempt !== undefined) {
        if (observedAttempt > authoritativeAttempt)
            return true;
        if (observedAttempt < authoritativeAttempt)
            return false;
    ...
    const observedAt = latestAttempt.startedAtMs ?? latestAttempt.observedAtMs ?? 0;
    const authoritativeAt = authoritative.completedAtMs ?? authoritative.updatedAtMs;
    return observedAt > authoritativeAt;
};
export const deriveLatestAgentPresentationStatus = (authoritative, latestAttempt) => {
    if (!latestAttempt)
 ...
 */
export const deriveOwnedAgentPresentationStatus = (ownerStatus, ownedStatuses) => {
    if (ownerStatus === "running")
        return "running";
    if (ownerStatus === "error" || ownerStatus === "canceled") {
        return ownerStatus;
    }
    if (ownedStatuses.includes("running"))
        return "running";
    if (ownedStatuses.includes("error"))
        return "error";
    return "completed";
};
const isRecordCurrentForCard = (record, options) => {
    if (options.attemptGeneration !== undefined &&
        record.attemptGeneration !== undefined) {
        if (record.attemptGeneration < options.attemptGeneration)
            return false;
        if (record.attemptGeneration === options.attemptGeneration &&
            options.rootRunId &&
            record.rootRunId &&
    ...
    if (options.rootRunId && record.rootRunId === options.rootRunId)
        return true;
 ...
 */
export const deriveThreadAndOwnedPresentationStatus = (records, threadId, options = {}) => {
    ...
    for (const record of records) {
        ...
        if (record.source === "claude-native")
            continue;
        ...
        if (children)
            ...
            childrenByParent.set(record.parentAgentId, [record]);
    }
    const derive = (id, ancestors) => {
        const record = recordById.get(id);
        if (!record || ancestors.has(id))
            return undefined;
    ...
    if (!owner || !isRecordCurrentForCard(owner, options))
        return undefined;
...
export const agentPresentationFallback = (status) => {
    switch (status) {
        case "running":
            return "Working…";
        case "canceled":
