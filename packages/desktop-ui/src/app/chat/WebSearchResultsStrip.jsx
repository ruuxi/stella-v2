...
export const WebSearchResultsStrip = ({ results, }) => {
    const [failed, setFailed] = useState(() => new Set());
    const visible = useMemo(() => results.filter((hit) => !failed.has(hit.url)), [failed, results]);
    if (visible.length === 0)
        return null;
    return (<div className="web-search-results">
      <div className="web-search-results__row" role="list" aria-label="Web search results">
        {visible.map((hit) => {
            const source = prettySource(hit.url);
            return (<a key={hit.url} className="web-search-results__card" href={hit.url} target="_blank" rel="noreferrer noopener" role="listitem" title={hit.title || source || hit.url}>
              <img className="web-search-results__image" src={hit.image} alt={hit.title || source} loading="lazy" onLoad={notifyAssistantScrollFollowLayoutChange} onError={() => setFailed((prev) => {
              ...
              <span className="web-search-results__source">
                {hit.favicon ? (<img className="web-search-results__favicon" src={hit.favicon} alt="" aria-hidden onError={(event) => {
