...
// ─── Blob generation ────────────────────────────────────────────────────
function generateBlobs(colors, mode = "soft") {
    if (mode === "flat") {
...
function renderGradient(ctx, width, height, bg, blobs, overlayAlpha) {
...
// ─── Component ──────────────────────────────────────────────────────────
export const ShiftingGradient = memo(function ShiftingGradient({ className, mode = "soft", colorMode = "relative", lightweight: lightweightProp = false, contained = false, }) {
    ...
    const getPalette = useCallback(() => {
        ...
        const accentColor = parseColor(tokens.textInteractive) ??
    ...
    return (<div ref={rootRef} aria-hidden="true" className={cn("shifting-gradient", contained && "shifting-gradient--contained", className)}>
      {lightweight ? (<div className="gradient-base" style={{
            ...
            }}/>) : (<canvas ref={canvasRef} className="gradient-base" style={{
                imageRendering: "auto",
            }}/>)}
    </div>);
});
