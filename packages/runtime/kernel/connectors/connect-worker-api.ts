...
Workflow: discover → actions → schema → call. If a connector is not connected, follow discover's "next" guidance (inline connect card via connector_status / Store) rather than retrying call. This client is the only connector surface — there is no shell CLI for connectors.
