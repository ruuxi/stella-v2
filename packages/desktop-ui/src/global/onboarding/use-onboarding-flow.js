...
import { SPLIT_STEP_ORDER } from "./onboarding-flow";
...
const PLATFORM_SKIPPED_PHASES = typeof window !== "undefined" && window.electronAPI?.platform !== "darwin"
    ? new Set(["permissions"])
    : new Set();
const advancePastSkipped = (index, direction, skippedPhases = PLATFORM_SKIPPED_PHASES) => {
    let cursor = index;
    while (cursor >= 0 &&
...
export function useOnboardingFlow({ initialPhase, onComplete, onEnterSplit, onInteract, onPhaseChange, skippedPhases, }) {
    const effectiveSkippedPhases = useMemo(() => {
        if (!skippedPhases || skippedPhases.size === 0)
            return PLATFORM_SKIPPED_PHASES;
        if (PLATFORM_SKIPPED_PHASES.size === 0)
            return skippedPhases;
        return new Set([...PLATFORM_SKIPPED_PHASES, ...skippedPhases]);
    }, [skippedPhases]);
    const [phase, setPhase] = useState(initialPhase);
    const [leaving, setLeaving] = useState(false);
    const [rippleActive, setRippleActive] = useState(initialPhase === "intro");
    const [prefersReducedMotion, setPrefersReducedMotion] = useState(false);
    const [maxVisitedSplitStepIndex, setMaxVisitedSplitStepIndex] = useState(() => Math.max(SPLIT_STEP_ORDER.indexOf(initialPhase), 0));
    ...
    const visibleSteps = useMemo(() => SPLIT_STEP_ORDER.filter((step) => !effectiveSkippedPhases.has(step)), [effectiveSkippedPhases]);
    return {
        phase,
        leaving,
        rippleActive,
