...
function attachmentsEqual(a, b) {
    ...
    for (let i = 0; i < a.length; i += 1) {
        const av = a[i];
        const bv = b[i];
        if ((av.id ?? null) !== (bv.id ?? null))
            return false;
        if ((av.url ?? null) !== (bv.url ?? null))
            return false;
        if ((av.mimeType ?? null) !== (bv.mimeType ?? null))
            return false;
        if ((av.name ?? null) !== (bv.name ?? null))
            return false;
    }
    return true;
}
const reactionsEqual = (a, b) => {
    const left = a ?? [];
    const right = b ?? [];
    if (left === right)
        return true;
    ...
    for (let i = 0; i < left.length; i += 1) {
        ...
        if (!av || !bv)
            return false;
        if (av.emoji !== bv.emoji)
            return false;
        if (av.action !== bv.action)
            return false;
...
const channelEnvelopeEqual = (a, b) => {
    if (a === b)
        return true;
    if (!a || !b)
        return a === b;
...
const scheduleReceiptEqual = (a, b) => {
    if (a === b)
        return true;
    if (!a || !b)
        return a === b;
...
const resourcePayloadEqual = (a, b) => {
    if (a === b)
        return true;
    if (!a || !b)
        return a === b;
    ...
    switch (a.kind) {
        case "canvas-html": {
            const bb = b;
            return (a.filePath === bb.filePath &&
                (a.title ?? null) === (bb.title ?? null) &&
                (a.slug ?? null) === (bb.slug ?? null) &&
                a.createdAt === bb.createdAt);
        }
        case "office":
            return (a.previewRef.sourcePath ===
                b.previewRef.sourcePath);
        case "markdown": {
            const bb = b;
            return (a.filePath === bb.filePath &&
        ...
        case "trash": {
            const bb = b;
            return ((a.title ?? null) === (bb.title ?? null) &&
                (a.createdAt ?? null) === (bb.createdAt ?? null));
        }
        case "media": {
            const bb = b;
            if ((a.presentation ?? null) !== (bb.presentation ?? null))
                return false;
            if ((a.numImages ?? null) !== (bb.numImages ?? null))
                return false;
            if ((a.imageIndex ?? null) !== (bb.imageIndex ?? null))
                return false;
            if (a.asset.kind !== bb.asset.kind)
                return false;
            if (a.asset.kind === "image" && bb.asset.kind === "image") {
                return a.asset.filePaths.join("|") === bb.asset.filePaths.join("|");
            }
            if ((a.asset.kind === "video" || a.asset.kind === "audio") &&
...
const sourceDiffPayloadsEqual = (a, b) => {
    if (a === b)
        return true;
    if (!a || !b)
    ...
    for (let index = 0; index < a.length; index += 1) {
        ...
        if (aa.kind !== "source-diff" || bb.kind !== "source-diff") {
            if (!resourcePayloadEqual(aa, bb))
                return false;
            continue;
        }
        if (aa.filePath !== bb.filePath)
            return false;
        if ((aa.patch ?? null) !== (bb.patch ?? null))
            return false;
        if ((aa.createdAt ?? null) !== (bb.createdAt ?? null))
            return false;
    }
    return true;
};
const backgroundWorkEqual = (a, b) => {
    if (a === b)
        return true;
    if (!a || !b)
        return a === b;
...
const agentCompletionEqual = (a, b) => {
    ...
    for (let i = 0; i < a.sections.length; i += 1) {
        ...
        if (as.agentId !== bs.agentId)
            return false;
        if (as.title !== bs.title)
            return false;
        if (as.startEventId !== bs.startEventId)
            return false;
        if (as.completionEventId !== bs.completionEventId)
            return false;
        if (as.rootRunId !== bs.rootRunId)
            return false;
        if (as.completedAtMs !== bs.completedAtMs)
            return false;
        if (as.summary !== bs.summary)
            return false;
        if (as.files.length !== bs.files.length)
            return false;
        for (let j = 0; j < as.files.length; j += 1) {
            ...
            if (af.timestamp !== bf.timestamp)
                return false;
            if (af.payload.kind !== bf.payload.kind)
                return false;
        }
    }
    return true;
};
const webSearchResultsEqual = (a, b) => {
    if (a === b)
        return true;
    if (!a || !b)
        return (a?.length ?? 0) === (b?.length ?? 0);
...
const mapArtifactsEqual = (a, b) => {
    ...
    for (let i = 0; i < a.length; i += 1) {
        ...
        if (a[i].map.markers.length !== b[i].map.markers.length)
            return false;
...
const appSelectionLabelsEqual = (a, b) => {
    if (a === b)
        return true;
...
const userRowEqual = (a, b) => a.id === b.id &&
    a.text === b.text &&
...
const assistantRowEqual = (a, b) => a.id === b.id &&
    a.text === b.text &&
...
export const eventRowEqual = (a, b) => {
    if (a === b)
        return true;
    if (a.kind !== b.kind)
        return false;
    if (a.kind === "user" && b.kind === "user")
        return userRowEqual(a, b);
