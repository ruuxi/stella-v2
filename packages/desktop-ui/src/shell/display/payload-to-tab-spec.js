...
import { UrlTabContent, MarkdownTabContent, SourceDiffTabContent, PdfTabContent, OfficeTabContent, OfficeFileTabContent, DelimitedTableTabContent, MediaTabContent, TrashTabContent, } from "./tab-content";
import { CanvasTabContent } from "./canvas-tab/CanvasTabContent";
import { AgentThreadChatTab } from "./AgentThreadChatTab";
import { addCanvasHtmlItem, canvasDisplayTabId, } from "./canvas-tab/canvas-items";
...
import { recordArtifactFileEntry, setFileEntries, } from "@/features/workspace-display/files-index";
 ...
 */
export const createSourceDiffTabSpec = () => ({
...
const isGeneratedMediaItem = (value) => {
...
const loadGeneratedMediaItems = () => {
    try {
        const parsed = JSON.parse(uiState.getItem(GENERATED_MEDIA_ITEMS_KEY) || "[]");
...
const persistGeneratedMediaItems = () => {
    uiState.setItem(GENERATED_MEDIA_ITEMS_KEY, JSON.stringify(generatedMediaItems.slice(-GENERATED_MEDIA_ITEMS_CAP)));
};
const generatedMediaItems = loadGeneratedMediaItems();
const generatedMediaItemIds = new Set(generatedMediaItems.map((item) => item.id));
...
// skip work.
let generatedMediaSnapshot = [];
const filePathForMediaItem = (item) => {
...
const idForMediaPayload = (payload) => {
    const { asset } = payload;
    switch (asset.kind) {
        case "image":
...
const addGeneratedMediaItem = (payload) => {
    const payloads = payload.asset.kind === "image" && payload.asset.filePaths.length > 1
...
const selectedIdForMediaPayload = (payload) => payload.asset.kind === "image" && payload.asset.filePaths.length > 1
 ...
 */
export const removeGeneratedMediaItem = (id) => {
...
export const payloadToTabSpec = (payload) => {
    ...
    switch (payload.kind) {
        ...
        case "url":
            return indexInFiles({
                ...
                render: () => createElement(UrlTabContent, {
                    url: payload.url,
                    title,
                }),
            }, payload);
        case "markdown":
            return indexInFiles({
                ...
                render: () => createElement(MarkdownTabContent, {
        ...
        case "office": {
            ...
            return indexInFiles({
                ...
                kind: kind === "office-spreadsheet" || kind === "office-slides"
                    ? kind
                    : "office-document",
                title,
                tooltip: sourcePath,
                metadata: { kind: "office", sourcePath },
                render: () => createElement(OfficeTabContent, { previewRef: payload.previewRef }),
            }, payload, sourcePath);
        }
        case "file-artifact":
            return indexInFiles({
                id: `file-artifact:${payload.filePath}`,
                kind: payload.artifactKind === "delimited-table"
                    ? "office-spreadsheet"
                ...
                render: () => payload.artifactKind === "delimited-table"
        ...
        case "pdf":
            return indexInFiles({
                ...
                render: () => createElement(PdfTabContent, {
                    filePath: payload.filePath,
                    ...(payload.title ? { title: payload.title } : {}),
                }),
            }, payload, payload.filePath);
...
export const createAgentThreadTabSpec = (args) => ({
    id: `agent-thread:${args.threadId}`,
    kind: "chat",
    title: args.title,
    tooltip: args.source === "claude-native"
    ...
    render: () => createElement(AgentThreadChatTab, {
        threadId: args.threadId,
        conversationId: args.conversationId,
