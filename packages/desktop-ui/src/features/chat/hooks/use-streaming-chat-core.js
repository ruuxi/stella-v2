...
import { buildAllLocalAttachments, toDisplayAttachments, } from '../streaming/message-context';
...
import { combineQueuedSendPayloads, orderQueuedMessages, removeQueuedUserMessageById, restoreQueuedMessagesAfterFailedDrain, timestampQueuedOptimisticEventForDrain, } from './queued-user-messages';
...
const buildContextMessageMetadata = (chatContext, base) => {
    ...
    const appSelectionLabel = appSelectionLabels.length > 0 ? appSelectionLabels.join(', ') : undefined;
...
export function useStreamingChatCore({ conversationId, locale, notifyTierRestrictedModel, persistedMessages, }) {
    const activeConversationId = conversationId;
    const [optimisticEvents, setOptimisticEvents] = useState([]);
    const [queuedUserMessages, setQueuedUserMessages] = useState([]);
    const queuedStreamPayloadsRef = useRef([]);
    const drainingQueuedPayloadsRef = useRef([]);
    const drainingQueuedMessageIdRef = useRef(null);
    ...
    const removeQueuedUserMessage = useCallback((messageId) => {
        queuedStreamPayloadsRef.current = queuedStreamPayloadsRef.current.filter((message) => message.id !== messageId);
        ...
        setQueuedUserMessages((current) => removeQueuedUserMessageById(current, messageId));
    }, []);
    const handleRunStarted = useCallback((event) => {
        const userMessageId = event.userMessageId;
        if (!userMessageId)
            return;
        queuedStreamPayloadsRef.current = queuedStreamPayloadsRef.current.filter((message) => message.id !== userMessageId);
        if (drainingQueuedMessageIdRef.current === userMessageId) {
            drainingQueuedMessageIdRef.current = null;
            drainingQueuedPayloadsRef.current = [];
        }
        setQueuedUserMessages((current) => removeQueuedUserMessageById(current, userMessageId));
    }, []);
    const handleRunFinished = useCallback((event) => {
    ...
    const clearOptimisticMessage = useCallback((messageId) => {
        setOptimisticEvents((current) => current.filter((event) => event._id !== messageId));
        setPendingUserMessageId((current) => current === messageId ? null : current);
    }, [setPendingUserMessageId]);
    const drainQueuedMessagesIfIdle = useCallback(() => {
        if (isStreaming || !activeConversationId)
            return;
        if (drainingQueuedMessageIdRef.current)
            return;
        if (queueDrainPausedRef.current)
            return;
        ...
        const drainable = orderQueuedMessages(queuedStreamPayloadsRef.current.filter((message) => message.conversationId === activeConversationId));
        ...
        queuedStreamPayloadsRef.current = queuedStreamPayloadsRef.current.filter((message) => !drainedIds.has(message.id));
        setQueuedUserMessages((current) => current.filter((message) => !drainedIds.has(message.id)));
        const dequeuedAtMs = issueDequeueTimestamp();
        const optimisticEventTemplate = drainable.length === 1
            ? combined.optimisticEvent
            : buildOptimisticUserEvent({
                id: combined.id,
                text: combined.userPrompt
        ...
        const optimisticEvent = timestampQueuedOptimisticEventForDrain(optimisticEventTemplate, dequeuedAtMs);
        setOptimisticEvents((current) => current.some((event) => event._id === combined.id)
            ? current
        ...
        startStream({
            ...
            onStartFailed: () => {
                ...
                queuedStreamPayloadsRef.current = restoreQueuedMessagesAfterFailedDrain(queuedStreamPayloadsRef.current, failedDrain);
                clearOptimisticMessage(combined.id);
                setQueuedUserMessages((current) => restoreQueuedMessagesAfterFailedDrain(current, failedDrain.map((message) => {
                    const payload = message.optimisticEvent.payload;
                    return {
                        id: message.id,
    ...
    useEffect(() => {
        ...
        const activeTurnAnswerVisible = persistedMessages.some((message) => {
            ...
            if (!message.payload || typeof message.payload !== 'object')
                return false;
            const payload = message.payload;
            if (payload.userMessageId !== pendingUserMessageId)
                return false;
            return typeof payload.text === 'string' && payload.text.trim().length > 0;
        });
        if (shouldTreatResumedAnswerAsStarted({
            isStreaming,
    ...
    useEffect(() => {
        const persistedIds = new Set(persistedMessages.map((message) => message._id));
        const queuedPayloads = queuedStreamPayloadsRef.current.filter((message) => !persistedIds.has(message.id));
    ...
    const sendMessage = useCallback(async (options) => {
        ...
        const contextState = resolveComposerContextState(options.chatContext, options.selectedText);
        const hasAttachments = Boolean(options.chatContext?.regionScreenshots?.length
        ...
        const attachments = isLocalStorage && hasAttachments
            ...
            : [];
        const messageMetadata = buildContextMessageMetadata(options.chatContext, options.metadata);
        ...
        const shouldQueueFollowUp = (isStreaming || drainingQueuedMessageIdRef.current !== null) &&
            (!pendingUserMessageId ||
                !persistedMessages.some((message) => {
                    ...
                    if (!message.payload || typeof message.payload !== 'object')
                        return false;
                    return (message.payload.userMessageId
                        === pendingUserMessageId);
                }));
        ...
        const optimisticText = cleanedText || options.selectedText?.trim() || 'Attached context';
        ...
        if (mode === 'follow_up') {
            console.log(`[stella:trace] sendMessage (follow_up queued) | convId=${resolvedConversationId}`);
            queueDrainPausedRef.current = false;
        ...
        console.log(`[stella:trace] sendMessage | convId=${resolvedConversationId} | text=${cleanedText.slice(0, 200)}`);
        startStream({
            userPrompt: cleanedText,
            selectedText: options.selectedText,
            chatContext: options.chatContext,
