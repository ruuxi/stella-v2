import { MAX_ACTIVE_RUNTIME_THREADS, MAX_GROUP_MEMBER_THREADS, THREAD_GROUP_KEY_PREFIX, normalizeRuntimeThreadId, } from "../runtime-threads.js";
...
export const tokenizeSearchQuery = (query) => {
    const rawTokens = (query ?? "")
        ...
        .filter(Boolean);
    const meaningful = rawTokens.filter((token) => !SEARCH_STOPWORDS.has(token.toLowerCase()));
    return (meaningful.length > 0 ? meaningful : rawTokens).slice(0, 12);
};
const toIsoTimestamp = (timestamp) => new Date(timestamp).toISOString();
const formatThreadCheckpointMessage = (summary) => [THREAD_CHECKPOINT_MARKER, "", summary.trim()].join("\n");
...
const previewFromAssistantPayload = (payload) => payload.content
    .flatMap((block) => {
    if (block.type === "text") {
...
const payloadByteLength = (payload) => rowSizeTextEncoder.encode(JSON.stringify(payload)).byteLength;
const customMessageByteLength = (message) => rowSizeTextEncoder.encode(JSON.stringify(message)).byteLength;
const truncatePreview = (value, maxChars = THREAD_ROW_PREVIEW_CHARS) => value.length <= maxChars ? value : `${value.slice(0, maxChars)}...`;
const truncateTextBlockForStorage = (text, label = "Text") => `[${label} truncated for storage (${text.length} chars). First ${Math.min(text.length, THREAD_ROW_PREVIEW_CHARS)} chars: ${truncatePreview(text)}]`;
const truncateToolOutputForStorage = (text) => `This tool output was too large to persist in storage (${text.length} chars). If the user asks about this data, suggest re-running the tool. Preview: ${truncatePreview(text)}`;
...
const enforceThreadPayloadRowSizeLimit = (payload) => {
    ...
    if (payload.role === "user") {
        const content = typeof payload.content === "string"
            ...
            : payload.content.map((block) => block.type === "text" &&
        ...
        if (typeof content !== "string") {
            const withoutImageData = {
                ...payload,
                content: content.map((block) => block.type === "image" ? strippedImageStorageNote(block) : block),
            };
        ...
        return {
            ...
            content: typeof payload.content === "string"
                ...
                : [
                    {
                        type: "text",
                        text: truncateTextBlockForStorage(JSON.stringify(payload.content), "User content"),
                    },
                ],
        };
    }
    if (payload.role === "assistant") {
        ...
        return {
            ...
            content: [
                {
                    type: "text",
                    text: truncateTextBlockForStorage(JSON.stringify(payload.content), "Assistant message"),
                },
            ],
        };
    }
    const compacted = {
        ...payload,
        content: payload.content.map((block) => block.type === "text" && block.text.length > THREAD_ROW_MAX_TEXT_CHARS
    ...
    return {
        ...
        content: typeof message.content === "string"
            ...
            : [
                {
                    type: "text",
                    text: truncateTextBlockForStorage(JSON.stringify(message.content), "Custom message"),
                },
            ],
    };
};
...
const parseThreadSessionEntry = (row) => {
    ...
    switch (row.entryType) {
        case "message": {
            const rawMessage = data && "message" in data
        ...
        case "compaction": {
            ...
            const fromEntryId = typeof data?.fromEntryId === "string" ? data.fromEntryId.trim() : "";
            const toEntryId = typeof data?.toEntryId === "string" ? data.toEntryId.trim() : "";
            const firstKeptEntryId = typeof data?.firstKeptEntryId === "string"
                ? data.firstKeptEntryId.trim()
                : "";
            const tokensBefore = typeof data?.tokensBefore === "number" &&
        ...
        case "custom_message": {
            const customType = typeof data?.customType === "string" ? data.customType.trim() : "";
...
const buildRawThreadMessages = (path) => path
...
const normalizeCompactionOverlay = (compaction, rawMessages) => {
    ...
    const firstKeptIndex = rawMessages.findIndex((message) => message.entryId === compaction.firstKeptEntryId);
    if (firstKeptIndex <= 0) {
        return null;
    }
...
const buildThreadCompactionOverlays = (path, rawMessages) => path
...
export class SessionStore {
    ...
    `);
        parts.forEach((part, index) => {
            stmt.run(`${messageId}:part:${index}`, sessionId, messageId, index, part.type, part.toolCallId ?? null, toJsonValueString(part.data), part.createdAt);
        });
    }
    upsertEventMessage(args) {
      ...
      ON CONFLICT(id) DO UPDATE SET
        ...
        updated_at = excluded.updated_at
    `)
            .run(args.eventId, args.sessionId, eventRoleForType(args.type), args.type, args.requestId ?? null, args.deviceId ?? null, args.targetDeviceId ?? null, toJsonString(args.channelEnvelope
            ? { channelEnvelope: args.channelEnvelope }
            : undefined), args.timestamp, args.timestamp);
    ...
    hasEventId(eventIdInput, typeInput) {
        ...
        if (!eventId)
            return false;
        const type = asTrimmedString(typeInput);
        const statement = this.db.prepare(type
    ...
    mergeEventPayload(args) {
        ...
        `)
            ...
            const existingPartCount = this.db
                .prepare(`SELECT COUNT(*) AS n FROM part WHERE message_id = ?`)
                .get(eventId)?.n ?? 0;
            if (existingPartCount > 1) {
                console.warn(`[session-store] mergeEventPayload destructively replaced ${existingPartCount} parts for event ${eventId} (conversation ${conversationId}); only ord:0 will survive. A multi-part event type now exists — switch this method to a part-level merge.`);
            }
            ...
            const now = Date.now();
            this.db
                .prepare(`UPDATE message SET updated_at = ? WHERE id = ? AND session_id = ?`)
    ...
    listRecentActivitySince(args) {
        const sinceMs = Number.isFinite(args.sinceMs)
            ...
            : 0;
        const normalizedLimit = Math.max(1, Math.min(Math.floor(args.limit ?? 80), 500));
        const rows = this.db
            .prepare(`
      SELECT
    ...
    listEventsBefore(conversationIdInput, opts) {
      ...
      ORDER BY recent.created_at ASC, recent.id ASC
    `)
            .all(conversationId, beforeTimestamp, beforeTimestamp, beforeId, normalizedLimit);
        return rows.map((row) => this.deserializeEventRow(row));
    }
    /**
    ...
    listMessages(conversationIdInput, args = {}) {
        const conversationId = this.sanitizeConversationId(conversationIdInput);
        const maxVisibleMessages = Math.max(1, Math.floor(args.maxVisibleMessages ?? 200));
        const cutoff = this.findVisibleMessageCutoff(conversationId, maxVisibleMessages);
    ...
    listMessagesBefore(conversationIdInput, args) {
        ...
        const cutoff = this.findVisibleMessageCutoffBefore(conversationId, maxVisibleMessages, before);
        const fetchCutoff = this.findTurnFetchCutoff(conversationId, cutoff);
    ...
    listMessagesAfter(conversationIdInput, args) {
        ...
        const rows = this.fetchTimelineRows(conversationId, fetchCutoff, null, null, CUTOFF_SCAN_CEILING);
        const sourceEvents = rows.filter((row) => compareTimelineCursor({ timestamp: row.timestamp, id: row._id }, after) > 0);
        return {
            ...this.limitChangedMessageWindow(this.assembleMessageWindow(rows), after, maxVisibleMessages),
            sourceEvents,
        };
    }
    ...
    `)
        ...
        for (const start of lifecycleEvents) {
            ...
            const turn = this.assembleMessageWindow(this.fetchTimelineRows(conversationId, turnStart, nextTurn));
            const anchor = turn.messages.find((message) => message.toolEvents.some((event) => event._id === start._id)) ?? turn.messages.find((message) => message.type === "user_message");
            if (!anchor)
                continue;
        ...
        const messages = [...anchorsById.values()].sort((a, b) => a.timestamp - b.timestamp || a._id.localeCompare(b._id));
        return {
            messages,
            visibleMessageCount: messages.filter((message) => !isUiHiddenChatMessagePayload(message.payload ?? null)).length,
        };
    }
    /**
    ...
    findVisibleMessageCutoff(conversationId, maxVisibleMessages) {
        return this.findVisibleMessageCutoffPaged(conversationId, maxVisibleMessages, null);
    }
    findVisibleMessageCutoffBefore(conversationId, maxVisibleMessages, before) {
        return this.findVisibleMessageCutoffPaged(conversationId, maxVisibleMessages, before);
    }
    findVisibleMessageCutoffPaged(conversationId, maxVisibleMessages, initialBefore) {
        const batchSize = Math.min(CUTOFF_SCAN_BATCH_MAX, Math.max(CUTOFF_SCAN_BATCH_MIN, maxVisibleMessages * 2));
    ...
    trimMessageWindow(window, cutoff) {
        ...
        const messages = window.messages.filter((message) => {
            const keep = compareTimelineCursor({ timestamp: message.timestamp, id: message._id }, cutoff) >= 0;
    ...
    limitChangedMessageWindow(window, after, maxVisibleMessages) {
        ...
        let visibleMessageCount = 0;
        for (const message of window.messages) {
            const messageChanged = compareTimelineCursor({ timestamp: message.timestamp, id: message._id }, after) > 0;
            const toolEventsChanged = message.toolEvents.some((event) => compareTimelineCursor({ timestamp: event.timestamp, id: event._id }, after) > 0);
            if (!messageChanged && !toolEventsChanged)
                continue;
            messages.push(message);
            if (!isUiHiddenChatMessagePayload(message.payload ?? null)) {
                visibleMessageCount += 1;
    ...
    listActivity(conversationIdInput, args = {}) {
        ...
        const before = typeof args.beforeTimestampMs === "number"
            ? {
    ...
    listSyncMessages(conversationIdInput, maxMessages = MAX_EVENTS_PER_CONVERSATION) {
        const conversationId = this.sanitizeConversationId(conversationIdInput);
    ...
    `)
        ...
        for (const row of rows) {
            const payload = parseJsonRecord(row.payloadJson);
            if (isUiHiddenChatMessagePayload(payload ?? null))
                continue;
    ...
    ensureThreadSession(threadKey, conversationId, timestamp) {
      ...
      VALUES (?, ?, ?, ?, NULL, ?, ?)
    `)
            .run(threadKey, sessionId, RUNTIME_THREAD_SESSION_VERSION, cwd, timestamp, timestamp);
        return {
            sessionId,
            createdAt: timestamp,
            cwd,
    ...
    appendThreadSessionEntry(args) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `)
            .run(entryId, args.threadKey, args.sessionId, parentEntryId, args.entryType, timestampIso, args.timestamp, toJsonValueString(args.data));
        return entryId;
    }
    ...
    appendThreadMessage(message) {
        ...
        const payload = enforceThreadPayloadRowSizeLimit(buildFallbackThreadPayload(message));
        this.withTransaction(() => {
            this.upsertSession(conversationId, message.timestamp);
            const threadSession = this.ensureThreadSession(threadKey, conversationId, message.timestamp);
    ...
    loadThreadMessages(threadKeyInput, limit) {
        ...
        return buildThreadMessagesFromEntries(this.loadThreadSessionEntries(threadKey, limit)).map((message) => ({
    ...
    compactThread(args) {
        ...
        this.withTransaction(() => {
            ...
            const normalizedFromEntryId = existingOverlays[0]?.fromEntryId ?? fromEntryId;
            const threadSession = this.ensureThreadSession(threadKey, conversationId, timestamp);
            this.appendThreadSessionEntry({
                threadKey,
                sessionId: threadSession.sessionId,
    ...
    recordRunEvent(event) {
        ...
        ON CONFLICT(id) DO UPDATE SET
          ...
          updated_at = excluded.updated_at
      `)
                .run(messageId, event.conversationId, event.runId, event.agentType, toJsonString({
  ...
  `;
    ...
    threadFtsAvailable() {
        if (this.hasThreadFts === undefined) {
            try {
                this.hasThreadFts = Boolean(this.db
                    .prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'thread_search_fts'")
                    .get());
            }
            catch {
    ...
    listThreadsForRecallIndex(args) {
        ...
        const activeSinceMs = Number.isFinite(args.activeSinceMs) && (args.activeSinceMs ?? 0) > 0
    ...
    transcriptFtsAvailable() {
        if (this.hasTranscriptFts === undefined) {
            try {
                this.hasTranscriptFts = Boolean(this.db
                    .prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'message_text_fts'")
                    .get());
            }
            catch {
    ...
    searchTranscriptsFts(tokens, limit) {
      ...
      LIMIT ?
    `)
            .all(matchQuery, ...tokens.map((token) => `%${escapeLike(token)}%`), limit);
    ...
    searchTranscriptsLike(tokens, limit) {
      ...
      LIMIT ?
    `)
            .all(...tokens.map((token) => `%${escapeLike(token)}%`), // WHERE binds,
    ...
    `;
        const rows = [
            ...(before > 0
                ? this.db
                    .prepare(`${base} AND message.created_at < ? AND message.created_at >= ?
    ...
    reactivateThread(conversationId, threadId) {
        if (this.listActiveThreadsByAge(conversationId).length >=
    ...
    mintUniqueKey(base) {
        if (!this.threadKeyExists(base))
            return base;
        for (let ordinal = 2;; ordinal++) {
            const candidate = `${base}-${ordinal}`;
            if (!this.threadKeyExists(candidate))
                return candidate;
        }
    }
    mintThreadKey(args) {
    ...
    `)
        ...
        let nextOrdinal = 1;
        for (const row of rows) {
            if (!row.threadId.startsWith(prefix))
                continue;
    ...
    resolveOrCreateActiveThread(args) {
      ...
      `)
        ...
        if (this.listActiveThreadsByAge(args.conversationId).length >=
            MAX_ACTIVE_RUNTIME_THREADS) {
        ...
        const name = args.nameHint?.trim().replace(/\s+/g, " ").slice(0, 200) || threadId;
        const now = Date.now();
    ...
    setThreadExternalSessionId(threadKey, externalSessionId) {
        this.ensureImplicitThreadRow(threadKey);
        const normalized = typeof externalSessionId === "string" &&
     ...
     */
    replaceAgentProgressSummaries(summariesByAgentId) {
        const deleteStmt = this.db.prepare(`DELETE FROM agent_progress_summaries WHERE agent_id = ?`);
        const insertStmt = this.db.prepare(`INSERT INTO agent_progress_summaries (agent_id, text, created_at) VALUES (?, ?, ?)`);
        for (const [rawAgentId, rawList] of Object.entries(summariesByAgentId ?? {})) {
            const agentId = rawAgentId.trim();
            if (!agentId || !Array.isArray(rawList))
                continue;
    ...
    listAgentProgressSummaries(agentId, limit = 3) {
        const cappedLimit = Math.max(1, Math.min(MAX_PERSISTED_PROGRESS_SUMMARIES_PER_AGENT, Math.floor(limit)));
        const rows = this.db
            .prepare(`
    ...
    `)
        ...
        const current = typeof currentState?.reminderTokensSinceLastInjection === "number"
    ...
    `)
            .get(conversationId);
        const reminderTokensSinceLastInjection = typeof currentState?.reminderTokensSinceLastInjection === "number"
            ? currentState.reminderTokensSinceLastInjection
            : 0;
        this.db
            .prepare(`
    ...
    `)
            .get(conversationId);
        return {
            userTurnsSinceReview: typeof row?.userTurnsSinceReview === "number"
                ? Math.max(0, Math.floor(row.userTurnsSinceReview))
                : 0,
            lastReviewedMessageTs: typeof row?.lastReviewedMessageTs === "number"
    ...
    resetUserTurnsSinceMemoryReview(conversationId, lastReviewedMessageTs) {
        const now = Date.now();
        const reviewedTs = typeof lastReviewedMessageTs === "number" &&
