# Post-build gate for the Windows release: proves every artifact Stella claims
# authorship of carries a valid Authenticode signature from the exact approved
# DigiCert certificate, with an RFC 3161 timestamp so the signature outlives
# the certificate's validity window.
#
# Both `Get-AuthenticodeSignature` (chain + signer identity) and Microsoft
# SignTool (`verify /pa`, the Authenticode policy the OS actually applies) are
# required: the cmdlet alone accepts signatures SmartScreen would still reject.
#
# The uninstaller only exists after NSIS runs, so the gate performs a silent
# install into a temp directory, verifies the generated uninstaller, then
# silently uninstalls. Emits a redacted JSON report for CI upload.
param(
...
# Thumbprints and serials arrive from repository variables in whatever
# formatting a human pasted (spaces, colons, mixed case); compare on hex only.
...
function Resolve-SignTool {
  ...
  # Hosted runners ship the Windows SDK but do not put SignTool on PATH.
...
# X.500 quotes an RDN value containing a comma, so accept both encodings.
...
# The pinned manifest is the source of truth for which native helpers ship, so
# the count check also catches an unsigned helper that slipped into the tree.
...
# The browser CLI may arrive with or without an .exe suffix depending on the
# immutable manifest, which is why package.json also lists it under signExts.
...
# The uninstaller is generated at install time, so it can only be verified by
# actually installing. `/D=` must come last and unquoted for NSIS.
...
# Report holds only public certificate identity, never the thumbprint/serial
# the workflow masks.
$report = [ordered]@{
  schemaVersion = 1
