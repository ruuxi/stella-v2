    ...
    return {
        ...
        parameters: {
            ...
            properties: {
                prompt: {
                    type: "string",
                    description: "Description of the image to generate. Be specific about subject, style, framing, color, lighting, and any text overlays.",
                },
                aspectRatio: {
                    type: "string",
                    description: "Optional aspect ratio (e.g. '1:1', '16:9', '9:16', '4:3'). Defaults to the gateway's recommended ratio.",
                },
                size: {
                    type: "object",
                    description: "Optional explicit pixel dimensions. Only set this when the default aspectRatio presets won't do (e.g. sprite atlases at non-standard sizes). Subject to the model envelope: max edge ≤ 3840, 655,360 ≤ width × height ≤ 8,294,400, longest edge ≤ 3× shortest edge.",
                    properties: {
                ...
                profile: {
                    type: "string",
                    enum: ["best", "fast"],
                    description: "Optional model profile. Use 'fast' for Fashion try-ons and quick drafts.",
                },
                referenceImagePaths: {
                    type: "array",
                    items: { type: "string" },
                    maxItems: MAX_MANAGED_IMAGE_REFERENCE_ITEMS,
                    description: "Optional local image paths to use as reference inputs. At most four total references may be supplied across paths and URLs. Managed generation safely normalizes local bytes into a bounded upload envelope. When any reference is provided the gateway switches from text_to_image to image_edit.",
                },
                referenceImageUrls: {
                    type: "array",
                    items: { type: "string" },
                    maxItems: MAX_MANAGED_IMAGE_REFERENCE_ITEMS,
                    description: "Optional remote http(s) image URLs or validated data:image URLs to use as reference inputs. At most four total references may be supplied across URLs and paths. Mix with referenceImagePaths when you have a local subject photo plus catalog product photos.",
                },
