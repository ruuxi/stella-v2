...
const port = randomInt(26_000, 27_000);
...
describe("Guarded model fetch in real Workerd", () => {
  ...
  beforeAll(async () => {
    persistencePath = await mkdtemp(
      join(tmpdir(), "stella-guarded-model-fetch-workerd-"),
    );
    ...
    const child = spawn(
      ...
      [
        ...
        "dev",
        "--config",
        "tests/fixtures/guarded-model-fetch-workerd.wrangler.jsonc",
        "--ip",
        "127.0.0.1",
        "--port",
        String(port),
  ...
  afterAll(async () => {
    ...
    try {
      ...
      if (persistencePath.includes("stella-guarded-model-fetch-workerd-")) {
  ...
  test("routes through a service binding and owner object before releasing the body", async () => {
    const response = await fetch(`${origin}/allow`);
    ...
    expect(report.result).toEqual({ status: 201, body: report.expectedBody, header: "fixture" });
    expect(report.bytes).toBe(new TextEncoder().encode(report.expectedBody).byteLength);
  });

  for (const [mode, error] of [["deny", "MEMORY_POLICY_CHANGED"], ["cancel", "exact turn canceled"]]) {
  ...
  test("preserves an early gateway refusal without sending private content", async () => {
    const response = await fetch(`${origin}/refuse`);
    const report = await response.json();
    expect(report.result).toEqual({ status: 403, body: "account suspended", header: null });
    expect(report.bytes).toBe(0);
  });
});
