...
export class ExplicitModelAgent {
  ...
  constructor(
    opts: AgentOptions & {
      initialState: Partial<AgentState> & Pick<AgentState, "model">;
    },
  ) {
    const { model, ...initialState } = opts.initialState;
    this._state = {
      systemPrompt: "",
      model,
      thinkingLevel: "off",
  ...
  private _processLoopEvent(event: AgentEvent): void {
    switch (event.type) {
      case "message_start":
        this._state.streamMessage = isAssistantMessage(event.message)
          ? event.message
    ...
    const config: AgentLoopConfig = {
      ...
      onTurnBoundary: this._onTurnBoundary
        ? async (boundaryContext, signal) => {
            try {
              const replacement = await this._onTurnBoundary?.(
                boundaryContext,
                signal,
              );
