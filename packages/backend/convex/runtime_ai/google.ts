...
function mapToolChoice(
  choice: GoogleToolChoice | undefined,
): GenerateContentConfig["toolConfig"] | undefined {
...
function buildParams(
  ...
  return {
    ...
    config: {
      ...
      toolConfig: mapToolChoice(
        context.tools && context.tools.length > 0 ? "auto" : undefined,
      ),
      thinkingConfig:
    ...
    try {
      ...
      const client = new GoogleGenAI({
        ...
        httpOptions: {
          ...
          // Every physical retry must reacquire the outer durable dispatch
          // lease; never let the SDK retry invisibly inside one lease.
      ...
      for await (const chunk of googleStream) {
        ...
        for (const part of candidate?.content?.parts ?? []) {
          if (typeof part.text === "string") {
            ...
            const active =
              activeIndex === null ? null : output.content[activeIndex];
            if (
              !active ||
              (isThinking && active.type !== "thinking") ||
              (!isThinking && active.type !== "text")
            ) {
              if (active?.type === "text") {
                stream.push({
                  type: "text_end",
          ...
          if (part.functionCall) {
            ...
            const toolCall: ToolCall = {
              ...
              id:
                ...
                `${part.functionCall.name}_${Date.now()}_${++toolCounter}`,
