...
Write `prompt` as what you're trying to find, in your own words. `memorySearchTerms` is required: 2-8 concrete grep-like terms from the user's wording, repo/module names, feature names, dates, file names, error text, or prior decision keywords. They pre-run the memory, past-thread, and transcript searches before the recall agent starts, so pick terms that would appear in the thing itself — the recall agent also reformulates and searches on its own. If `Recall` returns `Nothing relevant found.`, continue from the visible request.
