...
const request = (signal?: AbortSignal) => new Request("https://gateway/model", {
  method: "POST", body: JSON.stringify({ messages: ["private memory"] }),
...
describe("guarded model fetch", () => {
  test("enters transport before authorization and withholds every body byte", async () => {
    ...
    const work = guardedModelFetch({
      request: request(), authorize: () => authorize.promise,
      fetch: async value => {
        entered.resolve();
        expect(value.headers.get("authorization")).toBe("Bearer test-capability");
        const body = await value.arrayBuffer();
        bytes = body.byteLength;
        return new Response(body, { status: 201, headers: { "x-upstream": "retained" } });
      },
    });
  ...
  test("cancels a pending guard without waiting for its remote call", async () => {
    ...
    let bytes = 0;
    const work = guardedModelFetch({
      request: request(abort.signal), authorize: () => authorize.promise,
      fetch: async value => {
        entered.resolve();
  ...
  test("returns an early gateway refusal intact without reading the source", async () => {
    ...
    const body = new ReadableStream<Uint8Array>({
      pull(controller) { sourceRead = true; controller.enqueue(new Uint8Array([1])); },
      cancel() { sourceCanceled = true; },
    }, { highWaterMark: 0 });
    const work = guardedModelFetch({
      request: new Request("https://gateway/model", { method: "POST", body }),
      authorize: () => authorize.promise,
      fetch: async () => { entered.resolve(); return new Response("suspended", { status: 403 }); },
    });
    await entered.promise;
    authorize.resolve();
    const response = await work;
  ...
  test("does not retry a failed transport", async () => {
    ...
    await expect(guardedModelFetch({
      request: request(), authorize: async () => undefined,
