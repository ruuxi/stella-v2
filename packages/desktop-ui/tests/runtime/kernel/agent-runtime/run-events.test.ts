...
describe("subscribeRuntimeAgentEvents", () => {
  ...
  it("keeps the indicator up while a spawned sub-agent runs (no early dismiss)", () => {
    ...
    // must STAY up — the submitted root turn owns it until the terminal run
    // event, regardless of background/spawned work.
    state = streamStoreReducer(state, {
      type: "task-upsert",
      runId,
      conversationId: "conversation-1",
    ...
    // The first visible provider delta no longer hands off — the indicator
    // owns the whole submitted turn.
    state = streamStoreReducer(state, { type: "mark-streaming-text", runId });
    expect(activeFor(state)).toBe(true);

    // Only the terminal run event dismisses it.
    state = streamStoreReducer(state, {
      type: "run-finished",
      runId,
      conversationId: "conversation-1",
  ...
  it("keeps the working indicator alive in reasoning gaps after an interim/preamble message", () => {
    ...
    // Model streams a preamble ("Let me check…") — the flag flips but the
    // indicator stays up (the whole turn owns it now).
    state = streamStoreReducer(state, { type: "mark-streaming-text", runId });
    ...
    // Final answer streams — still visible; only the terminal run event
  ...
  it("re-arms the working indicator at a preamble→tool-call boundary before the tool starts", () => {
    ...
    // Model streams a preamble ("Let me check…") — the flag flips while the
    // indicator stays mounted for the whole turn.
    state = streamStoreReducer(state, { type: "mark-streaming-text", runId });
    expect(state.runsById[runId]?.isStreamingText).toBe(true);
    expect(activeFor(state)).toBe(true);

    // The preamble message finalizes and it ends with a tool call. The
    // boundary clears the streaming-text flag so downstream consumers see
    // the pre-tool thinking phase again.
    state = streamStoreReducer(state, {
  ...
  it("rejects a stale response marker after the preamble boundary", () => {
    ...
    // The post-tool answer's first visible delta flips the flag; the
    // indicator stays up until the run goes terminal.
    state = streamStoreReducer(state, { type: "mark-streaming-text", runId });
    expect(state.runsById[runId]?.isStreamingText).toBe(true);
    expect(activeFor(state)).toBe(true);
    state = streamStoreReducer(state, {
  ...
  it("keeps the indicator up when a stale preamble marker arrives after tool-start", () => {
    ...
    // Only the post-tool answer's own visible delta flips the flag; the
    // indicator itself stays up until the run goes terminal.
    state = streamStoreReducer(state, { type: "mark-streaming-text", runId });
    expect(state.runsById[runId]?.isStreamingText).toBe(true);
    expect(activeFor(state)).toBe(true);
    state = streamStoreReducer(state, {
