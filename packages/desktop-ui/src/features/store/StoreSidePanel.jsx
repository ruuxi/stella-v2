...
import { featureKeyOf, refreshFeatureSnapshot, storeSidePanelStore, useStoreSidePanelState, } from "./store-side-panel-store";
import "./store.css";
import { ShareDialog, } from "./store-side-panel/ShareDialog";
...
export function StoreSidePanel() {
    ...
    const [publishFeatures, setPublishFeatures] = useState([]);
    ...
    const handlePublished = useCallback(async (_args) => {
        storeSidePanelStore.clearSelections();
        setPublishFeatures([]);
        setPublishOpen(false);
    }, []);
    return (<div className="right-sidebar__rich right-sidebar__rich--store store-side-panel" data-store-display-tab="store">
      <RecentChangesList snapshot={state.snapshot} snapshotLoading={state.snapshotLoading} selectedFeatureKeys={state.selectedFeatureKeys} olderEntries={state.olderEntries} rosterTotal={state.rosterTotal} olderLoading={state.olderLoading} onPublishSelected={() => void handlePublishSelected()}/>

      {(state.snapshot?.items ?? []).length === 0 && !state.snapshotLoading ? (<div className="store-side-panel-empty-state">
      ...
      <ShareDialog open={publishOpen} selectedFeatures={publishFeatures} onClose={() => {
            setPublishOpen(false);
            setPublishFeatures([]);
