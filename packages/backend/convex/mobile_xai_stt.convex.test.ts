...
describe("mobile and CarPlay cloud dictation", () => {
  it("sends the shared transcription route directly to xAI without a model field", async () => {
    ...
    const response = await t.fetch("/api/mobile/transcribe", {
      ...
      body: JSON.stringify({
        requestId: "mobile-stt:xai-shared-route-0001",
    ...
    // xAI's REST STT endpoint has a single model and rejects a model field;
    // the container only shows up as the file's name/MIME type.
