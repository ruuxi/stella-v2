...
describe("session-store", () => {
  ...
  // Appends 4k+ events synchronously; sits at the 5s default's edge when the
  // whole suite runs in parallel on a loaded machine.
  it("keeps listMessages bounded when the requested visible window exceeds the scan ceiling", { timeout: 20_000 }, () => {
    const { store } = createTestContext();
    const conversationId = store.getOrCreateDefaultConversationId();
