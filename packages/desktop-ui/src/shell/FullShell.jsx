...
import { CREATURE_HIDDEN_PHASES, } from "@/global/onboarding/onboarding-flow";
...
import { readLocalOnboardingCompleted, useOnboardingState, } from "@/global/onboarding/use-onboarding-state";
...
const onboardingChunkPromise = {
    current: null,
};
const loadOnboardingChunk = () => {
    if (!onboardingChunkPromise.current) {
        onboardingChunkPromise.current = import("@/global/onboarding/OnboardingOverlay");
    }
    return onboardingChunkPromise.current;
};
const OnboardingView = lazy(() => import("@/global/onboarding/OnboardingOverlay").then((module) => ({
...
function OnboardingExperience({ activeConversationId, onEnteredApp, }) {
    ...
    return (<div className="onboarding-layout" data-split={onboarding.splitMode || undefined}>
      <Suspense fallback={null}>
        <OnboardingView hasExpanded={onboarding.hasExpanded} onboardingDone={onboarding.onboardingDone} onboardingExiting={onboarding.onboardingExiting} isAuthenticated={onboarding.isAuthenticated} splitMode={onboarding.splitMode} splitEntering={onboarding.splitEntering} hasDiscoverySelections={onboarding.hasDiscoverySelections} hasStarted={onboarding.hasStarted} stellaAnimationRef={onboarding.stellaAnimationRef} stellaAnimationPaused={pauseStellaAnimation} stellaAnimationHidden={stellaHiddenByPhase} onboardingKey={onboarding.onboardingKey} initialPhase={onboarding.initialPhase} creatureInitialBirth={onboarding.creatureInitialBirth} triggerFlash={onboarding.triggerFlash} startOnboarding={onboarding.startOnboarding} completeOnboarding={onboarding.completeOnboarding} handleEnterSplit={onboarding.handleEnterSplit} onDiscoveryConfirm={handleDiscoveryConfirm} onSelectionChange={onboarding.setHasDiscoverySelections} onPhaseChange={handleOnboardingPhaseChange} discoveryWelcomeExpected={discoveryWelcomeExpected} discoveryWelcomeReady={discoveryWelcomeReady}/>
      </Suspense>
    </div>);
}
export const FullShell = () => {
    ...
    const { completed: onboardingDone, hydrated: onboardingHydrated } = useOnboardingState();
    ...
    const [hasEnteredApp, setHasEnteredApp] = useState(() => readLocalOnboardingCompleted());
    ...
    useEffect(() => {
        if (!onboardingResolved || !onboardingDone)
            return;
    ...
    return (<div className="window-shell full" data-window-mode={needsOnboarding ? "onboarding" : "app"}>
      <ShiftingGradient mode={gradientMode} colorMode={gradientColor} lightweight={false}/>
      <div className="full-body">
        {appReady ? (<>
            <RouterProvider router={router}/>
            <AskStellaSelectionChip />
          </>) : needsOnboarding ? (<OnboardingExperience activeConversationId={activeConversationId} onEnteredApp={() => setHasEnteredApp(true)}/>) : null}
      </div>
    </div>);
};
