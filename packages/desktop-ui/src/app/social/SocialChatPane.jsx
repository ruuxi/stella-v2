...
import { useSocialSession, } from "./hooks/use-social-session";
 ...
 */
const SocialItemSeparator = () => (<div style={{ height: 16 }} aria-hidden="true"/>);
function getProfileForOwner(roomData, extraProfiles, ownerId) {
...
export function SocialChatPane({ roomId, currentOwnerId, onBack, }) {
    const roomData = useQuery(api.social.rooms.getRoom, {
        roomId,
    });
    const { messages, sendMessage, loadOlder: loadOlderMessages, hasOlder: hasOlderMessages, isLoadingOlder: isLoadingOlderMessages, } = useSocialMessages(roomId, currentOwnerId);
    ...
    useEffect(() => {
        ...
        if (latestMessageAt === undefined)
            return;
        if (lastReadAt !== undefined && latestMessageAt <= lastReadAt)
            return;
    ...
    const { sessionSummary, turns, loadOlderTurns, hasOlderTurns, isLoadingOlderTurns, } = useSocialSession(sessionLookupId);
    ...
    const stellaArmed = armedState.roomId === roomId && armedState.armed && sessionIsLive;
    ...
    const timelineRows = useMemo(() => {
        ...
        for (const turn of turns) {
            ...
            const responseBody = turn.status === "completed"
    ...
    const renderGroup = useCallback(({ item: group, }) => {
        ...
        if (isSystem) {
            ...
            return (<div key={msg.id} className="social-message-bubble" data-role="system">
            {msg.body}
          </div>);
        }
        const role = isStella ? "stella" : isSelf ? "self" : "other";
        return (<div className="social-message-group">
          {!isSelf && (<div className="social-message-sender">
              {isStella ? (<span className="social-message-sender-avatar social-message-sender-avatar--stella">
                  <img src="stella-logo.svg" alt=""/>
                </span>) : (<Avatar fallback={profile.username} src={profile.avatarUrl} size="small"/>)}
              <span className="social-message-sender-name" data-stella={isStella || undefined}>
          ...
          {isSelf && (<div className="social-message-sender" style={{ justifyContent: "flex-end" }}>
              <span className="social-message-sender-time">
                {formatMessageTime(group.firstTimestamp)}
              </span>
            </div>)}
          {group.messages.map((msg) => {
                ...
                const shareLink = parsed?.kind === "view-store-package"
                    ? {
                        authorUsername: parsed.authorUsername,
                        packageId: parsed.packageId,
                    }
                    : null;
                const inviteLink = parsed && parsed.kind !== "view-store-package" ? parsed : null;
                return (<div key={msg.id} className="social-message-bubble" data-role={role} data-pending={msg.pending || undefined} data-embed={shareLink
                        ? "addon-share"
                        : inviteLink
                            ? "social-invite"
                            : undefined}>
                {shareLink ? (<AddonShareCard link={shareLink} variant="wide"/>) : inviteLink ? (<SocialInviteCard invite={inviteLink}/>) : (msg.body)}
              </div>);
            })}
        </div>);
    }, [currentOwnerId, roomData]);
    const groupKeyExtractor = useCallback((group) => group.messages[0].id, []);
    ...
    const handleStartSession = useCallback(async () => {
        ...
        try {
            ...
            showToast({
                variant: "error",
                description: getSocialActionErrorMessage("Couldn't start Stella here. Please try again.", error),
            });
        }
        finally {
    ...
    const handleUpdateSessionStatus = useCallback(async (status) => {
        ...
        try {
            ...
            showToast({
                variant: "error",
                description: getSocialActionErrorMessage("Couldn't update Stella right now. Please try again.", error),
            });
        }
        finally {
    ...
    const handleSend = useCallback(async (body) => {
        if (stellaArmed && activeSession && socialSessionsApi) {
            try {
                ...
                showToast({
                    variant: "error",
                    description: getSocialActionErrorMessage("Couldn't send that to Stella. Please try again.", error),
                });
            }
            return;
        }
        try {
            ...
            showToast({
                variant: "error",
                description: getSocialActionErrorMessage("Couldn't send your message. Please try again.", error),
            });
        }
    ...
    return (<div className="social-chat-pane">
      <div className="social-chat-header">
        {onBack ? (<button type="button" className="social-chat-back" aria-label="Back to conversations" title="Back" onClick={onBack}>
            <ChevronLeft size={20}/>
          </button>) : null}
        <div className="social-chat-header-info">
          <div className="social-chat-header-name">{displayName}</div>
          {roomData.memberProfiles.length > 2 && (<div className="social-chat-header-meta">
        ...
        */}
        {!activeSession && (<button type="button" className="social-stella-pill" onClick={() => void handleStartSession()} disabled={isStartingSession} title="Bring Stella into this conversation">
            <img src="stella-logo.svg" alt="" className="social-stella-pill-logo"/>
            <span>{isStartingSession ? "Starting..." : "Start Stella"}</span>
          </button>)}
      </div>

      {activeSession && (<div className="social-session-bar" data-state={activeSession.status}>
          <div className="social-session-bar-info">
            <img src="stella-logo.svg" alt="" className="social-session-header-logo"/>
          ...
          <div className="social-session-actions">
            {activeSession.status === "ended" ? (<button type="button" className="social-session-button" onClick={() => void handleStartSession()} disabled={isStartingSession}>
                {isStartingSession ? "Starting..." : "Start Again"}
              </button>) : null}
            {isHost && activeSession.status !== "ended" ? (<>
                <button type="button" className="social-session-button" onClick={() => void handleUpdateSessionStatus(activeSession.status === "active" ? "paused" : "active")} disabled={isUpdatingSession}>
                  {activeSession.status === "active" ? "Pause" : "Resume"}
                </button>
                <button type="button" className="social-session-button" data-variant="danger" onClick={() => void handleUpdateSessionStatus("ended")} disabled={isUpdatingSession}>
                  End
                </button>
              </>) : null}
          </div>
      ...
      <div className="social-messages-viewport">
        {messageGroups.length === 0 ? (<div className="social-empty-state">
          ...
          </div>) : (<LegendList ref={socialScroll.listRef} data={messageGroups} keyExtractor={groupKeyExtractor} renderItem={renderGroup} estimatedItemSize={60} recycleItems maintainVisibleContentPosition maintainScrollAtEnd={{ animated: false }} maintainScrollAtEndThreshold={0.02} initialScrollAtEnd ListHeaderComponent={isLoadingOlder ? (<div className="social-messages-older-loading" aria-live="polite">
                  Loading older messages…
                </div>) : undefined} ItemSeparatorComponent={SocialItemSeparator} className="social-messages-list" contentContainerStyle={{ padding: 24 }} style={{ height: "100%", width: "100%" }}/>)}
      </div>

      <div className="social-composer-stack">
        {sessionIsLive && (<button type="button" className="social-stella-arm-button" data-active={stellaArmed || undefined} onClick={() => setArmedState((previous) => ({
                roomId,
            ...
            <img src="stella-logo.svg" alt="" className="social-stella-arm-button-logo"/>
            <span>{stellaArmed ? "Telling Stella" : "Tell Stella"}</span>
          </button>)}
        <SocialComposer onSend={handleSend} armed={stellaArmed} placeholder={stellaArmed
