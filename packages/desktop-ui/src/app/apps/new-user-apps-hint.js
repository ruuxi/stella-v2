  ...
  try {
    ...
    const seen =
      parsed.seen && typeof parsed.seen === "object"
        ? Object.fromEntries(
            Object.entries(parsed.seen).filter(([, value]) => value === true),
          )
        : {};
    return {
...
const seedIfNeeded = (state) => {
  ...
  if (registry.phase !== "ready") return state;
  const seen = {};
...
export const markAllUserAppsSeen = () => {
  if (!platformCapabilities.localFiles) return;
...
const getHintSnapshot = () => {
  ...
  const registry = getUserAppsSnapshot();
  if (registry.phase !== "ready") return false;
  for (const app of registry.apps) {
...
export function useNewUserAppsHint() {
  const active = useSyncExternalStore(
    platformCapabilities.localFiles ? subscribe : () => () => undefined,
    platformCapabilities.localFiles ? getHintSnapshot : getServerSnapshot,
    getServerSnapshot,
  );
