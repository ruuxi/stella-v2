export const SETTINGS_SEARCH_ENTRY_DEFS = [
    // ---------- General ----------
    {
        tab: "general",
 ...
 * every keystroke.
 */
export function includesAsWordStart(text, term) {
    if (!term)
        return false;
    ...
    while (from <= textLen - termLen) {
        ...
        const isAlphanum = (prev >= 97 && prev <= 122) || (prev >= 48 && prev <= 57);
        if (!isAlphanum)
            return true;
        from = idx + 1;
    }
...
export function matchesAllTokenGroups(text, groups) {
    if (groups.length === 0)
        return true;
...
let normalizedCacheKey = null;
let normalizedCache = [];
function getNormalizedEntries(t) {
    if (normalizedCacheKey === t)
    ...
    normalizedCache = resolveEntries(t).map((entry) => ({
        ...
        searchText: normalizeSearchText([entry.title, entry.description, ...entry.keywords, entry.tab].join(" ")),
    }));
 ...
 */
function scoreEntry(normalized, tokens) {
    if (tokens.length === 0)
        return 0;
    ...
    if (titleText === fullQuery)
        return 1000;
    if (titleText.startsWith(fullQuery))
        return 500;
    const onlyLiteral = (text) => {
        for (const token of tokens) {
            if (!includesAsWordStart(text, token))
                return false;
        }
        return true;
    };
    ...
    if (onlyLiteral(descriptionText))
        return 100;
    if (matchesAllTokenGroups(descriptionText, groups))
        return 80;
    if (onlyLiteral(searchText))
        return 50;
    if (matchesAllTokenGroups(searchText, groups))
        return 40;
    return -1;
}
/**
 ...
 */
export function searchSettings(query, t) {
 ...
 */
export function expandedMatchTerms(query) {
