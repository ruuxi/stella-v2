const signInRequiredMatchers = ['sign in required'];
...
const includesAny = (normalized, matchers) => matchers.some((matcher) => normalized.includes(matcher));
const statusCodeFromError = (normalized) => {
    const match = normalized.match(/\b(?:http(?: status)?|status(?: code)?|error(?: code)?|code)\s*[:=]?\s*([45]\d{2})\b/i);
    if (!match?.[1])
        return null;
    const status = Number(match[1]);
    return Number.isFinite(status) ? status : null;
};
const nestedErrorMessage = (value, depth = 0) => {
...
const extractJsonErrorMessage = (message) => {
...
const redactErrorSecrets = (message) => message
    .replace(/\bbearer\s+[a-z0-9._~+/=-]+/gi, 'Bearer [redacted]')
    .replace(/\b(?:sk|pk)-[a-z0-9_-]{8,}\b/gi, '[redacted]')
    .replace(/([?&](?:api[_-]?key|access[_-]?token|token|secret)=)[^&\s]+/gi, '$1[redacted]')
    .replace(/((?:api[_-]?key|access[_-]?token|token|secret)\s*[:=]\s*)[^\s,;]+/gi, '$1[redacted]');
const friendlyUnknownErrorDetail = (message) => {
    const extracted = extractJsonErrorMessage(message) ?? message;
    const firstUsefulLine = extracted.split(/\n\s*(?:at\s+|caused by:)/i, 1)[0] ?? '';
    const cleaned = redactErrorSecrets(firstUsefulLine)
        .replace(/^\s*(?:codex|provider|stella runtime)\s+error(?:\s*\([^)]*\))?\s*[:=-]\s*/i, '')
        .replace(/^\s*\[(?:error|fatal)[^\]]*\]\s*/i, '')
        .replace(/^\s*\{\{\s*(?:error|fatal)[^}]*\}\}\s*/i, '')
        .replace(/^\s*(?:error|fatal|exception)\s*[:=-]\s*/i, '')
        .replace(/^\s*(?:http(?: status)?|status(?: code)?|error(?: code)?|code)\s*[:=]?\s*[45]\d{2}\s*[:=-]?\s*/i, '')
        .replace(/\s+/g, ' ')
        .trim();
    if (!cleaned ||
        /^(?:unknown error|an unknown error occurred|something went wrong)$/i.test(cleaned)) {
...
export const classifyStellaProviderError = (reason) => {
    ...
    if (includesAny(normalized, providerCredentialMatchers) ||
        statusCode === 401 ||
        statusCode === 403) {
    ...
    if (includesAny(normalized, timeoutMatchers) ||
        statusCode === 408 ||
        statusCode === 504) {
...
const LIMIT_OR_AUTH_KINDS = new Set([
    'account-auth',
    'billing',
    'chatgpt-usage-limit',
    'claude-code-login',
...
export const isStellaLimitOrAuthClassification = (classification) => LIMIT_OR_AUTH_KINDS.has(classification.kind);
