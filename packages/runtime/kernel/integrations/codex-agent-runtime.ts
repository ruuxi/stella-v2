...
export const codexServiceTierRequestValue = (
  serviceTier: CodexServiceTier,
): string => (serviceTier === "fast" ? "priority" : "default");
      ...
      this.child.stdin.write(line, (error) => {
        if (error) {
          const wrapped = new Error(
            `Codex app-server write failed: ${error.message}`,
          );
...
export const runCodexAgentTurn = async (request: {
  ...
  modelOverride?: string;
  /** Per-turn service tier pin; falls back to the saved preference. */
  serviceTier?: CodexServiceTier;
  ...
  /** Exposes live steering for the active turn; returns an optional detach. */
  onTurnControl?: (controls: {
    steer: (input: {
      prompt: string;
      attachments?: RuntimeAttachmentRef[];
    }) => Promise<void>;
  ...
  let detachTurnControl: (() => void) | undefined;
  let steeringInputCount = 0;
  const steeringCleanupDirs: string[] = [];
