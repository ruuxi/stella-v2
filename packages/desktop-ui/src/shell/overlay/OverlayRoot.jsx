...
const pointInRect = (point, rect) => point.x >= rect.left &&
...
// ---------------------------------------------------------------------------
function useOverlayIPC(dispatch) {
    useEffect(() => {
        ...
        const cleanups = [
            ...
            api.overlay.onShowDictation?.((data) => {
                dispatch({
                    type: "dictation:show",
...
// interactive region.
// ---------------------------------------------------------------------------
function useOverlayHitTesting(state, updateInteractive) {
    const { regionCaptureActive, dictationVisible, dictationPosition, } = state;
    useEffect(() => {
        ...
        const handleMouseMove = (e) => {
            ...
            updateInteractive(rects.some((rect) => pointInRect({ x: e.clientX, y: e.clientY }, rect)));
        };
...
function useOverlayDictation() {
    ...
    const stopAndComplete = useCallback(async (sessionId) => {
        const session = sessionRef.current;
        if (!session || sessionIdRef.current !== sessionId)
            return;
    ...
    useEffect(() => {
        ...
        const cleanupStart = api.onOverlayStart(({ sessionId }) => {
            ...
            void session
                ...
                onLevel: (level) => {
                    setLevels((prev) => appendRollingLevel(prev, level, MAX_DICTATION_OVERLAY_LEVELS));
                },
...
function DictationOverlay({ visible, position, levels, elapsedMs, onCancel, onConfirm, }) {
    if (!visible || !position)
        return null;
    return (<div className="dictation-overlay" style={{
            left: position.x,
            top: position.y,
        }}>
      <DictationRecordingBar levels={levels} elapsedMs={elapsedMs} onCancel={onCancel} onConfirm={onConfirm} showControls={false}/>
    </div>);
}
// ---------------------------------------------------------------------------
...
export function OverlayRoot() {
    ...
    const updateInteractive = useCallback((shouldBeInteractive) => {
    ...
    return (<div className="overlay-root" style={{
            position: "fixed",
            inset: 0,
            pointerEvents: state.regionCaptureActive ? "auto" : "none",
            overflow: "hidden",
        }}>
      {state.windowHighlightBounds && (<div className={state.windowHighlightTone === "subtle"
                ? "capture-window-ring capture-window-ring--subtle"
                : "capture-window-ring"} style={{
      ...
      {state.regionCaptureActive && (<div style={{
                position: "absolute",
                inset: 0,
                zIndex: 3,
                pointerEvents: "auto",
      ...
      <DictationOverlay visible={state.dictationVisible} position={state.dictationPosition} levels={dictation.levels} elapsedMs={dictation.elapsedMs} onCancel={dictation.cancel} onConfirm={dictation.confirm}/>
