...
/**
 * The mark is a hand-animated SVG on its own rAF loop, so its cost is the
 * frames it schedules. These drive the loop through a fake clock and count
 * pending frames: the contract is that it ticks while it can be seen and
 * schedules nothing at all while it cannot.
 */
describe("working indicator mark animation budget", () => {
  ...
  it("still paints once when it mounts paused, then sleeps", () => {
    // Chat can mount while the window is blurred. Without the first paint the
    // indicator would show an empty box until the window came back.
