...
export type ConversationHomeSnapshot = {
  key: string;
  content: PreparedHomeContent;
};
...
export class ConversationHomeCache {
  ...
  async load(args: {
    ...
    const key = JSON.stringify([
      args.ownerId,
      args.metadata.memory,
      args.metadata.skills.entries,
    ]);
    ...
    if (
      new TextEncoder().encode(JSON.stringify(snapshot)).byteLength < 100_000
    ) {
