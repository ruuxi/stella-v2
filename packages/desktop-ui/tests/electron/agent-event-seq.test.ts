...
describe("agent event seq spaces", () => {
  ...
  it("advances the recorder cursor from sourceSeq, ignoring synthetics", () => {
    expect(nextAgentRecorderSeqCursor(3, { seq: 1_800_000_000_001, sourceSeq: 9 })).toBe(
      9,
    );
    expect(nextAgentRecorderSeqCursor(9, { seq: 1_800_000_000_002, sourceSeq: 4 })).toBe(
      9,
    );
    expect(nextAgentRecorderSeqCursor(0, { seq: Number.MAX_SAFE_INTEGER })).toBe(0);
  });
});
