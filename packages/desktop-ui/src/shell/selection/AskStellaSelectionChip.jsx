...
const isInsideHiddenSurface = (node) => {
    if (!node)
        return false;
...
const clampLeft = (rawLeft, pillWidth) => {
...
const computePillPosition = (rect) => {
    const pillWidth = Math.max(PILL_MIN_WIDTH, rect.width * 0.5);
    const centerX = rect.left + rect.width / 2;
    const left = clampLeft(centerX - pillWidth / 2, pillWidth + COPY_BUTTON_EXTENT);
    const naturalTop = rect.top - PILL_HEIGHT - PILL_OFFSET;
    const top = naturalTop < VIEWPORT_MARGIN ? rect.bottom + PILL_OFFSET : naturalTop;
...
const isCanvasSelectionMessage = (value) => Boolean(value && typeof value === "object" && value.type === "stella:canvas-selection");
const isCanvasComposeMessage = (value) => Boolean(value && typeof value === "object" && value.type === "stella:canvas-compose");
const findCanvasIframe = (source) => {
    if (!source)
        return null;
    for (const iframe of document.querySelectorAll(".canvas-tab__iframe")) {
        if (iframe.contentWindow === source)
            return iframe;
    }
    return null;
};
const readCanvasSelectionState = (event) => {
    if (!isCanvasSelectionMessage(event.data))
        return undefined;
    ...
    if (text.trim().length < MIN_CHARS)
        return null;
    const rect = event.data.rect && typeof event.data.rect === "object"
        ? event.data.rect
        : null;
    ...
    const selectionRect = new DOMRect(iframeRect.left + left, iframeRect.top + top, width, height);
...
export function AskStellaSelectionChip() {
    ...
    if (!chip)
        return null;
    return (<div ref={chipRef} className="ask-stella-selection-chip-group" style={{ left: chip.left, top: chip.top }}>
      <button type="button" className="floating-selection-chip ask-stella-selection-chip" onMouseDown={(event) => event.preventDefault()} onClick={handleClick} title="Ask Stella about this selection">
        <img src="stella-logo.svg" alt="" aria-hidden="true" className="floating-selection-chip__logo ask-stella-selection-chip__logo"/>
        <span className="floating-selection-chip__label ask-stella-selection-chip__label">
          Ask Stella
        </span>
      </button>
      <button type="button" className="floating-selection-chip ask-stella-selection-chip ask-stella-selection-chip--icon" onMouseDown={(event) => event.preventDefault()} onClick={handleCopy} aria-label={copied ? "Copied" : "Copy"} title={copied ? "Copied" : "Copy"}>
