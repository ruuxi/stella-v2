...
import { resetMessageStorage } from "@stella/runtime/kernel/storage/reset-message-storage";
import { broadcastLocalChatUpdated } from "./context.js";
export const scheduleBootstrapRuntimeShutdown = (context, options = {}) => {
...
export const createBootstrapResetFlows = (context, options) => ({
    hardResetLocalState: async () => {
        ...
        try {
            await Promise.allSettled(config.hardResetMutableHomePaths.map((relativePath) => fs.rm(path.join(stellaDataDirPath, relativePath), {
                recursive: true,
                force: true,
            })));
        }
