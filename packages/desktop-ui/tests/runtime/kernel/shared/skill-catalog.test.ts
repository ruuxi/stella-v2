...
describe("skill catalog", () => {
  it("omits configured skill ids from the prompt block", async () => {
    ...
    await writeSkill(stellaAppDir, "humanizer", "Rewrite text naturally.");
    ...
    expect(state.entries.map((entry) => entry.id)).toEqual(["humanizer"]);
    expect(block).toContain("`humanizer`");
    expect(block).not.toContain("stella-browser");
    expect(block).not.toContain("pdf");
  });

  it("never exposes retired bundled skill ids from stale home copies", async () => {
    const stellaAppDir = await createStellaAppDir();
    await writeSkill(stellaAppDir, "humanizer", "Rewrite text naturally.");
    for (const skillId of RETIRED_BUNDLED_SKILL_IDS) {
      await writeSkill(stellaAppDir, skillId, `Stale ${skillId} payload.`);
    }

    const state = await buildSkillCatalogPromptState(stellaAppDir);
    const inline = await renderSkillCatalogBlock(stellaAppDir);
    const full = await renderFullSkillCatalogBlock(stellaAppDir);

    expect(state.totalSkills).toBe(1);
    expect(state.entries.map((entry) => entry.id)).toEqual(["humanizer"]);
    for (const skillId of RETIRED_BUNDLED_SKILL_IDS) {
      expect(inline).not.toContain(`\`${skillId}\``);
      expect(full).not.toContain(`\`${skillId}\``);
    }
  });

  it("does not count retired skill ids toward automatic Explore", async () => {
    const stellaAppDir = await createStellaAppDir();
    for (let i = 0; i < INLINE_SKILL_CATALOG_THRESHOLD; i += 1) {
      await writeSkill(
        stellaAppDir,
    ...
    await expect(shouldUseAutomaticSkillExplore(stellaAppDir)).resolves.toBe(
      false,
    );
  });
