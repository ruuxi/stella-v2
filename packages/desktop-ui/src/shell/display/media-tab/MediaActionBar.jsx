...
export const MediaActionBar = ({ item, onDelete, }) => {
    ...
    const handleSave = useCallback(async () => {
        if (!filePath)
            return;
        const result = await window.electronAPI?.system?.saveFileAs?.(filePath, filePath.split(/[\\/]/).pop() ?? filePath);
    ...
    return (<Fragment>
      {canSave ? (<button type="button" className="media-tab__action-btn" onClick={handleSave} aria-label="Save" title="Save">
          <Download size={14} strokeWidth={1.85}/>
        </button>) : null}
      <button type="button" className="media-tab__action-btn" onClick={handleCopy} aria-label="Copy" title="Copy">
        <Copy size={14} strokeWidth={1.85}/>
      </button>
      <button type="button" className={`media-tab__action-btn${armed ? " media-tab__action-btn--armed" : ""}`} onClick={handleTrash} aria-label={armed ? "Click again to delete" : "Delete"} title={armed ? "Click again to delete" : "Delete"}>
        <Trash2 size={14} strokeWidth={1.85}/>
      </button>
      {message ? (<span className="media-tab__action-status">{message}</span>) : null}
    </Fragment>);
};
