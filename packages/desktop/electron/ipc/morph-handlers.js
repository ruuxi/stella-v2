...
import { captureWindowDataUrl, waitForOverlayMorphSignal, } from "../windows/morph-transition-helpers.js";
...
export const registerMorphHandlers = (options) => {
    let activeOnboardingTransitionId = null;
    let activeOnboardingCaptureRect = null;
    const normalizeCaptureRect = (fullWindow, rect) => {
     ...
     */
    ipcMain.handle("morph:start", async (_event, payload) => {
        ...
        const { captureRect, screenBounds } = normalizeCaptureRect(fullWindow, payload?.rect);
        ...
        const readyPromise = waitForOverlayMorphSignal("overlay:morphReady", transitionId, ONBOARDING_MORPH_OVERLAY_READY_TIMEOUT_MS);
        overlay.startMorphForward(transitionId, screenshot, screenBounds, captureRect ? null : fullWindow, "onboarding");
     ...
     */
    ipcMain.handle("morph:complete", async (_event, payload) => {
        const fullWindow = options.windowManager.getFullWindow();
        const overlay = options.getOverlayController();
        if (!fullWindow || fullWindow.isDestroyed() || !overlay) {
            return { ok: false };
        ...
        const { captureRect } = normalizeCaptureRect(fullWindow, payload?.rect ?? activeOnboardingCaptureRect ?? undefined);
        const screenshot = await captureWindowDataUrl(fullWindow, captureRect);
        if (!screenshot) {
        ...
        const donePromise = waitForOverlayMorphSignal("overlay:morphDone", transitionId, ONBOARDING_MORPH_DONE_TIMEOUT_MS);
        const handoffStarted = overlay.startMorphHandoff(transitionId, screenshot, false);
        if (!handoffStarted) {
            activeOnboardingTransitionId = null;
            activeOnboardingCaptureRect = null;
            return { ok: false };
