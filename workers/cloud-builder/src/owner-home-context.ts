...
export class OwnerHomeContextCache {
  ...
  async changed(ownerGeneration: string, revision: number): Promise<void> {
    ...
    await this.storage.transaction(async txn => {
      ...
      if (cached?.memory.preference.ownerGeneration === ownerGeneration) await txn.delete(CACHE);
    });
  ...
  async load(args: {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      const revision = await this.storage.get<number>(VERSION + args.ownerGeneration) ?? 0;
      const cached = this.resident ?? await this.storage.get<OwnerHomeContext>(CACHE);
      if (cached && cached.revision === revision && cached.memory.preference.ownerGeneration === args.ownerGeneration) {
        try {
          await args.assertPolicy(cached.memory.preference);
          // Policy validation can yield to a notification.
          if ((await this.storage.get<number>(VERSION + args.ownerGeneration) ?? 0) === revision) {
      ...
      if ((await this.storage.get<number>(VERSION + args.ownerGeneration) ?? 0) !== revision) continue;
      ...
      if (new TextEncoder().encode(JSON.stringify(loaded)).byteLength < 100_000) {
        await this.storage.put(CACHE, loaded);
      } else await this.storage.delete(CACHE);
      this.resident = loaded;
      return loaded;
    }
    throw new Error("Cloud home context changed repeatedly during preparation.");
  }
}
