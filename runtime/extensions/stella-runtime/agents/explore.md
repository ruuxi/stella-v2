...
- `state/notes/` - daily task summaries from the last 14 days when the task seems to overlap prior work.
...
2. Use Grep to find candidate skills or notes. Prefer narrow patterns over walking trees.
3. Use Read to confirm a file is actually relevant before listing it.
4. Prioritize `state/skills/` matches. Only include notes when they contain concrete prior work that would help.
...
Stop conditions:
- You have a populated `relevant` list and have checked notes for any obvious recent matches.
