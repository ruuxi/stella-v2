...
export class Executor extends DurableObject<Env> {
  ...
  private bytes = 0;
  async stats() { return { enteredAt: this.enteredAt, firstBodyAt: this.firstBodyAt, bytes: this.bytes }; }
  async fetch(request: Request) {
    this.enteredAt = Date.now();
    if (new URL(request.url).pathname === "/refuse") return new Response("account suspended", { status: 403 });
    ...
    return new Response(new Blob(parts), { status: 201, headers: { "x-provider": "fixture" } });
  }
}

export class Gateway extends WorkerEntrypoint<Env> {
  async fetch(request: Request) {
    if (request.headers.get("authorization") !== "Bearer fixture") return new Response("bad fixture token", { status: 401 });
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const body = JSON.stringify({ prompt: "private memory", chunks: "x".repeat(256_000) });
    let authorizationFinished: (() => void) | undefined;
    const guardFinished = new Promise<void>(resolve => { authorizationFinished = resolve; });
    let result: { status: number; body: string; header: string | null } | { error: string };
    try {
      const response = await guardedModelFetch({
        request: new Request(`https://gateway/${mode}`, {
          method: "POST", headers: { authorization: "Bearer fixture", "x-test-id": id },
          ...
          try {
            ...
            while (Date.now() < deadline) {
              ...
              if (state.enteredAt > 0) {
                ...
                if (state.bytes !== 0) throw new Error("body leaked before authorization");
                break;
              }
              await scheduler.wait(5);
            }
            if (!gatewayEnteredDuringGuard) throw new Error("gateway did not enter during guard");
      ...
      result = { status: response.status, body: await response.text(), header: response.headers.get("x-provider") };
    } catch (error) {
      result = { error: error instanceof Error ? error.message : String(error) };
    }
    await guardFinished;
    return Response.json({ result, authorizedAt, gatewayEnteredDuringGuard, expectedBody: body, ...await executor.stats() });
  },
};
