...
import { CHAT_DISPLAY_TAB_ID, HOME_DISPLAY_TAB_ID, STORE_DISPLAY_TAB_ID, TRASH_DISPLAY_TAB_ID, registerWorkspaceDefaultTabs, } from "@/features/workspace-display/default-tabs";
export { CHAT_DISPLAY_TAB_ID, HOME_DISPLAY_TAB_ID, STORE_DISPLAY_TAB_ID, TRASH_DISPLAY_TAB_ID, } from "@/features/workspace-display/default-tabs";
/**
 ...
 */
function ChatDisplayTab({ openRequest, }) {
    const panelExpanded = useDisplayPanelExpanded();
    const chat = useChatRuntime();
    const messages = useChatMessages();
    return (<ChatPanelTab openRequest={openRequest} wideLayout={panelExpanded} messages={messages} isStreaming={chat.conversation.isStreaming} isStreamingResponseText={chat.conversation.streaming.isStreamingResponseText} runtimeStatusText={chat.conversation.streaming.runtimeStatusText} activeToolCallId={chat.conversation.streaming.activeToolCallId} activeToolName={chat.conversation.streaming.activeToolName} latestCompletedTool={chat.conversation.streaming.latestCompletedTool} hasToolActivity={chat.conversation.streaming.hasToolActivity} isToolActive={chat.conversation.streaming.isToolActive} pendingUserMessageId={chat.conversation.pendingUserMessageId} queuedUserMessages={chat.conversation.streaming.queuedUserMessages} removeQueuedUserMessage={chat.conversation.streaming.removeQueuedUserMessage} hasOlderMessages={chat.conversation.hasOlderMessages} isLoadingOlder={chat.conversation.isLoadingOlder} isInitialLoading={chat.conversation.isInitialLoading} onLoadOlder={chat.conversation.loadOlderMessages} onSend={chat.conversation.sendMessageWithContext} onStop={chat.conversation.cancelCurrentStream}/>);
}
...
registerWorkspaceDefaultTabs({
    openChatDisplayTab: (openRequest, opts) => openChatDisplayTab(openRequest, opts),
    openHomeDisplayTab,
