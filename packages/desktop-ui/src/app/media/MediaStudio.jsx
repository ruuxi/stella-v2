...
import { addHistoryEntry, extractOutput, generateThumb, loadFormState, loadHistory, openOutputsFolder, saveFormState, saveOutputToStella, updateHistoryEntry, } from "./media-store";
...
export default function MediaStudio() {
    ...
    const [capabilityId, setCapabilityId] = useState(savedForm.capabilityId);
    const [prompt, setPrompt] = useState(savedForm.prompt);
    const [sourceUri, setSourceUri] = useState(null);
    const [sourceFileName, setSourceFileName] = useState(null);
    const [aspectRatio, setAspectRatio] = useState(savedForm.aspectRatio);
    const [profile, setProfile] = useState(savedForm.profile);
    const [extraValues, setExtraValues] = useState(savedForm.extraValues);
    ...
    const savedJobRef = useRef(new Set());
    ...
    const job = useQuery(api.media_jobs.getByJobId, activeJobId ? { jobId: activeJobId } : "skip");
    ...
    useEffect(() => {
        if (!activeJobId)
            return;
        if (savedJobRef.current.has(activeJobId))
            return;
    ...
    const persistForm = useCallback((patch) => {
    ...
    const handleCapabilitySelect = useCallback((id) => {
        const cap = CAPABILITIES.find((c) => c.id === id);
        const newProfile = cap?.profiles?.[0]?.id ?? null;
        const newExtra = Object.fromEntries((cap?.extraFields ?? []).map((f) => [f.key, f.default]));
        startTransition(() => {
            setCapabilityId(id);
            setPrompt("");
            setAspectRatio(null);
    ...
    const handleAspectRatioToggle = useCallback((ar) => {
    ...
    const handleSendTo = useCallback((targetCapId, url) => {
        const cap = CAPABILITIES.find((c) => c.id === targetCapId);
        if (!cap)
            return;
        const targetCat = cap.category;
        ...
        startTransition(() => {
            ...
            setExtraValues(Object.fromEntries((cap.extraFields ?? []).map((f) => [f.key, f.default])));
            persistForm({
                category: targetCat,
    ...
    const liveOutput = useMemo(() => activeJobId && jobStatus === "succeeded" && jobOutput
        ? extractOutput(jobOutput)
        : null, [activeJobId, jobStatus, jobOutput]);
    const activeOutput = viewingEntry?.output ?? liveOutput;
    const showPending = activeJobId &&
        !viewingEntry &&
    ...
    return (<div className={`ms ${dragging ? "ms--dragging" : ""}`} onDragEnter={handleDragEnter} onDragLeave={handleDragLeave} onDragOver={handleDragOver} onDrop={handleDrop}>
      {/* ── Left: controls ── */}
      <div className="ms-controls">
        ...
        <nav className="ms-categories">
          {CATEGORIES.map((cat) => (<button key={cat.id} type="button" className={`ms-category ${category === cat.id ? "ms-category--active" : ""}`} onClick={() => handleCategoryChange(cat.id)}>
              {cat.label}
            </button>))}
        </nav>
        ...
        <div className="ms-controls-body">
          ...
          <div className="ms-capabilities">
            {filteredCapabilities.map((cap) => (<button key={cap.id} type="button" className={`ms-capability ${capabilityId === cap.id ? "ms-capability--active" : ""}`} onClick={() => handleCapabilitySelect(cap.id)}>
          ...
          {capability && (<>
              ...
              <div className="ms-form">
                {capability.profiles && capability.profiles.length > 1 && (<div className="ms-field">
                    <label className="ms-label">Quality</label>
                    <div className="ms-tags">
                      {capability.profiles.map((p) => (<button key={p.id} type="button" className={`ms-tag ${profile === p.id ? "ms-tag--active" : ""}`} onClick={() => handleProfileChange(p.id)}>
                          {p.name}
                        </button>))}
                    </div>
                  </div>)}

                {capability.needsPrompt && (<div className="ms-field">
                    <label className="ms-label">Prompt</label>
                    <textarea className="ms-textarea" value={prompt} onChange={(e) => handlePromptChange(e.target.value)} placeholder="Describe what you'd like…" rows={3}/>
                  </div>)}

                {capability.needsSource && (<div className="ms-field">
                    <label className="ms-label">
                      {capability.sourceLabel ?? "Source file"}
                    </label>
                    {sourceFileName && sourceCompatible ? (<div className="ms-source-info">
                        {sourceType === "image" && sourceUri && (<img src={sourceUri} alt="" className="ms-source-preview"/>)}
                        <span className="ms-source-name">{sourceFileName}</span>
                        <button type="button" className="ms-source-clear" onClick={handleClearSource}>
                          ✕
                        </button>
                      </div>) : (<button type="button" className="ms-upload" onClick={() => fileInputRef.current?.click()}>
                        {sourceFileName
                        ? "Choose a different file"
                        : "Choose file or drop anywhere"}
                      </button>)}
                    <input ref={fileInputRef} type="file" accept={capability.sourceAccept} onChange={handleFileChange} className="ms-file-input"/>
                  </div>)}

                {capability.supportsAspectRatio && (<div className="ms-field">
                    <label className="ms-label">Aspect ratio</label>
                    <div className="ms-tags">
                      {ASPECT_RATIOS.map((ar) => (<button key={ar} type="button" className={`ms-tag ${aspectRatio === ar ? "ms-tag--active" : ""}`} onClick={() => handleAspectRatioToggle(ar)}>
                          {ar}
                        </button>))}
                    </div>
                  </div>)}

                {capability.extraFields?.map((field) => (<div key={field.key} className="ms-field">
                    <label className="ms-label">{field.label}</label>
                    <input type="number" className="ms-number-input" value={extraValues[field.key] ?? field.default} min={field.min} max={field.max} onChange={(e) => {
                ...
                <button type="button" className="ms-generate" disabled={!canSubmit} onClick={handleGenerate}>
      ...
      <div className="ms-right">
        <div className="ms-output-panel">
          {dragging && (<div className="ms-drop-overlay">
              <div className="ms-drop-label">Drop file</div>
            </div>)}

          {!showPending && !showFailed && !showOutput && !dragging && (<div className="ms-empty">
          ...
          {showPending && (<div className="ms-status">
          ...
          {showOutput && activeOutput && (<div className="ms-output">
              {activeOutput.kind === "image" && (<>
                  <div className="ms-output-images">
                    {activeOutput.urls.map((url, i) => (<button key={url} type="button" className="ms-output-image-btn" onClick={() => setLightboxUrl(url)}>
              ...
              {activeOutput.kind === "audio" && (<audio src={activeOutput.url} controls className="ms-output-audio"/>)}
              {activeOutput.kind === "text" && (<div className="ms-output-text">
                  <p>{activeOutput.text}</p>
                </div>)}
              {activeOutput.kind === "download" && (<a href={activeOutput.url} target="_blank" rel="noreferrer" className="ms-output-download">
                  {activeOutput.label}
                </a>)}
            </div>)}
        </div>
        ...
        {/* History strip */}
        {history.length > 0 && (<div className="ms-strip">
            <div className="ms-strip-scroll">
              {history.map((entry) => {
                const isActive = viewingEntry?.id === entry.id || activeJobId === entry.id;
                const thumbSrc = entry.thumb ?? null;
                return (<button key={entry.id} type="button" className={`ms-strip-item ${isActive ? "ms-strip-item--active" : ""}`} onClick={() => handleHistoryClick(entry)} title={entry.prompt ?? entry.capabilityName}>
                    {thumbSrc ? (<img src={thumbSrc} alt="" className="ms-strip-thumb"/>) : (<div className={`ms-strip-placeholder ms-strip-placeholder--${entry.output?.kind ?? "pending"}`}>
              ...
              <button type="button" className="ms-strip-folder" onClick={() => void openOutputsFolder()} title="Open outputs folder">
                <FolderIcon />
              </button>
            </div>
          </div>)}
      ...
      {/* Lightbox */}
      {lightboxUrl && (<div className="ms-lightbox" onClick={() => setLightboxUrl(null)}>
          <img src={lightboxUrl} alt="" className="ms-lightbox-img" onClick={(e) => e.stopPropagation()}/>
          <button type="button" className="ms-lightbox-close" onClick={() => setLightboxUrl(null)}>
            ✕
          </button>
        </div>)}
    </div>);
