...
export function EngineScopedModelList({ engineLabel, models, value, onSelect, loading = false, disabled = false, emptyMessage, hideHead = false, }) {
    return (<div className="engine-scoped-model-list" aria-label={`${engineLabel} models`}>
      ...
      <div className="engine-scoped-model-list__rows">
        {loading && models.length === 0 ? (<p>Loading {engineLabel} models…</p>) : models.length === 0 ? (emptyMessage === null ? null : (<p>{emptyMessage ?? `No ${engineLabel} models are available.`}</p>)) : (models.map((model) => {
            ...
            return (<button key={model.id} type="button" aria-pressed={selected} aria-disabled={model.unavailable || undefined} data-selected={selected || undefined} disabled={disabled || model.unavailable} onClick={() => onSelect(model.id)}>
