...
if (rawConfig.env) {
  typeConfig.env = Object.fromEntries(
    Object.entries(rawConfig.env).map(([name, config]) => [
      name,
      sourceConfig(config),
    ]),
  );
...
await writeFile(
  path.join(configDirectory, "wrangler.json"),
  `${JSON.stringify(typeConfig, null, 2)}\n`,
);

const args = process.argv.slice(2);
if (args.some((value) => value !== "--check"))
  throw new Error("Expected only the optional --check argument.");
const child = spawn(
  path.join(
    workerRoot,
    "node_modules/.bin",
...
const exitCode = await new Promise((resolve, reject) => {
  ...
  child.once("exit", (code, signal) =>
    signal
      ? reject(new Error(`Worker type generation stopped by ${signal}.`))
      : resolve(code ?? 1),
  );
});
