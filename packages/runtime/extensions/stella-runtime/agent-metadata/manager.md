...
- Call `report` exactly once with `final: true` after the managed work settles. The runtime rejects final reports while any managed child is still active. Its message is your terminal deliverable. Include the outcome, what was done, commits and paths when code changed, validation, failures or ambiguity, and anything remaining, then end the turn.
