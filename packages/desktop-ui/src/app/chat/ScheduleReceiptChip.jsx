...
const labelFor = (affected, nowMs) => {
    ...
    // single-line summary.
    const next = affected.reduce((min, entry) => Math.min(min, entry.nextRunAtMs), Number.POSITIVE_INFINITY);
    return {
        primary: `${affected.length} schedules`,
        meta: Number.isFinite(next) ? `next ${formatNextRun(next, nowMs)}` : "",
    };
};
export function ScheduleReceiptChip({ affected, summary, }) {
    ...
    const { primary, meta } = useMemo(() => labelFor(affected, nowMs), [affected, nowMs]);
    if (affected.length === 0)
        return null;
    return (<>
      <button type="button" className="schedule-receipt-chip" onClick={() => {
            setNowMs(Date.now());
            setOpen(true);
        }}>
      ...
      <ScheduleDetailsDialog open={open} onOpenChange={(nextOpen) => {
            if (nextOpen)
                setNowMs(Date.now());
            setOpen(nextOpen);
