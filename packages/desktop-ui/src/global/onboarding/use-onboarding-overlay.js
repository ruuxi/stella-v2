...
import { readOnboardingPhase, useOnboardingState, } from "@/global/onboarding/use-onboarding-state";
import { SPLIT_PHASES } from "@/global/onboarding/onboarding-flow";
...
export function useOnboardingOverlay() {
    const { completed: onboardingDone, complete: completeOnboarding, reset: resetOnboarding, persistPhase, } = useOnboardingState();
    ...
    const initialPhase = resumePhase ?? "intro";
    const creatureInitialBirth = onboardingDone || isResuming ? 1 : CREATURE_INITIAL_SIZE;
    const [hasExpanded, setHasExpanded] = useState(() => onboardingDone || isResuming);
    const [hasStarted, setHasStarted] = useState(() => onboardingDone || isResuming);
    const [splitMode, setSplitMode] = useState(() => isResuming && SPLIT_PHASES.has(initialPhase));
    ...
    const splitEnterTimerRef = useRef(null);
    ...
    const stellaAnimationRef = useRef(null);
