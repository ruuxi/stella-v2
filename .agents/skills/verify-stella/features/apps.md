...
- `apps-library` opens populated app cards. The `/apps` page also has search and sort controls; the sidebar library does not.
...
- On `/apps`, search or sort the populated library, then choose an app card.
...
- **Populated library.** Inspect a named card in the sidebar. Use the `/apps` page when proving search and sort. Capture the selected app tab.
