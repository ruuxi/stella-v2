...
import { expandedMatchTerms, searchSettings, } from "@/global/settings/lib/settings-search-index";
 ...
 */
export function SettingsSearchResults({ query, onSelect, onClear, }) {
    const t = useT();
    ...
    if (results.length === 0) {
        return (<div className="settings-search-results settings-search-results--empty">
        ...
        <div className="settings-search-results-empty-body">
          ...
          <button type="button" className="settings-search-results-link" onClick={onClear}>
    ...
    const countKey = resultsCount === 1
        ? "settings.search.resultsOne"
        : "settings.search.resultsOther";
    const ariaKey = resultsCount === 1
        ? "settings.search.resultsAriaOne"
        : "settings.search.resultsAriaOther";
    return (<div className="settings-search-results" role="listbox" aria-label={t(ariaKey, { count: resultsCount })}>
      ...
      <ul className="settings-search-results-list">
        {results.map((entry, index) => (<li key={`${entry.tab}:${entry.title}`} className="settings-search-result-item">
            <button type="button" role="option" aria-selected={index === 0} className="settings-search-result-button" onClick={() => onSelect(entry)}>
              ...
              <div className="settings-search-result-meta">
                ...
                <ArrowRight size={13} strokeWidth={1.85} className="settings-search-result-arrow" aria-hidden/>
              </div>
            </button>
          </li>))}
      </ul>
 ...
 */
function highlightTokens(text, terms) {
    ...
    return parts.map((part, index) => {
        if (index % 2 === 1) {
            return (<mark key={index} className="settings-search-match">
          {part}
        </mark>);
        }
