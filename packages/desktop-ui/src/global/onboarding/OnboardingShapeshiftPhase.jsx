...
import { DEMO_DEFAULT_SIDEBAR, DemoBubble, DemoChat, DemoComposer, DemoShell, DemoWorking, } from "./demo/DemoShell";
...
function FocusTimerCard() {
    return (<div className="oshape-timer-card">
...
function Receipt({ morph, undone, live, onUndo, }) {
    return (<div className="oshape-receipt" data-undone={undone || undefined} data-live={live || undefined}>
      ...
      {onUndo ? (<button type="button" className="oshape-receipt__undo" onClick={onUndo}>
          <RotateCcw size={10}/>
          Undo
        </button>) : null}
    </div>);
}
export function OnboardingShapeshiftPhase({ splitTransitionActive, onContinue, }) {
    ...
    const appliedIds = useMemo(() => {
        const ids = new Set(log.filter((entry) => !entry.undone).map((entry) => entry.morph.id));
        // The shell morphs at the `morph` cue, before the run is committed
        // to the log, so mid-run application is derived here.
        if (activeMorph && has("morph"))
            ids.add(activeMorph.id);
        return ids;
    }, [activeMorph, has, log]);
    const startMorph = useCallback((morph) => {
        if (activeMorph)
            return;
        setJustUndid(false);
    ...
    const handleUndo = useCallback(() => {
        if (activeMorph || undoTimerRef.current)
            return;
        ...
        undoTimerRef.current = setTimeout(() => {
            ...
            setLog((entries) => {
                ...
                if (!lastApplied)
                    return entries;
                return entries.map((entry) => entry === lastApplied ? { ...entry, undone: true } : entry);
            });
            setJustUndid(true);
        }, 420);
    }, [activeMorph]);
    const sidebarItems = useMemo(() => {
        if (!appliedIds.has("timer"))
            return DEMO_DEFAULT_SIDEBAR;
        return [
    ...
    const remainingMorphs = MORPHS.filter((morph) => morph.id !== activeMorph?.id &&
    ...
    return (<div className="onboarding-step-content oshape-step">
      <div className="oshape-stage" data-dusk={appliedIds.has("dusk") || undefined}>
        <DemoShell className="oshape-shell" sidebarItems={sidebarItems} tabs={appliedIds.has("tabs") ? ["Home", "Chats", "Notes"] : undefined} activeTab="Home">
          <DemoChat>
         ...
         * collapse to receipt-only. */}
            ...
            return (<div key={`${entry.morph.id}-${index}`} className="oshape-run">
                  {showFull ? (<DemoBubble role="user" visible>
                      {entry.morph.prompt}
                    </DemoBubble>) : null}
                  <Receipt morph={entry.morph} undone={entry.undone} onUndo={isLast && !entry.undone && !splitTransitionActive
                    ? handleUndo
                    : undefined}/>
                 ...
                 * (the composer below is bottom-pinned). */}
                  {entry.morph.id === "timer" && !entry.undone ? (<FocusTimerCard />) : null}
                  {showFull ? (<DemoBubble role="assistant" visible>
                      {entry.morph.reply}
                    </DemoBubble>) : null}
                </div>);
        })}

            {activeMorph ? (<div className="oshape-run">
                <DemoBubble role="user" visible={has("send")}>
                  {activeMorph.prompt}
                </DemoBubble>
                <DemoWorking visible={has("working") && !has("receipt")} label="Rewriting the app…"/>
                {has("receipt") ? <Receipt morph={activeMorph} live/> : null}
                {activeMorph.id === "timer" && has("morph") ? (<FocusTimerCard />) : null}
                <DemoBubble role="assistant" visible={has("settled")}>
                  {activeMorph.reply}
                </DemoBubble>
              </div>) : null}

            {justUndid && !activeMorph ? (<DemoBubble role="assistant" visible>
                Reverted. Ask again whenever.
              </DemoBubble>) : null}

            {remainingMorphs.length > 0 ? (<div className="oshape-chips" data-locked={activeMorph !== null || undefined}>
                {remainingMorphs.map((morph) => (<button key={morph.id} type="button" className="oshape-chip" disabled={activeMorph !== null || splitTransitionActive} onClick={() => startMorph(morph)}>
                    {morph.prompt}
                  </button>))}
              </div>) : null}

            <DemoComposer value={activeMorph && !has("send") ? typed.value : ""} typing={typed.typing && !has("send")} sending={Boolean(activeMorph && has("send") && !has("settled"))} placeholder="Ask for any change..."/>
          </DemoChat>
        </DemoShell>

        {/* Morph flash — the mock's stand-in for the real capture →
         * cross-fade transition that covers live self-edits. */}
        {activeMorph && has("morph") ? (<div className="oshape-flash" key={`run-${activeMorph.id}`}/>) : null}
        {undoFlashKey > 0 ? (<div className="oshape-flash" key={`undo-${undoFlashKey}`}/>) : null}
      </div>
      ...
      <button className="onboarding-confirm oshape-continue" data-visible={true} data-emphasized={completedRuns > 0 || undefined} disabled={splitTransitionActive} onClick={onContinue}>
        Continue
      </button>
    </div>);
}
