...
import { getTaskDecorationsSnapshot, subscribeTaskDecorations, } from '@/features/chat/streaming/task-decoration-store';
/**
 ...
 */
export function useTaskDecorationPublisher() {
    const lastPublishedRef = useRef('');
    useEffect(() => {
        const publish = () => {
            const api = window.electronAPI?.localChat;
            if (!api?.publishTaskDecoration)
                return;
            const statusTextByAgentId = {};
            for (const [agentId, decoration] of Object.entries(getTaskDecorationsSnapshot())) {
                if (decoration.status !== 'completed' &&
