...
export const hashImageOperationRequest = (requestBody) => createHash("sha256")
...
const openDatabase = (stellaDataDir) => {
    ...
    `);
        ...
        if (!operationColumns.has("submission_state")) {
            db.exec("ALTER TABLE image_tool_operations ADD COLUMN submission_state TEXT NOT NULL DEFAULT 'pending'");
        }
...
const toOperation = (row, reattached) => ({
    operationId: row.operation_id,
    ...(row.job_id ? { jobId: row.job_id } : {}),
    ...(row.terminal_result_json
        ? (() => {
            const terminal = JSON.parse(row.terminal_result_json);
...
export const reserveDurableImageOperation = (args) => {
    ...
    try {
        db.exec("BEGIN IMMEDIATE");
        const alias = db
            .prepare(`SELECT o.operation_id, o.job_id, o.state, o.submission_state,
         ...
         WHERE a.conversation_id = ? AND a.tool_call_id = ?`)
            .get(args.conversationId, args.toolCallId);
        if (alias &&
        ...
        const recoverable = db
         ...
         ORDER BY updated_at DESC LIMIT 1`)
            .get(args.conversationId, requestHash);
        const operationId = recoverable?.operation_id ?? randomUUID();
        if (!recoverable) {
            db.prepare(`INSERT INTO image_tool_operations
         (operation_id, conversation_id, request_hash, state, created_at, updated_at)
         VALUES (?, ?, ?, 'pending', ?, ?)`).run(operationId, args.conversationId, requestHash, now, now);
        }
        else {
            db.prepare(`UPDATE image_tool_operation_aliases SET identity_version = 2
         WHERE operation_id = ?`).run(operationId);
        }
        db.prepare(`INSERT INTO image_tool_operation_aliases
       ...
       ON CONFLICT(conversation_id, tool_call_id) DO UPDATE SET
         ...
         created_at = excluded.created_at`).run(args.conversationId, args.toolCallId, operationId, requestHash, now);
        db.exec("COMMIT");
        return recoverable
...
export const claimImageOperationSubmission = (args) => {
    ...
    try {
        const result = db
            .prepare(`UPDATE image_tool_operations
...
export const markImageOperationSubmitted = (args) => {
    const db = openDatabase(args.stellaDataDir);
    try {
        db.prepare(`UPDATE image_tool_operations
       SET submission_state = 'submitted', job_id = COALESCE(?, job_id),
           updated_at = ?
       WHERE operation_id = ? AND state = 'pending'
         AND submission_state = 'dispatching'`).run(args.providerRequestId ?? null, Date.now(), args.operationId);
    }
    finally {
        db.close();
    }
...
export const attachImageOperationJob = (args) => {
    const db = openDatabase(args.stellaDataDir);
    try {
        db.prepare(`UPDATE image_tool_operations SET job_id = ?, updated_at = ?
       WHERE operation_id = ? AND state = 'pending'`).run(args.jobId, Date.now(), args.operationId);
    }
    finally {
        db.close();
    }
...
export const settleImageOperation = (args) => {
    ...
    try {
       ...
       WHERE operation_id = ? AND state = 'pending'`).run(state, args.result.ok ? args.result.job.jobId : (args.result.jobId ?? null), JSON.stringify(args.result), Date.now(), args.operationId);
    }
    finally {
        db.close();
    }
...
export const markImageOperationDelivered = (args) => {
    ...
    const db = openDatabase(args.stellaDataDir);
    try {
        db.prepare(`UPDATE image_tool_operations SET delivered_at = ?, updated_at = ?
       WHERE operation_id = (
         SELECT operation_id FROM image_tool_operation_aliases
         WHERE conversation_id = ? AND tool_call_id = ?
       ) AND state != 'pending'`).run(Date.now(), Date.now(), args.conversationId, args.toolCallId);
    }
    finally {
        db.close();
    }
...
export const pruneImageOperationLedger = (args) => {
    const db = openDatabase(args.stellaDataDir);
    const deliveredBefore = args.deliveredBefore ?? Date.now() - 30 * 24 * 60 * 60_000;
    const limit = Math.max(1, Math.min(args.limit ?? 500, 2_000));
    try {
        db.exec("BEGIN IMMEDIATE");
        const rows = db
            .prepare(`SELECT operation_id FROM image_tool_operations
         WHERE state != 'pending' AND delivered_at IS NOT NULL
           AND delivered_at < ?
         ORDER BY delivered_at ASC LIMIT ?`)
            .all(deliveredBefore, limit);
        const removeAliases = db.prepare("DELETE FROM image_tool_operation_aliases WHERE operation_id = ?");
        const removeOperation = db.prepare("DELETE FROM image_tool_operations WHERE operation_id = ?");
