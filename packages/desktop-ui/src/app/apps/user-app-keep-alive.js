...
export const promoteRetainedUserApp = (retained, slug) => {
    if (retained[0] === slug)
        return retained;
    return [slug, ...retained.filter((entry) => entry !== slug)].slice(0, MAX_RETAINED_USER_APPS);
};
/**
 ...
 * is measuring.
 */
export const countingDownUserApps = (retained, onScreenSlug) => retained.filter((slug) => slug !== onScreenSlug);
/**
 ...
 */
export const mountedUserApps = (retained, onScreenSlug) => onScreenSlug === null || retained.includes(onScreenSlug)
    ? retained
 ...
 */
export const onScreenUserAppSlug = ({ activeSlug, activeSection, panelOpen, }) => panelOpen && activeSection === "apps" ? activeSlug : null;
