...
import type { Model, ProviderRequestLifecycleProof } from "../ai/types.js";
...
describe("OpenAI Responses gateway transport", () => {
  it("posts managed gateway JSON without loading the OpenAI SDK", async () => {
    ...
    const transport = vi.fn(async (input: Parameters<typeof fetch>[0], init?: Parameters<typeof fetch>[1]) => {
      request = input instanceof Request ? input : new Request(input, init);
      return Response.json(responseBody, { headers: { "x-provider": "gateway" } });
    ...
    expect(request?.headers.get("idempotency-key")).toMatch(/^stella-response-/);
    expect(request?.headers.get("idempotency-key")).not.toBe("stale-idempotency-key");
    expect(request?.headers.get("x-stella-request-id")).toBeTruthy();
    expect(request?.headers.get("x-stella-request-id")).not.toBe("stale-request-id");
    ...
    expect(onResponse).toHaveBeenCalledWith(
      { status: 200, headers: { "content-type": "application/json", "x-provider": "gateway" } },
    ...
    expect(result.content).toEqual([
      {
        ...
        thinkingSignature: JSON.stringify(responseBody.output[0]),
      },
    ...
    expect(new Set(lifecycle.map((proof) => proof.requestIdSha256)).size).toBe(1);
  ...
  it("refreshes authorization once with the same logical identity", async () => {
    ...
    expect(lifecycle.filter((proof) => proof.phase === "request-dispatched")).toHaveLength(2);
    expect(new Set(lifecycle.map((proof) => proof.requestIdSha256)).size).toBe(1);
  });
  ...
  it("cancels while gateway JSON is still draining", async () => {
    ...
    const transport = vi.fn(async (_input: Parameters<typeof fetch>[0], init?: Parameters<typeof fetch>[1]) => {
      const body = new ReadableStream<Uint8Array>({
        start(streamController) {
          streamController.enqueue(new TextEncoder().encode('{"id":"resp_pending","output":'));
