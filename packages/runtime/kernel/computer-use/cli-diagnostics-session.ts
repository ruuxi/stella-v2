...
const requireObservedStateId = (command: ComputerUseActionCommand): string => {
  const observedStateId = command.observedStateId;
  if (!observedStateId) {
    throw new Error(
      `${command.action.type} requires observedStateId before diagnostics dispatch.`,
    );
  }
  return observedStateId;
};

const encodeAction = (command: ComputerUseActionCommand): string[] => {
  const target = targetArgs(command.target);
  const action = command.action;
  const observedStateId = requireObservedStateId(command);
  const provenance = [
    "--observed-state-id",
    observedStateId,
...
export const createCliDiagnosticsComputerUseSession = (
  ...
  const request: ComputerUseSession["request"] = async (
    ...
    if (typedRequest.type === "wait_for_change") {
      ...
      try {
        for (;;) {
          ...
          await abortableDelay(Math.min(150, remainingAfterPoll), signal);
        }
      } finally {
    ...
    const validateActionObservation = async (
      ...
      if (baseline.semanticStateId !== observedStateId) {
        return staleResponse(observedStateId, baseline.semanticStateId);
      }
      if (
        command.observedVisualStateId &&
      ...
      try {
        ...
        if (current.semanticStateId !== observedStateId) {
          return staleResponse(observedStateId, current.semanticStateId);
        }
        if (
          command.observedVisualStateId &&
