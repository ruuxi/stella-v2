...
const pendingRef = makeFunctionReference<
  ...
  number | null
>("cloud_home_context:pending");
const deliveredRef = makeFunctionReference<
  "mutation",
  { ownerId: string; ownerGeneration: string; revision: number },
  null
>("cloud_home_context:delivered");
export const pending = internalQuery({
  ...
  handler: async (ctx, args) => {
    const row = await ctx.db
      ...
      .withIndex("by_ownerId_and_ownerGeneration", (q) =>
        q
...
export const delivered = internalMutation({
  args: { ...ownerArgs, revision: v.number() },
  returns: v.null(),
  handler: async (ctx, args) => {
    const row = await ctx.db
      .query("cloud_home_context_updates")
    ...
    if (row)
      await ctx.db.patch(row._id, {
        ...
        pending: args.revision < row.revision,
      });
    return null;
  },
});
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const response = await fetch(
        `${endpoint.url}/internal/owners/home-context/changed`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({ ...args, revision }),
          signal: AbortSignal.timeout(8_000),
        },
      );
      if (!response.ok)
        throw new Error("Context notification not acknowledged");
...
export const retryPending = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    const rows = await ctx.db
      .query("cloud_home_context_updates")
      .withIndex("by_pending_and_retryAt", (q) =>
        q.eq("pending", true).lte("retryAt", now),
      )
      .take(100);
    for (const row of rows) {
      await ctx.db.patch(row._id, { retryAt: now + 60_000 });
      await ctx.scheduler.runAfter(0, publishHomeContextRef, {
        ownerId: row.ownerId,
