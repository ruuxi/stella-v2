...
// agent-home/<sha256(ownerId)>/memories/ and are read and written by the
// orchestrator DO through its AGENT_HOME bucket binding — Convex holds only
// the registry row for each (cloud_agent_home_docs).
//
...
export const searchOwnerMessagesInternal = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const needle = terms.map((term) => term.text).join(" ").slice(0, 512);
