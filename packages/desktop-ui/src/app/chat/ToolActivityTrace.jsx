...
import { AlertCircle, Box, Check, ChevronRight, Clock, Code, FileText, Globe, Lightbulb, MessageSquare, Pencil, Search, Wand2, } from "@/ui/icons";
...
function SwapLabel({ text }) {
    ...
    return (<span className="tool-activity__swap">
      {previous !== null && (<span key={`out:${previous}`} className="tool-activity__swap-layer tool-activity__swap-layer--out" aria-hidden="true">
          {previous}
        </span>)}
      <span key={`in:${current}`} className="tool-activity__swap-layer tool-activity__swap-layer--in">
        {current}
      </span>
    </span>);
}
function StepRow({ step }) {
    return (<li className="tool-activity__step" data-status={step.status}>
      <span className="tool-activity__step-glyph" aria-hidden="true">
        {step.status === "error" ? (<AlertCircle size={12} strokeWidth={2}/>) : (<Check size={12} strokeWidth={2}/>)}
      </span>
...
export function ToolActivityTrace({ group, traceKey, }) {
    ...
    return (<div className={`tool-activity${intro ? " tool-activity--intro" : ""}`} data-expanded={expanded ? "true" : undefined}>
      <button type="button" className="tool-activity__header" aria-expanded={expanded} onClick={() => setExpanded((value) => !value)}>
        ...
        <ChevronRight className="tool-activity__chevron" size={13} strokeWidth={2} aria-hidden="true"/>
      </button>
      <div className="tool-activity__steps-wrap" ref={wrapRef} aria-hidden={!expanded}>
        <ul className="tool-activity__steps" ref={innerRef} aria-label={`${steps.length} tool calls`}>
