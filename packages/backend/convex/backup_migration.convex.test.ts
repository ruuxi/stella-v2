...
describe("backup ownership migration", () => {
  ...
  it("keeps a partially progressed migration fenced when a later manifest locator needs repair", async () => {
    const t = createTest();
    const fromOwnerId = "backup-repair-source";
    const toOwnerId = "backup-repair-destination";
    const fence = await seedMigration(t, { fromOwnerId, toOwnerId });
    await insertEscrow(t, {
      ownerId: fromOwnerId,
      ownerGeneration: fence.fromOwnerGeneration,
    ...
    for (let index = 0; index < 9; index += 1) {
      const orderedObjectId = index.toString(16).padStart(64, "0");
      await insertObject(t, {
        ownerId: fromOwnerId,
        ownerGeneration: fence.fromOwnerGeneration,
        keyFingerprint: keyA,
        objectId: orderedObjectId,
        plaintextSha256: plaintextA,
        r2Key: `backups/source/ordered-${index}.bin`,
        createdAt: index + 1,
      });
    }
    const sharedManifestLocator = "backups/shared/incompatible-manifest.bin";
    await insertManifest(t, {
      ownerId: fromOwnerId,
      ownerGeneration: fence.fromOwnerGeneration,
      keyFingerprint: keyA,
      snapshotId: "source-snapshot",
      r2Key: sharedManifestLocator,
      sourceDeviceId: "source-device",
      isLatest: true,
      createdAt: 10,
    });
    ...
    await expect(
      ...
      /backup_migration_repair_required: .*manifest R2 locator .*incompatible metadata/u,
    );

    const state = await t.run(async (ctx) => ({
      migration: await ctx.db.get(
        fence.migrationId as Id<"auth_owner_migrations">,
      ),
      sourceObjects: await ctx.db
        .query("backup_objects")
        .withIndex("by_ownerId_and_createdAt", (q) =>
    ...
    expect(state.migration?.status).toBe("running");
    expect(state.sourceObjects).toEqual([]);
    expect(state.destinationObjects).toHaveLength(9);
    expect(state.sourceManifests).toHaveLength(1);

    // End the simulated lost-response pass as pending, then let the real outer
    // worker reclaim it. It must classify the repeated backup repair condition
    // as retryable and persist pending again, never terminally expose the split
    // source/destination rows through a failed migration status.
    await t.run(async (ctx) => {
      await ctx.db.patch(fence.migrationId as Id<"auth_owner_migrations">, {
        cloudProductStage: "complete",
      });
      await seedReadyMigrationBackupSweep(ctx, {
        migrationId: fence.migrationId as Id<"auth_owner_migrations">,
      });
    });
    await t.mutation(internal.auth_migration.finishOwnershipMigrationPass, {
      fromOwnerId,
      toOwnerId,
    ...
    await t.action(internal.auth_migration.migrateOwnership, {
      fromOwnerId,
      toOwnerId,
    });
    const afterOuterWorker = await t.run(async (ctx) =>
      ctx.db.get(fence.migrationId as Id<"auth_owner_migrations">),
    );
    expect(afterOuterWorker).toMatchObject({
      status: "pending",
      lastError: "Account data is still moving and will retry automatically.",
    });
    expect(afterOuterWorker?.leaseId).toBeUndefined();
    expect(afterOuterWorker?.leaseExpiresAt).toBeUndefined();
    await expect(
      t.query(internal.backups.listBackupsForOwnerInternal, {
        ownerId: toOwnerId,
        deviceId: "destination-device",
