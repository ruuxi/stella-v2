...
type ShareEmojiPackDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  pack: EmojiPackRecord;
};

const EMOJI_SHARE_PREFIX = "stella://emoji/";

const buildShareLink = (handle: string, packId: string): string =>
  `${EMOJI_SHARE_PREFIX}${handle.toLowerCase()}/${packId.toLowerCase()}`;

/**
 * Share an emoji pack with friends. Mirrors `ShareAddonDialog` —
 * one-line link via DM (`getOrCreateDmRoom` + `sendRoomMessage`) plus
 * a copy-to-clipboard fallback. Private packs surface a one-click
 * "Make unlisted" CTA so the user's intent ("share with this friend")
 * isn't blocked by the visibility tier they happened to pick.
 */
export function ShareEmojiPackDialog({
  open,
  onOpenChange,
  pack,
}: ShareEmojiPackDialogProps) {
  ...
  const setVisibility = useMutation(api.data.emoji_packs.setVisibility);
  ...
  const [shadowVisibility, setShadowVisibility] = useState<
    EmojiPackVisibility | undefined
  >(pack.visibility);
  const effectiveVisibility = shadowVisibility ?? pack.visibility;
  const isPrivate = effectiveVisibility === "private";

  const link = useMemo(
    () => (pack.authorHandle ? buildShareLink(pack.authorHandle, pack.packId) : null),
    [pack.authorHandle, pack.packId],
  );
    ...
    try {
      ...
      for (const ownerId of selected) {
        try {
          ...
          // best-effort; aggregate error toast at the end
        }
      }
  ...
  }, [getOrCreateDm, handleOpenChange, link, selected, sendRoomMessage, sending]);

  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="credential-modal share-addon-dialog">
        <DialogCloseButton />
        <DialogTitle>Share {pack.displayName}</DialogTitle>
        <DialogDescription>
          {!link
            ? "This pack doesn't have a public author handle yet, so it can't be shared."
            : isPrivate
            ? "This pack is private. Make it unlisted so people you share with can use it."
            ...
            <div className="share-addon-private-banner">
              ...
              <div className="share-addon-private-banner-text">
                <div className="share-addon-private-banner-title">
                  Private packs can't be opened by anyone else
                </div>
