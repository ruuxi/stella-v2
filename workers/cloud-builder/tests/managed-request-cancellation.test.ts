...
const setup = () => {
  ...
  return {
    abort, calls, background,
    args: {
      request: new Request("https://gateway/v2/relay", {
        headers: { "x-stella-request-id": "request-1" }, signal: abort.signal,
      }),
      capability: "signed-turn-capability",
      control: { async cancelManagedRequest(value: unknown) { calls.push(value); return { canceled: true }; } },
...
describe("private managed cancellation", () => {
  test("sends exact cancellation when a fetch hop ignores AbortSignal", async () => {
    ...
    const work = fetchWithManagedCancellation({ ...h.args,
      fetch: async () => new Promise(resolve => { release = resolve; }),
    });
    await Promise.resolve();
    h.abort.abort();
    await Promise.all(h.background);
    expect(h.calls).toEqual([{ capability: "signed-turn-capability", requestId: "request-1" }]);
  ...
  test("keeps cancellation connected after headers and until response consumption", async () => {
    const h = setup();
    const response = await fetchWithManagedCancellation({ ...h.args,
  ...
  test("retries a lost cancellation response without retrying inference", async () => {
    ...
    const response = await fetchWithManagedCancellation({ ...h.args,
      control: { async cancelManagedRequest() {
        attempts++;
        if (attempts === 1) throw Error("lost response");
        return { canceled: true };
      } },
      fetch: async () => { executions++; return new Response("ok"); },
    });
