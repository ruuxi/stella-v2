...
const MOODS = ["Auto", "Focus", "Calm", "Energy", "Sleep", "Lo-fi"];
...
export function MusicPlayer() {
    const { status, mood, error, currentPromptLabel, elapsedSeconds, userHint, lyrics, analyserRef, play, selectMood, stop, setUserHint, toggleLyrics, } = useLyriaMusic();
    ...
    return (<div className="dashboard-card">
      ...
      <div className="music-left-group" data-active={isActive || undefined}>
        <div className="music-prompt-bar">
          <input type="text" className="music-prompt-input" placeholder="Describe your vibe..." value={localHint} onChange={(e) => setLocalHint(e.target.value)} onKeyDown={(e) => {
            if (e.key === "Enter") {
                e.preventDefault();
                handlePlay();
            }
        ...
        {isActive && (<div className="music-track-info">
      ...
      <button className={`music-play-btn${isActive ? " music-play-btn--active" : ""}`} onClick={isActive ? stop : handlePlay} onMouseEnter={preloadMusic} onFocus={preloadMusic} disabled={status === "loading"} aria-label={isActive ? "Stop" : "Play"}>
        {status === "loading" ? (<span className="music-loading-dot"/>) : isActive ? (<Square size={14}/>) : (<Play size={14}/>)}
      </button>

      {/* Right side: Mood chips + Lyrics toggle */}
      <div className="music-right-group">
        <div className="music-moods">
          {MOODS.map((m) => (<button key={m} className={`music-mood-chip${m === mood ? " music-mood-chip--selected" : ""}`} onClick={() => selectMood(m)}>
              {m}
            </button>))}
        </div>
