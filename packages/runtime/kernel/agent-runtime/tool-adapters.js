...
import { sanitizeToolError, sanitizeToolResult, sanitizeToolVisibleText, } from "../tools/safety.js";
...
const formatToolWorkingText = (metadata) => metadata.workingText ??
    `Running ${metadata.label ?? formatToolLabel(metadata.name)}`;
export const getRequestedRuntimeToolNames = (toolsAllowlist) => Array.isArray(toolsAllowlist) && toolsAllowlist.length > 0
    ? toolsAllowlist
    : [...STELLA_LOCAL_TOOLS];
/**
...
export const getRuntimeToolMetadata = (opts) => {
    ...
    if (missing.length > 0) {
        ...
        console.warn(`[tool-adapters] dropped unknown tools from allowlist: ${missing.join(", ")}`);
    }
    return resolved;
};
const mergeToolSideEffectsIntoDetails = (details, fileChanges, producedFiles) => {
...
const formatToolResult = (toolResult, toolName) => {
    if (toolResult.error) {
        ...
        return {
            text: `Error: ${error}`,
            details: mergeToolSideEffectsIntoDetails(sanitizeToolResult(toolResult.details ?? { error }), toolResult.fileChanges, toolResult.producedFiles),
        };
    }
...
const parseAttachImageMatches = (text) => {
    ...
    while (markerStart >= 0) {
        const nextMarker = text.indexOf(STELLA_ATTACH_IMAGE_MARKER, markerStart + STELLA_ATTACH_IMAGE_MARKER.length);
        const newline = text.indexOf("\n", markerStart);
        const markerEnd = Math.min(nextMarker < 0 ? text.length : nextMarker, newline < 0 ? text.length : newline);
        ...
        const quotedFallback = pathAssignment
            ? null
            : /(?:^|\s)("(?:\\.|[^"\\])*")/.exec(marker);
        const legacyFallback = pathAssignment || quotedFallback
            ? null
            : /(?:^|\s)((?:\/[^\s"]+|[A-Za-z]:[\\/][^\s"]+?)\.(?:png|jpg|jpeg|gif|webp))/i.exec(marker);
            ...
            if (typeof value === "string") {
                ...
                for (const match of parseAttachImageMatches(value)) {
                    ...
                    if (replacement?.full === match.full) {
                        replaced = replaced.replace(replacement.full, replacement.replacement);
                    }
                }
                return replaced;
            }
            if (Array.isArray(value))
                return value.map(replaceJsonStrings);
...
export const executeRuntimeToolCall = async (args) => {
    ...
    if (args.hookEmitter) {
        const hookResult = await args.hookEmitter.emit("before_tool", { tool: args.toolName, args: args.args, context }, { tool: args.toolName, agentType: args.agentType });
    ...
    let toolResult = await args.toolExecutor(args.toolName, effectiveArgs, context, args.signal, args.onUpdate);
    if (args.hookEmitter) {
        const hookResult = await args.hookEmitter.emit("after_tool", {
            tool: args.toolName,
            args: effectiveArgs,
            result: toolResult,
            context,
