...
test("cloud hand-off balances the desktop start and ignores later dispatch updates", async () => {
  ...
  const accepted = await StellaRuntimeHost.prototype.startPlacedChat.call(
    host,
    {
  ...
  expect(accepted).toEqual({
    runId: "placed:dispatch-1",
    userMessageId: "dispatch-1",
  });
  ...
  expect(events.map((event) => event.userMessageId)).toEqual([
    "dispatch-1",
    "dispatch-1",
  ]);
...
test("private chats stay on this computer with a previously selected cloud or remote target", async () => {
  ...
  const host = {
    ...
    requestWorker: async (_method: string, payload: unknown) => {
      requests.push(payload);
      return { runId: "local-run" };
    },
  };
  for (const executionTarget of [
    { mode: "cloud" },
    { mode: "device", deviceId: "other-computer" },
  ]) {
    await StellaRuntimeHost.prototype.startChat.call(host, {
      conversationId: "local_private",
      storageMode: "local",
      userPrompt: "Private",
      executionTarget,
    });
