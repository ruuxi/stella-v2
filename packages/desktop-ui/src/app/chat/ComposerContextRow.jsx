...
import { ComposerActivityContextSection, ComposerAppSelectionContextSection, ComposerCaptureContextSection, ComposerFileContextSection, ComposerPastedTextContextSection, ComposerSelectedTextContextSection, ComposerWindowContextSection, } from "./ComposerContextSections";
import { appChipToChatContext, tabChipToChatContext, useAutoContextChips, } from "@/features/chat/hooks/use-auto-context-chips";
import { truncateChipLabel } from "@/features/chat/composer-context";
export function ComposerContextRow({ variant = "full", chatContext, selectedText, setChatContext, setSelectedText, onPreviewScreenshot, }) {
    return (<div className="composer-context-actions">
      <ComposerCaptureContextSection variant={variant} chatContext={chatContext} setChatContext={setChatContext} onPreviewScreenshot={onPreviewScreenshot}/>
      <ComposerFileContextSection variant={variant} chatContext={chatContext} setChatContext={setChatContext}/>
      <ComposerPastedTextContextSection variant={variant} chatContext={chatContext} setChatContext={setChatContext}/>
      <ComposerSelectedTextContextSection variant={variant} selectedText={selectedText} setSelectedText={setSelectedText} setChatContext={setChatContext}/>
      <ComposerAppSelectionContextSection variant={variant} chatContext={chatContext} setChatContext={setChatContext}/>
      <ComposerActivityContextSection variant={variant} chatContext={chatContext} setChatContext={setChatContext}/>
      <ComposerWindowContextSection variant={variant} chatContext={chatContext} setChatContext={setChatContext}/>
    </div>);
}
export function ComposerSuggestionContextRow({ active = true, chatContext, setChatContext, }) {
    const { lanes, dismissSlot } = useAutoContextChips(active);
    const rowRef = useRef(null);
    const lanesRef = useRef(null);
    const [hiddenLaneIndexes, setHiddenLaneIndexes] = useState(() => new Set());
    ...
    const isChipAttached = useCallback((chip) => {
        if (chip.kind === "tab") {
            return Boolean(attachedUrl && attachedUrl === chip.url);
        }
        return Boolean(attachedAppName && chip.name.toLowerCase().trim() === attachedAppName);
    }, [attachedAppName, attachedUrl]);
    const laneVisibilityKey = useMemo(() => lanes
        .map((lane) => {
        const current = lane.current && !isChipAttached(lane.current.chip)
    ...
    useLayoutEffect(() => {
        ...
        const syncVisibleLanes = () => {
            ...
            frame = requestAnimationFrame(() => {
                const laneEls = Array.from(lanesEl.querySelectorAll(".composer-context-suggestion-lane"));
                ...
                for (const laneEl of laneEls) {
                    ...
                    if (laneWidth <= 0)
                        continue;
                    const nextWidth = usedWidth + (visibleCount > 0 ? gap : 0) + laneWidth;
    ...
    return (<div ref={rowRef} className="composer-context-actions composer-context-actions--suggestions">
      <div ref={lanesRef} className="composer-context-suggestion-lanes">
        {lanes.map((lane, index) => (<SuggestionLaneView key={`lane-${index}`} index={index} lane={lane} setChatContext={setChatContext} onDismissCurrent={dismissSlot} isChipAttached={isChipAttached} overflowHidden={hiddenLaneIndexes.has(index)}/>))}
      </div>
    </div>);
}
/**
 ...
 */
export function useComposerContextSuggestions(active, chatContext, setChatContext) {
    const { lanes, dismissSlot } = useAutoContextChips(active);
    const attachedAppName = chatContext?.window?.app?.toLowerCase().trim() ?? null;
    const attachedUrl = chatContext?.browserUrl ?? null;
    const suggestions = useMemo(() => lanes.flatMap((lane) => {
        const slot = lane.current;
        if (!slot)
            return [];
        const attached = slot.chip.kind === "tab"
            ? Boolean(attachedUrl && attachedUrl === slot.chip.url)
            : Boolean(attachedAppName &&
                slot.chip.name.toLowerCase().trim() === attachedAppName);
        return attached ? [] : [slot];
    }), [attachedAppName, attachedUrl, lanes]);
    const selectSuggestion = useCallback((slot) => {
        if (slot.chip.kind === "tab") {
            setChatContext(tabChipToChatContext(slot.chip));
            captureAppWindowAsync({ appName: slot.chip.browser, pid: null }, setChatContext);
        }
        else {
 ...
 */
function SuggestionLaneView({ index, lane, setChatContext, onDismissCurrent, isChipAttached, overflowHidden, }) {
    const showCurrent = Boolean(lane.current && !isChipAttached(lane.current.chip));
    const showOutgoing = Boolean(lane.outgoing);
    if (!showCurrent && !showOutgoing)
        return null;
    return (<div className="composer-context-suggestion-lane" data-lane-index={index} data-overflow-hidden={overflowHidden || undefined}>
      {showOutgoing && lane.outgoing ? (<SuggestionChipSlot slot={lane.outgoing} setChatContext={setChatContext} onDismiss={() => { }}/>) : null}
      {showCurrent && lane.current ? (<SuggestionChipSlot slot={lane.current} setChatContext={setChatContext} onDismiss={() => onDismissCurrent(lane.current.key)}/>) : null}
    </div>);
}
function SuggestionChipSlot({ slot, setChatContext, onDismiss, }) {
    const handleClick = () => {
        if (slot.chip.kind === "tab") {
            setChatContext(tabChipToChatContext(slot.chip));
    ...
    if (slot.chip.kind === "tab") {
        return (<TabSuggestionChip tab={slot.chip} phase={slot.phase} onClick={handleClick}/>);
    }
    return (<AppSuggestionChip app={slot.chip} phase={slot.phase} onClick={handleClick}/>);
}
/**
 ...
 */
const captureAppWindowAsync = (target, setChatContext) => {
    const api = getElectronApi();
    const captureFn = api?.home?.captureAppWindow;
    if (!captureFn) {
        setChatContext((prev) => prev ? { ...prev, capturePending: false } : prev);
        return;
    }
    ...
    void (async () => {
        try {
            ...
            setChatContext((prev) => {
                if (!prev?.window)
                    return prev;
                if (prev.window.app !== appName)
                    return prev;
                if (prev.capturePending !== true)
                    return prev;
                if (!capture) {
...
function AppSuggestionChip({ app, phase, onClick, }) {
    const detail = app.windowTitle
        ? `${app.name} — ${app.windowTitle}`
        : app.name;
    return (<button type="button" className="composer-context-suggestion" data-phase={phase} title={app.isActive
            ? `Add ${detail} (current window) as context`
            : `Add ${detail} as context`} onClick={(event) => {
...
function TabSuggestionChip({ tab, phase, onClick, }) {
    return (<button type="button" className="composer-context-suggestion composer-context-suggestion--tab" data-phase={phase} title={tab.title ? `${tab.title} — ${tab.url}` : tab.url} onClick={(event) => {
            event.preventDefault();
            onClick();
            event.currentTarget.blur();
        }}>
 ...
 */
function ChipAppGlyph({ iconDataUrl, fallbackLabel, }) {
    if (iconDataUrl) {
        return (<img src={iconDataUrl} alt="" aria-hidden="true" className="composer-context-suggestion__icon" draggable={false}/>);
    }
    return (<span className="composer-context-suggestion__label">
