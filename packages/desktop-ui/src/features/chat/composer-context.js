...
export const truncateChipLabel = (text, max = CHIP_LABEL_MAX_CHARS) => {
    if (text.length <= max)
        return text;
 ...
 */
export const getComposerAppSelections = (chatContext) => {
...
const appSelectionDedupeKey = (selection) => JSON.stringify([
    selection.label,
    selection.snapshot,
    selection.bounds,
 ...
 */
export const hasAttachedComposerChips = (chatContext, selectedText) => {
    if (selectedText)
        return true;
    if (!chatContext)
        return false;
    ...
    if (chatContext.window)
        return true;
    if (getComposerAppSelections(chatContext).length > 0)
        return true;
    if (chatContext.browserUrl)
        return true;
    if (chatContext.regionScreenshots && chatContext.regionScreenshots.length > 0)
        return true;
    if (chatContext.files && chatContext.files.length > 0)
        return true;
    if (chatContext.pastedTexts && chatContext.pastedTexts.length > 0)
        return true;
    if (chatContext.capturePending)
        return true;
    return false;
};
export const resolveComposerContextState = (chatContext, selectedText) => {
    const hasVisibleWindowContext = Boolean(chatContext?.window);
    const windowContextEnabled = Boolean(chatContext?.window && chatContext.windowContextEnabled !== false);
    ...
    const hasAppSelectionContext = getComposerAppSelections(chatContext).some((selection) => Boolean(selection.snapshot));
    ...
    const hasSubmittableContext = Boolean(hasScreenshotContext
        || hasFileContext
    ...
    return {
        ...
        hasComposerContext: Boolean(hasSubmittableContext || hasPendingCaptureContext || hasVisibleWindowContext),
    };
};
const resolveComposerPlaceholder = ({ contextState, }) => {
...
export const deriveComposerState = ({ message, chatContext = null, selectedText = null, conversationId = null, requireConversationId = false, }) => {
    ...
    const canSubmit = Boolean(hasConversation && (hasMessage || contextState.hasSubmittableContext));
    return {
        contextState,
...
export const clearComposerWindowContext = (setChatContext) => {
...
export const clearComposerAppSelectionContext = (setChatContext) => {
    setChatContext((prev) => (prev ? { ...prev, appSelection: null, appSelections: [] } : prev));
};
/**
 ...
 */
export const attachComposerAppSelectionContext = (selection, setChatContext) => {
    setChatContext((prev) => {
        const base = prev ?? {
            window: null,
            browserUrl: null,
...
export const removeComposerAppSelectionContext = (index, setChatContext) => {
    setChatContext((prev) => {
        if (!prev)
            return prev;
        const existing = getComposerAppSelections(prev);
...
export const clearComposerActivityContext = (setChatContext) => {
    setChatContext((prev) => (prev ? { ...prev, activity: null } : prev));
};
export const clearComposerSelectedTextContext = (setSelectedText, setChatContext) => {
    setSelectedText(null);
    setChatContext((prev) => (prev ? { ...prev, selectedText: null } : prev));
};
export const removeComposerScreenshotContext = (index, setChatContext) => {
...
export const removeComposerFileContext = (index, setChatContext) => {
    setChatContext((prev) => {
        if (!prev)
            return prev;
...
export const removeComposerPastedTextContext = (index, setChatContext) => {
    setChatContext((prev) => {
        if (!prev)
            return prev;
