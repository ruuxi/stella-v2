...
import { getModelRestrictionActionLabel, getModelRestrictionDescription, isRestrictedModelOverrideAudience, resolveBillingAudience, } from "@/global/billing/audience";
import { BYOK_TOAST_ACTION } from "@/global/billing/byok-action";
import { resolveTierRestrictedModelNotice, } from "./tier-restricted-model-notice";
import { showToast } from "@/ui/toast";
const buildToastDedupeKey = (audience, agent, model) => `${audience}|${agent}|${model}`;
export function useTierRestrictedModelToast() {
    ...
    const audience = useMemo(() => resolveBillingAudience({ hasConnectedAccount, billingStatus }), [billingStatus, hasConnectedAccount]);
    ...
    return useCallback(async () => {
        ...
        if (!isRestrictedModelOverrideAudience(audience) || !audience)
            return;
        let preferences;
        try {
            preferences =
