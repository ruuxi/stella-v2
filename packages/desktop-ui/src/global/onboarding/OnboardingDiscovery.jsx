...
export const OnboardingDiscovery = ({ categoryStates, onToggleCategory, }) => {
    ...
    const hasFdaCategories = DISCOVERY_CATEGORIES.some((cat) => cat.requiresFDA && categoryStates[cat.id]);
    const canShowFdaNote = platform === "darwin";
    const showFdaNote = canShowFdaNote && hasFdaCategories;
    const openFullDiskAccess = () => window.electronAPI?.system.openFullDiskAccess?.();
    return (<div className="onboarding-discovery" data-visible={true}>
      ...
      {canShowFdaNote && (<div className="onboarding-fda-note" data-visible={showFdaNote || undefined} aria-hidden={!showFdaNote}>
          ...
          <button className="onboarding-fda-link" onClick={openFullDiskAccess} tabIndex={showFdaNote ? 0 : -1}>
