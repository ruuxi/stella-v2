...
const normalizeInviteCode = (raw: string): string => {
  ...
  if (normalized.length !== INVITE_CODE_LENGTH) {
    throw new ConvexError({
      ...
      message:
        "That invite code doesn\u2019t look right. Codes are 8 characters.",
    });
  }
  return normalized;
};
...
export const createCommunity = mutation({
  ...
  handler: async (ctx, args) => {
    assertC8RetiredSurfaceUnavailable("Social communities");
  ...
  handler: async (ctx, args) => {
    ...
    if (args.memberOwnerId === ownerId) {
      throw new ConvexError({
        code: "INVALID_ARGUMENT",
        message:
          "You can\u2019t remove yourself. Delete the community instead.",
      });
    }
...
export const leaveCommunity = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (membership.role === "owner") {
      ...
      throw new ConvexError({
        code: "FORBIDDEN",
        message: "You created this community. Delete it instead of leaving it.",
      });
    }
    await ctx.db.delete(membership._id);
