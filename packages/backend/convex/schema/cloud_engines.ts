...
export const cloudEnginesSchema = {
  ...
  cloud_llm_credentials: defineTable({
    ...
    // credential for this provider. New rows store a domain-separated source
    // digest (legacy rows with a raw owner id are repaired before source purge)
    // so the encrypted copy remains recoverable without principal residue.
    ...
    // Rolling repair/source-residue audit for legacy imports that persisted a
    // raw source principal instead of the new opaque digest.
    .index("by_importedFromOwnerId", ["importedFromOwnerId"]),
  ...
  cloud_engine_connects: defineTable({
    connectId: v.string(),
    ownerId: v.string(),
    /** Exact owner-data generation in which this PKCE capability was minted. */
  ...
  cloud_engine_settings: defineTable({
    ...
    // with this field absent remains the active singleton. The value is an
    // opaque source digest; only defined-vs-undefined is product semantics.
    importedFromOwnerId: v.optional(v.string()),
    updatedAt: v.number(),
  })
    .index("by_ownerId", ["ownerId"])
