...
export const registerSystemHandlers = (options) => {
    ...
    ipcMain.handle(IPC_LINK_WALLET_STATUS, (event) => {
        if (!options.externalLinkService.assertPrivilegedSender(event, IPC_LINK_WALLET_STATUS)) {
            throw new Error("Blocked untrusted Link wallet status.");
        }
        return options.getLinkWalletStatus();
    });
    ipcMain.handle(IPC_LINK_WALLET_CONNECT, (event, payload) => {
        if (!options.externalLinkService.assertPrivilegedSender(event, IPC_LINK_WALLET_CONNECT)) {
            throw new Error("Blocked untrusted Link wallet connect.");
        }
        return options.connectLinkWallet(payload ?? {});
    });
    ipcMain.handle(IPC_LINK_WALLET_DISCONNECT, (event) => {
        if (!options.externalLinkService.assertPrivilegedSender(event, IPC_LINK_WALLET_DISCONNECT)) {
            throw new Error("Blocked untrusted Link wallet disconnect.");
        }
        return options.disconnectLinkWallet();
    });
    ipcMain.handle(IPC_LINK_WALLET_ADD_CARD, (event) => {
        if (!options.externalLinkService.assertPrivilegedSender(event, IPC_LINK_WALLET_ADD_CARD)) {
            throw new Error("Blocked untrusted Link wallet add card.");
        }
        return options.addLinkWalletCard();
    });
    ipcMain.handle(IPC_LINK_WALLET_RESPOND, (event, payload) => {
        if (!options.externalLinkService.assertPrivilegedSender(event, IPC_LINK_WALLET_RESPOND)) {
            throw new Error("Blocked untrusted Link wallet response.");
        }
        if (payload.action !== "accept" &&
            payload.action !== "decline" &&
            payload.action !== "cancel") {
            throw new Error("Invalid Link wallet action.");
        }
        return options.respondLinkWallet(payload);
    });
    ...
    ipcMain.handle(IPC_CUSTOMIZATIONS_RESET, async (event) => {
        ...
        try {
            ...
            if (stellaInstallDir && result.movedEntries.some((entry) => entry.startsWith(`skills${path.sep}`))) {
