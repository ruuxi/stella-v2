...
import { setPendingSocialInvite, usePendingSocialInvite, } from "./social-invite-store";
...
export function SocialInviteLayer() {
    ...
    if (!invite)
        return null;
    return (<SocialInviteDialog key={invite.kind === "join-community"
...
function SocialInviteDialog() {
    ...
    const sendFriendRequestMutation = useMutation(api.social.relationships.sendFriendRequest);
    const communityPreview = useQuery(api.social.communities.getCommunityPreviewByInviteCode, invite?.kind === "join-community" && hasConnectedAccount
        ? { inviteCode: invite.inviteCode }
        : "skip");
    const friendPreview = useQuery(api.social.profiles.getProfileByUsername, invite?.kind === "add-friend" && hasConnectedAccount
    ...
    const storePreview = useQuery(api.data.store_packages.getPublicPackage, invite?.kind === "view-store-package"
    ...
    const handleConfirm = useCallback(async () => {
        ...
        try {
            ...
            setError(getSocialActionErrorMessage(invite.kind === "join-community"
                ? "Couldn't join that community. The invite may have expired."
                : "Couldn't send that friend request. Please try again.", err));
        }
        finally {
            setBusy(false);
        }
    ...
    const copy = isStore
        ...
        : isJoin
            ? {
                ...
                missing: "No community was found for this invite code. Ask for a fresh link.",
            }
            : {
                title: "Add friend",
    ...
    return (<Dialog open onOpenChange={(next) => (next ? null : close())}>
      <DialogContent fit className="friends-dialog-content">
        ...
        <DialogBody className="friends-dialog-body">
          ...
          <div className="social-invite-preview">
            {needsAccount ? (<div className="friends-empty">
                Sign in to Stella to accept this invite.
              </div>) : previewLoading ? (<div className="friends-empty">Looking up the invite…</div>) : previewMissing ? (<div className="friends-empty">{copy.missing}</div>) : isStore && storePreview ? (<div className="friends-item">
                <Avatar fallback={storePreview.displayName} src={storePreview.iconUrl} size="normal"/>
              ...
              </div>) : isJoin && communityPreview ? (<div className="friends-item">
              ...
              </div>) : friendPreview ? (<div className="friends-item">
                <Avatar fallback={friendPreview.username} src={friendPreview.avatarUrl} size="normal"/>
                <div className="friends-item-info">
                  <div className="friends-item-name">
          ...
          {error ? (<div className="friends-status-message" data-type="error">
              {error}
            </div>) : null}

          <div className="friends-dialog-footer">
            <button type="button" className="pill-btn pill-btn--lg" onClick={close} disabled={busy}>
              Cancel
            </button>
            <button type="button" className="pill-btn pill-btn--primary pill-btn--lg" onClick={() => void handleConfirm()} disabled={busy ||
            needsAccount ||
