...
export const shouldRestartElectronForBuildPath = (filename) => {
  ...
  if (
    sidecarOwnedPackagePrefixes.some((prefix) => normalized.startsWith(prefix))
  ) {
    return false;
  }
  ...
  if (
    hostOwnedPackagePrefixes.some((prefix) => normalized.startsWith(prefix))
  ) {
    return true;
  }
  ...
  // webview navigation/reload. The store webview's preload used to trigger
  // spurious restarts:
