...
import { getCompactionTriggerTokens, getThreadTokenEstimate, } from "../thread-runtime.js";
import { CONTEXT_DELTA_CUSTOM_TYPE_PREFIX, PINNED_INSTRUCTION_ENTRY_ID_MARKER, } from "./resident-context.js";
import { checkPromptPrefixStability, clearPromptPrefixSnapshot, } from "./prompt-prefix-guard.js";
import { clearProviderContextWindow, estimateProviderPayloadTokens, getLastProviderPayloadTokens, providerInputBudgetTokens, setProviderContextWindow, withForcedThreadCompaction, } from "./context-budget.js";
...
const durableComparableMessage = (message) => {
    if (message?.role === "assistant") {
        const { stellaRunId: _runId, stellaAttemptGeneration: _attemptGeneration, ...comparable } = message;
        return comparable;
    }
...
const hasExactDurableCompletedSuffix = (history, completedMessages) => {
    if (completedMessages.length === 0 || completedMessages.length > history.length) {
        return false;
    }
    const offset = history.length - completedMessages.length;
    return completedMessages.every((message, index) => safeSchemaJson(durableComparableMessage(history[offset + index])) ===
...
const hasExactDurableRawTail = (args) => {
    ...
    const checkpointIndex = args.threadHistory.findIndex((entry) => entry.entryId === checkpointId);
    if (checkpointIndex < 0) {
        return false;
    }
    const rawTailEntries = args.threadHistory
        .slice(checkpointIndex + 1)
        .filter((entry) => !entry.entryId?.includes(PINNED_INSTRUCTION_ENTRY_ID_MARKER));
    const durableRawTail = buildHistorySource({
        ...args.agentContext,
        threadHistory: rawTailEntries,
    });
    const rawTailLength = durableRawTail.length;
    if (rawTailLength > args.residentMessages.length || rawTailLength > args.refreshedMessages.length) {
        return false;
    }
    ...
    for (let index = 0; index < rawTailLength; index += 1) {
        const durable = safeSchemaJson(durableComparableMessage(durableRawTail[index]));
        if (safeSchemaJson(durableComparableMessage(args.refreshedMessages[refreshedOffset + index])) !== durable ||
            safeSchemaJson(durableComparableMessage(args.residentMessages[residentOffset + index])) !== durable) {
            return false;
        }
    }
    return true;
...
export class PiSessionCore {
    ...
    async refreshActiveWorkingSetAtBoundary(args) {
        ...
        if (!agent || !scheduler || args.signal?.aborted)
            return undefined;
        ...
        if (!this.pendingHistoryRefresh) {
            try {
                ...
                if (lastProviderTokens === undefined) {
                    ...
                    measuredTokens = getThreadTokenEstimate(args.opts.store.loadThreadMessages(this.threadKey));
                }
                else {
                    ...
                    measuredTokens = lastProviderTokens +
                        estimateProviderPayloadTokens({ messages: args.completedMessages }, Number.POSITIVE_INFINITY);
                }
            }
            catch {
            ...
            const triggerTokens = getCompactionTriggerTokens(args.opts.resolvedLlm, args.opts.agentType);
            if (measuredTokens < triggerTokens)
                return undefined;
            args.onCompacting?.();
        }
        try {
            ...
            if (this.agent !== agent || args.signal?.aborted)
                return undefined;
            if (!this.pendingHistoryRefresh) {
                let compacted = false;
                const inputBudget = providerInputBudgetTokens(args.opts.resolvedLlm.model.contextWindow);
                const runCompaction = () => runCompactionWithHooks({
                    opts: args.opts,
                    threadKey: this.threadKey,
                    runId: args.runId,
                ...
                const scheduled = scheduler.schedule({
                    threadKey: this.threadKey,
                    run: async () => {
                        const result = inputBudget && measuredTokens >= inputBudget
                            ? await withForcedThreadCompaction(this.threadKey, runCompaction)
                            : await runCompaction();
            ...
            const completedSuffixExact = hasExactDurableCompletedSuffix(refreshed, args.completedMessages);
            const currentTurnSuffixExact = !args.requiredResidentSuffix ||
    ...
    dropDurableErroredTailForRetry(store, runId, options) {
        ...
        try {
            ...
            const errored = payload?.role === "assistant" &&
                (payload.stopReason === "error" || payload.stopReason === "aborted");
            const empty = options?.allowEmpty === true &&
                payload?.role === "assistant" &&
                !payload.content.some((block) => block.type === "toolCall" ||
