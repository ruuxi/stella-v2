...
const cardTitle = (map) => {
    ...
    if (map.route) {
        ...
        const destination = map.markers.find((m) => m.id === map.route?.destinationId);
...
export const MapRouteCards = ({ cards }) => {
    if (cards.length === 0)
        return null;
    return (<div className="map-route-cards">
      {cards.map((card) => (<MapCard key={card.id} map={card.map}/>))}
    </div>);
};
