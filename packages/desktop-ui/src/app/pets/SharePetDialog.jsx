...
import { Dialog, DialogBody, DialogCloseButton, DialogContent, DialogDescription, DialogTitle, } from "@/ui/dialog";
...
export function SharePetDialog({ open, onOpenChange, pet, }) {
    ...
    const [shadowVisibility, setShadowVisibility] = useState(pet.visibility);
    ...
    const toggleFriend = useCallback((ownerId) => {
        setSelected((prev) => {
    ...
    const handleSend = useCallback(async () => {
        if (!link || selected.size === 0 || sending)
            return;
        setSending(true);
        try {
            let sent = 0;
    ...
    return (<Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent fit className="credential-modal-content share-addon-dialog">
        ...
        <DialogBody className="credential-modal-body">
          ...
          {link && isPrivate ? (<div className="share-addon-private-banner">
              ...
              <Button variant="primary" type="button" disabled={promoting} onClick={() => void handlePromote()}>
                {promoting ? "Updating…" : "Make unlisted"}
              </Button>
            </div>) : null}

          {link && !isPrivate ? (<div className="share-addon-link-row">
          ...
          {link && !isPrivate ? (<div className="share-addon-friends">
              <div className="share-addon-friends-header">
                Send to friends
                {selected.size > 0 ? (<span className="share-addon-friends-count">
                    {selected.size} selected
                  </span>) : null}
              </div>
              {friends.length === 0 ? (<div className="share-addon-empty">
                  No friends yet. Add a friend in Social to share directly.
                </div>) : (<div className="share-addon-friend-list">
                  {friends.map((friend) => {
                    const isSelected = selected.has(friend.profile.ownerId);
                    return (<button key={friend.profile.ownerId} type="button" className="share-addon-friend-row" data-selected={isSelected || undefined} onClick={() => toggleFriend(friend.profile.ownerId)}>
                        <Avatar src={friend.profile.avatarUrl} fallback={friend.profile.username} size="small"/>
              ...
              <div className="share-addon-actions">
                <Button variant="primary" type="button" disabled={selected.size === 0 || sending} onClick={() => void handleSend()}>
                  <Send size={14}/>
                  {sending
                ? "Sending…"
                : selected.size === 0
