import { isToolRequest, isToolResult, } from "@/features/chat/lib/event-transforms";
import { AGENT_IDS, isOrchestratorAgentType } from "@stella/contracts/agent-runtime";
...
const plural = (one, many) => (n) => n === 1 ? one : many(n);
...
const humanizeToolName = (toolName) => toolName
    .replace(/([a-z0-9])([A-Z])/g, "$1 $2")
    .replace(/[_-]+/g, " ")
    .trim()
    .toLowerCase();
const descriptorFor = (toolName) => {
...
const aggregateKey = (step) => DEV_CATEGORIES.has(step.category)
    ? `cat:${step.category}`
    : `tool:${step.toolName.toLowerCase()}`;
const str = (value) => typeof value === "string" && value.trim().length > 0 ? value.trim() : undefined;
const basename = (path) => path.split(/[\\/]/).pop() || path;
const clamp = (text, max) => text.length > max ? `${text.slice(0, max)}…` : text;
/** Per-call title shown in the expanded step list. */
const titleForCall = (toolName, args) => {
    const a = args ?? {};
 ...
 */
export function deriveToolActivity(events) {
    if (events.length === 0)
        return undefined;
    // Pair tool_request → tool_result by requestId, preserving request order.
    const steps = [];
    const indexByRequestId = new Map();
    for (const event of events) {
        ...
        if (isToolRequest(event)) {
            ...
            if (EXCLUDED_TOOLS.has(toolName.toLowerCase()))
                continue;
            ...
            steps.push({
                ...
                toolName,
                category,
                title: titleForCall(toolName, event.payload.args),
                status: "running",
            });
            continue;
        }
    ...
    const summary = joinPhrases(order.map((key) => {
 ...
 */
export function toolActivityEqual(a, b) {
    if (a === b)
        return true;
    if (!a || !b)
        return a === b;
    ...
    for (let i = 0; i < a.steps.length; i += 1) {
        if (a.steps[i].id !== b.steps[i].id)
            return false;
        if (a.steps[i].status !== b.steps[i].status)
            return false;
        if (a.steps[i].title !== b.steps[i].title)
            return false;
    }
    return true;
}
