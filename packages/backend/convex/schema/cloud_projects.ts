...
export const cloudProjectsSchema = {
  ...
  cloud_github_install_states: defineTable({
    stateId: v.string(),
    ownerId: v.string(),
    // Snapshotted when the browser handshake starts. Every later redirect leg
    // must validate it so a pre-reset state cannot bind into post-reset data.
  ...
  /**
   * Durable external-deletion debt. The installation/state locator rows and
   * this outbox row remain until GitHub acknowledges revocation (204/404), so
   * a transient API failure can never turn a live grant into unaddressable
   * external state.
   */
  cloud_github_installation_deletions: defineTable({
    ownerId: v.string(),
    installationId: v.string(),
    ...
    .index("by_installationId", ["installationId"])
    .index("by_ownerId_and_nextRetryAt", ["ownerId", "nextRetryAt"]),
};
