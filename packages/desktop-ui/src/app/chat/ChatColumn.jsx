...
export const ChatColumn = memo(function ChatColumn({ conversation, composer, scroll, composerEntering, conversationId, showHomeContent, onDismissHome, }) {
    ...
    const handleThumbMove = useCallback((e) => {
        ...
        const next = Math.max(0, Math.min(scrollRange, dragStartRef.current.scrollTop + scrollDelta));
        el.scrollTop = next;
    }, [getScrollNode, noteManualScroll]);
    ...
    const { showScrollButton, scrollToBottom, thumbRef, listRef, isFollowingLatest, isUserScrolling, } = scroll;
    ...
    const messages = useDeferredChatMessages(liveMessages, isUserScrolling, conversationId);
    ...
    useEffect(() => {
        const scheduleIdle = window.requestIdleCallback ??
            ((callback) => window.setTimeout(() => callback({ didTimeout: false, timeRemaining: () => 0 }), 200));
    ...
    const indicatorProps = useMemo(() => buildInlineWorkingIndicatorProps({
        isStreaming: Boolean(conversation.streaming.isStreaming),
        isStreamingResponseText: Boolean(conversation.streaming.isStreamingResponseText),
    ...
    const { setMessage: setComposerMessage, requestFocus: requestComposerFocus } = composer;
    const handleCancelQueued = useCallback((message) => {
        removeQueuedUserMessage(message.id);
        setComposerMessage((current) => restoreQueuedTextToComposer(current, message.text));
    ...
    // enter/leave fades.
    return (<div className="full-body-row">
      ...
      <div className={`full-body-chat-layer${showHomeContent ? " full-body-chat-layer--hidden" : ""}`} inert={showHomeContent || undefined}>
        <div className="full-body-main" {...dropHandlers}>
          ...
          <div className="chat-viewport-region">
            <ConversationEvents messages={messages} conversationId={conversationId} agentModelConfigByThread={agentModelConfigByThread} pendingUserMessageId={conversation.streaming.pendingUserMessageId} queuedUserMessages={conversation.streaming.queuedUserMessages} onCancelQueued={handleCancelQueued} indicator={indicatorProps} hasOlderMessages={conversation.history.hasOlderMessages} isLoadingOlder={conversation.history.isLoadingOlder} isLoadingHistory={conversation.history.isInitialLoading} listRef={listRef} className="session-content" contentContainerStyle={FULL_CHAT_CONTENT_STYLE} estimatedItemSize={140}/>

            {showScrollButton && !assistantReplyPeek.visible && (<button className="scroll-to-bottom" onClick={() => scrollToBottom("smooth")} aria-label="Scroll to bottom">
          ...
          <div className={composerEntering
        ...
        <div className="chat-scrollbar">
          <div ref={thumbRef} className="chat-scrollbar__thumb" onPointerDown={handleThumbDown} onPointerMove={handleThumbMove} onPointerUp={handleThumbUp} onPointerCancel={handleThumbUp}/>
        </div>
      </div>
