...
describe("manager orchestration production routing", () => {
  ...
  it("routes native persistent-empty exhaustion to the owning Manager only", async () => {
    ...
    try {
      ...
      // Retry backoffs are armed by Effect fibers, so a fixed advance
      // sequence can race the arming under load. Keep advancing fake time
      // until the exhaustion terminal lands.
