...
Status: revised story prepared after the user rejected the earlier model/history narrative. No current images are approved for upload. The old exports and native captures are preserved only in ignored `out/archive-rejected-story-20260906/`; they are not current candidates.
...
Root will direct final content after reviewing these examples. Voice verification owns Simulator priority. No native UI automation begins until that handoff. Use one isolated dev Pro owner and synthetic nonprivate documents, with one ongoing conversation. Do not expose diagnostic tokens, IDs, development overlays, private files, account details or credentials.

## Capture and export gates

Actual native screenshots go in `public/captures/{iphone,ipad,android}/{computer,browser,shopping,memory}.png`. New slugs intentionally prevent the rejected old captures from exporting under the revised copy. Missing sources block export.

Capture actual iPhone and iPad viewports independently; do not stretch phone pixels into a tablet. Android must use its own actual app binary and UI. Native status-bar overrides may show 9:41/full battery, as authorized by root. Preserve native pixels; no UI mockups, retouching or fabricated results.
