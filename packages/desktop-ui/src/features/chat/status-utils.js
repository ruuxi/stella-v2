...
const RAW_TOOL_STATUS_PATTERN = /^(?:running|executing|calling|invoking)\s+(.+)$/i;
/**
 ...
 */
const toToolStatusKey = (value) => value
    .trim()
 ...
 */
export function normalizeDisplayStatusText(statusText) {
    if (!statusText)
        return undefined;
...
const pickVariation = (options, seed) => {
    if (options.length === 0)
        return "";
    if (options.length === 1 || !seed)
        return options[0];
    return options[hashSeed(seed) % options.length];
};
export function computeStatus({ toolName, seed, isReasoning, } = {}) {
    if (toolName) {
        const normalizedToolName = toToolStatusKey(toolName);
        const mapped = TOOL_STATUS_BY_NAME[normalizedToolName];
        if (mapped)
            return pickVariation(mapped, seed ?? normalizedToolName);
    ...
    if (isReasoning)
        return pickVariation(REASONING_VARIATIONS, seed);
