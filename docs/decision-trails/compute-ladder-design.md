...
**D6. Durability is a closed union.** `TurnDurability = none(reason) | transcript_only(transcript) | workspace_checkpoint(transcript, checkpoint)`. A resident-only success commits `transcript_only`: seal the per-turn SQL journal (write-ahead recovery buffer, synchronous `message_end` appends, tail repair for interrupted tool-call groups), append the exact batch to Convex, verify the canonical cursor, deliver terminal, clear the journal. No turn-state operation, no archive. An attached success runs the existing archive-before-transcript order: drain/report produced files, quiesce the daemon (join shells), optional requested interior build in a trusted session, deterministic turn-state operation, archive upload from the quiesced session, transcript append + verify, owner-world publication, retirement, teardown. A resident turn that recorded an interior-build request attaches POST-LOOP with reason `interior_build` (the build is container work by constraint 3: trusted session restores the world, builds, uploads the candidate, and the `interior-source.json` update makes the turn finalize as attached with an archive). The deploy tool must never be silently useless from resident turns.
...
7. **Committed follow-up, not optional, and now the blocker for completing stage 6:** resident `code` and resident browser resume. Dynamic Worker execution with a browser session factory wired to the `BROWSER_GATEWAY` service binding from the DO, and browser suspension bound to the canonical transcript cursor (`transcript_only` receipts) instead of a checkpoint receipt. Reclassifies `code` from `container` to `js_sandbox`. Until this lands, every code call boots a container (stated trade-off, accepted 2026-08-29).
8. **Committed follow-up, not optional:** move per-owner exclusivity from Convex spawn admission to an attach-time world lease (single holder per owner, acquired inside `AgentComputeLadder`, model-visible wait-or-refuse for a second agent's container tool). Unblocks concurrent resident agents per owner. Depends on stage 5.
