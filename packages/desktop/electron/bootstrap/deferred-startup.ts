...
const isChronicleEnabled = (context) => context.state.chronicleController?.isEnabledCached() ?? false;
...
const armChronicleTick = (context, window, intervalMs) => {
    const runOnce = async () => {
        if (!isChronicleEnabled(context))
            return;
        const runner = context.lifecycle.getRunner();
        if (!runner)
            return;
        try {
            ...
            console.debug(`[chronicle] ${window} tick failed:`, error instanceof Error ? error.message : String(error));
        }
    };
    void runOnce();
