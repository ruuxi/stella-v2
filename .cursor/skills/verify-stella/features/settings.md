...
- `settings-tabs` selects General, Shortcuts, Account & Legal, and Audio.
...
- **Tab remaining.** Choose `Account & Legal`, then `Audio` the same way. Each selected tab's accessible name matches the tab, and General is no longer `aria-selected`.
...
- The tab set is `SETTINGS_TABS` in `packages/desktop-ui/src/global/settings/settings-tabs.ts`. Check it before reporting a missing tab as a regression.
...
- Account & Legal can mention sign-in. Signed-out copy is expected for an isolated profile. Do not treat a sign-in prompt as a harness failure.
- The gear is the standalone signed-out entry point. Once signed in, the same destinations fold into the account button's menu (`ShellTopBarAccount`), so a signed-in profile reaches Settings through a different trigger.
