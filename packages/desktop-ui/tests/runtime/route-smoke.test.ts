...
describe("route smoke", () => {
  ...
  it.each(["social", "store", "apps.index", "c.$handle"])(
  ...
  // The cloud pivot turned /apps into a pass-through layout and /apps/$slug
  // into a compatibility deep link: it resolves the cloud app, hands it to
  // the workspace-panel system, then restores Chat as the home route.
  it("apps is a pass-through layout, not a page", () => {
    const source = readFileSync(join(ROUTES_DIR, "apps.tsx"), "utf-8");
    expect(source).toMatch(/component:\s*Outlet/);
    expect(() => statSync(join(ROUTES_DIR, "apps.lazy.tsx"))).toThrow();
  });

  it("apps.$slug deep-links into the cloud app panel then restores Chat", () => {
    const source = readFileSync(join(ROUTES_DIR, "apps.$slug.tsx"), "utf-8");
    expect(source).toMatch(/openCloudAppPanel/);
    expect(source).toMatch(/navigate\(\{\s*to:\s*["']\/chat["'],\s*replace:\s*true\s*\}\)/);
    expect(() => statSync(join(ROUTES_DIR, "apps.$slug.lazy.tsx"))).toThrow();
  });
});
