...
function AgentGroupRow({ hierarchy, conversationId, expanded, onToggle }) {
  ...
  const descendants = useMemo(
    () => flattenActivityTasks(hierarchy.children),
    [hierarchy.children],
  );
  const summary = useMemo(
    () => summarizeCompactActivity(descendants),
    [descendants],
  );
  ...
  return (
    <>
      <li
        className="files-list__item files-list__item--group"
        data-expanded={expanded ? "true" : undefined}
      >
        <button
          type="button"
          className="files-list__open files-list__group-toggle"
          data-compact="true"
          onClick={onToggle}
        ...
        >
          ...
          <CompactChildState
            summary={summary}
            prioritizeFailure={owner.status === "running" && !expanded}
            startedAtMs={owner.startedAtMs}
            running={owner.status === "running"}
          />
        </button>
        <button
          type="button"
      ...
      {expanded
        ? descendants.map((child) => (
            <li
              key={`agent:${child.id}`}
              className="files-list__item files-list__item--nested"
            >
...
export function WorkList({ section = "files", idleContent = null }) {
  ...
  const renderedAccountScopeRef = useRef(accountScope);
  useLayoutEffect(() => {
    if (renderedAccountScopeRef.current === accountScope) return;
    renderedAccountScopeRef.current = accountScope;
  ...
  // Home search must reach the complete remote agent-thread history, not
  // only the first cloud page already projected into `tasks`. Each page
  // updates `hasOlder`/`isLoadingOlder`, so this drains one cursor at a time
  // while the query remains active. The local renderer feed uses the same
  // contract and therefore needs no separate search path.
  useEffect(() => {
    if (!query || !activity.hasOlder || activity.isLoadingOlder) return;
    activity.loadOlder();
  }, [activity.hasOlder, activity.isLoadingOlder, activity.loadOlder, query]);
  const searchingOlderActivity = Boolean(
  ...
  const itemGroups = useMemo(
    () => bucketByRecency(visibleItems, (item) => item.timestamp, bucketNowMs),
    [bucketNowMs, visibleItems],
  );
  useEffect(() => {
    setVisibleItemCount(WORK_PAGE_SIZE);
  ...
  return (
    <div
      ...
      data-search-open={(showSearch && searchOpen) || undefined}
      onDragEnter={(event) => {
        ...
        <div ref={scrollRef} className="sidebar-section__scroll">
          <ul className="files-list__items">
            {itemGroups.map((group) => (
              <li key={group.id} className="files-list__group">
                  ...
                  {group.items.map((item) =>
                    item.kind === "agentGroup" ? (
                      <AgentGroupRow
                        ...
                        expanded={query ? true : expandedGroupIds.has(item.id)}
                      ...
                      <li key={item.id} className="files-list__item">
                        <button
                          ...
                          className="files-list__open"
                          onClick={() =>
                            openDisplayPayloadTab(item.entry.payload)
                          }
