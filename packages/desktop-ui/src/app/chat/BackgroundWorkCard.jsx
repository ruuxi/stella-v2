...
import { agentPresentationFallback, deriveAgentCardPresentationStatus, deriveThreadAndOwnedPresentationStatus, } from "@/features/chat/lib/agent-activity-presentation";
...
function resolveDescriptions(threadIds, descriptions) {
    const out = [];
...
export function BackgroundWorkCard({ threadIds, completedThreadIds, pausedThreadIds, failedThreadIds, supersededThreadIds, spawnedAtMs, descriptions, statusTexts, followUpThreadIds, cardId, startEventIdsByThread, attemptGenerationsByThread, rootRunIdsByThread, terminalEventIdsByThread, conversationId, }) {
    ...
    const presentationStatus = useMemo(() => {
        ...
        const statuses = threadIds
            ...
            .map((threadId) => {
            const authoritative = deriveThreadAndOwnedPresentationStatus(threadActivity, threadId, {
    ...
    const followUpId = !multi && followUpThreadIds?.includes(threadIds[0])
        ? threadIds[0]
        : undefined;
    ...
    const assistantSummaryExcludedThreadIds = useMemo(() => (supersededThreadIds ?? []).filter((threadId) => attemptGenerationsByThread?.[threadId] === undefined), [attemptGenerationsByThread, supersededThreadIds]);
    const latestAssistantSummary = useMemo(() => selectLatestThreadAssistantSummary(threadActivity, {
        threadIds,
    ...
    const openThread = (threadId) => {
        const record = threadActivity.find((candidate) => candidate.threadId === threadId);
        openAgentThreadTab({
            threadId,
            conversationId,
    ...
    const terminalEventIds = threadIds
        ...
        .filter(Boolean);
    return (<div className="background-work-card" role="button" tabIndex={0} aria-label={`Open ${title} agent thread`} onClick={(event) => {
            ...
            openThread(primaryThreadId);
        }} data-state={failed ? "failed" : isFollowUp ? "follow-up" : "started"} data-lifecycle-status={presentationStatus} data-working={presentationStatus === "running" ? "true" : undefined} data-paused={showPaused ? "true" : undefined} data-activity-card-id={cardId} data-agent-ids={threadIds.join(",")} data-start-event-ids={startEventIds.join(",")} data-root-run-ids={rootRunIds.join(",")} data-terminal-event-ids={terminalEventIds.join(",")}>
      <span className="background-work-card__glyph" aria-hidden="true">
        <AgentLifecycleStatusIcon status={presentationStatus} size={16} strokeWidth={1.75}/>
      </span>
      <span className="background-work-card__text">
        <span className="background-work-card__title">
          {working ? (<TextShimmer text={title} durationMs={TITLE_SHIMMER_MS}/>) : (title)}
        </span>
        <span className="background-work-card__subtitle">{subtitle}</span>
      </span>
      <span className="background-work-card__actions">
        {threadIds.map((threadId) => (<button key={threadId} type="button" className="background-work-card__chat" onClick={(event) => {
                event.stopPropagation();
                openThread(threadId);
            }} aria-label="Open agent thread">
            <AgentModelIcon snapshot={threadActivity.find((record) => record.threadId === threadId)
                ?.modelConfigSnapshot}/>
          </button>))}
      </span>
    </div>);
