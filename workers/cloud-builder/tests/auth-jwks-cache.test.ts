...
const saved = (
  fetchedAt: number,
  keys: unknown[] = [{ ...publicKey, kid: "one" }],
) =>
  Response.json(
    { keys },
    { headers: { "x-stella-jwks-fetched-at": String(fetchedAt) } },
  );
...
describe("public signing keys across worker restart", () => {
  ...
  test("a cold isolate verifies the signature from a fresh shared public key without origin I/O", async () => {
    ...
    globalThis.fetch = (async () => {
      reads++;
      return Response.json({ keys: [{ ...publicKey, kid: "one" }] });
    }) as typeof fetch;
    const jwt = await token();
    expect((await verifyConvexToken(jwt, issuer)).ok).toBe(true);
    const age = entries
      .get(jwksUrlFor(issuer))
      ?.headers.get("x-stella-jwks-fetched-at");
    resetJwksCacheForTests();
    expect((await verifyConvexToken(jwt, issuer)).ok).toBe(true);
    expect(reads).toBe(1);
    expect(
      entries.get(jwksUrlFor(issuer))?.headers.get("x-stella-jwks-fetched-at"),
    ).toBe(age);
    ...
    expect(await verifyConvexToken(forged.join("."), issuer)).toMatchObject({
      ok: false,
  ...
  test("a missing key bypasses shared caching after the existing rotation cooldown", async () => {
    ...
    globalThis.fetch = (async () => {
      reads++;
      return Response.json({ keys: [{ ...publicKey, kid: "two" }] });
    }) as typeof fetch;
    expect((await verifyConvexToken(await token("two"), issuer)).ok).toBe(true);
    expect(reads).toBe(1);
    expect(
      await verifyConvexToken(await token("unknown"), issuer),
    ).toMatchObject({ ok: false, reason: "unknown_kid" });
    expect(reads).toBe(1);
  });
  ...
  test("expired, future and non-signing shared keys cannot authorize while origin is unavailable", async () => {
    ...
    for (const response of [
      saved(Date.now() - JWKS_TTL_MS),
      saved(Date.now() + 60_000),
      saved(Date.now(), [{ ...publicKey, kid: "one", use: "enc" }]),
      saved(Date.now(), [{ ...publicKey, kid: "one", alg: "RSA-OAEP" }]),
    ]) {
      resetJwksCacheForTests();
      entries.set(jwksUrlFor(issuer), response);
      expect(await verifyConvexToken(jwt, issuer)).toMatchObject({
        ok: false,
        retryable: true,
      });
    }
  ...
  test("edge cache failure falls back to origin and never persists private key fields", async () => {
    ...
    globalThis.fetch = (async () =>
      Response.json({
        keys: [{ ...publicKey, kid: "one", d: "must-not-be-cached" }],
      })) as typeof fetch;
    const jwt = await token();
    expect((await verifyConvexToken(jwt, issuer)).ok).toBe(true);
    resetJwksCacheForTests();
    installCache();
    expect((await verifyConvexToken(jwt, issuer)).ok).toBe(true);
    expect(await entries.get(jwksUrlFor(issuer))?.text()).not.toContain(
      "must-not-be-cached",
    );
  });
});
