...
export function registerCloudProjectRoutes(http: HttpRouter) {
  ...
  http.route({
    ...
    handler: httpAction(async (ctx, request) => {
      ...
      if (!body.ownerId) return json({ error: "ownerId required" }, 400);
      ...
      try {
        ...
        return json({ ...base, remoteUrl: project.remoteUrl, error: message }, 502);
      }
    }),
  });
    ...
    handler: httpAction(async (ctx, request) => {
      ...
      const result = await ctx.runMutation(
        internal.cloud_projects.recordProjectSetupInternal,
        {
          ...
          instanceSize:
            typeof body.instanceSize === "string" ? body.instanceSize : undefined,
          lastCheckpointAt:
