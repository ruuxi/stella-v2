...
import { createLifecycleRoutingState, routeLifecycleEvents, } from "@/features/chat/lib/route-lifecycle-events";
const SCHEDULED_EVENTS_OVERLAY_MAX = 200;
export const getMessageChronologicalTimestamp = (message) => {
    if (message.type === "assistant_message") {
        const payload = message.payload;
        const streamStartedAtMs = payload?.metadata?.runtime?.streamStartedAtMs;
        if (typeof streamStartedAtMs === "number" &&
...
const defaultSortTimestamp = getMessageChronologicalTimestamp;
/**
 ...
 */
export const createOwningUserClampedSortTimestampResolver = (messages, baseResolver = defaultSortTimestamp) => {
    const userTimestampById = new Map();
 ...
 */
export const createDisplayOrderComparator = (messages, getSortTimestamp = defaultSortTimestamp) => {
    const inputOrderById = new Map();
    for (let index = 0; index < messages.length; index += 1) {
        const id = messages[index]._id;
        if (!inputOrderById.has(id))
            inputOrderById.set(id, index);
    }
    return (a, b) => {
        ...
        if (ta !== tb)
            return ta - tb;
        const inputOrder = (inputOrderById.get(a._id) ?? Number.MAX_SAFE_INTEGER) -
...
const isUserTurnAssistant = (message) => {
    const payload = message.payload;
 ...
 */
export const keepAssistantTurnsContiguous = (messages) => {
    const userIndexById = new Map();
    const nextUserIndexAfter = new Array(messages.length);
    let nextUserIndex;
    ...
    for (let index = 0; index < messages.length; index += 1) {
        ...
        if (pending)
            ...
            insertBefore.set(boundaryIndex, [message]);
    }
    if (movedIds.size === 0)
        return messages;
    const result = [];
 ...
 */
const orderByResolver = (messages, getSortTimestamp) => {
    const compareDisplayOrder = createDisplayOrderComparator(messages, getSortTimestamp);
...
const mergeMessageSources = (getSortTimestamp, ...sources) => {
    const seen = new Map();
...
export const getPersistedAssistantSlots = (persistedMessages) => {
    const slots = new Map();
 ...
 */
export const overlayToMessageRecord = (overlay, persisted) => ({
    ...(persisted ?? {}),
    _id: overlay._id,
    ...
    payload: {
        ...(persisted?.payload ?? {}),
        text: overlay.locked && typeof persisted?.payload?.text === "string"
        ...
        metadata: {
            ...
            runtime: {
                ...(persisted?.payload?.metadata?.runtime ?? {}),
...
export const mergeConversationDisplayMessageSources = (args) => {
    const { persistedMessages, overlayMessages, streamingAssistants, persistedAssistantSlots, getSortTimestamp = defaultSortTimestamp, } = args;
    const effectiveSortTimestamp = createOwningUserClampedSortTimestampResolver(overlayMessages.length > 0
        ? [...persistedMessages, ...overlayMessages]
        : persistedMessages, getSortTimestamp);
    const resolverActive = effectiveSortTimestamp !== defaultSortTimestamp;
    const persistedById = new Map(persistedMessages.map((message) => [message._id, message]));
    if (streamingAssistants.length === 0) {
        ...
        return mergeMessageSources(effectiveSortTimestamp, persistedMessages, overlayMessages);
    }
    const maskedPersistedIds = new Set();
    ...
    const persistedMessagesForDisplay = maskedPersistedIds.size === 0
        ? persistedMessages
        : persistedMessages.filter((message) => !maskedPersistedIds.has(message._id));
    if (overlayMessages.length === 0) {
        return resolverActive
            ? orderByResolver(persistedMessagesForDisplay, effectiveSortTimestamp)
            : persistedMessagesForDisplay;
    }
    return mergeMessageSources(effectiveSortTimestamp, persistedMessagesForDisplay, overlayMessages);
};
/* -------------------------------------------------------------------------
 ...
 */
export const findOverlayWinnerIndices = (result, overlayMessages) => {
    if (overlayMessages.length === 0)
        return [];
    const overlaySet = new Set(overlayMessages);
    const indices = [];
    for (let i = 0; i < result.length; i += 1) {
        if (overlaySet.has(result[i]))
            indices.push(i);
    }
    return indices;
};
 ...
 */
export const overlayMergeShapeUnchanged = (prev, next) => {
    if (prev === next)
        return true;
 ...
 */
export const rebuildDisplayMessagesFromCachedOrder = (cachedResult, overlayWinnerIndices, currentOverlayMessages) => {
    ...
    if (overlayWinnerIndices.length === 0)
        return cachedResult;
    const currentOverlayById = new Map();
    for (const m of currentOverlayMessages)
        currentOverlayById.set(m._id, m);
...
export const useConversationDisplayMessages = ({ conversationId, persistedMessages, optimisticEvents, streamingAssistants, }) => {
    const scheduledEvents = useScheduledEvents({
        conversationId: conversationId ?? undefined,
        enabled: Boolean(conversationId),
        maxItems: SCHEDULED_EVENTS_OVERLAY_MAX,
    });
    const persistedAssistantSlots = useMemo(() => getPersistedAssistantSlots(persistedMessages), [persistedMessages]);
    const persistedMessagesById = useMemo(() => new Map(persistedMessages.map((message) => [message._id, message])), [persistedMessages]);
    /**
     ...
     */
    const frozenSlotSortTsRef = useRef(new Map());
    const frozenConversationIdRef = useRef(conversationId);
    ...
    const getSortTimestamp = useMemo(() => {
        if (sortTimestampByMessageId.size === 0)
            return undefined;
        return (message) => sortTimestampByMessageId.get(message._id) ??
    ...
    const overlayMessages = useMemo(() => {
        ...
        for (const event of scheduledEvents) {
            ...
            if (overlayEvents.some((other) => other._id === event._id))
                continue;
            overlayEvents.push(event);
        }
        const fromEvents = overlayEvents.length > 0 ? groupEventsIntoMessages(overlayEvents) : [];
    ...
    const merged = useMemo(() => {
        ...
        if (cache &&
            ...
            const reused = rebuildDisplayMessagesFromCachedOrder(cache.result, cache.overlayWinnerIndices, overlayMessages);
            if (reused) {
     ...
     */
    const lifecycleRoutingRef = useRef(createLifecycleRoutingState());
    const routingConversationIdRef = useRef(conversationId);
    ...
    return useMemo(() => routeLifecycleEvents(merged, lifecycleRoutingRef.current), [merged]);
};
