 ...
 */
function useResolvedRows(affectedRefs, open) {
            ...
            try {
                ...
                for (const ref of affectedRefs) {
                    if (ref.kind === "cron") {
                        ...
                        if (found)
                            next.push(cronToRow(found));
                        continue;
                    }
                    const found = heartbeatById.get(ref.id);
                    if (found)
                        next.push(heartbeatToRow(found));
                }
                setRows(next);
            }
            catch {
...
const initialActionState = { busy: "none" };
function ScheduleDialogRow({ row, nowMs, onMutation, }) {
    const [state, setState] = useState(initialActionState);
    const api = window.electronAPI?.schedule;
    const callMutation = async (busy, fn) => {
        if (!api)
            return;
        setState({ busy });
        try {
    ...
    const onRunNow = () => {
        if (!api)
            return;
        void callMutation("run", () => row.kind === "cron"
    ...
    const onTogglePause = () => {
        ...
        const busy = next ? "resume" : "pause";
        void callMutation(busy, () => row.kind === "cron"
    ...
    const onDelete = () => {
        ...
        if (row.kind === "cron") {
            ...
            void callMutation("delete", () => api.upsertHeartbeat({
                conversationId: row.conversationId,
                enabled: false,
            }));
        }
    ...
    const badge = row.enabled
        ...
        : "Paused";
    return (<li className="schedule-details-dialog__row" data-enabled={row.enabled}>
      ...
      <div className="schedule-details-dialog__row-actions">
        <Button type="button" className="pill-btn" disabled={state.busy !== "none" || !row.enabled} onClick={onRunNow}>
          {state.busy === "run" ? "Running…" : "Run now"}
        </Button>
        <Button type="button" className="pill-btn" disabled={state.busy !== "none"} onClick={onTogglePause}>
          {state.busy === "pause"
            ? "Pausing…"
            : state.busy === "resume"
                ? "Resuming…"
        ...
        <Button type="button" className="pill-btn pill-btn--danger" disabled={state.busy !== "none"} onClick={onDelete}>
          {state.busy === "delete" ? "Removing…" : "Delete"}
        </Button>
      </div>
      {state.error && (<p className="schedule-details-dialog__row-error" role="alert">
          {state.error}
        </p>)}
    </li>);
}
export function ScheduleDetailsDialog({ open, onOpenChange, affected, summary, }) {
    ...
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <Dialog.Content size="md" fit className="schedule-details-dialog">
        ...
        {summary && (<Dialog.Description>{summary}</Dialog.Description>)}
        <Dialog.Body>
          {rows.length === 0 ? (<p className="schedule-details-dialog__empty">
              This schedule no longer exists.
            </p>) : (<ul className="schedule-details-dialog__rows">
              {rows.map((row) => (<ScheduleDialogRow key={`${row.kind}:${row.id}`} row={row} nowMs={nowMs} onMutation={bumpRefresh}/>))}
            </ul>)}
        </Dialog.Body>
      </Dialog.Content>
    </Dialog>);
