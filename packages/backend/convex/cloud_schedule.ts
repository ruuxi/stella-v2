...
const scheduleMutationReceiptValidator = v.object({
  replayed: v.boolean(),
  resultJson: v.string(),
});

const SCHEDULE_REQUEST_ID_PATTERN = /^[A-Za-z0-9._:-]{8,128}$/u;

const normalizeScheduleRequestId = (value: string): string => {
  const requestId = value.trim();
  if (!SCHEDULE_REQUEST_ID_PATTERN.test(requestId)) {
    throw new ConvexError("Schedule request id is invalid.");
  }
  return requestId;
};

const replayScheduleReceipt = async (
...
const recordScheduleReceipt = async (
  ctx: Pick<MutationCtx, "db">,
  args: {
    ownerId: string;
    ownerGeneration: string;
  ...
  await ctx.db.insert("cloud_schedule_receipts", {
...
export const createScheduleInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const description = cleanDescription(args.description, prompt);
    ...
    if (replay !== null) return { replayed: true, resultJson: replay };
    ...
    const purged = await ctx.db
      ...
      .withIndex("by_ownerId_and_status_and_updatedAt", (q) =>
        q.eq("ownerId", args.ownerId).eq("status", SCHEDULE_STATUS_PURGED),
      )
      .first();
    if (purged) {
      throw new ConvexError(
    ...
    // Count the statuses that can fire or be resumed through exact indexed
    // queries. A long history of newer one-shot `done` rows must not crowd an
    // older live row out of a bounded updatedAt window.
    ...
    if (active.length + paused.length >= MAX_SCHEDULES_PER_OWNER) {
      throw new ConvexError(
    ...
    const resultJson = JSON.stringify({
      ok: true,
      schedule: toScheduleRow(row),
    });
    await recordScheduleReceipt(ctx, {
      ownerId: args.ownerId,
      ownerGeneration: args.ownerGeneration,
      requestId,
      action: "create",
...
export const updateScheduleInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const requestedPrompt =
      args.prompt === undefined ? undefined : cleanPrompt(args.prompt);
    const requestedSchedule =
      args.schedule === undefined
        ? undefined
        : normalizeScheduleInput(args.schedule);
    ...
    const intentJson = JSON.stringify({
      ...
      prompt: requestedPrompt ?? null,
      schedule: requestedSchedule ?? null,
      description:
        args.description === undefined ? null : args.description.trim(),
    ...
    if (requestedSchedule !== undefined) {
      assertScheduleAllowed(requestedSchedule, args.now);
      serialized = JSON.stringify(requestedSchedule);
      nextRunAt = computeNextRunAt(requestedSchedule, args.now);
    }
    ...
    const resultJson = JSON.stringify({
      ok: true,
      schedule: toScheduleRow({ ...row, ...patch }),
    });
    await recordScheduleReceipt(ctx, {
      ownerId: args.ownerId,
      ownerGeneration: args.ownerGeneration,
...
export const removeScheduleInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      ...
      const resultJson = JSON.stringify({ ok: false, removed: false });
      await recordScheduleReceipt(ctx, {
        ownerId: args.ownerId,
        ownerGeneration: args.ownerGeneration,
        requestId,
    ...
    const resultJson = JSON.stringify({ ok: true, removed: true });
    await recordScheduleReceipt(ctx, {
      ownerId: args.ownerId,
      ownerGeneration: args.ownerGeneration,
      requestId,
...
export const finishScheduleFireInternal = internalMutation({
  ...
  returns: v.null(),
  handler: async (ctx, args) => {
    // Every completion, including parse-error repair, is fenced to the owner
    // generation captured by the dispatcher before it claimed the fire.
    await assertOwnerDataWriteAllowed(ctx, args.ownerId, args.ownerGeneration);
    const row = await ctx.db
      .query("cloud_scheduled_turns")
      .withIndex("by_scheduleId", (q) => q.eq("scheduleId", args.scheduleId))
    ...
    if (
      (row.activeFireId !== undefined || args.fireId !== undefined) &&
      row.activeFireId !== args.fireId
    ) {
      return null;
    }
...
export const dispatchDueSchedulesInternal = internalAction({
  ...
  handler: async (ctx) => {
    ...
    for (const row of due) {
      ...
      try {
        ownerGeneration = (await assertOwnerDataAccessActive(ctx, row.ownerId))
          .generation;
      } catch {
        continue;
      }
