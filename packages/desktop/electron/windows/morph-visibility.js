...
import { getWindowInfoAtPoint, getWindowInfoBatchAtPoints, } from '../window-capture.js';
const DEFAULT_VISIBLE_RATIO_THRESHOLD = 0.5;
const SAMPLE_COLUMNS = 4;
const SAMPLE_ROWS = 3;
const area = (bounds) => Math.max(0, bounds.width) * Math.max(0, bounds.height);
...
const getDisplayScaleFactor = (display) => process.platform === 'darwin' ? 1 : (display.scaleFactor ?? 1);
const toNativePoint = (point) => {
...
export const isLikelySameWindowBounds = (targetBounds, candidateBounds) => {
    const targetArea = area(targetBounds);
    const candidateArea = area(candidateBounds);
    if (targetArea <= 0 || candidateArea <= 0)
        return false;
...
export const createMorphVisibilitySamplePoints = (bounds) => {
...
export async function shouldShowMorphForWindow(targetWindow, options) {
    ...
    if (options?.queryWindowInfo) {
        infos = await Promise.all(nativePoints.map((point) => options.queryWindowInfo(point.x, point.y)));
    }
    else {
        ...
        if (batchInfos) {
            ...
            infos = await Promise.all(nativePoints.map((point) => getWindowInfoAtPoint(point.x, point.y)));
        }
    }
    const results = infos.map((info) => Boolean(info &&
        info.pid === currentPid &&
    ...
    const threshold = options?.visibleRatioThreshold ?? DEFAULT_VISIBLE_RATIO_THRESHOLD;
