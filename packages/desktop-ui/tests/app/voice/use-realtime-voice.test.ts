...
describe("useRealtimeVoice transcript persistence", () => {
  ...
  it("writes a visible assistant voice-session summary when voice deactivates", () => {
    ...
    // Cloud-durable transcripts now carry a stable eventId and timestamp so
    // the turn can be bound to the canonical cloud conversation record.
    expect(persistTranscript).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        eventId: expect.stringMatching(/^[0-9a-f-]{36}:\d+$/),
        timestamp: 85_000,
      }),
    );
  });
