 ...
 */
export function SocialInviteCard({ invite }) {
    ...
    // is reading.
    const communityPreview = usePersistentConvexOneShot(api.social.communities.getCommunityPreviewByInviteCode, isJoin ? { inviteCode: invite.inviteCode } : "skip", {
        scope: "public",
        ttlMs: INVITE_CARD_CACHE_TTL_MS,
    });
    const friendPreview = usePersistentConvexOneShot(api.social.profiles.getProfileByUsername, !isJoin ? { username: invite.username } : "skip", {
        scope: "public",
        ttlMs: INVITE_CARD_CACHE_TTL_MS,
    });
    ...
    const detail = isJoin
        ? communityPreview
            ? `${communityPreview.memberCount}${communityPreview.memberCountTruncated ? "+" : ""} ${communityPreview.memberCount === 1 ? "member" : "members"} · tap to join`
            : ""
        : "Tap to send a friend request";
    if (loading) {
        return (<div className="social-invite-card" data-loading>
    ...
    if (missing) {
        return (<div className="social-invite-card" data-missing>
    ...
    return (<button type="button" className="social-invite-card" onClick={() => setPendingSocialInvite(invite)}>
      {!isJoin && friendPreview ? (<Avatar fallback={friendPreview.username} src={friendPreview.avatarUrl} size="normal"/>) : (<div className="social-invite-card-art social-invite-card-art--icon">
          <Users size={18}/>
        </div>)}
      <div className="social-invite-card-body">
        <div className="social-invite-card-eyebrow">{eyebrow}</div>
        <div className="social-invite-card-name">{name}</div>
        {detail ? (<div className="social-invite-card-detail">{detail}</div>) : null}
      </div>
    </button>);
}
