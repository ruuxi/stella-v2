...
describe("browser auth handoff landing", () => {
  ...
  test("script clears the fragment before verification and stores the bearer", () => {
    ...
    expect(script).toContain('"set-auth-token"');
    expect(script).toContain("localStorage.setItem(SESSION_TOKEN_KEY");
  ...
  test("clears the fragment before POST and redirects only after storing the bearer", async () => {
    ...
    expect(values.get("better-auth_session_token")).toBe(
      "signed-session.signature",
    );
  ...
  test("requires the verification response itself to issue a session token", async () => {
    ...
    const fetch = async () => new Response("{}", { headers: {} });
    const run = new Function(
      "document",
      "location",
      "history",
    ...
    expect(values.get("better-auth_session_token")).toBe("old-session.signature");
    expect(
