...
const hmacKey = async (
  ...
  if (bytes.byteLength < 32 || bytes.byteLength > 4_096)
    throw new Error("World capability secret is invalid.");
...
const signingInput = (payload: string): Uint8Array =>
...
export const verifyWorldCapability = async (args: {
  ...
  if (!payloadBytes || !signatureBytes || signatureBytes.byteLength !== 32)
    return { ok: false };
  ...
  try {
    ...
    if (
      Object.keys(value).sort().join(",") !== "e,g,n,t,v,w" ||
      value.v !== 1 ||
      value.w !== args.worldName ||
      typeof value.t !== "string" ||
      !value.t ||
      !Number.isSafeInteger(value.g) ||
      Number(value.g) < 1 ||
      !Number.isSafeInteger(value.e) ||
      Number(value.e) <= args.now ||
      typeof value.n !== "string" ||
      !/^[A-Za-z0-9_-]{22}$/u.test(value.n)
    )
      return { ok: false };
    const claims: WorldCapabilityClaims = {
      v: 1,
    ...
    if (base64Url(new TextEncoder().encode(JSON.stringify(claims))) !== payload)
      return { ok: false };
...
export const worldCapabilityFromRequest = (request: Request): string => {
  const match = /^Bearer ([A-Za-z0-9._-]+)$/u.exec(
