...
describe("WorldStore in real Workerd", () => {
  ...
  beforeAll(async () => {
    ...
    child = spawn(process.execPath, ["x", "wrangler", "dev", "--config", "tests/fixtures/world-store-workerd.wrangler.jsonc", "--ip", "127.0.0.1", "--port", String(port), "--local", "--persist-to", persistence, "--inspector-port", String(await allocateWorkerdInspectorPort()), "--show-interactive-dev-session=false"], { cwd: packageRoot, stdio: ["ignore", "pipe", "pipe"] });
  ...
  afterAll(async () => {
    ...
    if (persistence.includes("stella-world-workerd-")) await rm(persistence, { recursive: true, force: true });
  }, 30_000);

  test("runs tools, manifests, diff/push, and tar export", async () => {
    expect(await (await fetch(`${origin}/health`)).text()).toBe("ok");
    expect(await (await fetch(`${origin}/entries`)).json()).toEqual({ entries: [] });
