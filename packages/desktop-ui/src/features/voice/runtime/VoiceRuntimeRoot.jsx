...
import { acquireSharedMicrophone, } from "@/features/voice/services/shared-microphone";
...
const computeEnergy = (analyser) => {
    const result = computeAnalyserEnergy(analyser, energyBuffer);
    energyBuffer = result.buffer;
    return result.energy;
};
const runtimeStateEquals = (a, b) => a.sessionState === b.sessionState &&
...
export function VoiceRuntimeRoot() {
    ...
    const [bootConversationId, setBootConversationId] = useState(state.conversationId);
    /**
    ...
    const conversationIdRef = useRef(state.conversationId ?? "voice-rtc");
    const inputActiveRef = useRef(state.isVoiceRtcActive);
    const managerRef = useRef(null);
    const warmMicLeaseRef = useRef(null);
    const publishedStateRef = useRef(DEFAULT_RUNTIME_STATE);
    ...
    const lastVoiceErrorToastRef = useRef(null);
    const resolvedConversationId = state.conversationId ?? bootConversationId;
    const publishRuntimeState = (patch) => {
        const next = {
    ...
    useEffect(() => {
        ...
        const getDefaultConversationId = window.electronAPI?.localChat.getOrCreateDefaultConversationId;
    ...
    useEffect(() => {
        ...
        void acquireSharedMicrophone()
            ...
            .catch((error) => {
            console.debug("[voice-runtime] Failed to warm microphone for wake word:", error.message);
        });
        return () => {
            cancelled = true;
        };
        ...
        const manager = new VoiceSessionManager({
            ...
            onStateChange: (sessionState, errorMessage) => {
                ...
                publishRuntimeState({
                    ...
                    micLevel: sessionState === "connected"
                        ? publishedStateRef.current.micLevel
                        : 0,
                    outputLevel: sessionState === "connected"
    ...
    useEffect(() => {
        ...
        conversationIdRef.current = resolvedConversationId;
        managerRef.current?.updateSession(resolvedConversationId, state.isVoiceRtcActive);
