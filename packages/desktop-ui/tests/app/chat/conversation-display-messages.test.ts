...
describe("conversation display message merge", () => {
  ...
  it("shows a whole assistant message the instant its event lands, before SQLite", () => {
    ...
    // Nothing is mid-flight anymore, so the row must not claim to be.
    expect(
    ...
    const liveMessage = overlayToMessageRecord(
      ...
      message({
        payload: {
          userMessageId: "u1",
          text: "stored text",
          metadata: { runtime: { streamStartedAtMs: 42 } },
        },
      }),
    );
...
describe("conversation display merge — ordering and dedup", () => {
