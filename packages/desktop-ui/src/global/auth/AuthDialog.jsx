...
const getConvexSiteUrl = () => {
    const url = readConfiguredConvexSiteUrl(import.meta.env.VITE_CONVEX_SITE_URL);
    if (!url) {
...
const pollDesktopSocialAuth = async (convexSiteUrl, requestId) => {
    const deadline = Date.now() + SOCIAL_AUTH_TIMEOUT_MS;
    while (Date.now() < deadline) {
        await wait(SOCIAL_AUTH_POLL_INTERVAL_MS);
        const response = await fetch(`${convexSiteUrl}/api/auth/link/status?requestId=${encodeURIComponent(requestId)}`);
        if (!response.ok) {
            continue;
        }
        const data = (await response.json().catch(() => null));
        if (data?.status === "completed" && data.sessionCookie) {
            await window.electronAPI?.system.applyAuthSessionCookie?.(data.sessionCookie);
    ...
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent fit className="auth-dialog-content">
        ...
        <DialogBody className="auth-dialog-body">
          ...
          <MagicLinkAuthFlow className="auth-dialog-flow" hideEmailLabel inputVariant="normal" emailPlaceholder="you@example.com" autoFocus formClassName="auth-dialog-form" inputClassName="auth-dialog-input" buttonClassName="pill-btn pill-btn--primary pill-btn--lg auth-dialog-cta" buttonVariant="primary" buttonSize="large" submitLabel="Continue" sendingLabel="Sending..." resendLabel="Resend email" extrasClassName="auth-dialog-extras" extrasInnerClassName="auth-dialog-extras-inner" sentClassName="auth-dialog-sent" sentMessage="We sent a sign-in link. Open it on this device to finish." openInboxClassName="pill-btn pill-btn--primary pill-btn--lg auth-dialog-open-inbox" errorClassName="auth-dialog-error"/>
        </DialogBody>
      </DialogContent>
    </Dialog>);
};
function GoogleAuthButton() {
    ...
    const handleSignIn = useCallback(async () => {
        ...
        setIsSigningIn(true);
        try {
            const { convexSiteUrl, requestId, callbackURL } = await startDesktopSocialAuth();
    ...
    return (<div className="auth-dialog-google-wrap">
      <button type="button" className="auth-dialog-google" onClick={handleSignIn} disabled={isSigningIn}>
        <GoogleIcon />
...
function GoogleIcon() {
    return (<svg className="auth-dialog-google-icon" viewBox="0 0 18 18" aria-hidden="true" focusable="false">
      <path fill="#4285F4" d="M17.64 9.2c0-.64-.06-1.25-.16-1.84H9v3.48h4.84a4.14 4.14 0 0 1-1.8 2.72v2.26h2.91c1.7-1.57 2.69-3.89 2.69-6.62Z"/>
      <path fill="#34A853" d="M9 18c2.43 0 4.47-.8 5.95-2.18l-2.91-2.26c-.8.54-1.84.86-3.04.86-2.34 0-4.32-1.58-5.03-3.71H.96v2.33A9 9 0 0 0 9 18Z"/>
      ...
      <path fill="#EA4335" d="M9 3.58c1.32 0 2.51.45 3.44 1.35l2.58-2.58C13.46.89 11.42 0 9 0A9 9 0 0 0 .96 4.96l3.01 2.33C4.68 5.16 6.66 3.58 9 3.58Z"/>
    </svg>);
}
