...
import { CLAUDE_CODE_MODEL_ALIASES, formatClaudeCodeResolvedModel, readClaudeCodeResolvedModels, recordClaudeCodeResolvedModel, } from "./claude-code-resolved-models.js";
 ...
 */
export const isClaudeCodeModelRefusalOrOverloadError = (message) => /unable to respond to this request|usage policy|overloaded/i.test(message);
/**
...
const CLAUDE_CODE_COMPACTION_LOOP_MESSAGE = "Claude Code entered a compaction loop.";
/**
...
const asRecoverableStepError = (error) => error instanceof ClaudeCodeProcessEndedError ||
 ...
 */
const buildSideEffectReconciliationPrompt = (mutations, mcpCalls = [], referenceContext) => {
...
const buildResultRetryPrompt = () => "Your previous reply produced no result text. Provide your complete final answer to the pending request now.";
const withStepRecoveryExhausted = (error) => new Error(`${normalizeErrorMessage(error)} Stella retried ${MAX_STEP_RECOVERIES_PER_TURN} time(s) but Claude Code kept ending the step without a usable result. Check the \`claude\` CLI health (\`claude --version\`, login status), then retry the request.`);
...
export const getClaudeCodeTruncatedToolUseFromStreamEvent = (event) => {
    ...
    if (!TRUNCATING_STOP_REASONS.has(stopReason))
        return null;
...
export const describeClaudeToolUseTruncation = (truncation) => `Claude's stream ended with stop_reason "${truncation.stopReason}"` +
...
const stableToolArgs = (value) => {
    if (Array.isArray(value))
        return `[${value.map(stableToolArgs).join(",")}]`;
...
export const createClaudeNativeToolUseCorrelator = () => {
    ...
    const observe = (args) => {
        if (observedIds.has(args.toolCallId))
            return;
        ...
        if (!values.includes(args.toolCallId))
            values.push(args.toolCallId);
    ...
    return {
         ...
         */
        async resolveToolUseIntegrity(toolName, toolArgs, signal, timeoutMs = TOOL_USE_INTEGRITY_SETTLE_MS) {
        ...
        observeStreamEvent(event) {
            ...
            if (!source || index === undefined || !Number.isInteger(index))
                return;
            ...
            if (source.type !== "content_block_stop")
                return;
        ...
        async claim(toolName, toolArgs, signal) {
            ...
            return await new Promise((resolve, reject) => {
                ...
                const timer = setTimeout(() => {
                    ...
                    if (index >= 0)
                        entries.splice(index, 1);
                    reject(new Error("Timed out waiting for Claude's durable tool_use identity."));
                }, 5_000);
                timer.unref?.();
                const onObserved = (id) => {
                    clearTimeout(timer);
...
const isSessionAlreadyInUseError = (message) => /Session ID .* is already in use\./i.test(message);
const isMissingResumeSessionError = (message) => /No conversation found with session ID:/i.test(message);
 ...
 */
const processIsDead = (child) => child.exitCode !== null || child.signalCode !== null;
const killProcess = (child) => {
    if (processIsDead(child))
        return;
    try {
        child.kill("SIGTERM");
    }
...
const parseClaudeCodeModel = (modelId) => {
    ...
    if (!normalized.startsWith(CLAUDE_CODE_MODEL_PREFIX))
        return undefined;
...
const ensureArtifactDir = (session) => {
    if (!session.artifactDir) {
        session.artifactDir = path.join(os.tmpdir(), "stella-claude-code", session.sessionId);
    }
...
const materializeAttachments = (session, attachments) => {
    ...
    for (const [index, attachment] of attachments.entries()) {
        ...
        const filePath = path.join(artifactDir, `attachment-${index + 1}-${crypto.randomUUID()}${mimeExtension(parsed.mimeType)}`);
...
const observeFinalizedClaudeToolUses = (event, observe) => {
    if (event.type !== "assistant" || !observe)
        return;
    const content = asObject(event.message)?.content;
    if (!Array.isArray(content))
        return;
...
const fileChangeDedupeKey = (record) => `${record.kind.type}:${record.path}:${record.kind.type === "update" ? (record.kind.move_path ?? "") : ""}`;
...
const mergeMcpCalls = (target, records) => {
    for (const record of records ?? []) {
        const existing = target.find((entry) => entry.toolCallId === record.toolCallId);
        if (existing) {
...
export const getClaudeCodeStatusChangeFromStreamEvent = (event) => {
    ...
    const hookEvent = typeof event.hook_event === "string"
        ? event.hook_event
 ...
 * generic labels.
 */
const CLAUDE_CODE_MODEL_FALLBACK_RE = /^Model fallback triggered(?::? switching from (\S+) to (\S+))?/;
/**
...
export const getClaudeCodeModelFallbackFromStreamEvent = (event) => {
    ...
    const toModel = match[2]
        ...
        : "a fallback model";
    const text = `Claude Code switched this session from ${fromModel} to ${toModel} ` +
...
const cleanupSessionProcess = (session) => {
    ...
    resetSessionMcpClients(session, new Error("Claude Code session process was closed."));
...
class ClaudeCodeSessionRuntime {
    ...
    async runTurn(request) {
        ...
        const session = ensureSessionState(this.sessions, request, request.sessionKey, request.cwd);
    ...
    scheduleSessionCloseWhenIdle(sessionKey, timeoutMs) {
        this.clearIdleCloseTimer(sessionKey);
        const timer = setTimeout(() => this.closeSessionWhenIdle(sessionKey), Math.max(1_000, timeoutMs));
        timer.unref?.();
    ...
    async executeTurn(session, request) {
        ...
        if (!request.vanilla) {
            ...
            session.activeMcpTurn = {
                ...
                claimNativeToolUseId: (toolName, toolArgs, signal) => nativeToolUseCorrelator.claim(toolName, toolArgs, signal),
                checkToolUseIntegrity: (toolName, toolArgs, signal) => nativeToolUseCorrelator.resolveToolUseIntegrity(toolName, toolArgs, signal),
                executeTool: async (toolCallId, toolName, toolArgs, toolSignal, onUpdate) => {
                    ...
                    const signal = request.abortSignal && toolSignal
                    ...
                    const toolResult = await executeToolWithInactivityBound({
                        toolName,
                        signal,
                        run: (boundedSignal, onActivity) => request.executeTool(toolCallId, toolName, toolArgs, boundedSignal, (update) => {
                            onActivity();
                            onUpdate?.(update);
                    ...
                    mergeFileChanges(turnFileChanges, turnFileChangeKeys, toolResult.fileChanges);
                    if (pending) {
                        const pendingKeys = new Set(pending.fileChanges.map(fileChangeDedupeKey));
                        mergeFileChanges(pending.fileChanges, pendingKeys, toolResult.fileChanges);
                    }
                    return toolResult;
                },
        ...
        try {
            const response = await this.executeStepWithRecovery(session, request, effectiveSystemPrompt, prompt, [], { remaining: MAX_STEP_RECOVERIES_PER_TURN });
     ...
     */
    async executeStepWithRecovery(session, request, effectiveSystemPrompt, prompt, promptImages, recoveryBudget) {
        ...
        for (;;) {
            try {
                const result = await this.executeStep(session, request, effectiveSystemPrompt, currentPrompt, currentPromptImages, failedAttemptFileChanges, failedAttemptMcpCalls);
                if (failedAttemptFileChanges.length === 0) {
                    return result;
                }
                mergeFileChanges(failedAttemptFileChanges, failedAttemptFileChangeKeys, result.fileChanges);
                ...
                const hasPossibleSideEffects = Boolean(recoverable &&
                ...
                mergeFileChanges(failedAttemptFileChanges, failedAttemptFileChangeKeys, recoverable.fileChanges);
                ...
                if (recoverable instanceof ClaudeCodeProcessEndedError) {
                    ...
                    if (failedAttemptFileChanges.length > 0 ||
                        ...
                        currentPrompt = buildSideEffectReconciliationPrompt(failedAttemptFileChanges, failedAttemptMcpCalls);
                        currentPromptImages = [];
                    }
                    continue;
                }
                currentPrompt = hasPossibleSideEffects
                    ? buildSideEffectReconciliationPrompt(failedAttemptFileChanges, failedAttemptMcpCalls)
    ...
    applyFableFallbackPolicy(session, request, error) {
        ...
        if (!modelName || !/\bfable\b/.test(modelName))
            return false;
        const message = error instanceof Error ? error.message : "";
        if (!isClaudeCodeModelRefusalOrOverloadError(message))
            return false;
        ...
        if (session.fableSafetyFailures < SAFETY_ABORT_FABLE_ATTEMPTS) {
            request.onStatusChange?.({
                state: "running",
                text: `${prettyFrom} refused this request — retrying ` +
        ...
        if (!session.modelFallbackNotified) {
            ...
            request.onStatusChange?.({
                ...
                text: `${prettyFrom} failed ${SAFETY_ABORT_FABLE_ATTEMPTS} attempts ` +
    ...
    async executeStep(session, request, effectiveSystemPrompt, prompt, promptImages, observedMutations = [], observedMcpCalls = []) {
        return await this.executeStepWithMode(session, request, effectiveSystemPrompt, prompt, session.resumeReady, true, promptImages, observedMutations, observedMcpCalls);
    }
    /**
     ...
     */
    async executeStepWithMode(session, request, effectiveSystemPrompt, prompt, useResume, allowCompactionLoopRestart = true, promptImages = [], observedMutations = [], observedMcpCalls = []) {
        ...
        const buildReseedPrompt = (mutations, mcpCalls) => mutations.length > 0 || mcpCalls.length > 0
            ? buildSideEffectReconciliationPrompt(mutations, mcpCalls, request.resumeFallbackPrompt ?? prompt)
            : (request.resumeFallbackPrompt ?? prompt);
        try {
            const processState = await this.ensureStreamingProcess(session, request, effectiveSystemPrompt, useResume);
            return await this.sendStreamingPrompt(session, processState, request, prompt, promptImages);
        }
        catch (error) {
            const message = normalizeErrorMessage(error);
            if (!useResume && isSessionAlreadyInUseError(message)) {
                this.resetStreamingProcess(request.sessionKey, session);
                return await this.executeStepWithMode(session, request, effectiveSystemPrompt, prompt, true, allowCompactionLoopRestart, promptImages, observedMutations, observedMcpCalls);
            }
            if (useResume && isMissingResumeSessionError(message)) {
                ...
                return await this.executeStepWithMode(session, request, effectiveSystemPrompt, buildReseedPrompt(observedMutations, observedMcpCalls), false, allowCompactionLoopRestart, promptImages, observedMutations, observedMcpCalls);
            }
            if (allowCompactionLoopRestart &&
                ...
                session.turnCount = 0;
                session.resumeReady = false;
                const result = await this.executeStepWithMode(session, request, effectiveSystemPrompt, buildReseedPrompt(mutations, mcpCalls), false, false, promptImages, mutations, mcpCalls);
    ...
    buildClaudeCodeArgs(session, request, effectiveSystemPrompt, useResume, mcpHost) {
        ...
        const modelName = session.modelOverride ?? parseClaudeCodeModel(request.modelId);
        const args = [
            "-p",
        ...
        if (!request.vanilla) {
            ...
            const allowedStellaTools = request.tools
                ...
                .join(",");
            args.push("--strict-mcp-config", "--mcp-config", session.mcpConfigPath, "--disable-slash-commands", "--tools", "mcp__stella__*", "--allowedTools", allowedStellaTools);
        }
    ...
    async ensureMcpHost(session, request) {
        ...
        session.mcpConfigPath = path.join(ensureArtifactDir(session), "claude-code-mcp.json");
        fs.writeFileSync(session.mcpConfigPath, JSON.stringify({
    ...
    async ensureStreamingProcess(session, request, effectiveSystemPrompt, useResume) {
        const mcpHost = await this.ensureMcpHost(session, request);
        const launchConfig = this.buildProcessLaunchConfig(session, request, effectiveSystemPrompt, mcpHost);
        if (session.process &&
        ...
        if (Number.isFinite(request.autoCompactWindowTokens) &&
            (request.autoCompactWindowTokens ?? 0) > 0) {
            childEnv.CLAUDE_CODE_AUTO_COMPACT_WINDOW = String(Math.floor(request.autoCompactWindowTokens));
        }
        if (Number.isFinite(request.autoCompactTriggerPct) &&
            (request.autoCompactTriggerPct ?? 0) > 0) {
            childEnv.CLAUDE_AUTOCOMPACT_PCT_OVERRIDE = String(Math.min(100, Math.max(1, Math.floor(request.autoCompactTriggerPct))));
        }
        const child = spawn(executablePath, this.buildClaudeCodeArgs(session, request, effectiveSystemPrompt, useResume, mcpHost), {
        ...
        const consumeStdout = (flush = false) => {
            ...
            for (const segment of completeSegments) {
                ...
                if (typeof parsedLine.session_id === "string" &&
                ...
                if (parsedLine.type === "system" &&
                    ...
                    void recordClaudeCodeResolvedModel(request.stellaAppDir, parseClaudeCodeModel(request.modelId) ?? "default", parsedLine.model);
                }
                ...
                // don't spam).
                const modelFallback = getClaudeCodeModelFallbackFromStreamEvent(parsedLine);
                const current = processState.pending[0];
                if (current) {
                    if (updateClaudeCodeNativeToolActivity(parsedLine, current.activeNativeToolUseIds)) {
                    ...
                    const nativeFileChanges = collectClaudeCodeNativeFileChanges(parsedLine);
                ...
                if (parsedLine.type === "result") {
                    ...
                    try {
                        const stepResult = this.parseResultPayload(session, parsedLine, processState.stderrText, Boolean(!completed.request.vanilla && session.allowEmptyNativeFinal));
                        completed.resolve(completed.fileChanges.length > 0
        ...
        child.stderr.on("data", (chunk) => {
            ...
            if (processState.stderrText.length > MAX_STDERR_CAPTURE) {
                processState.stderrText = processState.stderrText.slice(0, MAX_STDERR_CAPTURE);
            }
        });
        child.once("error", (error) => {
            const wrapped = new Error(`Failed to start Claude Code: ${normalizeErrorMessage(error)}`);
            ...
            for (const pending of processState.pending.splice(0)) {
                ...
                pending.reject(new ClaudeCodeProcessEndedError(wrapped.message, null, pending.fileChanges, pending.mcpCalls));
            }
        });
        child.once("close", (code) => {
            ...
            const ownsSessionProcess = session.process === processState;
            if (ownsSessionProcess) {
                resetSessionMcpClients(session, new Error("Claude Code process exited."));
                session.process = undefined;
            }
            // A restart may already have registered a replacement child under this
            // session key; only remove OUR child from tracking.
            ...
            const message = processState.stderrText.trim() ||
                `Claude Code exited with code ${code ?? "unknown"} before returning a result.`;
            for (const pending of processState.pending.splice(0)) {
                this.detachAbortListener(pending);
                pending.reject(pending.request.abortSignal?.aborted
                    ? new Error("Claude Code run aborted.")
                    : new ClaudeCodeProcessEndedError(message, code, pending.fileChanges, pending.mcpCalls));
            }
        });
    ...
    async sendStreamingPrompt(session, processState, request, prompt, promptImages = []) {
        ...
        return await new Promise((resolve, reject) => {
            ...
            if (request.abortSignal) {
                pending.abortListener = () => {
                    resetSessionMcpClients(session, request.abortSignal?.reason ??
                        new Error("Claude Code run aborted."));
            ...
            const content = promptImages.length > 0
                ? [
            ...
            processState.child.stdin.write(`${payload}\n`, (error) => {
                ...
                reject(new ClaudeCodeProcessEndedError(`Failed to write Claude Code prompt: ${normalizeErrorMessage(error)}`, null, pending.fileChanges, pending.mcpCalls));
            });
        });
    }
    detachAbortListener(pending) {
        ...
        if (pending.abortListener && pending.request.abortSignal) {
            pending.request.abortSignal.removeEventListener("abort", pending.abortListener);
        }
    }
    refreshPendingIdleTimer(processState, pending) {
        ...
        const timeoutMs = toolsInFlight
            ? configuredTimeoutMs("STELLA_CLAUDE_CODE_TOOL_IDLE_TIMEOUT_MS", DEFAULT_STEP_TOOL_IDLE_TIMEOUT_MS)
            : pending.hasOutput
                ? configuredTimeoutMs("STELLA_CLAUDE_CODE_IDLE_TIMEOUT_MS", DEFAULT_STEP_IDLE_TIMEOUT_MS)
                : configuredTimeoutMs("STELLA_CLAUDE_CODE_STARTUP_IDLE_TIMEOUT_MS", DEFAULT_STEP_STARTUP_IDLE_TIMEOUT_MS);
        pending.idleTimer = setTimeout(() => {
            ...
            this.detachAbortListener(pending);
            abortProcess(processState.child);
            pending.reject(new Error(toolsInFlight
    ...
    parseResultPayload(session, parsed, stderrText, allowEmptyFinal = false) {
        ...
        if (parsed.is_error === true) {
            const parsedError = (typeof parsed.result === "string" && parsed.result.trim()) ||
        ...
        if (!message && !allowEmptyFinal) {
            throw new ClaudeCodeMalformedResultError(stderrText.trim() || "Claude Code returned an empty result.", "empty_result");
        }
        session.turnCount += 1;
        session.lastUsedAt = Date.now();
        return {
    ...
    failCompactionLoop(sessionKey, session, processState) {
        ...
        for (const pending of failed) {
            ...
            pending.reject(new ClaudeCodeCompactionLoopError(pending.fileChanges, pending.mcpCalls));
        }
    }
    resetStreamingProcess(sessionKey, session) {
        ...
        const child = session.process.child;
        resetSessionMcpClients(session, new Error("Claude Code process is restarting."));
        killProcess(child);
        session.process = undefined;
...
export const isClaudeCodeModel = (modelId) => modelId.trim().startsWith(CLAUDE_CODE_MODEL_PREFIX);
export const runClaudeCodeTurn = async (request) => await runtime.runTurn(request);
/** Diagnostic/test hook: is a live CLI process tracked for this session key? */
export const claudeCodeSessionHasActiveProcess = (sessionKey) => runtime.hasActiveProcess(sessionKey);
export const closeClaudeCodeSessionWhenIdle = (sessionKey) => {
    runtime.closeSessionWhenIdle(sessionKey);
};
export const scheduleClaudeCodeSessionCloseWhenIdle = (sessionKey, timeoutMs) => {
...
export const listClaudeCodeModels = async (auth, stellaAppDir) => {
    ...
    const oauthToken = auth?.oauthToken?.trim() || process.env.ANTHROPIC_OAUTH_TOKEN?.trim();
