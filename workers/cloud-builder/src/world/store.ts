...
import { baseName, normalizeWorldPath, parentPath, pathWithin } from "./path.js";
...
const tarHeader = (entry: WorldEntry): Uint8Array => {
  ...
  if (entry.kind === "symlink" && encoder.encode(entry.target ?? "").byteLength > 100) {
    throw new Error(`Symlink target is too long for ustar export: ${entry.path}`);
  }
  ...
  header[156] = entry.kind === "file" ? 0x30 : entry.kind === "dir" ? 0x35 : 0x32;
...
export class WorldSqlStore implements WorldToolFileApi {
  ...
  initialize(): void {
    ...
    const head = this.sql.exec<HeadRow>("SELECT value FROM world_meta WHERE key = 'head'").toArray()[0];
    if (head) return;
    ...
    this.sql.exec("INSERT INTO world_meta(key, value) VALUES ('head', ?)", live);
  }

  private liveManifest(): string {
    return this.sql.exec<HeadRow>("SELECT value FROM world_meta WHERE key = 'head'").one().value;
  }

  private entryRow(path: string, manifestId = this.liveManifest()): EntryRow | null {
    const parts = direntParts(path);
    return this.sql.exec<EntryRow>(
  ...
  async stat(input: string): Promise<WorldEntry | null> {
    ...
    if (path === "") return { path: "", kind: "dir", mode: 0o755, mtime: 0, size: 0 };
    const row = this.entryRow(path);
    return row ? rowEntry(row) : null;
  }

  async list(input: string, options: { cursor?: string; limit?: number } = {}): Promise<{ entries: WorldEntry[]; cursor?: string }> {
  ...
  private async allEntries(prefix: string): Promise<WorldEntry[]> {
    ...
    for (;;) {
      const page = await this.list(prefix, { ...(cursor ? { cursor } : {}), limit: 10_000 });
  ...
  private blobRow(sha256: string): BlobRow | null {
    return this.sql.exec<BlobRow>("SELECT sha256, size, storage FROM world_blobs WHERE sha256 = ?", sha256).toArray()[0] ?? null;
  }

  private async storeBlob(bytes: Uint8Array, expectedSha?: string): Promise<string> {
    if (bytes.byteLength > WORLD_FILE_LIMIT_BYTES) throw new Error("File exceeds the 256 MiB world file limit.");
    const sha256 = await sha256BytesHex(bytes);
    if (expectedSha && expectedSha !== sha256) throw new Error(`Blob sha256 mismatch: expected ${expectedSha}, received ${sha256}.`);
    if (this.blobRow(sha256)) return sha256;
    if (bytes.byteLength > WORLD_R2_THRESHOLD_BYTES) {
      await this.bucket.put(`blobs/${sha256}`, bytes);
      this.sql.exec("INSERT OR IGNORE INTO world_blobs(sha256, size, storage) VALUES (?, ?, 'r2')", sha256, bytes.byteLength);
      return sha256;
    }
    for (let offset = 0, ordinal = 0; offset < bytes.byteLength || (bytes.byteLength === 0 && ordinal === 0); offset += WORLD_CHUNK_BYTES, ordinal += 1) {
      const chunk = bytes.slice(offset, Math.min(bytes.byteLength, offset + WORLD_CHUNK_BYTES));
      const chunkSha = await sha256BytesHex(chunk);
      this.sql.exec("INSERT OR IGNORE INTO world_chunks(sha256, size, bytes) VALUES (?, ?, ?)", chunkSha, chunk.byteLength, chunk);
      this.sql.exec("INSERT INTO world_blob_chunks(blob_sha256, ordinal, chunk_sha256) VALUES (?, ?, ?)", sha256, ordinal, chunkSha);
    }
    this.sql.exec("INSERT INTO world_blobs(sha256, size, storage) VALUES (?, ?, 'sqlite')", sha256, bytes.byteLength);
    return sha256;
  }

  private async blobBytes(sha256: string, offset: number, length: number): Promise<Uint8Array> {
    const blob = this.blobRow(sha256);
    if (!blob) throw new Error(`Missing world blob ${sha256}.`);
    if (length === 0) return new Uint8Array();
    if (blob.storage === "r2") {
      const object = await this.bucket.get(`blobs/${sha256}`, { range: { offset, length } });
      if (!object) throw new Error(`Missing world R2 blob ${sha256}.`);
      return new Uint8Array(await object.arrayBuffer());
    }
    const first = Math.floor(offset / WORLD_CHUNK_BYTES);
    const last = Math.floor(Math.max(offset, offset + length - 1) / WORLD_CHUNK_BYTES);
    const rows = this.sql.exec<ChunkRow>(
    ...
    const joined = new Uint8Array(rows.reduce((total, row) => total + byteView(row.bytes).byteLength, 0));
    let cursor = 0;
    for (const row of rows) { const bytes = byteView(row.bytes); joined.set(bytes, cursor); cursor += bytes.byteLength; }
    const inside = offset - first * WORLD_CHUNK_BYTES;
    return joined.slice(inside, inside + length);
  }

  async readFile(input: string, options: { offset?: number; length?: number } = {}): Promise<Uint8Array | null> {
    const path = normalizeWorldPath(input);
    const row = this.entryRow(path);
    if (!row) return null;
    if (row.kind !== "file" || !row.blob_sha256) throw new Error(`Path is not a file: ${path}`);
    ...
    const requested = options.length ?? Math.min(available, WORLD_READ_LIMIT_BYTES);
    if (requested > WORLD_READ_LIMIT_BYTES) throw new Error("readFile length exceeds 8 MiB.");
    return this.blobBytes(row.blob_sha256, offset, Math.min(available, requested));
  }
  ...
  private ensureParents(path: string, manifest = this.liveManifest()): void {
    ...
    for (const segment of segments) {
      ...
      if (existing) {
        if (existing.kind !== "dir") throw new Error(`Parent path is not a directory: ${current}`);
        continue;
      }
      const node = this.sql.exec<{ node_id: number }>(
      ...
      this.sql.exec("INSERT INTO world_dirents(manifest_id, parent_path, name, node_id) VALUES (?, ?, ?, ?)", manifest, parts.parent, parts.name, node.node_id);
    }
  }

  private putNode(entry: WorldListingEntry, manifest = this.liveManifest()): WorldEntry {
    ...
    this.sql.exec("DELETE FROM world_tombstones WHERE manifest_id = ? AND path = ?", manifest, path);
    return {
      path,
      kind: entry.kind,
      mode: entry.mode,
  ...
  async writeFile(input: string, bytes: Uint8Array, options: { mode?: number; mtime?: number } = {}): Promise<WorldEntry> {
    const path = normalizeWorldPath(input);
    if (bytes.byteLength > WORLD_READ_LIMIT_BYTES) throw new Error("writeFile bytes exceed 8 MiB; use beginBlob/appendBlob/finishBlob.");
    const manifest = this.liveManifest();
    const prior = this.entryRow(path, manifest);
    if (prior && prior.kind !== "file") throw new Error(`Path is not a file: ${path}`);
    ...
    if (projectedSize > WORLD_QUOTA_BYTES) throw new Error("world_quota_exceeded");
    const sha256 = await this.storeBlob(bytes);
    return this.putNode({ path, kind: "file", mode: options.mode ?? prior?.mode ?? 0o644, mtime: options.mtime ?? this.now(), size: bytes.byteLength, sha256 }, manifest);
  }
  ...
  async mkdir(input: string, options: { mode?: number } = {}): Promise<void> {
    ...
    if (existing) {
      if (existing.kind !== "dir") throw new Error(`Path already exists and is not a directory: ${path}`);
      return;
    }
    this.putNode({ path, kind: "dir", mode: options.mode ?? 0o755, mtime: this.now(), size: 0 });
  }

  async remove(input: string, options: { recursive?: boolean } = {}): Promise<void> {
    const path = normalizeWorldPath(input);
    const manifest = this.liveManifest();
    const parentManifest = this.sql.exec<ManifestRow>(
    ...
    if (!options.recursive && entries.some((entry) => entry.path !== path)) throw new Error(`Directory is not empty: ${path}`);
    for (const entry of entries.sort((left, right) => right.path.length - left.path.length)) {
      const parts = direntParts(entry.path);
      this.sql.exec("DELETE FROM world_dirents WHERE manifest_id = ? AND parent_path = ? AND name = ?", manifest, parts.parent, parts.name);
      if (parentManifest && this.entryRow(entry.path, parentManifest)) {
        this.sql.exec("INSERT OR IGNORE INTO world_tombstones(manifest_id, path) VALUES (?, ?)", manifest, entry.path);
      } else {
        this.sql.exec("DELETE FROM world_tombstones WHERE manifest_id = ? AND path = ?", manifest, entry.path);
      }
    }
  }
  ...
  async rename(fromInput: string, toInput: string): Promise<void> {
    ...
    if (pathWithin(to, from)) throw new Error("Cannot rename a path into itself.");
    ...
    for (const entry of entries) {
      ...
      this.sql.exec("INSERT INTO world_dirents(manifest_id, parent_path, name, node_id) VALUES (?, ?, ?, ?)", manifest, parts.parent, parts.name, row.node_id);
    }
  ...
  async symlink(input: string, target: string): Promise<void> {
    ...
    this.putNode({ path, kind: "symlink", mode: 0o777, mtime: this.now(), size: encoder.encode(target).byteLength, target });
  }

  async beginBlob(): Promise<{ uploadId: string }> {
    const uploadId = crypto.randomUUID();
    this.sql.exec("INSERT INTO world_uploads(upload_id, size) VALUES (?, 0)", uploadId);
    return { uploadId };
  }

  async appendBlob(uploadId: string, bytes: Uint8Array): Promise<void> {
    if (bytes.byteLength > WORLD_READ_LIMIT_BYTES) throw new Error("appendBlob bytes exceed 8 MiB.");
    const upload = this.sql.exec<{ size: number }>("SELECT size FROM world_uploads WHERE upload_id = ?", uploadId).toArray()[0];
    if (!upload) throw new Error("Unknown blob upload.");
    if (upload.size + bytes.byteLength > WORLD_FILE_LIMIT_BYTES) throw new Error("File exceeds the 256 MiB world file limit.");
    const ordinalStart = this.sql.exec<{ next_ordinal: number }>("SELECT COALESCE(MAX(ordinal) + 1, 0) AS next_ordinal FROM world_upload_parts WHERE upload_id = ?", uploadId).one().next_ordinal;
    let ordinal = ordinalStart;
    for (let offset = 0; offset < bytes.byteLength; offset += WORLD_CHUNK_BYTES) {
      this.sql.exec("INSERT INTO world_upload_parts(upload_id, ordinal, bytes) VALUES (?, ?, ?)", uploadId, ordinal, bytes.slice(offset, offset + WORLD_CHUNK_BYTES));
      ordinal += 1;
    }
    this.sql.exec("UPDATE world_uploads SET size = size + ? WHERE upload_id = ?", bytes.byteLength, uploadId);
  }

  async finishBlob(uploadId: string, options: { path?: string; sha256?: string; mode?: number; mtime?: number }): Promise<WorldEntry> {
    const upload = this.sql.exec<{ size: number }>("SELECT size FROM world_uploads WHERE upload_id = ?", uploadId).toArray()[0];
    if (!upload) throw new Error("Unknown blob upload.");
    const parts = this.sql.exec<UploadPartRow>("SELECT ordinal, bytes FROM world_upload_parts WHERE upload_id = ? ORDER BY ordinal", uploadId).toArray();
    ...
    if (options.sha256 && options.sha256 !== sha256) {
      this.sql.exec("DELETE FROM world_upload_parts WHERE upload_id = ?", uploadId);
      this.sql.exec("DELETE FROM world_uploads WHERE upload_id = ?", uploadId);
      throw new Error(`Blob sha256 mismatch: expected ${options.sha256}, received ${sha256}.`);
    }
    ...
    if (!this.blobRow(sha256)) {
      if (upload.size > WORLD_R2_THRESHOLD_BYTES) {
        ...
        const uploadPromise = this.bucket.put(`blobs/${sha256}`, stream.readable);
        for (const part of parts) await writer.write(byteView(part.bytes));
        await writer.close();
        await uploadPromise;
        this.sql.exec("INSERT OR IGNORE INTO world_blobs(sha256, size, storage) VALUES (?, ?, 'r2')", sha256, upload.size);
      } else {
        for (const part of parts) {
          const chunk = byteView(part.bytes);
          const chunkSha = await sha256BytesHex(chunk);
          this.sql.exec("INSERT OR IGNORE INTO world_chunks(sha256, size, bytes) VALUES (?, ?, ?)", chunkSha, chunk.byteLength, chunk);
          this.sql.exec("INSERT INTO world_blob_chunks(blob_sha256, ordinal, chunk_sha256) VALUES (?, ?, ?)", sha256, part.ordinal, chunkSha);
        }
        if (parts.length === 0) {
          const empty = new Uint8Array();
          const chunkSha = await sha256BytesHex(empty);
          this.sql.exec("INSERT OR IGNORE INTO world_chunks(sha256, size, bytes) VALUES (?, 0, ?)", chunkSha, empty);
          this.sql.exec("INSERT INTO world_blob_chunks(blob_sha256, ordinal, chunk_sha256) VALUES (?, 0, ?)", sha256, chunkSha);
        }
        this.sql.exec("INSERT INTO world_blobs(sha256, size, storage) VALUES (?, ?, 'sqlite')", sha256, upload.size);
      }
    }
    this.sql.exec("DELETE FROM world_upload_parts WHERE upload_id = ?", uploadId);
    this.sql.exec("DELETE FROM world_uploads WHERE upload_id = ?", uploadId);
    if (!path) {
      ...
      return { path: "", kind: "file", mode: options.mode ?? 0o644, mtime: options.mtime ?? this.now(), size: upload.size, sha256 };
    }
    return this.putNode({ path, kind: "file", mode: options.mode ?? prior?.mode ?? 0o644, mtime: options.mtime ?? this.now(), size: upload.size, sha256 });
  }
  ...
  private manifestEntries(manifestId: string, cursor: string, limit: number): EntryRow[] {
    return this.sql.exec<EntryRow>(
      `SELECT CASE WHEN d.parent_path = '' THEN d.name ELSE d.parent_path || '/' || d.name END AS path,
              n.node_id, n.kind, n.mode, n.mtime, n.size, n.blob_sha256, n.target
         FROM world_dirents d JOIN world_nodes n ON n.node_id = d.node_id
  ...
  async checkpoint(options: { historyCursor: string }): Promise<{ manifestId: string }> {
    ...
    const tuples = entries.map((entry) => [entry.path, entry.kind, entry.mode, entry.size, entry.sha256 ?? null, entry.target ?? null]);
    const manifestId = await sha256Hex(JSON.stringify(tuples));
    const existing = this.sql.exec<{ manifest_id: string }>("SELECT manifest_id FROM world_manifests WHERE manifest_id = ? AND sealed = 1", manifestId).toArray()[0];
    if (existing) {
      ...
      this.sql.exec("UPDATE world_manifests SET history_cursor = ? WHERE manifest_id = ?", options.historyCursor, manifestId);
    } else {
      this.sql.exec("UPDATE world_manifests SET manifest_id = ?, history_cursor = ?, sealed = 1 WHERE manifest_id = ?", manifestId, options.historyCursor, live);
      this.sql.exec("UPDATE world_dirents SET manifest_id = ? WHERE manifest_id = ?", manifestId, live);
      this.sql.exec("UPDATE world_tombstones SET manifest_id = ? WHERE manifest_id = ?", manifestId, live);
    }
    const next = `live:${crypto.randomUUID()}`;
    this.sql.exec("INSERT INTO world_manifests(manifest_id, parent_manifest_id, history_cursor, created_at, sealed) VALUES (?, ?, NULL, ?, 0)", next, manifestId, this.now());
    this.sql.exec("INSERT INTO world_dirents(manifest_id, parent_path, name, node_id) SELECT ?, parent_path, name, node_id FROM world_dirents WHERE manifest_id = ?", next, manifestId);
    this.sql.exec("UPDATE world_meta SET value = ? WHERE key = 'head'", next);
    await this.collectGarbage(100);
    return { manifestId };
  }

  async manifest(manifestId: string, options: { cursor?: string; limit?: number } = {}): Promise<{ entries: WorldEntry[]; cursor?: string } | null> {
    const exists = this.sql.exec<{ manifest_id: string }>("SELECT manifest_id FROM world_manifests WHERE manifest_id = ?", manifestId).toArray()[0];
    if (!exists) return null;
    const limit = Math.max(1, Math.min(10_000, Math.floor(options.limit ?? 1_000)));
    const rows = this.manifestEntries(manifestId, options.cursor ?? "", limit + 1);
    const entries = rows.slice(0, limit).map(rowEntry);
  ...
  async head(): Promise<{ manifestId: string; parentManifestId?: string }> {
    const live = this.liveManifest();
    const row = this.sql.exec<ManifestRow>("SELECT manifest_id, parent_manifest_id FROM world_manifests WHERE manifest_id = ?", live).one();
    return { manifestId: live, ...(row.parent_manifest_id ? { parentManifestId: row.parent_manifest_id } : {}) };
  }

  async diff(listing: WorldListingEntry[]): Promise<{ changed: string[]; deleted: string[] }> {
    const current = new Map((await this.allEntries("")).map((entry) => [entry.path, entry]));
    const incoming = new Map(listing.map((entry) => [normalizeWorldPath(entry.path), entry]));
    const changed: string[] = [];
    for (const [path, entry] of incoming) {
      const existing = current.get(path);
      if (!existing || existing.kind !== entry.kind || existing.mode !== entry.mode || existing.size !== entry.size || existing.sha256 !== entry.sha256 || existing.target !== entry.target) changed.push(path);
    }
    const deleted = [...current.keys()].filter((path) => !incoming.has(path));
    return { changed: changed.sort(), deleted: deleted.sort() };
  }

  async pushDiff(input: { entries: WorldListingEntry[]; deleted: string[] }): Promise<{ missingBlobs: string[] }> {
    const missing = new Set<string>();
    for (const entry of input.entries) {
      normalizeWorldPath(entry.path);
      if (!Number.isSafeInteger(entry.mode) || entry.mode < 0 || !Number.isSafeInteger(entry.size) || entry.size < 0) {
        throw new Error(`Invalid world listing entry: ${entry.path}`);
      }
      if (entry.kind !== "file") continue;
      if (!entry.sha256) throw new Error(`File listing is missing sha256: ${entry.path}`);
      const blob = this.blobRow(entry.sha256);
      if (!blob) missing.add(entry.sha256);
      else if (blob.size !== entry.size) throw new Error(`Blob size mismatch for ${entry.path}.`);
    }
    const projected = new Map((await this.allEntries("")).map((entry) => [entry.path, entry]));
    for (const deleted of input.deleted) {
      const path = normalizeWorldPath(deleted);
      for (const candidate of projected.keys()) if (pathWithin(candidate, path)) projected.delete(candidate);
    }
    for (const entry of input.entries) {
      if (entry.kind !== "file" || !entry.sha256 || !missing.has(entry.sha256)) {
        projected.set(entry.path, { ...entry, mtime: entry.mtime ?? this.now() });
      }
    }
    ...
    if (projectedSize > WORLD_QUOTA_BYTES) throw new Error("world_quota_exceeded");
    for (const path of input.deleted.sort((left, right) => right.length - left.length)) if (await this.stat(path)) await this.remove(path, { recursive: true });
    for (const entry of [...input.entries].sort((left, right) => left.path.length - right.path.length)) {
      if (entry.kind === "file" && entry.sha256 && missing.has(entry.sha256)) continue;
      this.putNode(entry);
      if (entry.kind === "file" && entry.sha256) {
        this.sql.exec("DELETE FROM world_blob_pins WHERE sha256 = ?", entry.sha256);
      }
    }
    ...
    async function* generate(): AsyncGenerator<Uint8Array> {
      ...
      while (true) {
        ...
        for (const row of rows) {
          ...
          if (entry.kind === "file" && entry.sha256) {
            for (let offset = 0; offset < entry.size; offset += WORLD_READ_LIMIT_BYTES) yield await store.blobBytes(entry.sha256, offset, Math.min(WORLD_READ_LIMIT_BYTES, entry.size - offset));
  ...
  async collectGarbage(limit: number): Promise<boolean> {
    this.sql.exec("DELETE FROM world_blob_pins WHERE expires_at <= ?", this.now());
    const blobs = this.sql.exec<BlobRow>(
    ...
    for (const blob of blobs) {
      if (blob.storage === "r2") await this.bucket.delete(`blobs/${blob.sha256}`);
      this.sql.exec("DELETE FROM world_blob_chunks WHERE blob_sha256 = ?", blob.sha256);
      this.sql.exec("DELETE FROM world_blobs WHERE sha256 = ?", blob.sha256);
    }
    this.sql.exec("DELETE FROM world_chunks WHERE NOT EXISTS (SELECT 1 FROM world_blob_chunks bc WHERE bc.chunk_sha256 = world_chunks.sha256)");
    this.sql.exec("DELETE FROM world_nodes WHERE NOT EXISTS (SELECT 1 FROM world_dirents d WHERE d.node_id = world_nodes.node_id)");
