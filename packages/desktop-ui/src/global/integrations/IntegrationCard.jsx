...
function useIntegrationConnectionStatus(provider) {
    const connection = useQuery(api.channels.utils.getConnection, { provider });
    return connection != null;
}
function SetupContent({ headline, instructions, error, children, }) {
    return (<div className="connect-pair-centered">
...
function ConnectedView({ integration }) {
...
function composeE164(dialCode, localNumber) {
    const localDigits = localNumber.replace(/\D/g, "");
    return `${dialCode}${localDigits}`;
}
function LinqSetupView({ integration }) {
    ...
    const handleSendSms = useCallback(async (e) => {
        e.preventDefault();
      ...
      {step === "phone" ? (<>
          ...
          <form className="connect-phone-form" onSubmit={handleSendSms}>
            <div className="connect-phone-input-group">
              <Select className="connect-phone-dial" value={dialCode} onValueChange={(value) => setDialCode(value)} aria-label="Country code" options={COUNTRY_DIAL_CODES.map((c) => ({
                value: c.code,
                label: `${c.country} ${c.code}`,
            }))}/>
              <input type="tel" className="connect-phone-input" placeholder="(555) 123-4567" value={phone} onChange={(e) => setPhone(e.target.value)} autoFocus/>
            </div>
            <Button type="submit" variant="ghost" disabled={!phone.trim() || sending}>
          ...
          <form className="connect-phone-form" onSubmit={handleVerify}>
            <input type="text" className="connect-code-input" placeholder="ABC123" value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} maxLength={6} autoFocus autoComplete="one-time-code"/>
            <Button type="submit" variant="ghost" disabled={code.trim().length < 6 || verifying}>
              {verifying ? "Verifying..." : "Verify"}
            </Button>
          </form>
          <button type="button" className="connect-bot-link" onClick={() => { setStep("phone"); setCode(""); setError(null); }}>
            Use a different number
          </button>
        </>)}
    </div>);
}
function BotSetupView({ integration, isExpanded, }) {
    ...
    useEffect(() => {
        ...
        const loadBotSetup = async () => {
            ...
            const botLinkPromise = integration.provider === "slack"
            ...
            if (cancelled)
                return;
            const nextBotLink = botLinkResult.status === "fulfilled"
                ? botLinkResult.value
            ...
            if (codeResult.status === "rejected") {
                setState({
                    status: "error",
                    message: getErrorMessage(codeResult.reason, "Failed to generate code"),
                    botLink: nextBotLink,
                });
                return;
            }
            if (botLinkResult.status === "rejected") {
                setState({
                    status: "error",
                    message: getErrorMessage(botLinkResult.reason, "Failed to prepare Slack install URL"),
                    botLink: nextBotLink,
                });
                return;
            }
    ...
    return (<SetupContent headline={`Connect ${integration.displayName}`} instructions={integration.instructions} error={error}>
      {botLink ? (<div className="connect-pair-qr-block">
          {qrDataUrl ? (<img src={qrDataUrl} alt={`Open ${integration.displayName}`} className="connect-pair-qr"/>) : (<div className="connect-skeleton connect-pair-qr"/>)}
        </div>) : null}
      ...
      {botLink ? (<a className="connect-bot-link" href={botLink} target="_blank" rel="noopener noreferrer">
          Open {integration.displayName} &#8599;
        </a>) : null}
    </SetupContent>);
}
function IntegrationGridCardComponent({ integration, isSelected, onClick, disabled, }) {
    const isConnected = useIntegrationConnectionStatus(integration.provider);
    return (<button className={`connect-grid-card${isSelected ? " connect-grid-card-selected" : ""}`} onClick={onClick} type="button" disabled={disabled} aria-disabled={disabled || undefined}>
      ...
      {isConnected && (<span className="connect-grid-card-badge">
...
export function IntegrationDetailArea({ integration, }) {
    const isConnected = useIntegrationConnectionStatus(integration.provider);
    const { hasConnectedAccount } = useAuthSessionState();
    let detailContent;
    if (!hasConnectedAccount) {
        detailContent = (<div className="connect-pair-centered">
    ...
    return (<div className="connect-detail-area">
