...
- `chat-cloud-lifecycle` shows a cloud agent's spawn row, follow-up rows, and terminal state from durable lifecycle journal cards. On both clients the reply that relays a completed task's result carries the completion card (task title, produced-file pills, a result excerpt when fileless); the spawn row only settles its glyph. Mobile opens every file a cloud turn reported through a signed drive URL, never the desktop bridge. A resident cloud turn that attached a sandbox announces its reply-linked files with an `output_files` event at quiesce, the same as a container turn; a turn that never attached still delivers no files (known gap).
