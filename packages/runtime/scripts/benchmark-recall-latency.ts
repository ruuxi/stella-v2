...
import type { LocalContextEvent } from "../kernel/storage/shared.js";
...
const QUERIES = [
  {
    id: "recall_sources",
    prompt:
      "What did we determine about profile.md, core-memory.md, delegated threads, transcripts, and the sources Recall searches?",
    terms: ["profile.md", "core-memory.md", "threads", "transcripts", "Recall"],
  },
  {
    id: "carplay_thread",
    prompt:
...
try {
  for (const query of QUERIES.slice(0, parseLimit())) {
    ...
    const localEvents: LocalContextEvent[] = [];
    ...
    try {
      const brief = await runRecall({
        ...
        store,
        localEvents,
        resolveRecallRoute: async () => recallRoute,
        recallReadQueries: {
