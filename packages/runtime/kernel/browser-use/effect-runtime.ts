/**
 * The one module-level ManagedRuntime for `kernel/browser-use` (M5 wave-4
 ...
 *   area-owned — browser-use modules never import another area's runtime.
...
/** Shared runtime for every Effect run in `kernel/browser-use`. */
export const browserUseRuntime = ManagedRuntime.make(Layer.empty);

/**
 * Run a browser-use Effect on the shared runtime, rejecting with the
 * ORIGINAL failure object (`Cause.squash`) so facade callers see the exact
 * error the pre-Effect code would have thrown.
 */
export const runBrowserUseEffect = async <A>(
  effect: Effect.Effect<A, unknown>,
): Promise<A> => {
  const exit = await browserUseRuntime.runPromiseExit(effect);
 ...
 * replacement for `setTimeout`/`clearTimeout` pairs guarding daemon
 * round-trips and connection attempts. Canceling after the timeout fired
 * (or firing after cancel) is a no-op, exactly like `clearTimeout` on a
 * settled timer.
 */
