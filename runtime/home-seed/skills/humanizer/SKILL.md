---
name: humanizer
description: "Strip AI-isms from text and add real human voice. Use when asked to humanize, de-AI, de-slop, or un-ChatGPT writing, to make a draft sound natural, to match a user's writing voice, or before publishing user-facing prose (posts, emails, docs, release notes)."
---
...
Also apply this skill to **your own** output when writing user-facing prose — release notes, summaries, long-form explanations, documents. Stella's baseline voice already strips most of these, but a focused pass catches what slips through.

## How to use it

The text usually arrives one of three ways:
1. **Inline** — user pastes the text directly into the message. Work on it in-place, reply with the rewrite.
2. **File** — user points at a file. Read it first, then apply edits with `apply_patch` (targeted, per section) or rewrite the whole file. A targeted patch per section is cleaner than rewriting everything.
...
1. Read the input text carefully (read the file first if it's a file).
...
10. If the text came from a file, apply the edit with `apply_patch` (targeted) or a full rewrite, and show the user what changed.
