...
import { readRecentModels, recordRecentModel, } from "@/global/settings/lib/recent-models";
import "./composer-model-mention-menu.css";
const TOKEN_CHARACTER_PATTERN = /[A-Za-z0-9._-]/;
export const COMPOSER_ENGINE_MENTION_OPTIONS = [
    {
        value: "stella",
        label: "Stella",
        brand: "stella",
...
let cachedEngineMentionPreferences = null;
export function resolveCurrentModelMentionValue(preferences) {
    if (preferences?.agentRuntimeEngine === "default") {
        const selectedModel = preferences.modelOverrides.orchestrator ??
...
function normalizeRecentMentionValue(value) {
    if (value === "default")
        return "stella";
 ...
 */
export function findComposerModelMentionTrigger(value, caret) {
...
export function applyComposerModelMention(value, trigger, mention) {
 ...
 */
export function scoreComposerModelMentionMatch(text, query) {
    ...
    if (compactIndex >= 0)
        return 760 - Math.min(compactIndex, 120);
    let queryIndex = 0;
    let firstMatch = -1;
    let lastMatch = -1;
    for (let index = 0; index < compactHaystack.length && queryIndex < compactNeedle.length; index += 1) {
        if (compactHaystack[index] !== compactNeedle[queryIndex])
            continue;
        if (firstMatch < 0)
            firstMatch = index;
        lastMatch = index;
    ...
    if (queryIndex !== compactNeedle.length)
        return 0;
    const span = lastMatch - firstMatch + 1;
    const gaps = span - compactNeedle.length;
    return Math.max(1, 520 - gaps * 12 - firstMatch * 3);
}
export function filterComposerModelMentionOptions(options, query, signals = {}) {
    const normalizedQuery = normalizeSearchText(query);
    const normalizedCurrent = signals.currentValue?.toLowerCase() ?? null;
    const recentOrder = new Map((signals.recentValues ?? []).map((value, index) => [
    ...
    return options
        ...
        const matchScore = normalizedQuery
            ? Math.max(...searchable.map((text) => scoreComposerModelMentionMatch(text, normalizedQuery)))
            : 1;
        const priority = (current ? 10_000 : 0) +
...
export const ComposerModelMentionMenu = forwardRef(function ComposerModelMentionMenu({ trigger, textarea, onSelect, onDismiss }, ref) {
    const [preferences, setPreferences] = useState(cachedEngineMentionPreferences);
    ...
    const filteredOptions = useMemo(() => filterComposerModelMentionOptions(COMPOSER_ENGINE_MENTION_OPTIONS, trigger.query, {
        currentValue,
        recentValues,
    ...
    if (!position)
        return null;
    return createPortal(<div id="composer-model-mention-options" className="composer-model-mention-menu" data-model-mention-menu="" role="listbox" aria-label="Engines" style={position} onMouseDown={(event) => event.preventDefault()}>
      <div className="composer-model-mention-menu__options">
        {filteredOptions.map((option, index) => (<button key={option.value} ref={(element) => {
                optionRefs.current[index] = element;
            }} id={`composer-model-mention-${index}`} type="button" role="option" aria-selected={index === activeIndex} className="composer-model-mention-menu__option" data-active={index === activeIndex ? "" : undefined} onMouseEnter={() => setActiveIndex(index)} onClick={() => selectOption(option)}>
        ...
        {filteredOptions.length === 0 && (<div className="composer-model-mention-menu__empty">
            No matching engine
          </div>)}
      </div>
    </div>, document.body);
