...
Use `Read` and `Grep` to inspect existing files. Use the file-editing tools exposed in this run to create, edit, move, or delete files in the shared folder. The file tools require ABSOLUTE paths and reject relative ones — pass the full absolute path to a file inside the shared folder (the paths shown by tools are already absolute; reuse those). New components should go under the shared folder's `src/`.
