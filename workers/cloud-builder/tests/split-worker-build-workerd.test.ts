...
describe("Split Worker build in real Workerd", () => {
  ...
  beforeAll(async () => {
    ...
    await writeFile(join(outdir, "obsolete-chunk.js"), "throw new Error('obsolete bundle');");
    ...
    for (const module of manifest.modules) {
      ...
      const map = JSON.parse(await readFile(join(outdir, `${module.file}.map`), "utf8"));
      expect(map.version).toBe(3);
      expect(Array.isArray(map.sources)).toBe(true);
      if (module.file === manifest.entry) expect(map.sources.length).toBeGreaterThan(0);
      for (const dependency of module.imports) await access(join(outdir, dependency.file));
    }
    const imports = manifest.modules.map(module => `import(${JSON.stringify(`./${module.file}`)})`).join(",");
    await writeFile(join(outdir, "smoke.js"), `export * from "./index.js";
    ...
    const compatibilityDate = configText.match(/"compatibility_date"\s*:\s*"([^\"]+)"/)?.[1];
    if (!compatibilityDate) throw new Error("Missing Worker compatibility date");
    await writeFile(join(persistencePath, "wrangler.json"), JSON.stringify({
      name: "stella-split-worker-build-test", main: join(outdir, "smoke.js"),
      compatibility_date: compatibilityDate, compatibility_flags: ["nodejs_compat"],
      no_bundle: true, find_additional_modules: true, base_dir: outdir,
  ...
  test("loads every emitted lazy module and retains durable object exports in real Workerd", async () => {
    ...
    for (const name of ["OrchestratorSession", "OwnerGate", "BuildSession", "Sandbox", "SandboxSmall", "AppBuildSandbox", "WorldStore"]) {
