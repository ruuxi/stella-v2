...
type SelfModFeatureRosterPage = {
  entries: StoreSidePanelOlderEntry[];
  total: number;
};

export type StoreSidePanelLegacyApi = ElectronStoreApi & {
...
export const getStoreSidePanelLegacyApi = (): StoreSidePanelLegacyApi | undefined =>
  window.electronAPI?.store as StoreSidePanelLegacyApi | undefined;
...
export const refreshFeatureSnapshot = async (): Promise<void> => {
  const api = getStoreSidePanelLegacyApi();
