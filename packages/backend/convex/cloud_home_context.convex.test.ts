...
const delivered = makeFunctionReference<
  "mutation",
  typeof owner & { revision: number },
  null
>("cloud_home_context:delivered");
const pending = makeFunctionReference<"query", typeof owner, number | null>(
  "cloud_home_context:pending",
);
const retry = makeFunctionReference<"mutation", {}, null>(
...
describe("context notification debt", () => {
  it("records changes atomically and an old acknowledgement cannot clear a newer change", async () => {
    ...
    await t.run(async (ctx) => {
      await homeContextChanged(ctx, owner.ownerId, owner.ownerGeneration);
      await homeContextChanged(ctx, owner.ownerId, owner.ownerGeneration);
    });
  ...
  it("the watchdog repairs a lost scheduled action and keeps generations separate", async () => {
    const t = convexTest(schema, modules);
    await t.run(async (ctx) => {
      await ctx.db.insert("cloud_home_context_updates", {
        ...owner,
        revision: 3,
        deliveredRevision: 1,
        pending: true,
    ...
    const rows = await t.run((ctx) =>
      ctx.db
        ...
        .withIndex("by_ownerId", (q) => q.eq("ownerId", owner.ownerId))
        .take(10),
    );
    expect(
      rows.find((r) => r.ownerGeneration === "gen-1")?.retryAt,
    ).toBeGreaterThan(Date.now());
    expect(await t.query(pending, owner)).toBe(3);
    expect(
      await t.query(pending, { ...owner, ownerGeneration: "gen-2" }),
    ).toBeNull();
  });
});
