/**
 * Pure motion math for `StellaMarkIndicator`, kept out of the component so it
 * can be unit-tested (the repo's `bun test` runner has no React Native
 * renderer, so only logic is testable).
 *
 * Every function here is a Reanimated worklet: the indicator evaluates them on
 * the UI thread from one shared clock, so no per-frame work ever crosses to JS.
