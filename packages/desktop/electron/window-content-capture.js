...
export async function captureWindowContent(x, y, options) {
    if (!hasMacPermission('accessibility') || !hasMacPermission('screen'))
        return null;
...
function cropWindowCapture(windowCapture, contentBounds) {
    ...
    const cropWidth = Math.min(imageSize.width - cropX, Math.round(Math.min(targetBounds.width, availableWidth) * scaleX));
    const cropHeight = Math.min(imageSize.height - cropY, Math.round(Math.min(targetBounds.height, availableHeight) * scaleY));
