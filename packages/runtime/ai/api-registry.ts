...
export function registerLazyApiProvider<
  TApi extends Api,
  TOptions extends StreamOptions,
>(provider: LazyApiProvider<TApi, TOptions>, sourceId?: string): void {
 ...
 * Throws when the host has not registered the requested api.
 */
export async function resolveApiProviderInternal(
  api: Api,
): Promise<ApiProviderInternal> {
  const entry = apiProviderRegistry.get(api);
  if (!entry) {
    throw new Error(
      `No provider registered for api ${api}; the host must call registerBuiltInApiProviders() or registerCloudApiProviders() at startup`,
    );
  }
