...
export const listWorldProjection = async (root: string): Promise<WorldListingEntry[]> => {
  ...
  const walk = async (relative: string): Promise<void> => {
    ...
    for (const name of names) {
      ...
      const common = { path: child, mode: stat.mode & 0o7777, mtime: Math.trunc(stat.mtimeMs), size: stat.size };
      if (stat.isSymbolicLink()) {
        entries.push({ ...common, kind: "symlink", target: await readlink(childPath) });
      } else if (stat.isDirectory()) {
        entries.push({ ...common, kind: "dir", size: 0 });
        await walk(child);
      } else if (stat.isFile()) {
        entries.push({ ...common, kind: "file", sha256: await fileSha256(childPath) });
      }
    }
  };
  await walk("");
...
const postListing = async (
  ...
  const value = await response.json().catch(() => null) as { missingBlobs?: unknown } | null;
  if (!response.ok || !value || !Array.isArray(value.missingBlobs) || value.missingBlobs.some((sha) => typeof sha !== "string" || !/^[0-9a-f]{64}$/u.test(sha))) {
    throw new Error(`World push failed with HTTP ${response.status}.`);
  }
  return value.missingBlobs;
};

const uploadBlob = async (access: WorldSyncAccess, sha256: string, filePath: string): Promise<void> => {
  const requestHeaders = headers(access);
  ...
  if (!response.ok) throw new Error(`World blob upload failed with HTTP ${response.status}.`);
...
export const pushWorldProjection = async (args: {
  ...
  const files = new Map(
    entries
      .filter((entry): entry is WorldListingEntry & { kind: "file"; sha256: string } => entry.kind === "file" && Boolean(entry.sha256))
  ...
  while (missing.length > 0) {
    for (const sha256 of missing) {
      ...
      if (!filePath) throw new Error(`World requested an unknown blob ${sha256}.`);
