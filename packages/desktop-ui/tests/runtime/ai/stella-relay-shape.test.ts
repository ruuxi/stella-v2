...
describe("Stella relay route shape", () => {
  ...
  // Since "make cloud conversations canonical" (74a586520), a refresh
  // capability alone is enough to create a route: the relay is the auth
  // authority and `hasConnectedAccount` no longer gates route creation.
  it("creates a refresh-only route even when account state is absent or false", async () => {
    for (const hasConnectedAccount of [
      undefined,
      () => false as const,
    ]) {
      const route = createStellaRoute({
  ...
  it("does not create a route with neither a token nor a refresh capability", () => {
    const route = createStellaRoute({
      site: {
        baseUrl: STELLA_SITE,
        getAuthToken: () => null,
