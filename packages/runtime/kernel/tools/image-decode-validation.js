...
import { detectImageMediaType, isCompleteImage, } from "../../ai/utils/image-payload.js";
...
const assertResourceBudget = (info) => {
    const pixels = info.width * info.height;
    if (info.width > MAX_IMAGE_EDGE_PIXELS ||
...
export const inspectEncodedImage = (bytes) => {
    ...
    if (mimeType === "image/png" && bytes.length >= 24) {
        ...
        frames = 0;
        for (let index = 13; index < bytes.length && frames <= MAX_IMAGE_FRAMES; index += 1) {
            if (bytes[index] === 0x2c)
                frames += 1;
        }
        ...
        if (kind === "VP8X") {
            ...
            if ((bytes[20] & 0x02) !== 0) {
                frames = 0;
                for (let index = 30; index + 4 <= bytes.length && frames <= MAX_IMAGE_FRAMES; index += 1) {
                    if (String.fromCharCode(...bytes.subarray(index, index + 4)) === "ANMF")
                        frames += 1;
                }
                frames = Math.max(1, frames);
            }
...
export const decodeBase64ImageBounded = (encoded, maxBytes = MAX_GENERATED_IMAGE_BYTES) => {
...
export const readResponseBodyBounded = async (response, options = {}) => {
...
export const validateDecodedImageFile = async (filePath, expectedMimeType) => {
    ...
    try {
        ...
        return Boolean(decoded && (!expectedMimeType || decoded.mimeType === expectedMimeType));
    }
    finally {
        await handle.close();
    }
