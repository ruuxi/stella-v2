...
export class StoreModStore {
  ...
  upsertPendingSelfModChangeSet(args: {
    changeSetId: string;
    repoRoot: string;
    payload: unknown;
    createdAt: number;
    ...
    this.db
      .prepare(
        `INSERT INTO self_mod_pending_change_sets
          (change_set_id, repo_root, payload_json, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?)
  ...
  listPendingSelfModChangeSets(repoRoot: string): Array<{
    changeSetId: string;
    payload: unknown;
    createdAt: number;
  }> {
    const rows = this.db
      .prepare(
        `SELECT change_set_id, payload_json, created_at
         FROM self_mod_pending_change_sets
         WHERE repo_root = ? ORDER BY created_at ASC`,
      )
  ...
  deletePendingSelfModChangeSet(changeSetId: string): void {
    this.db
      .prepare(
        "DELETE FROM self_mod_pending_change_sets WHERE change_set_id = ?",
      )
      .run(changeSetId);
  }
