...
import { useCreateUserPetUploadUrl, useUserPetMutations, } from "./user-pet-data";
import { extractFirstImageUrl, processUserPetAtlasImage, submitUserPetAtlasJob, uploadUserPetSpritesheetToR2, } from "./user-pet-generation";
...
const isMediaJobSnapshot = (value) => Boolean(value) && typeof value === "object";
export function CreatePetDialog({ open, onOpenChange }) {
    ...
    const job = useQuery(api.media_jobs.getByJobId, state.jobId ? { jobId: state.jobId } : "skip");
    const processedJobsRef = useRef(new Set());
    useEffect(() => {
        const currentJobId = state.jobId;
        if (!currentJobId || !isMediaJobSnapshot(job))
            return;
        if (job.status === "succeeded") {
            if (processedJobsRef.current.has(currentJobId))
                return;
    ...
    const handleStartGeneration = useCallback(async () => {
        ...
        try {
            ...
            if (generation !== generationRef.current)
                return;
            const message = err instanceof Error ? err.message : "Couldn't start generation";
    ...
    const handlePublish = useCallback(async () => {
        ...
        try {
            ...
            if (previewBlob && upload.preview) {
                uploads.push(uploadUserPetSpritesheetToR2(previewBlob.blob, upload.preview));
            }
    ...
    const warningSummary = useMemo(() => {
        if (!state.blob || state.blob.warnings.length === 0)
            return null;
    ...
    return (<Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent fit className="user-pet-create-dialog">
        ...
        <DialogBody className="user-pet-create-body">
          <section className="user-pet-create-stage" data-state={state.blob
            ? "ready"
            : state.busy
                ? "busy"
                : state.error
                    ? "error"
                    : "empty"}>
            {state.blob ? (<PetSprite spritesheetUrl={state.blob.objectUrl} state={previewState} size={180}/>) : (<div className="user-pet-create-empty">
          ...
          {state.blob ? (<div className="user-pet-create-state-row">
              {PREVIEW_STATES.map((s) => (<button key={s} type="button" className="user-pet-create-state-pill" data-active={previewState === s || undefined} onClick={() => setPreviewState(s)}>
                  {s.replace("-", " ")}
                </button>))}
            </div>) : null}

          {warningSummary ? (<div className="user-pet-create-warning">{warningSummary}</div>) : null}

          <form className="user-pet-create-form" onSubmit={(event) => {
            ...
            <label className="user-pet-create-field">
              ...
              <textarea className="user-pet-create-textarea" value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="A leafy green sprout with bright eyes, small leaf ears, and a tiny acorn cap." rows={3} maxLength={2000} autoFocus/>
            </label>
            ...
            <div className="user-pet-create-field">
              ...
              <div className="user-pet-create-visibility">
                {VISIBILITY_OPTIONS.map((option) => (<button type="button" key={option.value} className="user-pet-create-visibility-pill" data-active={visibility === option.value || undefined} onClick={() => setVisibility(option.value)}>
            ...
            <div className="user-pet-create-actions">
              {hasDraft ? (<Button type="button" variant="secondary" size="normal" className="pill-btn user-pet-create-discard" onClick={handleDiscard} disabled={submitting}>
                  Discard
                </Button>) : null}
              <Button type="button" variant="secondary" size="normal" className="pill-btn pill-btn--lg" onClick={() => void handleStartGeneration()} disabled={state.busy || submitting || prompt.trim().length === 0}>
                <StellaLogoIcon size={14} aria-hidden/>
                {generateLabel}
              </Button>
              <Button type="submit" variant="primary" size="normal" className="pill-btn pill-btn--primary pill-btn--lg" disabled={!ready || submitting}>
