...
const stepDreamRotationMemoryMap = async ({
  context,
  secrets,
  state,
  rawLog,
  ...
  const owner = await recoverOwnerIdentity(context, secrets, state, rawLog);
  const memory = requireRecord(state.memory, "Memory acceptance state");
  const inputMemoryHead = requireRecord(memory.memoryHead, "Input MEMORY head");
  const beforeMap = await cloudMemoryHead(
    context,
    secrets,
    owner,
  ...
  for (let index = 0; index < 5; index += 1) {
    const sourceKey = `acceptance-dream/${context.runId}/${index}`;
    const payload = {
      title: `Acceptance Dream ${index}`,
      summary: `DREAM-${context.runId}-${index}\n${"dream-memory-".repeat(900)}`,
    };
    const response = await workerRequest(
      context,
      secrets,
      ...
      "cloud-home.dream.enqueue",
      rawLog,
    );
    inputReceipts.push(requireRecord(response.body, "Dream enqueue receipt"));
  }
  let dreamSettled = false;
  const dreamPromise = workerRequest(
    context,
    secrets,
    ...
    "cloud-home.dream.run",
    rawLog,
  ).finally(() => {
    dreamSettled = true;
  });
  const conflictChurn = (async () => {
    for (let attempt = 0; attempt < 40 && !dreamSettled; attempt += 1) {
      const exported = await loadCloudHomeExport(context, secrets, rawLog);
      const current = exported.documents.find(
        (document) => document?.name === "memories/memory_map.md",
      );
      const content = `${current?.content ?? "# Memory Map\n"}\n<!-- acceptance-cas-${runId}-${attempt} -->\n`;
      await cloudHomeUserRequest(
        context,
        secrets,
        "/cloud-home/memory/write",
        {
          ...
          body: JSON.stringify({
            expectedOwnerGeneration: owner.ownerGeneration,
            expectedMemoryEpoch: memory.memoryEpoch,
            name: "memories/memory_map.md",
            kind: "memory_map",
            source: `acceptance-dream-cas:${runId}`,
            expectedRevision: current
              ? requireInteger(current.revision, "Memory map churn revision")
              : 0,
            content,
            writer: "user_edit",
            idempotencyKey: `dream-cas:${runId}:${attempt}`.slice(0, 128),
          }),
        },
        "cloud-home.dream.cas-churn",
        rawLog,
      );
      await new Promise((resolve) => setTimeout(resolve, 25));
    }
  })();
  const [dreamResponse] = await Promise.all([dreamPromise, conflictChurn]);
  const conflictResult = requireRecord(
    dreamResponse.body,
    "Dream conflict run result",
  );
  requireBoolean(
    conflictResult.conflictRetryObserved,
    true,
    "Dream conflict retry observation",
  );
  requireInteger(
    conflictResult.attemptCount,
    "Dream conflict attempt count",
    2,
  );
  requireInteger(
    conflictResult.conflictRetryCount,
    "Dream conflict retry count",
    1,
  );
  assert(
    conflictResult.status === "completed" || conflictResult.processedCount >= 5,
    "Dream conflict run did not complete its claimed input set.",
  );

  // Establish an authoritative post-race head after every churn promise has
  // settled. The conflict receipt above remains the proof that the writer
  // really retried; this clean follow-up prevents a late successful churn
  // write from being mislabeled as the Dream output head.
  const settlementRunId = randomUUID();
  const settlementLeaseId = randomUUID();
  const settlementInput = requireRecord(
    (
      await workerRequest(
        context,
        secrets,
        "/internal/cloud-home/dream/enqueue",
        {
          ...
          body: JSON.stringify({
            ...
            memoryEpoch: memory.memoryEpoch,
            kind: "memory_note",
            sourceKey: `acceptance-dream/${context.runId}/settlement`,
            sourceRevision: 1,
            title: "Acceptance Dream Settlement",
            payload: {
              summary: `DREAM-SETTLED-${context.runId}`,
            },
          }),
        },
        "cloud-home.dream.enqueue-settlement",
        rawLog,
      )
    ).body,
    "Dream settlement input receipt",
  );
  inputReceipts.push(settlementInput);
      ...
      await workerRequest(
        ...
        "/internal/cloud-home/dream/run",
        {
          ...
          body: JSON.stringify({
            ...
            leaseId: settlementLeaseId,
            limit: 20,
          }),
        },
        "cloud-home.dream.run-settlement",
        rawLog,
      )
    ).body,
    "Dream settlement run result",
  );
  assert(
    result.status === "completed" || result.processedCount >= 1,
    "Dream settlement run did not complete.",
  );
  const afterMemory = requireRecord(
    await cloudMemoryHead(
      context,
      secrets,
      owner,
    ...
    "Dream output MEMORY head",
  );
  const afterMap = requireRecord(
    await cloudMemoryHead(
      context,
      secrets,
      owner,
    ...
    "Dream output memory map head",
  );
  assert(
    afterMemory.versionId === result.memoryVersionId &&
      afterMap.versionId === result.memoryMapVersionId,
    "Dream result version ids do not match the authoritative heads.",
  );
  const archiveVersionIds = [
    ...(Array.isArray(conflictResult.archiveVersionIds)
      ? conflictResult.archiveVersionIds
      : []),
    ...(Array.isArray(result.archiveVersionIds)
      ? result.archiveVersionIds
      : []),
  ];
  assert(
    archiveVersionIds.length > 0,
    "Dream did not rotate a real archive version.",
  );
  const exported = await loadCloudHomeExport(context, secrets, rawLog);
  const archive = exported.documents.find(
    (document) =>
      document?.kind === "archive" &&
      archiveVersionIds.includes(document.versionId),
  );
  assert(
    archive,
    "Dream archive version is absent from the authenticated memory export.",
  );
  const archiveHead = requireRecord(
    await cloudMemoryHead(
      context,
      secrets,
      owner,
  ...
  await checkpoint({
    ...
    dream: {
      runId,
      settlementRunId,
      memoryHead: afterMemory,
      memoryMapHead: afterMap,
      archiveHead,
      conflictRetryCount: conflictResult.conflictRetryCount,
    },
  });
  const rotatedBlockCount = (
    String(archive.content).match(/<!-- stella:dream:[0-9a-f]{24}:begin/gu) ??
    []
  ).length;
  assert(
    rotatedBlockCount > 0,
    "Dream archive contains no rotated memory block.",
  );
  assert(
    String(
      exported.documents.find(
        (document) => document?.name === "memories/memory_map.md",
      )?.content ?? "",
    ).includes(`~/.stella/memories/${archive.name}`),
    "Dream memory map does not reference the exact archive document.",
  );
  return {
    observations: {
      conversationId: owner.conversationId,
      dreamRunId: runId,
      dreamLeaseIdSha256: sha256(leaseId),
      settlementDreamRunId: settlementRunId,
      settlementDreamLeaseIdSha256: sha256(settlementLeaseId),
      inputMemoryVersionId: inputMemoryHead.versionId,
      outputMemoryVersionId: afterMemory.versionId,
      outputMemoryMapVersionId: afterMap.versionId,
      memoryRevisionBefore: inputMemoryHead.revision,
      memoryRevisionAfter: afterMemory.revision,
      memoryMapRevisionBefore: beforeMap?.revision ?? 0,
      memoryMapRevisionAfter: afterMap.revision,
      dreamInputSha256: sha256(canonicalJson(inputReceipts)),
      memoryOutputSha256: afterMemory.sha256,
      memoryMapOutputSha256: afterMap.sha256,
      archiveDocumentName: archiveHead.name,
      archiveDocumentId: archiveHead.documentId,
      archiveVersionId: archiveHead.versionId,
      archiveContentSha256: archiveHead.sha256,
      archiveR2Key: archiveHead.r2Key,
      archiveR2Etag: archiveObject.etag,
      dreamRunCompleted:
        result.status === "completed" || result.processedCount >= 5,
      memoryUpdated: afterMemory.revision > inputMemoryHead.revision,
      memoryMapUpdated: afterMap.revision > (beforeMap?.revision ?? 0),
      rotationObserved: archiveVersionIds.length > 0,
      rotatedBlockCount,
      memoryMapReferencesArchive: true,
      conflictRetryObserved: conflictResult.conflictRetryObserved,
      attemptCount: conflictResult.attemptCount,
      conflictRetryCount: conflictResult.conflictRetryCount,
    },
    patch: {
      dream: {
...
const STEP_HANDLERS = Object.freeze({
  ...
  dream_rotation_memory_map: stepDreamRotationMemoryMap,
