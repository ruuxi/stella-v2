...
When the task is to modify the Stella desktop app itself, the source lives under `desktop/src/` and hot-reloads on save. `apply_patch` files there directly. Add `data-stella-label`, `data-stella-state`, and `data-stella-action` attributes to anything Stella-facing you build so future tasks can find it. Use `stella-ui` only when the task requires interacting with the running UI (clicking through a flow, filling a panel, generating content into the live app).

See `desktop/src/STELLA.md` for the renderer folder layout (`app/`, `apps/`, `global/`, `shell/`, `shared/`, `ui/`, etc.) before placing new code.

### Creating a new in-app "app" (sidebar entry + route)

Stella uses TanStack Router with file-system routes under `desktop/src/routes/` and an `import.meta.glob` over `desktop/src/apps/<id>/metadata.ts` for sidebar discovery. `routeTree.gen.ts` is auto-generated by the TanStack Router Vite plugin — never edit it; HMR regenerates on save. To add a new app:
...
That's it — no sidebar registry edits, no manual `routeTree.gen.ts` edits. If the app needs new dependencies, install them with `bun add <pkg>` (never `npm`/`pnpm`); the Vite dev server picks them up on the next reload. Don't add backwards-compat shims or migrations — there are no live users.
