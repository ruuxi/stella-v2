...
function getAnthropicCompat(model: Model<"anthropic-messages">): Required<AnthropicMessagesCompat> {
	return {
		...
		// Dormant since tool_search removal; nothing reads it (see types.ts).
		supportsToolReferences: model.compat?.supportsToolReferences ?? false,
	};
}
	...
	(async () => {
		...
		try {
			...
			if (options?.client) {
				...
				const createForKey = (requestApiKey: string) =>
					createClient(
						model,
						requestApiKey,
 ...
 * Shared by history replay and the pause_turn continuation path.
 */
