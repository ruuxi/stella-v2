...
export function OnboardingEnginePhase({ splitTransitionActive, onContinue, }) {
    ...
    const [choice, setChoice] = useState("stella");
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                ...
                const hasAnyByok = credentials.apiKeys.length > 0 ||
    ...
    const handleLoginOAuth = useCallback(async (providerKey, label) => {
    ...
    const providerEntries = useMemo(() => BYOK_PROVIDER_KEYS.map((key) => {
        const entry = LLM_PROVIDERS.find((p) => p.key === key);
        if (!entry)
            return null;
        const apiKey = findApiKey(credentials.apiKeys, key);
        const oauthCredential = findOauthCredential(credentials.oauthCredentials, key);
        const oauthProvider = findOauthProvider(credentials.oauthProviders, key);
    ...
    const activeEntry = useMemo(() => activeProvider
        ? providerEntries.find(({ entry }) => entry.key === activeProvider) ??
            null
        : null, [activeProvider, providerEntries]);
    return (<div className="onboarding-step-content onboarding-engine-step">
      ...
      <div className="onboarding-engine-tiles">
        <EngineTile variant="hero" active={choice === "stella"} onClick={handleSelectStella} icon={<StellaLogoIcon size={32} aria-hidden/>} label="Stella" description="Use Stella's defaults, or pick any model you want."/>
        <div className="onboarding-engine-tiles-row">
          <EngineTile variant="compact" active={choice === "claude_code"} onClick={handleSelectClaudeCode} icon={<ClaudeLogoIcon size={18} aria-hidden/>} label="Claude Code" description="Use your Claude Code CLI."/>
          <EngineTile variant="compact" active={choice === "byok"} onClick={handleSelectByok} icon={<KeyRound size={16} strokeWidth={1.5} aria-hidden/>} label="Bring your own provider" description="API key or OAuth."/>
        </div>
      </div>

      <div className="onboarding-engine-byok-reveal" data-visible={choice === "byok" || undefined} aria-hidden={choice !== "byok"}>
        <div className="onboarding-engine-byok-reveal-inner">
          <div className="onboarding-engine-byok">
          <div className="onboarding-engine-providers" role="list" aria-label="Available providers">
            {providerEntries.map(({ entry, connected, supportsOAuth, supportsApiKey }) => {
            ...
            const Icon = connected
                ...
                : supportsOAuth
                    ? LogIn
                    : KeyRound;
            return (<button key={entry.key} type="button" role="listitem" className="onboarding-engine-provider" data-active={isActive || undefined} data-connected={connected || undefined} data-pending={inFlight || undefined} disabled={inFlight} onClick={() => handleProviderClick(entry.key)} title={connected
                    ...
                    <Icon size={12} strokeWidth={1.75} className="onboarding-engine-provider-icon" aria-hidden/>
          ...
          {activeEntry ? (<div className="onboarding-engine-auth" role="region" aria-label={`Connect ${activeEntry.entry.label}`}>
              ...
              {activeEntry.supportsOAuth ? (<div className="onboarding-engine-auth-row">
                  <LogIn size={13} aria-hidden/>
                  <Button type="button" variant="ghost" onClick={() => void handleLoginOAuth(activeEntry.entry.key, activeEntry.entry.label)} disabled={oauthInFlight === activeEntry.entry.key}>
              ...
              {activeEntry.supportsApiKey ? (<div className="onboarding-engine-auth-row">
                  <KeyRound size={13} aria-hidden/>
                  <TextField label={`${activeEntry.entry.label} API key`} hideLabel type="password" placeholder={activeEntry.entry.placeholder} value={draftKey} onChange={(e) => setDraftKey(e.target.value)} onKeyDown={(e) => {
                    if (e.key === "Enter") {
                        void handleSaveKey(activeEntry.entry.key, activeEntry.entry.label);
                    }
                }} autoFocus={!activeEntry.supportsOAuth} disabled={savingKey}/>
                  <Button type="button" variant="primary" onClick={() => void handleSaveKey(activeEntry.entry.key, activeEntry.entry.label)} disabled={!draftKey.trim() || savingKey}>
      ...
      {error ? (<p className="onboarding-engine-error" role="alert">
          {error}
        </p>) : null}

      <button className="onboarding-confirm" data-visible={true} disabled={splitTransitionActive} onClick={handleContinue}>
        Continue
      </button>
      <button type="button" className="onboarding-engine-skip" data-visible={choice === "byok" || undefined} disabled={splitTransitionActive || choice !== "byok"} aria-hidden={choice !== "byok"} onClick={handleContinue}>
