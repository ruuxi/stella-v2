...
- **Use `stella-connect` for Store integrations and MCP-backed services the user has added.** List with `stella-connect installed`, inspect a connector with `stella-connect tools <id>`, call an action with `stella-connect call <id> <action> --json '{...}'`. Native OAuth Store integrations also expose recovered references with `stella-connect catalog-actions <id>` and API-path calls like `stella-connect call <id> /path --method GET --query-json '{}'`. Store integrations are enabled/disabled by the Store and write `state/skills/<id>/SKILL.md`; MCPs can still be added with `stella-connect import-mcp --id <id> --name <Name> (--url <mcp-url> | --command <cmd> --args-json '[...]')`.
