...
const materializeRemoteImageAttachment = async (attachment) => {
...
const materializePromptAttachments = async (attachments, target) => {
    if (!attachments?.length)
        return attachments;
    const materialized = await Promise.all(attachments.map(materializeRemoteImageAttachment));
    ...
    try {
        ...
        const promptInputs = args.promptMessages && args.promptMessages.length > 0
            ? await Promise.all(args.promptMessages.map(async (message) => ({
                ...message,
                attachments: await materializePromptAttachments(message.attachments, imageTarget),
            })))
            : [
                {
                    text: args.promptText ?? "",
                    attachments: await materializePromptAttachments(args.attachments, imageTarget),
                },
            ];
        const promptTimestamp = now();
        const promptMessages = (await Promise.all(promptInputs.map(async (input, index) => {
            const message = createRuntimePromptAgentMessage(input, promptTimestamp + index * 2);
