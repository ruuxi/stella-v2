...
export const runsHandlers: WorkerRpcHandlers = {
  ...
  [METHOD_NAMES.INTERNAL_WORKER_ACK_EVENTS]: (params) =>
    Effect.gen(function* () {
      ...
      const pruned =
        sessions.current()?.runEvents.ack({ runId, lastSeq }) ?? 0;
      return { pruned };
    }),
  ...
  [METHOD_NAMES.INTERNAL_WORKER_DREAM_TRIGGER_NOW]: (params) =>
    Effect.gen(function* () {
      const session = yield* WorkerSessions.sessionOrFail(
        () => new RunnerUnavailableError(),
      );
      const runner = yield* session.runner.current;
      const trigger =
        (params as { trigger?: "manual" | "startup_catchup" } | undefined)
          ?.trigger ?? "manual";
      return yield* fromPromise(() => runner.triggerDreamNow(trigger));
    }),

  [METHOD_NAMES.INTERNAL_WORKER_CHRONICLE_SUMMARY_TICK]: (params) =>
    Effect.gen(function* () {
      const session = yield* WorkerSessions.sessionOrFail(
        () => new RunnerUnavailableError(),
      );
      ...
      return yield* fromPromise(() => runner.runChronicleSummaryTick(window));
    }),
};
