...
import { getOrCreateLocalConversationId, isLocalChatApiAvailable, } from '@/features/chat/services/local-chat-store';
...
export const useConversationBootstrap = () => {
    ...
    const { bootstrapAttempt, markFailed, markPreparing, markReady, } = useBootstrapState();
    useEffect(() => {
        let cancelled = false;
        const run = async () => {
            markPreparing();
            ...
            try {
                ...
                markFailed(error instanceof Error && error.message
                    ? error.message
