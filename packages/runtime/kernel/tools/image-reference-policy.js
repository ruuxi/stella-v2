...
import { decodeAndValidateImage, decodeBase64ImageBounded, } from "./image-decode-validation.js";
...
const IMAGE_SIGNATURES = [
    ...
    {
        mimeType: "image/webp",
        matches: (bytes) => bytes.subarray(0, 4).toString("ascii") === "RIFF" &&
            bytes.subarray(8, 12).toString("ascii") === "WEBP",
    },
];
export const detectSupportedImageMimeType = (bytes) => IMAGE_SIGNATURES.find((entry) => entry.matches(bytes))?.mimeType ?? null;
const isWithin = (candidate, root) => {
    const relative = path.relative(root, candidate);
    return (relative === "" ||
        (!relative.startsWith("..") && !path.isAbsolute(relative)));
};
const authorizedRoots = async (context) => {
    const configured = [
    ...
    const roots = await Promise.all(configured.map(async (root) => await fs.realpath(path.resolve(root)).catch(() => null)));
    return roots.filter((root) => root !== null);
};
const sameOpenedObject = (left, right) => left.dev === right.dev &&
    left.ino === right.ino &&
    left.size === right.size &&
    left.mtimeMs === right.mtimeMs &&
    left.ctimeMs === right.ctimeMs;
const readBoundedFromHandle = async (handle) => {
    const chunks = [];
    let total = 0;
    let position = 0;
    for (;;) {
        ...
        if (total > MAX_IMAGE_REFERENCE_BYTES) {
            throw new Error(`reference image exceeds the ${MAX_IMAGE_REFERENCE_BYTES} byte limit`);
        }
        chunks.push(chunk.subarray(0, bytesRead));
    }
    return Buffer.concat(chunks, total);
};
export const readValidatedImageFileNoFollow = async (filePath, options = {}) => {
    ...
    if (typeof constants.O_NOFOLLOW !== "number") {
        throw new Error("safe no-follow image reads are unavailable on this platform");
    }
    ...
    try {
        ...
        if (openedStat.size <= 0 || openedStat.size > MAX_IMAGE_REFERENCE_BYTES) {
            throw new Error(`reference image must be between 1 byte and ${MAX_IMAGE_REFERENCE_BYTES} bytes`);
        }
        ...
        if (roots.length > 0 &&
            ...
            throw new Error("reference image changed outside the authorized directories");
        }
        ...
        if (!decoded) {
            throw new Error("reference file is not a complete, decodable PNG, JPEG, GIF, or WebP image");
        }
...
export const readAuthorizedImageReference = async (filePath, context, hooks) => {
    const roots = await authorizedRoots(context);
    if (roots.length === 0) {
        throw new Error("reference image is outside the active workspace or Stella attachment/media directories");
    }
...
export const authorizedReferenceAsDataUri = async (filePath, context) => {
...
export const validateImageDataUri = async (value) => {
    const match = value.match(/^data:([^;,]+);base64,(.+)$/s);
    if (!match)
        throw new Error("reference data URI must be base64 encoded");
    ...
    if (bytes.length <= 0 || bytes.length > MAX_IMAGE_REFERENCE_BYTES) {
        throw new Error(`reference data URI exceeds the ${MAX_IMAGE_REFERENCE_BYTES} byte limit`);
    }
    const decoded = await decodeAndValidateImage(bytes);
    if (!decoded || decoded.mimeType !== match[1].trim().toLowerCase()) {
        throw new Error("reference data URI MIME type does not match supported image bytes");
    }
};
