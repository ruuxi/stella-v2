...
import { remarkStellaFileLinks, STELLA_FILE_TAG, STELLA_FILE_TAG_ATTRIBUTES, } from "@/features/chat/lib/stella-file-links";
import { StellaFileLink } from "./StellaFileLink";
import { hasCompleteEmojiSpritePack, useActiveEmojiPack, } from "./emoji-sprites/active-emoji-pack";
import { remarkEmojiSprites } from "./emoji-sprites/remark-emoji-sprites";
import { cellToRowCol, getEmojiSpriteGridSize, parseEmojiSpriteUrl, } from "./emoji-sprites/sprite-map";
import "./markdown.css";
/*
...
const areMarkdownPropsEqual = (prev, next) => prev.text === next.text &&
    prev.cacheKey === next.cacheKey &&
    prev.className === next.className &&
    prev.mode === next.mode &&
    Boolean(prev.hideHorizontalRules) === Boolean(next.hideHorizontalRules);
const MarkdownImage = ({ src, alt, className, ...rest }) => {
    ...
    return (<span className={cn("ai-emoji", className)} style={{
...
const buildComponents = (hideHorizontalRules) => ({
    ...(hideHorizontalRules ? { hr: () => null } : {}),
    img: MarkdownImage,
    [STELLA_FILE_TAG]: StellaFileLink,
});
let nextAnonCacheKey = 0;
export const Markdown = memo(function Markdown({ text, cacheKey, className, mode = "static", hideHorizontalRules = false, }) {
    /*
     ...
     */
    const anonCacheKeyRef = useRef(null);
    ...
    const components = useMemo(() => buildComponents(hideHorizontalRules), [hideHorizontalRules]);
    const [activeEmojiPack] = useActiveEmojiPack();
    const emojiSpritesEnabled = hasCompleteEmojiSpritePack(activeEmojiPack);
    const remarkPlugins = useMemo(() => emojiSpritesEnabled
        ? [...BASE_REMARK_PLUGINS, remarkEmojiSprites]
        : BASE_REMARK_PLUGINS, [emojiSpritesEnabled]);
    /*
     ...
     * during debugging.
     */
    const emojiVars = useMemo(() => {
        if (!emojiSpritesEnabled || !activeEmojiPack)
            return undefined;
        return Object.fromEntries(activeEmojiPack.sheetUrls.map((url, index) => [
    ...
    return (<div style={emojiVars}>
      <Streamdown key={streamdownKey} mode={mode} className={cn("markdown", className)} remarkPlugins={remarkPlugins} components={components} allowedTags={ALLOWED_TAGS} linkSafety={LINK_SAFETY}>
        {text}
      </Streamdown>
    </div>);
