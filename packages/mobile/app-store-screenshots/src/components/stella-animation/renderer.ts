...
export const initRenderer = (
  ...
  if (uListening) {
    gl.uniform1f(uListening, 0);
  }

  if (uSpeaking) {
    gl.uniform1f(uSpeaking, 0);
  }

  if (uVoiceEnergy) {
    gl.uniform1f(uVoiceEnergy, 0);
  }
  ...
  const render = (
    ...
    if (uListening) {
      gl.uniform1f(uListening, listening);
    }

    if (uSpeaking) {
      gl.uniform1f(uSpeaking, speaking);
    }

    if (uVoiceEnergy) {
      gl.uniform1f(uVoiceEnergy, voiceEnergy);
    }
