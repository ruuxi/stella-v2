...
function formatPendingDreamInputs(pendingItems, t) {
    if (pendingItems <= 0)
        return undefined;
    return t("settings.memory.dreamResults.pendingPrefix", {
        items: t(pendingItems === 1
            ? "settings.memory.dreamResults.pendingItem"
            : "settings.memory.dreamResults.pendingItems", { count: pendingItems }),
    });
}
function formatChronicleEnableFailure(args, t) {
    switch (args.reason) {
        case "no-stella-root":
...
function formatDreamRunResult(args, t) {
...
function ChronicleSettingsCard() {
    ...
    const [chronicleOverride, setChronicleOverride] = useState("");
    const [chronicleOverrideLoaded, setChronicleOverrideLoaded] = useState(false);
    ...
    useEffect(() => {
        ...
        return () => {
            cancelled = true;
            window.removeEventListener("stella:local-model-preferences-changed", onChange);
        };
    }, []);
    ...
    const hasChronicleByokCredential = useMemo(() => {
        if (!chronicleProvider || chronicleProvider === "stella")
            return false;
        return Boolean(findApiKey(credentials.apiKeys, chronicleProvider) ??
    ...
    const hasStellaPaidPlan = hasConnectedAccount &&
        billingStatus !== undefined &&
        billingStatus.plan !== "free";
    const canEnable = hasChronicleByokCredential || hasStellaPaidPlan;
    const billingLoading = hasConnectedAccount &&
        billingStatus === undefined &&
    ...
    const refresh = useCallback(async () => {
        ...
        try {
            ...
            setError(getSettingsErrorMessage(caught, t("settings.memory.errors.loadStatus")));
        }
        finally {
            setLoading(false);
        }
        ...
        try {
            ...
            if (!result.ok) {
                ...
                showToast({
                    ...
                    description: result.reason === "already-running"
            ...
            setError(getSettingsErrorMessage(caught, t("settings.memory.errors.update")));
        }
        finally {
            setBusy(null);
        }
    ...
    const handleDreamNow = async () => {
        ...
        try {
            ...
            setError(getSettingsErrorMessage(caught, t("settings.memory.errors.dream")));
        }
        finally {
            setBusy(null);
        }
    ...
    const handleOpenFolder = async () => {
        if (!chronicleApi?.openMemoriesFolder)
            return;
        setBusy("open");
        try {
    ...
    const handleWipe = async () => {
        ...
        try {
            ...
            setError(getSettingsErrorMessage(caught, t("settings.memory.errors.wipe")));
        }
        finally {
            setBusy(null);
        }
    ...
    const screenMemoryDescription = accessLoading || canEnable
    ...
    const statusDescription = loading
        ? t("common.loading")
        : enabled
            ? `${running ? t("settings.memory.status.running") : t("settings.memory.status.stopped")}${typeof fps === "number"
    ...
    return (<div className="settings-card">
      ...
      <div className="settings-row">
        ...
        <div className="settings-row-control">
          <Button type="button" variant="ghost" className="pill-btn" disabled={busy !== null || loading || accessLoading} onClick={() => handleToggle(!enabled)}>
            {busy === "toggle"
      ...
      <div className="settings-row">
        ...
        <div className="settings-row-control">
          <Button type="button" variant="ghost" className="pill-btn" disabled={busy !== null} onClick={handleOpenFolder}>
            {busy === "open"
      ...
      <div className="settings-row">
        ...
        <div className="settings-row-control">
          <Button type="button" variant="ghost" className="pill-btn" disabled={busy !== null} onClick={handleDreamNow}>
            {busy === "dream"
      ...
      <div className="settings-row">
        ...
        <div className="settings-row-control">
          <Button type="button" variant="ghost" className="pill-btn pill-btn--danger" disabled={busy !== null} onClick={handleWipe}>
            {busy === "wipe"
      ...
      {error ? (<div className="settings-row">
          <div className="settings-row-info">
