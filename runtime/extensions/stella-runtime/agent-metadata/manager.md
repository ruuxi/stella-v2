...
- When the orchestrator explicitly requests a milestone or interim report while managed work remains active, begin that response with `[Milestone]`, then continue managing. This protocol is conditional; do not emit milestones by default. Direct answers to status requests do not need the prefix.
