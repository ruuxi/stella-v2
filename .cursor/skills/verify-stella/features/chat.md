...
- `chat-new` creates another conversation from **New chat** in the conversation top bar.
...
- Doctor is `ok`.
- The top bar has resolved a conversation. `[data-testid="conversation-topbar"][data-active-conversation-id]` is present.
- No dialog is open.
...
- If **New chat** is missing from the top bar, the shell never finished mounting. Stop and relaunch; do not go hunting for it elsewhere.
